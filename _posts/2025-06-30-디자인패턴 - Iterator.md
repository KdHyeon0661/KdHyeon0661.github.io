...
## 4. Python — Iterator/Iterable 프로토콜, 제너레이터, 커스텀 반복자
...
### 4.5 슬라이딩 윈도우, 배칭, 피커블(look-ahead)
```python
...
## 5. Java — Iterable/Iterator, for-each, 역방향·필터·연결 반복자
...
## 6. C# — IEnumerable/IEnumerator, `yield return`, LINQ
...
### 6.2 제너레이터(`yield return`)와 커스텀 순회
```csharp
...
### 8.2 병합(정렬 병합), 교차 지퍼(zip), 병렬 이터레이션
```python
...
## 10. 동시 수정(Concurrent Modification)·스냅샷·락
...
### 11.2 대용량 CSV 스트리밍 처리(물질화 없이 집계)
```python
import csv
...
## 13. Iterator vs Composite vs Visitor (확장 표)
