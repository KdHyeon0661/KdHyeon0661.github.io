...
## 4. 논블로킹과 `EAGAIN` — 준비도(ready) + 시도 + 재대기
...
## 5. 부분 I/O(Short read/write) — **반드시 루프**
...
## 8. 시그널과 블로킹 I/O — `SA_RESTART`, `pselect/ppoll`
...
## 9. FD 수명/누수 방지 — `O_CLOEXEC`와 “예약 FD”
...
## 10. 경로/권한/TOCTTOU 완화 — `*at`, `openat2`(Linux), 안전 플래그
...
## 11. `mmap`/`msync`/`fsync` — SIGBUS와 동기화 오류
...
## 12. 실전 유틸(바로 복붙) — `robio.h` / `robio.c`
...
## 17. “견고한 종료” 미니 예제 — SIGPIPE 무시 + 타임아웃 + 부분 I/O
...
## 19. 보너스: 수학적 관점에서의 데드라인 오차 누적(간단)
