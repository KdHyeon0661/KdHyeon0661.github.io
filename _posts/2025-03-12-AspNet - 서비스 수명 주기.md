...
## 4) Lifetime 혼용 시 발생하는 대표 함정과 솔루션

### 4.1 Singleton이 Scoped/Transient를 **직접 주입**(Constructor Injection)하면?
...
### 4.2 Captive Dependency(포획된 의존성)
...
## 6) 미들웨어, 컨트롤러/페이지, 최소 API에서의 Lifetime 규칙

### 6.1 컨트롤러/페이지/핸들러
- 기본적으로 **스코프**에서 만들어진다.  
- 페이지/컨트롤러는 **Scoped 서비스 안전**.

### 6.2 전통 미들웨어(델리게이트 기반)
```csharp
...
## 7) EF Core `DbContext`와 Lifetime
...
## 8) `HttpClientFactory`와 Lifetime
...
## 10) 백그라운드 서비스(HostedService/BackgroundService)와 Scoped 사용
...
## 11) Keyed Services(.NET 8) — 같은 인터페이스, 다른 구현 구분
...
## 13) `TryAdd*`, `Replace`, `RemoveAll` — 등록 제어
...
## 14) 테스트 전략 — WebApplicationFactory/TestServer

### 14.1 통합 테스트에서 DI 오버라이드
```csharp
...
### 17.2 Singleton에서 Scoped 사용 — 안전 패턴
...
### 17.3 Options Snapshot/Monitor 비교
