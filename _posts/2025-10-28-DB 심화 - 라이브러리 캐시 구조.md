...
## 큰 그림: 라이브러리 캐시는 **“실행 가능한 코드/계획(커서)”와 메타데이터 저장소**
...
## 구조: 해시 버킷 → 해시 체인 → 핸들(Parent) → 오브젝트(Child/Heaps)
...
## 무효화(Invalidation)와 재파싱(Re-Parse)
...
## SQL 커서 구조(Parent/Child)와 공유 실패 원인
...
### `library cache: mutex X` / `library cache: mutex S`
