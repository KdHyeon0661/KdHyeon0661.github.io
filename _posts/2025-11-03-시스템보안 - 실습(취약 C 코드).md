...
category: 시스템보안
---
# 17.3 실습: 취약 C 코드 → PR 게이트 실패 → 수정 → 자동 재검증
...
## 17.3.5 Makefile(ASan/UBSan/포트러블 플래그)
...
## 17.3.6 GitHub Actions: PR 게이트(실패 → 통과)
...
## 17.3.8 테스트 업데이트(오버플로우 대신 동작 확인)
...
# 17.4 방어: 서명/검증, 프로비저닝 비밀 회전, 릴리즈 SBOM 공개

## 17.4.1 컨테이너/바이너리 서명 & 검증(cosign, OIDC 키리스)

### (A) 빌드 시 서명 + SBOM 어테스테이션
```yaml
...
## 17.4.2 프로비저닝 비밀 관리 & **회전 전략**

### (A) CI/CD에서 **정적 시크릿 금지**: OIDC + 단기 크레덴셜
...
### (C) **KMS + SOPS**로 Git에 암호화 저장 & **회전 파이프라인**
...
## 17.4.3 릴리즈 SBOM 공개 & 소비자 신뢰 사슬

### (A) SBOM 생성(CycloneDX/SPDX) – **언어 + 컨테이너**
```bash
# 애플리케이션(소스) SBOM
...
## 17.4.4 배포 게이트: “서명·SBOM 없는 이미지는 거부”
