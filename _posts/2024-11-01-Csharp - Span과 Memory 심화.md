...
### 5.2 UTF-8 <-> UTF-16 변환 (부분 변환/확장 버퍼)
...
### 5.3 BinaryPrimitives로 엔디언 안전 접근
...
### 7.2 MemoryPool<T> — `IMemoryOwner<T>` 수명 명시
...
### 8.2 SequenceReader<T> (ReadOnlySequence<T> 기반)
...
### 11.5 MemoryPool + IBufferWriter로 가변 출력
