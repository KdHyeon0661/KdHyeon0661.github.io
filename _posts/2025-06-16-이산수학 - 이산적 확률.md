...
### 음이항 \( \mathrm{NegBin}(r,p) \) (r번째 성공까지 실패 포함 시도 횟수)
...
### 체르노프(이항 합 \(S_n\sim\sum \mathrm{Bern}(p)\))
...
### 이항 vs 포아송 근사 검증(\(n=1000,p=0.004,\lambda=4\))
