...
## 1. WHERE 절의 평가 모델: 3값 논리와 연산자 우선순위

### 1.1 SQL의 3값 논리(Three-Valued Logic)
...
## 2. WHERE 절 문법 심화: 스칼라 비교·범위·집합·패턴
...
### 2.4 패턴/검색: `LIKE`, 와일드카드, `ESCAPE`, 정규식
...
### 2.5 `NOT IN`과 NULL 함정 → `NOT EXISTS` 권장
...
## 6. EXISTS / IN / JOIN / SEMI-ANTI JOIN 선택 가이드
...
## 7. 부정 조건 최적화: `NOT LIKE`, `<>`, `NOT BETWEEN`
...
## 12. 실무 예제 — 전형적 필터링 시나리오

### 12.1 “최근 30일, 한국 고객, 유효 주문 합계 10만 원 이상”
