...
## 2) CPU/메모리/보드 — 정적·상세 정보

### 2.1 `lscpu` — 마이크로아키텍처/토폴로지
```bash
lscpu
...
### 2.2 메모리·보드/BIOS — `dmidecode`/`lshw`
```bash
...
### 2.3 열/전력/주파수 — `sensors`/`turbostat`/`powertop`
```bash
...
### 2.4 커널·하드웨어 ID — `/proc`·`lspci`·`lsusb`
```bash
...
### 3.1 블록 구성 — `lsblk`/`blkid`/`parted`/`fdisk`
```bash
...
### 3.4 SMART/NVMe 건강 — `smartctl`/`nvme-cli`
```bash
...
### 4.3 대역폭/지연 측정 — `iperf3`/`mtr`/`traceroute`
```bash
...
### 5.1 `pidstat`/`mpstat` — per-process/per-CPU
```bash
pidstat 1 5              # 프로세스별 CPU/메모리/IO
mpstat -P ALL 1 5
```

### 5.2 `perf` — 샘플링/플레임그래프(기초)
```bash
...
## 7) 컨테이너/가상화/NUMA/IRQ 고급 주제

### 7.1 가상화 지원/하이퍼바이저 감지
```bash
