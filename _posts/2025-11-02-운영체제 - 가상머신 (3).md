...
# 1) 클라우드 이미지(예: ubuntu cloud image)를 'base.qcow2'로 보관했다고 가정
qemu-img create -f qcow2 -b base.qcow2 /var/lib/libvirt/images/vm.qcow2

# 2) cloud-init user-data 준비
cat > user-data <<'YAML'
#cloud-config
users:
  - name: dev
...
### 18.7.5 macOS: Hypervisor.framework(HVF) 이용
...
### 18.7.7 WASM/WASI: 프로세스 VM로서의 샌드박스
...
### 18.7.9 vTPM + Secure Boot (libvirt)
...
### 18.8.1 Confidential VMs (CVM): 메모리 암호화 & 원격 검증
...
### 18.8.2 초고속 부팅: MicroVM 스냅샷/리스토어
...
### 18.8.6 메모리 연구: Balloon, Hinting, 압축, 대페이지
...
### 18.8.8 GPU 가상화: mdev/SR-IOV/타임 슬라이싱
...
### 18.8.10 VM Introspection(VMI) & 보안 가시성
...
### 18.8.11 Record/Replay & Time-Travel Debugging
...
### 18.8.13 분해(Disaggregation)/CXL 시대
