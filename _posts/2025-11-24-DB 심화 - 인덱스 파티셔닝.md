...
## 3) **비파티션(글로벌 Non-Partitioned) 인덱스** (GNI)
...
## 4) **글로벌 파티션 인덱스** (GPI: Global Partitioned Index)
...
## 5) **Prefixed vs Non-Prefixed** (정의/영향/예시)
...
## 6) **컴포지트(서브파티션) 테이블**에서의 로컬 인덱스

### 테이블: RANGE(월) × HASH(cust_id)
