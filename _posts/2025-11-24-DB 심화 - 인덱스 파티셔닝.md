...
## 1. 인덱스 파티셔닝의 본질: “프루닝 + 유지보수 + 병렬성”의 삼각형
...
## 2. 인덱스 파티션 유형 3종(로컬 / 글로벌 비파티션 / 글로벌 파티션)
...
### 3.2 Prefixed vs Non-Prefixed 로컬
...
## 6. Prefixed vs Non-Prefixed (로컬/글로벌 공통 개념)
...
## 8. 파티션-와이즈 조인(Partition-Wise Join)과 인덱스 파티셔닝
...
## 10. 운영 DDL/정책 모음(실전에서 가장 사고가 잦은 영역)
...
### 10.4 EXCHANGE PARTITION + 로컬 인덱스
...
### 11.2 Prefixed/Non-Prefixed 설계 규칙
...
## 13. 실측 검증 루틴(이 가이드의 핵심 “증거 기반 튜닝”)
