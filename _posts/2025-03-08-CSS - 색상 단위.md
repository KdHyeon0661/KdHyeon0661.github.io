...
아래 글은 당신이 준 초안(HEX/RGB/RGBA/HSL/HSLA/색상 이름/예제/선택 가이드)의 **핵심을 유지**하면서,  
**최신 CSS Color** 스펙(쉼표 없는 신문법, `#RRGGBBAA`, `hwb()`, `lab()/lch()/oklab()/oklch()`, `color()`, `color-mix()`, **상대 색상 문법**, 넓은 색역 P3, 대비/접근성, 감마·명도 개념**)까지** 실전에 맞게 확장한 가이드입니다.

모든 코드는 ``` 코드블록으로, 필요한 수식은 $$ 로 감쌉니다.
