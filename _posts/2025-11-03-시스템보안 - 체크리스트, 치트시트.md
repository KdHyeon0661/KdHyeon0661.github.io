---
layout: post
title: 시스템보안 - 체크리스트/치트시트
date: 2025-11-03 18:30:23 +0900
category: 시스템보안
---
# 20. 체크리스트/치트시트 모음

## 20.1 로컬 권한 상승 점검표(Win/Linux)

> 목표: “**권한 상승 경로**가 존재하는지”를 빠르게 스크리닝하고, 발견 시 **취약 구성을 수정**합니다.  
> (※ 실운영에서는 모든 명령을 **읽기 모드**로 수행하고, 변경은 변경관리 절차에 따르세요.)

### 20.1.1 Windows LPE 점검표

**A) 계정/권한/토큰**
- ✅ 로컬 관리자/서드파티 서비스 계정 **최소화**  
- ✅ **UAC**: 관리자 승인 모드, Secure Desktop  
- ✅ **LSA 보호(의무)**: `RunAsPPL` 활성화  
- ✅ **Credential Guard** 가능 시 사용  
- ✅ **감사 정책**: 프로세스 생성(4688), 서비스 설치(7045), 작업 생성(4698)
...
# UAC / LSA 보호 / Device Guard / Credential Guard
...
**B) 서비스/스케줄러/이미지 경로 취약**
- ✅ 서비스 **바이너리 경로 공백** + **따옴표 미설정** 금지  
- ✅ 서비스 **ACL**: 비관리자 쓰기 금지  
- ✅ 작업 스케줄러 **동작 파일 경로** 검증, 사용자 컨텍스트 확인  
- ✅ **DLL 검색 순서** 하이재킹 방지(작업 디렉터리/상대경로 금지)

**🧪 명령 스니펫**
```powershell
# 서비스 바이너리 경로/계정/권한 확인
Get-WmiObject win32_service | Select Name, StartName, PathName, State | Format-Table -Auto

# 취약 가능성(따옴표X 경로 중 공백 포함)
Get-WmiObject win32_service | ? { $_.PathName -match '^[A-Za-z]:\\[^"]* [^"]*\.exe' -and $_.PathName -notmatch '"' } `
| Select Name, PathName

# 작업 스케줄러 요약
schtasks /query /fo LIST /v | findstr /i /c:"작업 이름" /c:"작업 실행" /c:"실행하는 계정"
```

**C) 파일/레지스트리/리소스 권한**
- ✅ 시스템 디렉터리(Program Files, Windows, System32) **Users 쓰기 금지**  
- ✅ **HKLM\...\Run/Services** 등 자동 시작 위치 **권한 확인**  
- ✅ **Temp/Downloads** → 실행권한 제한(AppLocker/WDAC)

**🧪 명령 스니펫**
```powershell
# 폴더 ACL 점검(Users에 Write/Modify 있으면 위험)
...
**D) 메모리/드라이버/커널 표면**
- ✅ 커널 드라이버 **서명강제**(HVCI) 사용 검토  
- ✅ ETW/AMSI **비활성 회피 시도 차단 정책** 적용  
- ✅ 불필요한 드라이버 제거

---

### 20.1.2 Linux LPE 점검표

**A) 계정/권한**
- ✅ `sudo` 제한: **NOPASSWD 최소화**, 명령 화이트리스트  
- ✅ `su`/root 원격 로그인 금지(SSH `PermitRootLogin no`)  
- ✅ `umask` 기본 강화(예: 027)

**🧪 명령 스니펫**
```bash
# sudoers(파일+include) 점검
...
- ✅ 불필요 **SUID/SGID 바이너리 제거**  
- ✅ **capabilities**(file caps) 부여 최소화  
- ✅ `LD_PRELOAD`/`PATH`/`IFS` 악용 가능 경로 제거
...
**C) 파일권한/경로 하이재킹**
- ✅ `PATH` 앞쪽에 **world-writable** 디렉터리 금지  
- ✅ **서비스(Unit) ExecStart** 경로 유효성 확인  
- ✅ **cron/anacron/systemd timer** 스크립트 권한

**🧪 명령 스니펫**
```bash
echo $PATH | tr ':' '\n' | while read d; do [ -w "$d" ] && ls -ld "$d"; done

# systemd 서비스 실행 경로 확인
...
- ✅ **악용 가능 모듈 블록**(USB storage 등 필요 없으면 blacklist)
...
## 20.2 포렌식 스텝 카드 & 명령어 스니펫

> 목표: **현장 30–60분 트리아지**에서 동일한 순서로 밟을 **카드형 절차**입니다. (변경·삭제·전파 행위 금지)

### 20.2.1 공통 원칙(현장)

- ✅ **사진/메모**: 화면/연결/시간/사용자 세션 기록  
- ✅ **변경 최소화**: 라이브/휘발성 수집 후 디스크 이미지  
- ✅ **체인오브커스터디(CoC)** 문서화(수집자, 시간, 해시)

---

### 20.2.2 Windows 트리아지 카드

**[W-01] 컨텍스트**
- 명령: `whoami /all`, `query user`, `ipconfig /all`, `netstat -abno`
- PowerShell:
```powershell
Get-Date
...
# 작업 스케줄러(키워드 필터)
schtasks /query /fo LIST /v | findstr /i /c:"작업 실행" /c:"ttplab" /c:"AppData"
```

**[W-04] 이벤트 로그(핵심 채널)**
```powershell
# 최근 2시간 4688/7045/4698
...
**[W-05] 메모리/아티팩트 보존**
- 메모리 덤프(도구 표준에 따름, 예: `winpmem`) → SHA256 해시  
- Prefetch, ShimCache, Amcache, SRUM, RecentFiles 백업(읽기)

---

### 20.2.3 Linux 트리아지 카드
...
**[L-05] 수집 패키징(읽기 전용)**
```bash
# tar로 메타만 수집(파일 내용 제외, -h 해시/추가도구 권장)
mkdir -p /tmp/triage
...
## 20.3 하드닝 베스트 프랙티스 요약(서버/워크스테이션)

> 목표: “**기본값 강화** + **정책 코드화** + **모니터링 기준**”의 3단계.  
> 서버/워크스테이션 공통, 서비스 성격에 따라 선택 적용.

### 20.3.1 공통 정책

- ✅ **최소 권한**: 로컬 관리자 금지, 서비스 계정 분리(권한·암호수명)  
- ✅ **패치 파이프라인**: 월간 정기 + 고위험 즉시, **자동 재부팅 윈도우 지정**  
- ✅ **EDR/AV**: 실시간 보호 + 정책 잠금, **Self-Protection** 모드  
- ✅ **로깅 표준**: Sysmon(Win) / auditd + journald(Linux), 공통 스키마  
- ✅ **원격 관리**: Just-In-Time(JIT) 접근, MFA, bastion 통제, 세션 로깅  
- ✅ **암호/비밀**: Password Manager/SSO, **Keyless(OIDC)** 활용, 비밀 회전

---

### 20.3.2 Windows 서버/워크스테이션

**구성(요약)**
- **UAC** 강제(관리자 승인 모드 + Secure Desktop)  
- **LSA 보호**: `RunAsPPL=1`, Credential Guard 가능 시 사용  
- **AppLocker/WDAC** 화이트리스트(업무 앱 서명 기반 허용)  
- **PowerShell 로깅**: Module/ScriptBlock/Transcription  
- **원격 데스크톱**: NLA 강제, 외부노출 금지, 게이트웨이 경유
...
**Sysmon 기본 권장(EventID)**
- 1(프로세스 생성), 3(네트워크), 7(DLL 로드), 11(파일 생성), 13(레지스트리), 22(DNS), 24/25(명령제어) 등

---

### 20.3.3 Linux 서버/워크스테이션

**구성(요약)**
- **SSH 하드닝**: `Protocol 2`, `PermitRootLogin no`, `PasswordAuthentication no`, `PubkeyAuthentication yes`, `AllowUsers`/`AllowGroups`  
- **파일시스템**: `/var/tmp`, `/home`, `/tmp`에 `nodev,nosuid,noexec`(업무 영향 검토)  
- **서비스 최소화**: `systemctl disable --now` 불필요 데몬  
- **Audit**: `auditd` 규칙(정책파일 관리)  
- **SELinux/AppArmor**: `enforcing`/프로파일 맞춤
...
- **컨테이너 런타임**: root 금지, `readOnlyRootFilesystem`, `capabilities: drop=["ALL"]`, `seccomp: RuntimeDefault`  
- **이미지**: SBOM 첨부, 서명(cosign), 취약점 스캔(Trivy/Grype)  
- **K8s**: Admission(OPA/Gatekeeper/Kyverno)로 정책 강제, 런타임 탐지(Falco)
...
## 20.4 개인 장비 보안 가이드(연구자용): VM 스냅샷·격리 습관

> 목표: 리서치/분석/테스트를 **안전하게 격리**하고, **사생활/본업 환경**에 영향을 주지 않게 하는 습관화.

### 20.4.1 기기 기본

- ✅ OS 최신 업데이트 자동  
- ✅ **디스크 암호화**(BitLocker/FileVault/LUKS)  
- ✅ **계정 분리**: 일상/연구 로컬 계정 분리, 관리자 세션 최소화  
- ✅ **패스워드 관리자** + **FIDO2/U2F 키**(가능하면)  
- ✅ 브라우저 **컨테이너/프로필 분리**(개인/업무/연구)

---

### 20.4.2 네트워크/프록시 격리

- ✅ 연구용 VM은 기본 **NAT** → 외부 노출 방지  
- ✅ 의심 샘플 테스트 시 **Host-only**로 완전 오프라인  
- ✅ 필요 시 **HTTP/DNS 프록시**를 통해 **로그/필터** 남기기

**🧪 예시: Linux 호스트에서 연구 VM용 프록시 로그**
```bash
# tinyproxy 설치/로그 확인
sudo apt-get install -y tinyproxy
sudo sed -i 's/^#LogLevel.*/LogLevel Info/' /etc/tinyproxy/tinyproxy.conf
sudo systemctl restart tinyproxy
# VM 브라우저 프록시를 호스트IP:8888로 설정 후 /var/log/tinyproxy/tinyproxy.log 확인
```

---

### 20.4.3 VM 스냅샷 습관(“깨끗한 기준선 → 분기 → 합류”)

1) **Baseline 스냅샷**: OS + 필수 도구 설치 후 `Baseline-YYYYMM`  
2) **분석 스냅샷**: 케이스별 `Case-<id>-start`  
3) **실험**: 파일/레지스트리/네트워크 변화 **메모**  
4) **종료 후 롤백**: `Case-<id>-start`로 되돌리기  
5) **주기적 재생성**: Baseline 분기 오래 유지 금지(패치 반영)
...
### 20.4.4 샌드박스/에뮬레이션 보조

- ✅ **QEMU/Bochs**: 사용자 모드 에뮬로 위험행위 차단  
- ✅ **Frida Gadget**: 동적 관찰(자체 코드 주입은 격리된 랩에서만)  
- ✅ **Docker 도구 상자**: 분석 툴 컨테이너화하여 호스트 오염 방지

**🧪 Docker 툴박스 예시**
```bash
docker run --rm -it --network none -v "$PWD:/work" python:3.11-slim bash
# 분석 스크립트 실행, 네트워크 차단 상태에서 처리
```

---

### 20.4.5 브라우저/문서 안전 열람

- ✅ **읽기 전용 뷰어**(PDF/SVG/미디어)는 **격리 VM**에서  
- ✅ 브라우저 **사이트 격리**, `chrome://flags`(Strict-Origin-Isolation 등)  
- ✅ **다운로드 자동 실행 금지**, 미확인 확장/플러그인 금지

---

### 20.4.6 수집/노트/폐기

- ✅ 분석 노트는 **타임스탬프+해시**와 함께 기록  
- ✅ 샘플/아티팩트는 **암호화 컨테이너**에 보관, **보존기한** 설정 후 폐기  
- ✅ 업무망 업로드 금지(격리 네트워크/전용 저장소 사용)

---

## 20.5 부록 — 자동 점검 미니 스크립트

### 20.5.1 Windows 권한/서비스 빠른 헬스체크 (PowerShell)
...
### 20.5.2 Linux SUID/Caps/SSH/NOPASSWD 빠른 헬스체크 (bash)
...
**LPE(Win)**
- [ ] Admin 그룹/서비스 계정 최소화  
- [ ] UAC(승인+보안 데스크톱), LSA 보호/CG  
- [ ] 서비스 경로 따옴표/ACL 안전  
- [ ] 스케줄러/Run 키 검토  
- [ ] Sysmon(EventID 1/3/7/11/13/22) 수집

**LPE(Linux)**
- [ ] sudo NOPASSWD 제거, SSH root 로그인 금지  
- [ ] SUID/SGID/Cap 최소화  
- [ ] PATH 앞쪽 world-writable 제거  
- [ ] ASLR/kptr_restrict 등 커널 보호  
- [ ] auditd 규칙 적용

**포렌식 트리아지**
- [ ] 컨텍스트 → 프로세스/네트워크 → 지속성 → 최근변경 → 수집/해시  
...
이 치트시트는 “**빨리 체크하고, 바로 고칠 수 있는**” 항목 중심입니다.  
위 스크립트(Windows PowerShell, Linux bash)를 저장소에 넣고 **주기적 리포트**를 수집하면, **권한 상승 노출·포렌식 준비 상태·하드닝 수준**을 꾸준히 **가시화**할 수 있습니다.