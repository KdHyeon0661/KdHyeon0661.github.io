...
## 2. 점수(score)·확률(probability)·결정(decision)
...
## 11. 실전 코드 모음 (scikit-learn)

### 11.1 기본 이진 분류 파이프라인 + 층화 CV + 지표
```python
import numpy as np
...
### 11.2 RBF-SVM 그리드서치 + 임계값 튜닝 + 혼동행렬
```python
...
### 11.3 확률 보정(Platt vs Isotonic) 비교
```python
...
### 11.4 불균형: class_weight vs SMOTE (파이프라인)
```python
