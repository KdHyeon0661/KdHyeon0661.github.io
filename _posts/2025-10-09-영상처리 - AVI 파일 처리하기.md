---
layout: post
title: 영상처리 - AVI 파일 처리하기
date: 2025-10-09 15:25:23 +0900
category: 영상처리
---
# 16.1 | **AVI 파일 처리하기**
_이 장에서는 **RIFF/AVI 구조**를 먼저 정확히 짚고, Windows **Video for Windows (VfW)** API를 사용해 **프레임 읽기/쓰기**를 모두 지원하는 `IppAvi` 클래스를 설계·구현합니다. 또한 **MJPEG 압축 스트림**과 **비압축(32bpp)** 스트림 모두를 다루고, `IppDib`(BGRA32, top-down)과 상호 변환하는 실전 코드를 제공합니다._

> 전제
> - 플랫폼: Windows, C++17, Win32(비-MFC) 기준.
> - 렌더링/실제 영상 보관은 앞 장의 `IppDib`(Top-Down **BGRA32**)를 사용.
> - AVI 접근: **Video for Windows (VfW)** API (`vfw.h`, `Vfw32.lib`).
> - 코드 블록은 모두 한 번만 ``` 로 감쌉니다.

---

## 1. AVI 파일의 구조와 사용법

### 1.1 RIFF/AVI 개요
AVI 파일은 RIFF(Resource Interchange File Format)를 따릅니다.

- **RIFF 헤더**
  - `"RIFF"` (4바이트) + 전체 파일 크기(4바이트) + `"AVI "`(4바이트)
- **리스트/청크(Chunk)** 로 구성
  - `LIST 'hdrl'` : 헤더 영역
    - `avih` : 메인 헤더(프레임 시간, 전체 프레임 수, 스트림 수 등)
    - `LIST 'strl'` : 스트림 헤더/포맷
      - `strh` : 스트림 헤더(비디오/오디오 구분, 스케일/레이트 등)
      - `strf` : 스트림 포맷(비디오=BITMAPINFOHEADER, 오디오=WAVEFORMATEX)
  - `LIST 'movi'` : 실제 **프레임 데이터**
    - 비디오 프레임 청크 ID 예: `"00db"`(uncompressed), `"00dc"`(compressed)
  - `idx1` : 인덱스(옵션이지만 대부분 존재)

### 1.2 FPS/길이/사이즈의 해석
- `avih.dwMicroSecPerFrame` → 프레임 시간(마이크로초)
  - FPS \(=\dfrac{1,000,000}{dwMicroSecPerFrame}\)
- `strh.dwScale`/`strh.dwRate` → 스트림의 시간 단위
  - FPS \(=\dfrac{dwRate}{dwScale}\) (비디오 스트림)
- `avih.dwTotalFrames` : 전체 프레임 수
- `BITMAPINFOHEADER` 의 `biWidth`, `biHeight`, `biBitCount` 로 픽셀 포맷 파악
  - `biHeight>0` → **bottom-up** DIB (하단부터 위로)
  - `biHeight<0` → **top-down** DIB (상단부터 아래로)

> 실제 파싱을 직접 구현할 수도 있지만, Windows에서는 **VfW** 고수준 API가 안전하고 편리합니다. 아래는 **VfW로 읽기/쓰기**를 구현한 `IppAvi` 클래스입니다.

---

## 2. `IppAvi` 클래스 설계

### 2.1 목표 기능
- **읽기(Read)**
  - `openRead(path)` / `close()`
  - 메타 정보: `width()`, `height()`, `fps()`, `totalFrames()`
  - 프레임 획득: `readFrame(n, IppDib& out)` (0 ≤ n < total)
- **쓰기(Write)**
  - `beginWrite(path, width, height, fps, FourCC)`
  - `writeFrame(const IppDib&)` (BGRA32 → (BGR24/32 bottom-up)로 변환하여 저장)
  - `endWrite()`
- **압축 옵션**
  - 기본: **비압축(BI_RGB 24/32)**
  - 선택: **MJPG**(일반적으로 폭넓게 설치되어 있음). `AVIMakeCompressedStream` 사용

### 2.2 의존성 & 링킹
```cpp
// 필요 헤더/라이브러리
#include <vfw.h>
#pragma comment(lib, "Vfw32.lib")
```

---

## 3. 구현 – `IppAvi.h / IppAvi.cpp`

### 3.1 헤더: `IppAvi.h`
```cpp
// IppAvi.h
#pragma once
#include <string>
#include <vector>
#include <windows.h>
#include <vfw.h>
#include "IppDib.h"

enum class AviCodec {
    Uncompressed,   // BI_RGB (24/32bpp)
    MJPG            // Motion JPEG
};

class IppAvi {
public:
    IppAvi();
    ~IppAvi();

    // 읽기
    bool openRead(const std::wstring& path);
    void close();
    bool isOpen() const { return m_file != nullptr; }
    int  width()  const { return m_w; }
    int  height() const { return m_h; }
    double fps()  const { return m_fps; }
    int  totalFrames() const { return m_total; }
    // n번째 프레임(0-based)을 IppDib(BGRA32, top-down)에 로드
    bool readFrame(int n, IppDib& out);

    // 쓰기
    bool beginWrite(const std::wstring& path, int w, int h, double fps, AviCodec codec=AviCodec::Uncompressed, int quality=90);
    bool writeFrame(const IppDib& frame); // frame은 BGRA32 top-down
    bool endWrite();

private:
    // 내부 유틸
    void releaseRead();
    void releaseWrite();
    static void TopDownBGRA_to_BottomUpBGR24(const IppDib& src, std::vector<uint8_t>& dst);
    static void TopDownBGRA_to_BottomUpBGRA32(const IppDib& src, std::vector<uint8_t>& dst);

private:
    // 공통
    bool m_vfwInited=false;

    // 읽기용
    PAVIFILE   m_file   = nullptr;
    PAVISTREAM m_vs     = nullptr;
    PGETFRAME  m_getfrm = nullptr;
    int m_w=0, m_h=0, m_total=0;
    double m_fps=0.0;

    // 쓰기용
    PAVIFILE     m_wfile   = nullptr;
    PAVISTREAM   m_wstream = nullptr;
    PAVISTREAM   m_cstream = nullptr; // 압축 스트림(옵션)
    LONG         m_nextFrame = 0;
    int          m_ww=0, m_hh=0;
    double       m_wfps=0.0;
    AviCodec     m_wcodec=AviCodec::Uncompressed;
    int          m_quality=90; // 0~100
};
```

### 3.2 구현: `IppAvi.cpp` — **읽기 파트**
```cpp
// IppAvi.cpp
#include "IppAvi.h"
#include <stdexcept>
#include <cassert>

IppAvi::IppAvi(){
    AVIFileInit(); // 전역 초기화
    m_vfwInited = true;
}
IppAvi::~IppAvi(){
    close();
    endWrite();
    if (m_vfwInited){ AVIFileExit(); m_vfwInited=false; }
}

// ---------- 읽기(Reader) ----------
bool IppAvi::openRead(const std::wstring& path){
    close();

    HRESULT hr = AVIFileOpenW(&m_file, path.c_str(), OF_READ, nullptr);
    if (FAILED(hr)){ m_file=nullptr; return false; }

    // 비디오 스트림 가져오기
    hr = AVIFileGetStream(m_file, &m_vs, streamtypeVIDEO /*fccType*/, 0 /*first*/);
    if (FAILED(hr)){ close(); return false; }

    // 스트림 정보
    AVISTREAMINFO si = {0};
    hr = AVIStreamInfo(m_vs, &si, sizeof(si));
    if (FAILED(hr)){ close(); return false; }

    // FPS 계산: rate/scale
    if (si.dwScale!=0) m_fps = (double)si.dwRate / (double)si.dwScale;
    else m_fps = 0.0;

    // 길이
    LONG len = AVIStreamLength(m_vs);
    m_total = (len>0)? (int)len : 0;

    // 프레임 크기 추정: strh.rcFrame 또는 strf(BITMAPINFO)에서 가져옴
    // GetFrameOpen 시 전달하는 포맷으로 결정 가능. 여기서는 NULL로 전달하여 원 포맷으로.
    m_getfrm = AVIStreamGetFrameOpen(m_vs, nullptr);
    if (!m_getfrm){ close(); return false; }

    // 첫 프레임으로 크기 파악
    void* p = AVIStreamGetFrame(m_getfrm, 0);
    if (!p){ close(); return false; }
    BITMAPINFOHEADER* bih = (BITMAPINFOHEADER*)p;
    m_w = bih->biWidth;
    m_h = (bih->biHeight>0)? bih->biHeight : -bih->biHeight; // 절대값

    return true;
}

void IppAvi::releaseRead(){
    if (m_getfrm){ AVIStreamGetFrameClose(m_getfrm); m_getfrm=nullptr; }
    if (m_vs){ AVIStreamRelease(m_vs); m_vs=nullptr; }
    if (m_file){ AVIFileRelease(m_file); m_file=nullptr; }
    m_w=m_h=m_total=0; m_fps=0.0;
}

void IppAvi::close(){ releaseRead(); }

// AVIStreamGetFrame → BITMAPINFOHEADER + 픽셀 → IppDib(BGRA32, top-down)
bool IppAvi::readFrame(int n, IppDib& out){
    if (!m_getfrm || n<0 || n>=m_total) return false;
    void* p = AVIStreamGetFrame(m_getfrm, n);
    if (!p) return false;

    BITMAPINFOHEADER* bih = (BITMAPINFOHEADER*)p;
    uint8_t* bits = (uint8_t*)p + bih->biSize + (bih->biClrUsed? bih->biClrUsed*4 : 0);

    int W=bih->biWidth;
    int H=(bih->biHeight>0? bih->biHeight : -bih->biHeight);
    int bpp=bih->biBitCount;

    // 출력은 top-down BGRA32
    out.create(W,H,32);

    // 입력이 bottom-up인지 판별
    bool bottomUp = (bih->biHeight>0);

    // 행별 소스 stride (DWORD 정렬)
    int srcStrideBytes;
    if (bpp==24){
        srcStrideBytes = ((W*3 + 3) / 4) * 4;
    }else if (bpp==32){
        srcStrideBytes = W*4;
    }else if (bpp==8 || bpp==16){ // 드물지만 대비
        // 간단화를 위해 여기서는 8/16bpp 미지원 처리
        return false;
    }else{
        return false;
    }

    // 복사/확장
    for (int y=0; y<H; ++y){
        int sy = bottomUp ? (H-1-y) : y;
        const uint8_t* srow = bits + (size_t)sy*srcStrideBytes;
        uint8_t* drow = (uint8_t*)out.bits() + (size_t)y*out.stride();

        if (bpp==24){
            for (int x=0; x<W; ++x){
                uint8_t B=srow[x*3+0], G=srow[x*3+1], R=srow[x*3+2];
                drow[x*4+0]=B; drow[x*4+1]=G; drow[x*4+2]=R; drow[x*4+3]=255;
            }
        }else{ // 32
            // 대부분 BI_RGB BGRA(알파가 0) 또는 비트필드일 수 있음. 여기서는 BGRA 가정.
            memcpy(drow, srow, W*4);
            // 알파 채우기(0이면 255로)
            for (int x=0; x<W; ++x) if (drow[x*4+3]==0) drow[x*4+3]=255;
        }
    }
    return true;
}
```

### 3.3 구현: `IppAvi.cpp` — **쓰기 파트**
```cpp
// ---------- 쓰기(Writer) ----------
bool IppAvi::beginWrite(const std::wstring& path, int w, int h, double fps, AviCodec codec, int quality){
    endWrite();
    m_ww=w; m_hh=h; m_wfps=fps; m_wcodec=codec; m_quality=std::clamp(quality,0,100);
    m_nextFrame=0;

    HRESULT hr = AVIFileOpenW(&m_wfile, path.c_str(), OF_CREATE | OF_WRITE, nullptr);
    if (FAILED(hr)){ m_wfile=nullptr; return false; }

    // 비디오 스트림 만들기
    AVISTREAMINFO si={0};
    si.fccType = streamtypeVIDEO; // 'vids'
    si.fccHandler = 0;            // 압축 선택은 이후 단계에서
    si.dwScale = 1000;            // 1초=1000단위
    si.dwRate  = (DWORD)std::lround(fps*1000.0); // → fps
    si.dwSuggestedBufferSize = w*h*4; // 대략
    SetRect(&si.rcFrame, 0, 0, w, h);

    HRESULT hr2 = AVIFileCreateStream(m_wfile, &m_wstream, &si);
    if (FAILED(hr2)){ endWrite(); return false; }

    // 압축 선택: Uncompressed → 바로 포맷 세트, MJPG → 압축 스트림 생성
    if (codec==AviCodec::Uncompressed){
        // 32bpp BI_RGB 포맷 사용(쓰기/속도 편의)
        BITMAPINFOHEADER bih={0};
        bih.biSize = sizeof(BITMAPINFOHEADER);
        bih.biWidth = w;
        bih.biHeight= h; // bottom-up
        bih.biPlanes= 1;
        bih.biBitCount = 32; // BGRA
        bih.biCompression = BI_RGB;
        bih.biSizeImage = w*h*4;

        hr2 = AVIStreamSetFormat(m_wstream, 0, &bih, sizeof(bih));
        if (FAILED(hr2)){ endWrite(); return false; }
        // 압축 스트림은 없음
    }else{
        // MJPG 압축 스트림 생성
        AVICOMPRESSOPTIONS opts={0};
        opts.fccType = streamtypeVIDEO;
        opts.fccHandler = mmioFOURCC('M','J','P','G');
        opts.dwQuality = (DWORD)((m_quality/100.0)*10000); // 0~10000

        PAVISTREAM tmp=nullptr;
        hr2 = AVIMakeCompressedStream(&tmp, m_wstream, &opts, nullptr);
        if (FAILED(hr2)){ endWrite(); return false; }
        m_cstream = tmp;

        // 압축 스트림 포맷 설정(입력 포맷)
        BITMAPINFOHEADER bih={0};
        bih.biSize = sizeof(BITMAPINFOHEADER);
        bih.biWidth = w;
        bih.biHeight= h; // bottom-up
        bih.biPlanes= 1;
        bih.biBitCount = 24; // 대부분 MJPG는 24bpp BGR 기대
        bih.biCompression = BI_RGB;
        bih.biSizeImage = ((w*3 + 3)/4*4)*h;
        hr2 = AVIStreamSetFormat(m_cstream, 0, &bih, sizeof(bih));
        if (FAILED(hr2)){ endWrite(); return false; }
    }
    return true;
}

void IppAvi::releaseWrite(){
    if (m_cstream){ AVIStreamRelease(m_cstream); m_cstream=nullptr; }
    if (m_wstream){ AVIStreamRelease(m_wstream); m_wstream=nullptr; }
    if (m_wfile){ AVIFileRelease(m_wfile); m_wfile=nullptr; }
    m_nextFrame=0; m_ww=m_hh=0; m_wfps=0.0;
}

bool IppAvi::endWrite(){ if (!m_wfile && !m_wstream && !m_cstream) return true; releaseWrite(); return true; }

// BGRA32(top-down) → BGR24(bottom-up)
void IppAvi::TopDownBGRA_to_BottomUpBGR24(const IppDib& src, std::vector<uint8_t>& dst){
    int W=src.width(), H=src.height();
    int stride = ((W*3 + 3)/4)*4;
    dst.assign((size_t)stride*H, 0);
    for (int y=0; y<H; ++y){
        const uint8_t* srow=(const uint8_t*)src.bits() + (size_t)y*src.stride();
        uint8_t* drow = dst.data() + (size_t)(H-1-y)*stride; // bottom-up
        for (int x=0; x<W; ++x){
            drow[x*3+0]=srow[x*4+0]; // B
            drow[x*3+1]=srow[x*4+1]; // G
            drow[x*3+2]=srow[x*4+2]; // R
        }
    }
}

// BGRA32(top-down) → BGRA32(bottom-up)
void IppAvi::TopDownBGRA_to_BottomUpBGRA32(const IppDib& src, std::vector<uint8_t>& dst){
    int W=src.width(), H=src.height();
    int stride = W*4;
    dst.assign((size_t)stride*H, 0);
    for (int y=0; y<H; ++y){
        const uint8_t* srow=(const uint8_t*)src.bits() + (size_t)y*src.stride();
        uint8_t* drow = dst.data() + (size_t)(H-1-y)*stride; // bottom-up
        memcpy(drow, srow, W*4);
        // 알파가 0이면 255 채우기(선택)
        for (int x=0; x<W; ++x) if (drow[x*4+3]==0) drow[x*4+3]=255;
    }
}

bool IppAvi::writeFrame(const IppDib& frame){
    if (!m_wfile || !m_wstream) return false;
    if (frame.width()!=m_ww || frame.height()!=m_hh) return false;

    HRESULT hr=E_FAIL;
    if (m_wcodec==AviCodec::Uncompressed){
        // 32bpp BI_RGB로 스트림 포맷 설정했으므로 BGRA32 bottom-up 필요
        std::vector<uint8_t> buf;
        TopDownBGRA_to_BottomUpBGRA32(frame, buf);
        LONG bytes = (LONG)buf.size();
        hr = AVIStreamWrite(m_wstream, m_nextFrame, 1, buf.data(), bytes, 0, nullptr, nullptr);
    }else{
        // MJPG: 입력 포맷을 24bpp BGR로 요청했음
        std::vector<uint8_t> buf;
        TopDownBGRA_to_BottomUpBGR24(frame, buf);
        LONG bytes = (LONG)buf.size();
        hr = AVIStreamWrite(m_cstream, m_nextFrame, 1, buf.data(), bytes, 0, nullptr, nullptr);
    }
    if (FAILED(hr)) return false;
    ++m_nextFrame;
    return true;
}
```

---

## 4. 사용 예제

### 4.1 예제 A — AVI 열고 특정 프레임 보기
```cpp
// open_avi_and_show.cpp (발췌)
#include "IppAvi.h"
#include "IppDib.h"

// ... 창 핸들/도큐먼트 유틸은 앞 장과 동일 가정
void Cmd_OpenAviAndShowFrame100(){
    std::wstring path = L"sample.avi";
    IppAvi avi;
    if (!avi.openRead(path)){ MessageBoxW(nullptr, L"열기 실패", L"AVI", MB_ICONERROR); return; }

    IppDib frame;
    int n = std::min(100, avi.totalFrames()-1);
    if (!avi.readFrame(n, frame)){ MessageBoxW(nullptr, L"프레임 읽기 실패", L"AVI", MB_ICONERROR); return; }

    // 새 문서 창에 표시
    CreateDocWindowFromDib(frame, L"AVI Frame 100");
}
```

### 4.2 예제 B — BMP 시퀀스로부터 AVI 만들기(30 FPS)
```cpp
// export_avi_from_bmps.cpp (발췌)
#include "IppAvi.h"
#include "IppDib.h"

void Cmd_ExportAviFromBmps(const std::vector<std::wstring>& bmpPaths){
    if (bmpPaths.empty()) return;

    // 첫 장 크기 확인
    IppDib tmp; tmp.load(bmpPaths[0]); // 앞 장의 IppDib::load 사용
    int W=tmp.width(), H=tmp.height();

    IppAvi avi;
    if (!avi.beginWrite(L"out_uncompressed.avi", W,H, 30.0, AviCodec::Uncompressed)){
        MessageBoxW(nullptr, L"AVI 쓰기 시작 실패", L"AVI", MB_ICONERROR); return;
    }
    for (auto& p : bmpPaths){
        IppDib f; if (!f.load(p)){ continue; }
        // 크기가 다르면 간단히 중앙 크롭/패드 또는 리사이즈(직접 구현)
        if (f.width()!=W || f.height()!=H) f = ResizeBgra32(f, W,H);
        avi.writeFrame(f);
    }
    avi.endWrite();
    MessageBoxW(nullptr, L"비압축 AVI 저장 완료", L"AVI", MB_OK);

    // MJPG로도 저장
    if (avi.beginWrite(L"out_mjpg.avi", W,H, 30.0, AviCodec::MJPG, 85)){
        for (auto& p : bmpPaths){
            IppDib f; f.load(p);
            if (f.width()!=W || f.height()!=H) f = ResizeBgra32(f, W,H);
            avi.writeFrame(f);
        }
        avi.endWrite();
        MessageBoxW(nullptr, L"MJPG AVI 저장 완료", L"AVI", MB_OK);
    }
}
```

### 4.3 예제 C — 간단 AVI 플레이어(타이머로 재생)
```cpp
// avi_player.cpp (핵심 부분)
#include "IppAvi.h"
static IppAvi g_avi;
static IppDib g_cur;
static int g_index=0;
static UINT_PTR g_timer=0;

void StartPlay(const std::wstring& path, HWND hView){
    if (!g_avi.openRead(path)){ MessageBoxW(hView,L"열기실패",L"AVI",MB_ICONERROR); return; }
    g_index=0;
    if (!g_avi.readFrame(g_index, g_cur)){ return; }
    InvalidateRect(hView,nullptr,FALSE);

    UINT interval = (UINT)std::max(1.0, 1000.0/g_avi.fps());
    g_timer = SetTimer(hView, 10, interval, nullptr);
}
void StopPlay(HWND hView){
    if (g_timer){ KillTimer(hView, 10); g_timer=0; }
    g_avi.close();
}
void OnTimer(HWND hView, WPARAM id){
    if (id!=10) return;
    ++g_index;
    if (g_index>=g_avi.totalFrames()) g_index=0; // 루프 재생
    if (g_avi.readFrame(g_index, g_cur)){
        InvalidateRect(hView,nullptr,FALSE);
    }
}
void OnPaint(HWND hView, HDC hdc){
    // g_cur (IppDib) 그리기 — 앞 장의 BitBlt/StretchDIBits 루틴 이용
    g_cur.draw(hdc, 0,0);
}
```

---

## 5. 실전 팁 & 주의사항

### 5.1 32/64비트 & 코덱
- 64비트 환경에서는 **일부 VfW 코덱이 제공되지 않을 수** 있습니다.
  - 이 경우 **Uncompressed** 또는 **MJPG** 와 같은 범용 코덱 사용을 권장합니다.
- 최신 API(Media Foundation)로도 구현 가능하지만, **레거시 AVI** 호환과 간편성을 위해 본 장에서는 **VfW** 를 채택했습니다.

### 5.2 픽셀 포맷 차이
- `AVIStreamGetFrame` 이 반환하는 DIB은 **bottom-up** 인 경우가 많습니다.
  - `biHeight>0` 이면 bottom-up이므로 위 코드처럼 y를 뒤집어 복사.
- 24bpp는 **BGR**, 32bpp는 **BGRA** 라는 점을 잊지 마세요.

### 5.3 MJPG 품질/용량 트레이드오프
- `AVICOMPRESSOPTIONS.dwQuality` 는 `0~10000` 범위를 사용(여기서는 0~100에서 변환).
- **품질↑ → 용량↑**, 반대로 품질이 낮으면 블록/링잉 등 압축 아티팩트 가능.

### 5.4 성능
- **읽기**: `AVIStreamGetFrame` 은 내부 디코딩을 수행하므로, 임의 접근 시 **코덱 씬크 비용**이 있습니다.
  - 순차 접근이 빠르며, 랜덤 접근을 자주 하면 캐싱 전략을 고려하세요.
- **쓰기**: 비압축은 빠르지만 용량이 큼. MJPG는 CPU 압축 비용이 붙습니다.

### 5.5 오디오
- 본 장은 **비디오 전용**. 오디오까지 다루려면 `streamtypeAUDIO` 로 별도 스트림을 추가/읽기 해야 합니다(추가 `strh/strf`).

---

## 6. 통합 — 메뉴/명령(선택)

다음 두 메뉴를 추가해 빠르게 실험할 수 있습니다.

- **File → Open AVI…** : `StartPlay(path, hView)` 호출
- **File → Export AVI…** : BMP 시퀀스/현재 창의 프레임들을 AVI로 내보내기

리소스/대화 상자는 이전 장의 패턴과 동일하므로 생략합니다.

---

## 7. 테스트 체크리스트

- [ ] 서로 다른 해상도의 AVI를 열어 크기/FPS/총 프레임이 올바른지 확인
- [ ] 프레임 0, 중간, 마지막 프레임을 읽어 **색상/위아래 뒤집힘** 문제 없나 확인
- [ ] 비압축 / MJPG 각각으로 저장한 뒤, 외부 플레이어에서 정상 재생되는지 확인
- [ ] 타이머 재생 시 FPS 에 맞춰 부드럽게 재생되는지 확인

---

## 8. 요약

- AVI는 **RIFF** 기반 컨테이너로, `hdrl`/`movi`/`idx1` 구조를 갖습니다.
- Windows **VfW** API를 사용하면 **간단하고 안정적으로** AVI **읽기/쓰기**를 구현할 수 있습니다.
- 본 장의 `IppAvi` 는
  - 읽기: `openRead` → `readFrame(n, IppDib&)`
  - 쓰기: `beginWrite` → `writeFrame` 반복 → `endWrite`
  - 코덱: **Uncompressed** / **MJPG**
  - `IppDib`(top-down BGRA32)과 **상호 변환**까지 포함합니다.
- 이 클래스를 바탕으로 **프레임 기반 영상 처리 파이프라인**(필터링/검출/추적)을 AVI 입출력과 **자연스럽게 연결**할 수 있습니다. 🚀
