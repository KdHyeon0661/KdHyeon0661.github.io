...
category: 정보보안기사
---
# SECTION 08 웹 애플리케이션 취약점 — 08. 경로 추적(Path Traversal) 취약점 완전 정리 (우회 벡터·OS별 특이점·언어별 취약 패턴·안전 정규화·심볼릭 링크/TOCTOU·Zip Slip·서버/클라우드 설정·체크리스트·실기 예상문제)
...
### Node.js (Express) — 경로 정규화 + BASE 프리픽스 + 심볼릭 링크 주의
...
### Java (Spring Boot) — `Path.normalize()` + prefix 검사
