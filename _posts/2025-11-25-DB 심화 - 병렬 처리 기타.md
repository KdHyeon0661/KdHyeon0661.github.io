...
## 병렬 실행(PQ)의 “그림 한 장” 리마인드

병렬 실행은 한 SQL을 여러 PX 서버(슬레이브)가 **나눠 읽고(Scan)**, **나눠 처리하고(Join/Agg/Sort)**, **QC가 합쳐 반환**하는 구조입니다. Oracle은 이 과정을 **DFO(Data Flow Operation)** 와 **TQ(Table Queue)** 라는 내부 파이프라인으로 모델링합니다.
...
Oracle은 다음 중 하나가 있어야 병렬을 고려합니다.
...
## `direct path read` — 버퍼 캐시 우회 대용량 읽기

### Direct Path Read의 본질

`direct path read`는 **블록을 SGA 버퍼 캐시에 올리지 않고**
**바로 PGA로 읽어오는** 읽기 방식입니다. 이때 세션은 `direct path read` 대기 이벤트를 보게 됩니다.

- **Buffered read**: 디스크 → SGA Buffer Cache → PGA
- **Direct read**: 디스크 → PGA (캐시 우회)

Oracle 공식 튜닝 가이드는 direct read를
“SGA를 거치지 않는 단일/멀티블록 직접 읽기”로 분류하고 있습니다.
...
1. **병렬 Full Table Scan / 병렬 Index Fast Full Scan**
   - PX 서버는 대용량 스트리밍 I/O를 선호하므로 direct read가 기본 패턴이 됩니다.
2. **큰 테이블의 직렬 FTS(Adaptive Direct Path Read)**
   - 11g 이후, 캐시에 넣을 가치가 없다고 판단되면 직렬 FTS도 direct read로 바뀔 수 있습니다.
3. **대용량 Sort/Hash가 TEMP로 스필될 때**
   - TEMP에서의 direct read/write는 `direct path read temp`, `direct path write temp`.
4. **Exadata 등에서 Smart Scan 오프로딩이 실패한 direct read**
   - Exadata 문서에서도 direct path read는 병렬 direct read에서 핵심 이벤트로 분류됩니다.

### 왜 빠른가 / 왜 느릴 수 있는가

**빠른 이유**

- 버퍼 캐시 LRU와 래치를 건드리지 않아 **캐시 오염을 막고** CPU/래치 비용을 줄입니다.
- 병렬/대용량 스캔은 OS의 비동기 I/O, 대형 I/O를 타기 좋아 **순차 대역폭을 최대로 씁니다.**

**느려질 수 있는 이유**

- 같은 세그먼트에 dirty 버퍼가 많으면, direct scan 직전에
  “디스크에 내려야 하는” 동기화가 생겨 추가 대기가 끼어들 수 있습니다(대규모 업데이트 직후 FTS 등).
- 반복 참조되는 **소테이블**까지 direct read로 우회하면
  캐시 히트 이점을 잃고 오히려 응답이 악화될 수 있습니다.
...
1. **DOP 과대 금지**
   - 스토리지 대역폭/CPU 대비 DOP를 올리면
     슬레이브끼리 **디스크를 경쟁**해서 `direct path read` 대기만 늘고 느려집니다.
2. **MBRC(멀티블록 읽기 크기)/스토리지 스트라이핑**
   - DPR은 “큰 덩어리 연속 읽기”가 전제입니다. MBRC/스트라이프가 작으면 효과가 줄어듭니다.
3. **캐시에 남겨야 하는 테이블인지 판단**
   - 반복 참조 소테이블은 DPR 강제보다 캐시 경로가 낫습니다.
4. **PGA/TEMP 밸런스**
   - 병렬 정렬/해시가 TEMP로 흘러가면 direct path read temp가 늘면서 I/O 병목.

---

## 병렬 DML(Parallel DML, PDML)
...
Oracle 병렬 실행 가이드에서도 DML이 병렬 서버에 의해 분할되어 수행됨을 명시합니다.
...
3. 실행계획 NOTE에 “Parallel DML”이 실제로 활성화되어야 함
   - 트리거/제약/인덱스 구조에 따라 **Serial 강등**될 수 있습니다.

### PDML INSERT = Direct-Path Insert

병렬 INSERT는 사실상 **direct-path load**로 동작합니다.
Ask TOM과 Oracle 문서 모두
`APPEND` 또는 `PARALLEL`을 사용한 INSERT AS SELECT가 direct-path임을 확인합니다.
...
- 대량 적재에서 스트리밍 I/O가 가능해짐

### ORA-12838 (PDML/Direct-Path 후 동일 트랜잭션 접근 금지)

Oracle 공식 문서는 direct-path INSERT 직후
**같은 트랜잭션에서 그 테이블을 읽거나 수정할 수 없고, COMMIT이 필요**하다고 명시합니다.
...
- 이 제약은 23c에서도 기본적으로 유지되며, direct-path 로드의 일관성 보장을 위한 설계입니다.

### PDML UPDATE/DELETE/MERGE의 실전 포인트
...
- 적재 직후 같은 트랜잭션 접근을 피했는가(ORA-12838)
- 인덱스/트리거/제약으로 Serial 강등되지 않는가

---

## 병렬 인덱스 스캔

### 병렬 친화도: IFFS > Full Scan > Range Scan

Oracle은 병렬 인덱스 경로로 **Index Fast Full Scan(IFFS)** 를 가장 선호합니다.
공식 Parallelism 규칙에도 IFFS가 **병렬화 가능한 스캔 타입**으로 들어 있습니다.

- **Index Fast Full Scan**
  - 인덱스를 “**얇은 테이블**”처럼 전체 멀티블록으로 읽음
  - **정렬 순서 보장 없음**
  - DPR과 궁합이 매우 좋고 병렬화가 쉽습니다.
...
- `direct path read`가 주 대기(인덱스도 direct read가 가능).

### “힌트가 먹히는데 왜 FTS로 가지?”의 이유

Jonathan Lewis는 `PARALLEL_INDEX` 힌트가
**테이블 측 parallel 비용 스케일링과 결합되지 않으면**
IFFS의 병렬 비용을 제대로 낮추지 못해
CBO가 FTS를 택하는 사례를 설명합니다.
...
- 테이블/인덱스 parallel 비용 스케일링이 일치하는가
- IFFS 후 Rowid 랜덤 I/O가 과다하면 해시 조인/FTS 쪽이 낫다

---

## 병렬 Nested Loops 조인(PX NLJ)
...
Ask TOM은 “대규모 NL은 확장성이 떨어지고 비용이 폭발한다”는 점을 반복적으로 강조합니다.
...
각 PX가 독립적으로 큰 테이블을 탐색하는 패턴을 권장합니다.
...
- Inner 탐색은 **`db file sequential read`** 중심(인덱스 단건 I/O).
  `db file sequential read`는 전형적인 인덱스/ROWID 단일블록 읽기 이벤트입니다.
...
Ask TOM이 말하는 “**대규모 NL의 LIO 폭발**” 전형입니다.

### 대안: 병렬 해시 조인 + HASH/HASH 분배
...
- 두 테이블이 큰 경우엔 HASH/HASH가 통상 가장 안정적입니다.
...
- `db file sequential read`가 폭증하는가
- Outer를 브로드캐스트하면 통신이 줄어드는가
- 크면 해시 조인/Partition-wise Join으로 바꿀 수 있는가

---

## 운영 관점: 실행계획/대기/스큐 관찰 루틴
...
| 이벤트 | 의미 | 흔한 원인 |
|---|---|---|
| `direct path read` | DPR / 병렬 FTS/IFFS | 대용량 스캔, 스토리지 병목  |
| `direct path read temp` | TEMP direct read | 해시/소트 스필, PGA 부족  |
| `db file sequential read` | 단일블록 인덱스/ROWID | NLJ, 랜덤 I/O 폭증  |
...
- 병렬 INSERT는 direct-path이므로 **COMMIT 전 접근 금지(ORA-12838)**
- 실행계획 NOTE에서 Serial 강등 여부 확인
- 다수 인덱스/트리거/RI로 비용이 커질 수 있음
- 적재 경쟁이 있으면 direct-path 락 경합 고려

### 병렬 인덱스

- IFFS가 가능하면 IFFS가 1순위
...
- Outer가 작을 때만(수천~수만 수준) 사용
- 작은 Outer는 `BROADCAST NONE`으로 복제해 통신 제거
- Outer가 커지면 `db file sequential read` 폭발 = 즉시 해시 조인 전환
