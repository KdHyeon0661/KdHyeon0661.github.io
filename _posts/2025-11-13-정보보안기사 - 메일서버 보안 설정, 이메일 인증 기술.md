...
### 3.3 기본 하드닝 sendmail.mc (주석 포함 샘플)
...
### 3.5 SASL(AUTH) — /etc/sasl2/Sendmail.conf
```conf
...
### 4.4 DMARC(Domain-based Message Authentication, Reporting & Conformance)
...
### 4.10 SpamAssassin/Rspamd 연동(개요)
...
### 4.11 종합 DNS 레코드 샘플(도메인: example.com)
```
; 수신 라우팅
...
### 4.13 TLSRPT/DMARC 리포트 수집·요약(샘플 파서)
```python
