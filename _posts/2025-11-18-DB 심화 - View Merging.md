...
- **CQT(Cost-Based Query Transformation)**: 옵티마이저 변환이 “항상”이 아니라 **비용 비교로 결정**되는 변환군. Complex View Merging이 대표적이다.

---

## View Merging이 일어나는 위치: 옵티마이저 파이프라인
...
즉, “병합 여부”는 **플랜 후보군을 바꿔** 성능을 수배 이상 갈라놓는다.

---

## View Merging의 3가지 타입

Oracle 공식 분류는 아래 3개다.
...
3) **Complex View Merging (CVM)**
   - `GROUP BY`, `DISTINCT` 등 “무거운” 뷰를 병합해
     **집계/중복제거 위치를 바꾸는** 변환
   - **항상 이득이 아니므로 비용 비교(CQT)**로 결정된다.

이제 각각을 **의미 보존 조건 + 비용 효과 + 실전 패턴**으로 깊게 들어가보자.

---

## 단수(Simple) View Merging - SPJ 뷰 병합의 정석
...
- **인덱스 NL 조인·조인 필터·파티션 프루닝** 같은 기회가 늘어남

Oracle 공식 예시도 “4개 후보 → 6개 후보”로 탐색공간이 늘어
NL 기반 인덱스 접근이 가능해져 비용이 떨어졌다는 구조다.

### Simple View Merging의 “의미 보존” 조건
...
## Outer-Join View Merging - NULL 의미 보존의 싸움
...
Oracle은 OJVM을 **의미 보존 가능할 때만 제한적으로 수행**한다.
...
## 복합(Complex) View Merging - 집계/중복제거 위치 최적화
...
- `c.region='KOR'` 조인이 `f_sales`를 **대폭 줄이는 필터**라면
  **집계 대상 행 자체가 줄어** Hash Group By 비용이 감소한다.
...
- 옵티마이저는 이 둘을 **비용으로 비교해 결정**한다.

### UNION ALL 뷰와 CVM + OR-Expansion
...
## “머징 불가(Non-mergeable)”의 정확한 이유

Oracle이 View Merging을 포기하는 이유는 2종류다.

### **의미상 불가(semantic danger)**
...
## 병합 판단의 핵심 변수: 카디널리티 & 통계

### CBO는 “중간행 크기”를 비용으로 본다
...
| 목적 | 힌트 | 의미 |
|---|---|---|
| 병합 강제 | `MERGE(qb)` | 인라인 뷰/서브쿼리 QB를 상위 QB로 병합 유도  |
| 병합 금지 | `NO_MERGE(qb)` | QB를 유지(필요시 Temp 물리화)  |
...
**주의**
- `NO_MERGE`는 Simple/Complex 모두 막을 수 있다(버전/케이스에 따라 다르게 보이기도 함).
...
- [ ] **Complex 뷰는 “집계 위치”의 비용 비교**가 핵심
      (Filtered join이면 Delayed가 유리, Explosion이면 Early가 유리)
...
**조인 그래프를 재구성해 CBO의 탐색공간과 비용구조를 바꾸는 핵심 변환**이다.
