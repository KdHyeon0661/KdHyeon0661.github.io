...
## 리소스 모니터: `docker stats` / 프로세스: `docker top`
...
## `docker update` — 실행 중 자원 제한 조정(가능 범위)

```bash
docker update --cpus=1 --memory=512m web
```

## `docker create` / `start` — 분리형 생성/시작
