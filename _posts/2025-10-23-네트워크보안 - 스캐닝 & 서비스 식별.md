...
## 3.1 포트 스캔 기법: TCP SYN/Connect, UDP, FIN/Xmas/NULL
...
### 3.1.2 TCP SYN 스캔(“하프 오픈”, 가장 많이 쓰이는 기본)
...
## 3.2 Nmap 심화: NSE, 방화벽 우회 팁, 호스트 디스커버리

### 3.2.1 Nmap 서비스 버전·OS 식별
...
### 3.2.2 NSE(Nmap Scripting Engine)로 심층 식별/취약점 점검
...
### 3.2.4 호스트 디스커버리(Host Discovery)
...
## 3.3 Masscan/ZMap: 대규모 스캔 윤리와 속도·정확도 트레이드오프
...
### 3.4.5 4단계 — NSE(안전 카테고리)로 풍부화(Enrichment)
```bash
...
### 3.4.6 5단계 — (옵션) masscan → nmap 파이프라인
```bash
