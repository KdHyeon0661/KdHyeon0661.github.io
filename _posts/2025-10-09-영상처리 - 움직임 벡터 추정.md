---
layout: post
title: 영상처리 - 움직임 벡터 추정
date: 2025-10-09 17:25:23 +0900
category: 영상처리
---
# 16.3 | **움직임 벡터 추정 (Block Matching Motion Estimation)**
_“정의 → 비용함수(SAD/SSD) → 경계/정합 실패 처리 → **IppFullSearch** 클래스(정수/하프펠) → **오버레이(화살표)** → **메뉴/대화상자** → 예제 시나리오”까지 한 번에._

> 전제
> - 플랫폼: Windows / C++17 / Win32 기반 **ImageTool** 연속 예제
> - 픽셀 컨테이너: `IppImage<T>`(float 그레이스케일 [0,1]) / 디스플레이: `IppDib`(Top-Down BGRA32)
> - 코드는 모두 한 번만 ``` 로 감싸며, 수식은 **MathJax** 로 표기합니다.

---

## 1. 블록 매칭 움직임 추정 방법

### 1.1 정의 (블록 기반 모션, 정수/서브픽셀)
영상 \(I_{t-1}\) (이전 프레임)과 \(I_t\) (현재 프레임)가 있을 때, 영상 좌표 \((x,y)\) 에서 **블록** \(\Omega=\{(i,j)\mid 0\le i,j<B\}\) 를 잡아, 탐색 영역 \(\mathcal{S}=\{(u,v)\mid -R\le u,v\le R\}\) 내에서 **유사도(오차)** 가 최소가 되는 **움직임 벡터** \(\mathbf{d}(x,y)=(u^\*,v^\*)\) 를 찾습니다.
...
> 일반적으로 **SAD** 가 빠르고 강인합니다. 조명 오프셋/스케일 변화가 크면 **ZNCC**(정규화 상관)도 선택지지만, 본 절은 **SAD/SSD** 중심의 **전수 탐색(Full Search)** 을 구현합니다.

### 1.2 경계/정합 실패
- **경계**: \((x+u, y+v)\) 로 블록이 이전 프레임 경계를 벗어나면 해당 후보를 **무효** 또는 패딩(경계 복제) 처리. 여기서는 **무효**로 처리합니다.
- **정합 실패**: 텍스처가 적거나 조명이 급변하면 비용이 낮은 후보가 없을 수 있음 → **최소 비용이 임계치 이상**이면 (0,0)으로 클램프 하거나, 비용 맵을 시각화하여 후처리(평활화/미디언) 합니다.

### 1.3 서브픽셀(하프펠) 정밀화
정수 화소에서 최적 \((u_0,v_0)\) 을 찾은 뒤, 그 주변 \((u_0\pm\frac12, v_0)\), \((u_0, v_0\pm\frac12)\), \((u_0\pm\frac12, v_0\pm\frac12)\) 를 **양선형 보간**으로 평가하여 더 낮은 비용을 선택합니다.

---

## 2. `IppFullSearch` 클래스 설계

### 2.1 API 목표
- 입력: `prev`(t-1), `cur`(t) — `IppImage<float>` (0..1)
- 파라미터: 블록 크기 B, 탐색 반경 R, 비용함수(SAD/SSD), **하프펠** on/off, **스텝**(표시 간격)
- 출력: **모션 필드**(블록 그리드에 대응하는 MV), **비용 맵**, **오버레이 드로잉**

### 2.2 자료 구조
- **그리드**: \(\lfloor W/B \rfloor \times \lfloor H/B \rfloor\) 크기
- `MV` : `{float dx, dy; float cost;}`
- `MVField` : `{int gridW, gridH; std::vector<MV> data;}`

---

## 3. 구현 — `IppFullSearch.hpp / .cpp`
...
struct FSParams {
    ...
    bool halfpel = true;      // 서브픽셀 정밀화
    int stepVis = 1;          // 시각화 간격(=그리드에서 샘플링 스텝)
    float rejectThr = -1.0f;  // 정합 거부 임계(<=0 이면 미사용). SAD/SSD 스케일에 맞춰 설정.
};
...
public:
    ...
    // 오버레이(화살표) — frame 위에 모션을 그립니다.
    // color: (R,G,B), thickness: 선 두께, skip: 시각화 간격 (p.stepVis와 별도로 추가 downsample)
...
// 양선형 샘플 (좌표는 픽셀 중심 기준 아님, 단순 [0..W), [0..H) 연속)
...
// 하프펠: 정수 (u0,v0) 주변 8개 지점(±0.5) + 중심 보정 검사
...
{
    // 평가 후보: (u0+du, v0+dv), du,dv ∈ { -0.5, 0, +0.5 } (중심 포함 9개)
    float best = baseCost;
...
bool DibToGray01(const IppDib& src, IppImage<float>& g); // BGRA32 → Gray[0,1]
...
INT_PTR MotionVectorDlg::OnInit(HWND h){
    ...
    SetDlgItemTextW(h, IDC_EDIT_MV_REJTHR, L"-1"); // 미사용
...
void MotionVectorDlg::Run(HWND h){
    ...
    if (ip<0||ic<0||ip>=(int)docs_.size()||ic>=(int)docs_.size()){
        MessageBoxW(h, L"문서 선택을 확인하세요.", L"Motion Vectors", MB_ICONWARNING); return;
    }
    IppDib* pPrev=docs_[ip].dib;
    IppDib* pCur =docs_[ic].dib;
    if (!pPrev || !*pPrev || !pCur || !*pCur){
        MessageBoxW(h, L"영상이 비었습니다.", L"Motion Vectors", MB_ICONERROR); return;
    }
    if (pPrev->width()!=pCur->width() || pPrev->height()!=pCur->height()){
        MessageBoxW(h, L"이전/현재 영상의 크기가 다릅니다.", L"Motion Vectors", MB_ICONERROR); return;
    }
    ...
    if (!DibToGray01(*pPrev, gPrev) || !DibToGray01(*pCur, gCur)){
        MessageBoxW(h, L"그레이 변환 실패", L"Motion Vectors", MB_ICONERROR); return;
    }
    ...
    if (!mv){ MessageBoxW(h, L"계산 실패", L"Motion Vectors", MB_ICONERROR); return; }
...
### 예제 A) **순수 평행 이동(합성 데이터)**
- **상황**: `prev` 를 오른쪽으로 **+4**, 아래로 **+2**만큼 시프트한 `cur` 를 생성.
- **설정**: `B=16, R=8, Metric=SAD, Half-pel=Off`
- **결과**: 대부분의 그리드에서 \((+4, +2)\) 검출. 경계 근처는 탐색 후보 부족으로 (0,0) 또는 근사치.

```cpp
// synth_shift_test.cpp (요지)
IppDib prev=MakeChecker(320,240);     // 임의 패턴
IppDib cur =ShiftImage(prev, +4,+2);  // 정수 이동
// ...
// 대화상자에서 prev=prev, cur=cur 선택 후 Run
```

### 예제 B) **보행 영상 (실사)**
- **상황**: 보행자 영상의 연속 두 프레임.
- **설정**: `B=16, R=8, Metric=SAD, Half-pel=On, VisStep=2, Scale=2.0`
- **결과**: 다리/팔 주변에서 큰 벡터, 정적 배경은 (0,0)에 가까움. 하프펠로 미세 이동이 부드럽게 표현.

### 예제 C) **조명 변화 / 질감 부족**
- **상황**: 실내에서 조명이 깜빡여 전체 밝기 변화.
- **대응**: `SSD` 보다 `SAD` 선호. `rejectThr` 를 적당히 높여 애매한 블록은 (0,0) 으로 거부.

---

## 6. 성능 & 품질 팁

- **B / R 선택**: 큰 B는 안정(노이즈 평균화) vs. 세부 움직임 소실. R은 충분해야 하지만 \((2R+1)^2\) 로 비용 증가.
- **Early Break**: SAD/SSD 누적이 **현재 best** 를 초과하면 내부 루프 중단(미세 최적화 가능).
- **피라미드(멀티스케일)**: Frame을 다운샘플링해 coarse-to-fine 검색 → R을 줄이되 전역 이동에 강인.
- **후처리**: MV 필드를 3×3 **미디언** 으로 한 번 정리 → 외란 제거.
- **하프펠 주의**: 경계에서 보간 샘플이 부족하지 않게 **조금 넉넉한 유효성 검사**(위 코드에서 +2 여유) 필요.
...
- **SAD / SSD 단위**
  ...
  \]
  (입력은 \([0,1]\) 정규화 값이므로 최대 차이는 1)
  `rejectThr` 는 예: \(B=16\) ⇒ \(B^2=256\). 정합이 나쁜 블록만 거부하려면 `SAD > 50~80` 수준부터 시도.

- **하프펠 후보**: 8방향 + 중심 1개 = 9개. 더 고급으로는 2차 곡면 근사(패러볼라 보간)도 가능.

---

## 8. 요약
- 블록 매칭 모션 추정의 핵심은 **비용함수**(SAD/SSD)와 **탐색 전략**입니다.
- 본 절의 **`IppFullSearch`** 는
  - 정수 전수 탐색 + (옵션) **하프펠 정밀화**
  - **거부 임계치**, **오버레이 시각화(화살표)**
  - 간단한 **메뉴/대화 상자** 로 바로 실습 가능한 형태
- 이 모듈은 추후 **피라미드 검색**, **ZNCC/비트코스트** 등으로 확장해도 인터페이스가 유지됩니다.
- 다음 단계: **AviView**에서 프레임 \(t{-}1\) ↔ \(t\) 자동 공급(재생 도중 “현재↔이전” 모션 필드 갱신)으로 **실시간 MV 보기**를 구현해보세요. 🚀
