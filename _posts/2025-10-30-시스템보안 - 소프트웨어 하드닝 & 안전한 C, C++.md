...
category: 시스템보안
---
# 12. 소프트웨어 하드닝 & 안전한 C/C++

## 12.1 언어 레벨: UB, 정수/포인터 산술, 메모리 모델

### 12.1.1 **UB(Undefined Behavior)** 핵심 맥락
...
### 12.1.6 **계약(Contract)·사전조건/사후조건·어설션**
...
## 12.2 빌드 하드닝: Stack Protector, FORTIFY, Control-Flow 보호

> 목적: **버그가 있어도** 악용 난이도를 올리고 **조기 탐지**되도록 **툴체인·링커·OS 기능**을 적극 사용.

### 12.2.1 **리눅스/유닉스 계열** 컴파일·링크 플래그(Clang/GCC)
...
### 12.2.4 **CFI/IFC/Return Address 보호**
...
## 12.3 Sanitizer(ASan/UBSan/MSan), fuzzing(libFuzzer/AFL) 파이프라인
...
### 12.3.3 **Fuzzing**: libFuzzer (in-process, coverage-guided)
...
### 12.3.4 **AFL++/honggfuzz** (fork-server 계열)
...
### 12.3.5 **커버리지/회귀**: crash 최소화 & CI 연동
...
### 12.3.6 **현실적 파이프라인**: Dev/PR/Release 단계
