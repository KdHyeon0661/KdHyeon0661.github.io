...
# 2) 실습 환경: JDK, Gradle vs Maven, IDE, Docker, DB
...
## 2.5 Hello, Spring Boot 빠른 예제

### 2.5.1 프로젝트 생성(Gradle/Kotlin DSL 예시)
...
## 2.6 Docker 이미지 빌드 (세 가지 전략)

### 2.6.1 멀티스테이지 Dockerfile
...
# 2) Run stage (distroless or slim)
...
## 2.7 DB 연결(JPA + PostgreSQL)

### 2.7.1 엔티티/리포지토리/컨트롤러 샘플
...
## 2.8 테스트: Testcontainers로 실DB 검증
...
## 2.11 프로파일 전략(dev/test/docker/prod)
