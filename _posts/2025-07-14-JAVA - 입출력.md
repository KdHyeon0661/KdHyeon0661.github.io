...
## 2. `BufferedReader` — 빠른 줄 입력 & 직접 파싱
...
## 6. 출력 짝궁: `BufferedWriter` / `PrintWriter`
...
### 8.2 빠른 합계: `BufferedReader` + `StringTokenizer`
```java
import java.io.*;
import java.util.*;
...
### 8.5 커스텀 초고속 FastScanner (바이트 파서)
```java
import java.io.*;
