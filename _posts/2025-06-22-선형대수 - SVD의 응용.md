...
## 1) 🖼️ 이미지 압축(Image Compression) — SVD로 간단하고 강력하게
...
### 1.4 PyTorch 실전 코드 (흑백/컬러 지원, 품질 지표 포함)
...
# 2) Color (RGB, 채널별 다른 keep_ratio 허용)
...
## 2) 🎯 추천 시스템(Recommender Systems) — 잠재요인으로 예측하기
...
### 2.3 사용자 기준 중심화 + Truncated SVD (프로토타입)
...
### 2.6 암묵적 피드백(Implicit) — 클릭/뷰/구매 로그
