...
category: Elixir
---
# 12장. Elixir 예외 처리와 예외 기반 설계 — `raise`, `try/rescue`, `exit`, `defexception`, 뱅(!) 관례까지

## 12.4 예외 발생시키기 (Raising exceptions)

### 12.4.1 `raise/1,2` 기본 — 예외 객체와 메시지

가장 기본적인 예외 발생 방식은 `raise/1`, `raise/2` 이다.

```elixir
# 1) 문자열만 넘기면 RuntimeError
raise "boom"
...
#### 12.4.1.1 예외는 값이다 (Exception struct)
...
### 12.4.2 `try/rescue/after` — 최소만 사용하기
...
### 12.4.3 `throw/catch` vs 예외 — 비국소 반환의 유산
...
### 12.4.4 `exit`와 프로세스 종료 — 동시성 맥락에서의 실패
...
### 12.4.8 예외 사례 모음 — 어떤 예외가 언제 발생하는가
...
## 12.5 예외를 이용해 설계하기 (Designing with exceptions)

지금까지는 “예외를 **어떻게 발생시키고, 잡는지**”를 봤다.  
이제는 “예외를 **어디까지 허용하고, 어디서 값으로 내려야 하는지**”를 설계 관점에서 본다.

### 12.5.1 실패 모델 분리: 예상 가능한 실패 vs 실패해서는 안 되는 상태
...
### 12.5.3 `with`와 예외 전략 — 한 층에서는 한 모델만
...
### 12.5.4 동시성과 예외 — “Let it crash” + Supervisor
...
### 12.5.5 `after`/리소스 안전 패턴 — 예외를 신경쓰지 않는 코드 만들기
...
### 12.5.6 예외 로깅/관찰: Logger + Exception.format
...
## 12.6 더 적은 것들로 더 큰 효과를 (Less is more)
