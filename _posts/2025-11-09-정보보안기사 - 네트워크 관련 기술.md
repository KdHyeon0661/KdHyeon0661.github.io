...
### 1.2 Linux nftables/iptables 예시
...
### 1.3 NAT와 ICMP/PMTUD·알고 있어야 할 문제들
...
## 2. 프록시/로드밸런서 — L4 vs L7, 알고리즘·보건검사·TLS 처리
...
## 3. QoS/트래픽 제어 — DSCP·큐잉·토큰 버킷·AQM
...
## 4. IPv6 핵심(IPv4 대비 개요) — 주소·NDP·RA/RS·프라이버시
...
### 4.2 NDP(Neighbor Discovery Protocol)
...
## 5. 터널링/오버레이 — GRE/IP-in-IP/VXLAN(개요)와 오버헤드
...
## 7. 정책 기반 라우팅(PBR)·다중 경로(ECMP)

### 7.1 `ip rule`/`ip route` (Linux)
```bash
...
## 9. 가시성/운영 — SPAN/TAP·NetFlow/IPFIX·패킷 캡처
...
### 실습 D — VRRP(keepalived) 게이트웨이 페일오버
