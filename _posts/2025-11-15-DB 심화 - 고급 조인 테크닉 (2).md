...
### (2) 브리지 도입 — “작은 키 집합”부터 만든 후 최종 결합
```sql
...
## 1.2 패턴 B — “기간/버전 브리지”로 선분조인 후보 축소
...
## 2.2 Flashback Query로 **SCN/시간** 기준 조회
...
# 3) 선분이력 조인 (Interval/Temporal Join)

> **선분(시작~끝) 구간**을 미리 만들어두고, 포인트 테이블과 **비등치**로 매칭.  
> (가격, 등급, 담당자, 계약 등)

## 3.1 선분 만들기(PRICE_EVT → PRICE_SEG)
...
## 4.3 “후보 선분 브리지”로 후보 줄이기 (앞 1.2 참조)
...
## 5.2 오른쪽에는 있는데 왼쪽에 **매칭 주문이 없는** 상품 (Right-ANTI)
...
## 5.3 선분 매칭 실패: 포인트가 어떤 구간에도 속하지 않음
