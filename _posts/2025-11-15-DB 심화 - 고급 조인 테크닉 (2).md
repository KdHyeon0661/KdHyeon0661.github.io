...
## 징검다리(Bridge / Stepping-Stone) 테이블 조인
...
## 브리지 패턴 A — “조건 요약 브리지”로 4-way 대형 조인 축소
...
### 변형 2 — 브리지 생성에서 `DISTINCT`로 중복 제거
...
### 가장 기본 패턴: LATERAL(APPLY) + Stopkey
...
### 변형 1 — ROW_NUMBER() 기반 Top-1 per key
...
### 변형 2 — 분석함수 `KEEP (DENSE_RANK LAST)`
...
## “조인에 실패한 레코드” 읽기(미매칭 탐지)

### ANTI-JOIN (왼쪽은 있는데 오른쪽에 없음)
