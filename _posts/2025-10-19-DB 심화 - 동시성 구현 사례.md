...
category: DB 심화
---
# 동시성 구현 사례 — **일련번호 채번(Serial/Sequence) 동시성 높이기**, **선분이력(유효기간) 정합성 유지**
(Oracle 중심·실전 설계/코드/운영 체크리스트)
...
> 현실 법칙: **“진짜 gap-less”** & **“글로벌 시간순”** & **“초고처리량”**을 **동시에** 달성하기는 매우 어렵다.
> 보통 **2가지를 택하고 1가지는 포기**한다. (예: *순서성+처리량*을 택하면 *gap-less* 포기)

---

### 1.2 기본: Oracle `SEQUENCE` 성능 최적화
...
### 1.3 RAC/분산에서 더 빠르게: “스트라이프드 시퀀스(Striped Sequences)”
...
### 1.4 Hi/Lo(대역 할당) 패턴 — 앱 측 대량 발급

> 아이디어: DB는 **큰 단위 블록(Hi)** 만 부여하고, 앱은 그 안에서 **Lo**를 자체 증가.
> DB round-trip을 **블록 당 1회**로 줄여 **폭증 처리량** 달성.
...
## 2. 선분이력 정합성 유지 (유효기간 이력, Non-Overlap/Contiguity)
...
- **포인트**
  ...
  - **동일 pid에 대한 병행 요청**이 많으면 큐잉(아래 §2.6)도 고려.

---

### 2.5 수정/중도 삽입(스플릿) — 중간 시점 교체
...
### 2.6 고동시성 큐잉 — `SKIP LOCKED`로 병행성 높이기
...
### 4.1 일련번호
1) **SEQUENCE**: `CACHE`↑, `NOORDER` (RAC), **스트라이프**(여러 시퀀스), **Hi/Lo**(앱 대역).
2) **인덱스 핫스팟**: Reverse Key/Hash 파티션/샤딩.
3) **갭 요구**: 진정한 gap-less → **단일행 잠금 카운터** + **낮은 TPS** + **장애/재시도 설계**.

### 4.2 선분이력
1) **pid 직렬화**: 최근 행 `FOR UPDATE`(또는 센티넬 행) — **겹침 금지**의 기초.
2) **검증 쿼리**: 겹침 조건(De Morgan) 정확히, 인덱스로 빠르게.
3) **연속성**: 정책(자동 보정 vs 거부)을 명시화.
4) **스플릿**: 한 트랜잭션에서 분할·삽입·보정까지 일괄.
5) **대량 동시성**: `SKIP LOCKED` 큐로 수평 확장.
6) **정기 헬스체크**: 야간 배치로 겹침/구멍 리포트 → 자동 정비 도구.
...
- **일련번호 동시성**은 **시퀀스 튜닝(CACHE/NOORDER)**, **스트라이핑**, **Hi/Lo** 같은 **감산적 동기화 전략**으로 **DB 경합을 멀리** 보낸다. 인덱스 핫스팟은 **Reverse/파티션**으로 분산.
- **선분이력 정합성**은 **pid 단위 직렬화 잠금 + 겹침 검증 + (선택) 연속성 보정**으로 데이터 레벨에서 **확실히** 보장한다. 병행 요청은 **SKIP LOCKED 큐**로 수평 확장.
- **정합성과 처리량**은 늘 트레이드오프다. 요구를 **수학/운영 비용**과 함께 정의하고, **현실적 최적점**(갭 허용/순서 해석/큐/재시도)을 선택하라.

> 한 줄 결론
> - **채번**: “**잠그지 말고** 캐시·분산·대역으로 **미리 벌려라**.”
> - **선분이력**: “**같은 개체는 한 줄로 세우고**(잠금), **겹침은 쿼리로 끊어라**(검증).”
