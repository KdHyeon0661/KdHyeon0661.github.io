...
## 표현(Representation): 히트맵·오프셋·PAF

### 히트맵(Heatmap) — 가장 널리 쓰이는 키포인트 표현
...
### 오프셋/리파인(Offset/Refine) 지도(선택)

- 히트맵 해상도 한계 보완: 각 픽셀에서 키포인트까지의 **오프셋 벡터** \(\Delta x,\Delta y\)를 추가 회귀하여 정밀도 향상.

### PAF(Part Affinity Field) — 연결 토포로지(Topology) 표현
...
## 파이프라인 개관: Top-down vs Bottom-up
...
## 간단 네트워크: Top-down SimpleBaseline 헤드(Pytorch)
...
## 간단 네트워크: Bottom-up(히트맵+PAF) 미니 헤드
...
## 소형 E2E 스니펫(Top-down: 검출→크롭→히트맵→좌표 복원, 개념형)

```python
# 사람 박스 detector로 얻었다고 가정: bboxes = [[x,y,w,h], ...]
# 각 박스 크롭(+패딩) → 정규화 → model → heatmap
# heatmap_nms → 키포인트 좌표 (크롭 좌표계) → 원본 복원
