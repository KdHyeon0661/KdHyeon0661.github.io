...
### A.1.5 예외 재발생(re-raise) (초안 유지 + “스택트레이스가 왜 중요한가”)
...
#### (C) GenServer에서 exit 대신 `{:stop, reason, ...}`
...
#### (A) Task.await/2는 실패를 exit로 전달할 수 있다
...
#### (B) Supervisor 아래 Task라면 실패를 “복구 정책”으로 흡수
...
#### (B) exit/throw는 `catch_exit`, `catch_throw`로 테스트
