...
## 대화 상자 — **차수(n_max), 가중치, 중심/반지름, 정규화, 오버레이**

`ZernikeDialog.hpp`, `resource.h`, `ImageTool.rc`

### 리소스 ID — `resource.h` (추가)
...
## 대화 상자 — **템플릿 파일 선택 · 메서드/탑K/NMS/멀티스케일/임계값**
...
### 대화 상자 로직 — `TemplateMatchDialog.hpp`
