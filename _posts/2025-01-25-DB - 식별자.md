...
## 1. 수학적 관점: 키와 함수적 종속(Functional Dependency)
...
### 4.2 UUID (v4 랜덤, v1 시간+노드, v7 시간정렬)
- **v4**: 균등 랜덤. 인덱스 분산(지역성↓).  
...
### 4.3 ULID (time-ordered, Crockford base32)
- 시간 정렬 + 비교적 짧은 문자열 표현.  
- 사전 순 정렬이 시간 순서와 일치.

### 4.4 Snowflake 스타일(64비트, 시간·노드·시퀀스)
...
## 8. 참조 무결성: ON DELETE/UPDATE 정책의 의미
...
## 15. 마이그레이션: PK 교체/분리의 안전 절차

### 15.1 자연키 PK → 인조키 PK 전환
