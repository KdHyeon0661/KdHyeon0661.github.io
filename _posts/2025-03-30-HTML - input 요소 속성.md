...
## 제출(submit)과 전송(payload)의 기본 원리 한 장 요약
...
## HTML5/현대 브라우저 확장 속성 (입력 UX/검증/연결)
...
### `spellcheck` / `autocapitalize` (텍스트 교정)
...
### 버튼 전용 `formaction` / `formmethod` / `formenctype` / `formnovalidate` / `formtarget`
...
## 타입별 특화 속성 (파일/이미지/비밀번호 등)

### 파일 업로드: `type="file"` — `accept`/`multiple`/`capture`
