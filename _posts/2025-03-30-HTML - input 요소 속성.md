...
category: HTML
---
# 🧾 HTML & HTML5의 `<input>` 요소 속성 완전 정리 (심화 확장판)

초안의 구조(기본 속성 → HTML5 확장 → 유효성 검사 → 실무 팁)를 유지하되, **접근성(a11y)**, **보안**, **모바일 UX**, **제출 동작 제어(form\*)**, **Constraint Validation API**, **자동완성 토큰**, **국소화/로케일 이슈**, **파일 업로드/카메라 속성**, **CSS 의사클래스**까지 보강했다. 모든 예제는 **복붙 후 즉시 실행** 가능하도록 최소한의 맥락을 포함한다.
