...
category: CSS
---
# 박스 모델(Box Model) 완벽 이해하기 — 개념·수식·엣지케이스·실전 패턴

아래 글은 당신이 준 초안(정의, 4요소, 계산 예시, box-sizing, 마진 겹침, 실무 팁)을 **핵심 유지**하면서,  
현대 CSS에서 자주 부딪히는 **논리 속성, 글쓰기 방향, 스크롤바/오버플로, 인라인 박스, 비율 박스, 배경 페인트, BFC(flow-root), gap** 등  
**실전 포인트와 엣지케이스**까지 빠짐없이 확장한 가이드입니다.  
...
## 5) 마진 겹침(Margin Collapse) — 반드시 아는 포인트
...
## 6) 논리 속성(Logical Properties)로 국제화 대응
...
## 7) 퍼센트 padding과 비율 박스(aspect ratio)
...
## 9) outline, box-shadow, filter — 크기에 영향 없음
...
## 12) gap(Flex/Grid) vs margin — 간격의 현대적 기준
