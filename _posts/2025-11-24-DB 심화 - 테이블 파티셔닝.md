...
- **Partition Pruning(파티션 프루닝)**  
  ...
  (프루닝은 정적/동적/서브쿼리/조인 기반으로 다양하게 동작한다. :contentReference[oaicite:0]{index=0})

- **Partition-Wise Join(파티션-와이즈 조인)**  
  양쪽 테이블이 같은 키/경계/개수로 파티셔닝 돼 있으면  
  **파티션끼리 1:1로 조인**해 **재분배·전역 해시빌드 없이 병렬 조인**이 가능해진다. :contentReference[oaicite:1]{index=1}

- **관리/통계 분할 효과**  
  파티션별 통계를 가지므로 **카디널리티 추정이 더 안정**되며,  
  증분 통계(Incremental)로 **새 파티션만 재수집**할 수 있다.

### 0.2 동시성/확장성(물리/병렬 관점)
...
- Heat Map/ADO(Automatic Data Optimization)는 **파티션 단위로 자동 압축/이관 정책**을 실행한다. :contentReference[oaicite:2]{index=2}
...
- **GLOBAL PARTITIONED INDEX**  
  **인덱스도 파티셔닝한 전역 인덱스**.  
  테이블 프루닝이 안 돼도 **인덱스 레벨 프루닝**을 할 수 있다. :contentReference[oaicite:3]{index=3}
...
아래 유형들은 옵티마이저가 실제로 구분해서 사용한다. :contentReference[oaicite:4]{index=4}

### 2.1 Static Pruning(정적 프루닝)
파싱 시점에 파티션 범위가 확정된다.
...
### 2.2 Dynamic Pruning(동적 프루닝)
바인드 변수 때문에 파싱 시점엔 범위를 몰라서, 실행 시점에 프루닝한다. :contentReference[oaicite:5]{index=5}
...
### 2.4 Join Pruning(조인 프루닝)
조인 상대의 조건으로 파티션을 제거한다. :contentReference[oaicite:6]{index=6}
...
### 3.2 월별 RANGE 파티션 예제(US/EU 리테일 매출)
...
- 스타 스키마/팩트-디멘전에서 관리/조인 최적화에 매우 유리. :contentReference[oaicite:7]{index=7}

### 8.2 예제: 주문(부모 RANGE) ↔ 주문상세(자식 REFERENCE)
...
- **Full P-Wise Join**  
  ...
  **파티션끼리 1:1 조인**한다. :contentReference[oaicite:8]{index=8}

- **Partial P-Wise Join**  
  한쪽만 파티셔닝되었거나 경계가 일부만 맞을 때 **부분 혜택**.

### 9.2 예제: 월×cust_hash 동일 구조의 2테이블 조인
...
### 10.2 Global(또는 Global Partitioned) Index가 필요한 상황
...
### 11.2 Incremental Global Stats(대용량 필수)
```sql
BEGIN
...
### 12.3 Heat Map/ADO 기반 자동 압축·이관
Heat Map은 파티션/세그먼트의 사용 패턴을 기록하고,  
ADO는 이를 기반으로 **파티션 단위 자동 압축/스토리지 티어링 정책**을 실행한다. :contentReference[oaicite:9]{index=9}
