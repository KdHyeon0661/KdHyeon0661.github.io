...
## 10. 실전 리팩토링 예(스텝 바이 스텝)

### 10.1 거대한 switch 제거 → Factory Method
나쁜 예:
```csharp
...
### 10.3 템플릿 복제 → Prototype
- 대형 객체를 표준 템플릿으로 보관(레지스트리) → `.clone()` → 부분 변경.

---

## 11. 패턴별 추가 예제(요청 잦은 도메인)

### 11.1 Abstract Factory(데이터베이스 제품군)
```
IDbFactory
...
### 15.1 이미지 인코더(Factory Method + 등록)
```python
class Encoder:
