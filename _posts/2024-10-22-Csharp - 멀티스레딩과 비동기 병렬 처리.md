...
## 2. ThreadPool & Task — 기본 실행 단위

### 2.1 Task.Run 으로 워커 큐에 던지기 (CPU-bound)
...
### 6.4 ReaderWriterLockSlim — 읽기 많고 쓰기 적을 때
```csharp
...
### 6.5 SemaphoreSlim — 동시 허용량 제한(스로틀)
```csharp
...
## 9. UI 스레드 — SynchronizationContext/Dispatcher
...
### 13.1 CPU-bound 배치 처리 — Parallel + 파티션 + 결과 수집(스레드 안전)
```csharp
...
### 13.2 I/O-bound 동시성 — SemaphoreSlim으로 동시 요청 제한
```csharp
