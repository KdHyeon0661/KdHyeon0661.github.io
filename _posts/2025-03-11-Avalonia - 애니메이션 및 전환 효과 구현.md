...
## 1. 페이지 전환: ContentControl + PageSlide/CrossFade
...
## 2. 재사용 가능한 전환 호스트(TransitionHost) 컴포넌트

> 팀/프로젝트 규모가 커질수록 **전환 설정을 한 곳**에서 관리하는 것이 중요하다.

### 2.1 코드: TransitionHost
...
## 3. 개별 컨트롤 애니메이션: Transitions vs Animation(KeyFrame)

### 3.1 Property Transitions(속성 전환)
...
### 3.2 키프레임 애니메이션(고급: `Avalonia.Animation`)
...
## 5. 리스트 항목 애니메이션(ItemsControl/TreeDataGrid 등)
