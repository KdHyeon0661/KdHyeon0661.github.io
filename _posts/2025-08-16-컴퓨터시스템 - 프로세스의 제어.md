...
## 3. 생성과 실행 이미지 교체: `fork/exec`, `posix_spawn`, `clone3`

### 3.1 표준 패턴: `fork()` → (자식에서) `execve()`
```c
#include <unistd.h>
#include <sys/wait.h>
#include <stdio.h>
...
## 7. 문맥 전환(Context Switch)과 스케줄링 제어
...
## 11. 현대 신기술: pidfd/clone3/io_uring
...
## 12. 안전한 스폰/슈퍼비전 패턴(누수·경쟁·좀비 방지)

### 12.1 부모가 파이프/시그널로 준비 확인 후 exec
```c
...
### 12.2 PR_SET_PDEATHSIG: 부모 죽으면 자식도 종료
```c
#include <sys/prctl.h>
prctl(PR_SET_PDEATHSIG, SIGTERM);
```

### 12.3 signalfd + event loop 로 수거/종료
...
## 15. 관찰/추적/디버깅: /proc, lsof, strace, perf, bpf
...
## 16. 보안·무결성: setuid, capabilities, seccomp, 환경 정리
...
### 17.2 “서비스를 격리해 CPU/메모리 제한 걸고 시작”
```bash
