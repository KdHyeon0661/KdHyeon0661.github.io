...
### 3.3 Amdahl의 법칙(부분 최적화의 전체 영향)
부분 비율 \(p\), 해당 부분 가속비 \(s\):
$$
\mathrm{Speedup}_{overall}=\frac{1}{(1-p)+\frac{p}{s}}.
$$

### 3.4 Roofline(메모리 vs 연산 한계)
$$
...
## 7. C 타이밍 기본기 — `clock_gettime` 해치(Skeleton)
...
## 10. 하드웨어 카운터 연동(perf_event_open 간단 스니펫)
...
## 15. 보고서/README에 반드시 포함할 것(재현성 체크리스트)
...
## 23. 부록 — 여러 번 실행하고 중앙값/표준편차/p95 출력(C)
