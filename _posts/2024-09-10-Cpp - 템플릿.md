...
### 정수/열거/포인터/참조/클래스 형식의 값 인수 (C++20 확대)
...
### `fixed_string`로 문자열 리터럴 길이/내용을 형식 차원에서 사용
...
## 가변 템플릿(Variadic Templates) & 폴드식(Fold Expression)
...
## CTAD (Class Template Argument Deduction) & Deduction Guides
...
## 전달 참조(Forwarding Reference) & 완벽 전달(Perfect Forwarding)
...
## 인스턴스화/ODR/링커 — 헤더 전개, 명시적 인스턴스화, extern template
...
## CRTP (Curiously Recurring Template Pattern) / 정책 기반 설계
