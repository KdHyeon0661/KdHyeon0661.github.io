...
## 4. **방법 A — KMS ReEncrypt로 EDK만 교체 (권장)**

### 4.1 CLI: 로컬 EDK 파일 교체
...
### 4.2 Python(boto3): S3 객체 메타데이터의 **EDK 재암호화 후 원자적 갱신**
...
### 4.3 Node.js(AWS SDK v3): DB(예: DynamoDB)에 저장한 EDK 컬럼 교체
...
## 5. **방법 B — S3 SSE-KMS 객체 키 변경 (Copy / Batch)**

### 5.1 단일/소량: `copy-object`
...
## 6. **방법 C — 클라이언트 복호화 → 재암호화** (최후의 수단)

### 6.1 Python 예시 (로컬에서 AES-GCM 복호화 → 새 키로 EDK 생성)
...
### 9.1 Python: **대량 S3 EDK 재암호화** (병렬 + 재시도)
...
## 11. **Step Functions로 안전한 일괄 오케스트레이션** (옵션)
...
## 14. **CloudFormation/CDK 스니펫** (키/권한 배포)

### 14.1 CloudFormation — 대상 CMK
...
### 14.2 CDK(TypeScript) — S3 기본 암호화에 새 키 지정
