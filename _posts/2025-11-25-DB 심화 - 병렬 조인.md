...
플랜의 `PX SEND <type>` / `PX RECEIVE`가 **TQ를 명시적으로 보여주는 자리**이며, 여기서 `<type>`이 곧 **분배 방식**이다.

`IN-OUT`은 한 오퍼레이션의 입력/출력이 **직렬(S)**인지 **병렬(P)**인지, 그리고 **병렬 간 재분배(P→P)**가 있는지를 나타낸다.
특히 `PCWP/PCWC`는 **부모-자식 오퍼레이션이 같은 PX 집합에서 “붙어서” 실행**됨을 뜻한다(=TQ를 줄여 통신 비용 감소).
...
**동일 파티션 쌍끼리만** 조인하여 **재분배(P→P)를 없애는** 조인이다.

**요건**
1) 조인 조건이 **파티션 키에 대한 동등 조인(equi-join)**
2) **HASH-HASH / RANGE-RANGE / LIST-LIST / RANGE-INTERVAL 등** “정렬 가능한” 동일 구조
...
Oracle 문서 기준 FPWJ의 **플랜 시그니처**는 다음이 핵심이다.
...
### “상위 방식이 달라도” FPWJ를 만드는 고급 패턴

Oracle은 **파티션-서브파티션** 수준이 맞으면
상위 방식이 달라도 **Full PWJ**를 만들 수 있다고 설명한다.
...
PPWJ는 **병렬에서만 수행 가능**하다고 Oracle이 명시한다.
...
- `V$PQ_TQSTAT`에서 TQ별 `bytes/num_rows`가 균등한지
...
플랜에서 반드시 `PX SEND BROADCAST`가 나타난다.
...
| 상황 | 최우선 선택 | 이유 |
|---|---|---|
| 양쪽 모두 조인키로 동일 파티션 가능 | **Full PWJ** | P→P 제거, 선형 확장  |
