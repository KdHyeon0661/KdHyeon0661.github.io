...
category: DB 심화
---
# Oracle 병렬 조인(Parallel Join) 완전 정복
...
# 1) Full Partition-Wise Join (FPWJ) — **둘 다 같은 기준으로 파티셔닝**
...
# 2) Partial Partition-Wise Join (PPWJ) — **한쪽만 파티셔닝**
...
# 3) Dynamic Partitioning(동적 파티셔닝) — **런타임 재분배로 가상 파티션 정렬**
...
# 5) RANGE 파티션 vs HASH 파티션 혼합 사례

## Range(날짜) 파티션 × Hash(cust) 파티션
...
# 6) IN-OUT, PX SEND/RECEIVE, TQ로 **데이터 흐름 읽는 법**
