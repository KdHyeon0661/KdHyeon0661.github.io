...
## 6. 수명(Lifetime)과 댕글링(dangling) 방지
...
## 7. 참조의 특수 규칙 — 임시/수명 연장, 참조 붕괴

### 7.1 const lvalue 참조와 임시
...
### 7.3 참조 붕괴 규칙 (Reference Collapsing)
$$
T\&  \&  \Rightarrow T\&,\quad
T\&  \&\&\Rightarrow T\&,\quad
T\&\&\& \Rightarrow T\&,\quad
...
## 8. 이동语义와 완전 전달(Perfect Forwarding)
...
### 9.3 함수 인자에서 `T*` vs `T(&)[N]`
- `T*`는 길이 정보가 **사라짐** → 별도 길이 전달 필요.
- 참조 시그니처(`T(&)[N]`)는 **컴파일 타임 길이 보존**.

---

## 10. 동적 메모리와 스마트 포인터 대비

### 10.1 `new/delete` 직접 사용 (교육용)
```cpp
int* p = new int(42);
delete p;
```
...
## 11. 함수 오버로드와 참조 — lvalue/rvalue 구분
...
## 15. 예제 모음 (컴파일 가능한 스니펫)

### 15.1 포인터/참조/값 — 마이크로벤치 스케치
```cpp
#include <vector>
#include <numeric>
