...
### 13.2 HTTP 응답 분할/CRLF 주입(Response Splitting)
...
### 13.6 Prototype Pollution(Node/브라우저)
...
### 13.10 HTTP Request Smuggling(프록시/백엔드 불일치)
...
### 13.19 ESI/에지 템플릿 인젝션

- **방어**: ESI 비활성 또는 **정적 경로만**, 사용자 입력 포함 금지.

---

### 13.20 쿠키 토싱(Cookie Tossing)

- **방어**: 도메인/경로 정책 정합, **Set-Cookie 충돌 모니터링**.

---

### 13.21 DOM 기반 취약점(간단 리마인드)
...
## 14. 개발 보안관리(Secure SDLC·SCA/SAST/DAST·비밀관리·CI/CD·서명/공급망·정책/코드리뷰)

### 14.1 보안 내재화 SDLC(개념 그림)
...
### 14.5 CI: SAST/SCA/Secret Scan/DAST 파이프라인
...
### 14.7 비밀관리(예: AWS Secrets Manager)
...
### 14.8 의존성 위생(Renovate/Bundler-Audit 등)
...
### 14.12 정책-코드(Policy as Code) 예: OPA/Rego
