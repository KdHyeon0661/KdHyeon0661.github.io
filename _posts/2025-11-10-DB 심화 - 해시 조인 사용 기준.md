...
> 꼭 기억할 것  
> - 해시 조인은 **“작은 BUILD를 메모리에 올리고 큰 PROBE를 한 번 훑는”** 대량 처리 조인이다. :contentReference[oaicite:0]{index=0}  
> - 성능의 80%는 **BUILD 크기/키 분포/워크에어리어(메모리)**에서 결정된다. :contentReference[oaicite:1]{index=1}  
...
- 이 해시 테이블은 **PGA(workarea, hash area)**에 생성된다. :contentReference[oaicite:2]{index=2}  
...
- PROBE 입력(큰 쪽)을 **순차 스캔**하며:
  ...
  2) 버킷 체인에서 실제 키 비교  
  3) 매칭 결과 출력  
- 이상적인 경우 PROBE는 **딱 한 번**만 읽는다. :contentReference[oaicite:3]{index=3}

### 1.3 Hash Workarea(메모리)와 종류

Oracle은 실행 시점에 “이 해시 조인이 메모리에 다 들어가는가?”를 보고 다음 세 부류로 동작한다. :contentReference[oaicite:4]{index=4}
...
- **Multi-pass**: “PROBE가 **여러 번 반복**”  
  → 멀티패스가 나타나면 해시 조인은 사실상 “스필 폭탄”이다. :contentReference[oaicite:5]{index=5}

---

## 2) BUILD 입력이 Hash Area를 초과할 때 — 하이브리드/멀티패스의 실제 동작

### 2.1 왜 파티셔닝(Grace Hash Join)이 필요한가?
...
그래서 Oracle은 **BUILD/PROBE를 동일 해시로 파티셔닝**하는 전략을 쓴다. :contentReference[oaicite:6]{index=6}

### 2.2 하이브리드(One-pass) 단계별 흐름
...
- 하지만 **각 파티션은 1번만** 재읽으므로 “one-pass”. :contentReference[oaicite:7]{index=7}

### 2.3 멀티패스(Multi-pass)의 흐름
...
- **멀티패스 = PROBE 다회 재스캔**  
- 성능은 TEMP 대역폭에 종속된다. :contentReference[oaicite:8]{index=8}

---

## 3) 스필을 “눈으로” 확인하는 방법(실측)

### 3.1 DBMS_XPLAN MEMSTATS / IOSTATS
...
- IOSTATS에서
  - `direct path write temp`, `direct path read temp`가 늘면  
    BUILD/PROBE 스필이 실제로 발생한 것. :contentReference[oaicite:9]{index=9}

### 3.2 V$SQL_WORKAREA(_ACTIVE)

해시/소트 워크에어리어의 실행 결과를 직접 본다. :contentReference[oaicite:10]{index=10}
...
### 4.1 (A) BUILD 축소: 스필이 줄거나 사라지는 케이스
...
- 19c 이후 Optimizer는 **SQL Plan Directive + Dynamic Statistics**로  
  **데이터 스큐 조인을 자동 보정하려는 기능**을 갖는다. :contentReference[oaicite:11]{index=11}

---

## 7) 스큐/스필 완화 전략 (실전)

### 7.1 BUILD 입력 축소(가장 확실)
...
### 7.2 BUILD/PROBE 뒤집기(SWAP_JOIN_INPUTS)
...
### 7.3 히스토그램/확장 통계/동적 통계

- 스큐 컬럼에 히스토그램  
- 결합 상관이 있으면 확장 통계  
- 반복 오판이면 Dynamic Statistics 수용

이 3개가 “해시 조인 설계 시 빌드 크기 예측”을 안정화한다. :contentReference[oaicite:12]{index=12}

### 7.4 병렬에서의 Bloom Filter(Join Filter)
...
- Bloom 효율은 **카디널리티 정확도**에 의존하므로  
  스큐 오판이 줄어들수록 Bloom도 잘 먹힌다. :contentReference[oaicite:13]{index=13}

---

## 8) 해시 조인을 쓰는 기준 — NL/SMJ와의 분기점

Oracle 공식 튜닝 가이드의 일반 원칙은 다음과 같다. :contentReference[oaicite:14]{index=14}
...
- BUILD 입력이 PGA(해시 workarea)를 넘으면 Oracle은  
  **Grace/Hybrid(One-pass) 해시 조인**으로 BUILD/PROBE를 파티셔닝해 TEMP로 스필한다.  
  이때 PROBE는 **파티션 단위로 재읽기**된다. :contentReference[oaicite:15]{index=15}
- 특정 파티션이 또 커지면 **Multi-pass**로 재파티셔닝이 반복되어  
  PROBE가 여러 번 읽히고 TEMP I/O가 폭증한다. :contentReference[oaicite:16]{index=16}
- 해시 키 스큐(중복/편중)는  
  **버킷/파티션 불균형 → 체인 길이 증가(CPU 폭증) → 특정 파티션 스필 집중(TEMP 폭증) → 병렬 로드 임밸런스**를 만든다.  
  스큐 조인의 자동 보정은 **동적 통계/SQL Plan Directive**로 일부 개선되지만,  
  **근본은 통계와 SQL 구조 교정**이다. :contentReference[oaicite:17]{index=17}
- 해시 조인의 성패는  
