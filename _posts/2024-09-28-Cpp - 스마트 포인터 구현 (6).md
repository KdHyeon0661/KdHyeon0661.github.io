...
## Catch2 단위 테스트 — 기본 기능 (파일: `tests/test_basic_catch.cpp`)
...
## `enable_shared_from_this` / `weak` (파일: `tests/test_enable_and_weak.cpp`)
...
## 스레드 스트레스 (파일: `tests/test_thread_stress.cpp`)
...
## 선형 퍼즈(랜덤 시퀀스) (파일: `tests/test_fuzz_linear.cpp`)
...
## GoogleTest 스모크 (파일: `tests/test_gtest_equiv.cpp`)
...
## CI 매트릭스 (GitHub Actions 예시: `.github/workflows/ci.yml`)
