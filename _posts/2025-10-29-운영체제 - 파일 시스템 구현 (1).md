...
### 14.1.5 무결성/일관성: 저널링 vs COW vs 로깅
...
### 14.2.3 쓰기(Write) 경로 & 지연쓰기(writeback)
...
### 14.2.4 fsync()/fdatasync() 알고리즘(요약)
...
### 14.2.8 (토이) 파일 생성/쓰기/트렁크/삭제 로직 예제(Python 시뮬레이터)
```python
...
### 14.3.8 (토이) 디렉터리 이미지 생성기: 선형 vs 해시 성능 비교
```python
