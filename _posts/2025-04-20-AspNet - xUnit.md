...
## xUnit 수명주기 — 생성자/`IDisposable`/Fixture
...
## ASP.NET Core — 컨트롤러/Minimal API/미들웨어 단위 테스트
...
## 통합 테스트 — `TestServer` / `WebApplicationFactory<T>`
...
## EF Core 테스트 — InMemory vs SQLite(InMemory)
...
## JSON 응답 검증 — `System.Text.Json` / `FluentAssertions`
