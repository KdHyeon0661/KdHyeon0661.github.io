...
## 4. 탑다운 레시피: 2분 내 윤곽 잡기

### 4.1 Wait Class/Event 프로파일
```sql
...
## 5. 라인 매핑: ASH → DBMS_XPLAN(SQL 라인 통계)
...
### 7.2 Client Identifier / Machine / Program
```sql
...
## 9. 대기 이벤트별 ASH 분석 → 실행계획 라인 연결

아래는 **대표 이벤트 → 라인**을 잇는 정석 루틴과 **예제 SQL**.

### 9.1 User I/O — `direct path read temp` (정렬/해시 스필)
...
### 9.2 User I/O — `db file sequential read` (랜덤 I/O)
...
### 9.3 Concurrency — `enq: TX - row lock contention` (행잠금)
...
### 9.5 RAC — `gc current/cr block busy`
...
## 10. “시간 라인” 보기(타임라인/스파이크 탐지)

### 10.1 초 단위 타임라인(Wait Class Stacked)
```sql
WITH sec_class AS (
...
## 11. 시나리오 케이스 스터디(끝까지 따라하기)

### 11.1 보고서 쿼리 느림(정렬 스필) — `direct path read temp` 1위
...
### 11.2 OLTP 지연(행잠금) — `enq: TX - row lock contention`
...
### 11.4 RAC 글로벌 캐시 — `gc current block busy`
...
### 12.1 최근 15분 Top (Wait/Event/SQL) 한번에
```sql
-- Wait/Event
