...
### 단위원에서의 최대곱: \(\max xy\) s.t. \(x^2+y^2=1\)
...
### 최단거리: 점에서 직선으로 \(\min (x-a)^2+(y-b)^2\) s.t. \(\alpha x+\beta y+\gamma=0\)
...
### 레일리 비(고유값 문제): \(\max x^\top A x\) s.t. \(x^\top x=1\)
