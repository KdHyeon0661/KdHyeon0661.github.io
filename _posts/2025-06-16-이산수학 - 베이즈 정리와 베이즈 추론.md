...
## 큰 그림: Posterior ∝ Likelihood × Prior
...
### **결합적 사고**: 모형 선택(사후모형확률), 모수 추정(사후), 예측(사후예측)까지 일관된 확률 프레임으로 처리.

---

## 공액 사전(Conjugate Priors) — 계산이 쉬운 조합들

### 베르누이/이항–베타 (성공확률 \(\theta\))
...
### 정규(평균·분산 미지) — Normal–Inverse-Gamma
...
### 베타–이항: 사후 & 신뢰가능구간 & \(P(\theta_B>\theta_A)\)
...
### 코인 치우침 \(\theta\)에 대한 **간단 Metropolis** (공액 없이도 가능 예시)
...
## 신뢰 가능한 구간(credible interval) vs 신뢰구간(confidence interval)
