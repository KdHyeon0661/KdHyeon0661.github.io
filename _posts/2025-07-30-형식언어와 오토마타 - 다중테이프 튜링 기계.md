...
### 5.2 팰린드롬 결정: \(L=\{ ww^R \mid w\in\{0,1\}^*\}\) (NPDA가 가능, DCFL은 아님)
...
## 6. **실전 구현 예제(Python)** — 미니 **다중테이프 TM 시뮬레이터**
...
delta = {}
# 규칙:
# (q0, a, □) -> (q0, a, a, R, R)   for a in {0,1}
for a in ['0','1']:
    delta[('q0', a, BLANK)] = ('q0', a, a, 'R','R')
# 입력 끝: (q0, □, x) -> (qacc, □, x, S, S)
for x in ['0','1', BLANK]:
...
### 9.1 k-테이프 → 1테이프 (1 스텝)
```text
# [Phase A] 머리기호 아래 읽기: 구획 #1..#k를 스캔하며 (a1..ak) 수집
# [Phase B] 전이계산: (q',(b1..bk),(m1..mk)) := δ(q,(a1..ak))
# [Phase C] 쓰기/이동: 다시 스캔하며 각 구획 caret 위치에 bi 기록, caret을 mi로 이동
# q := q'
```

### 9.2 2D → 1D (좌표 인코딩+희소 저장)
```text
# 1D 테이프는 (key=indexOf(i,j), symbol) 블록을 구분자로 정렬 저장
loop:
...
## 11. 손으로 따라 해보는 **작은 실습 시나리오**

### 11.1 2-테이프 역복사 후 비교(팰린드롬 스타일)
