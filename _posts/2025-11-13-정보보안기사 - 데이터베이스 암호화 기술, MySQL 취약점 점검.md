...
## 1. 암호화 총론 — 범위·성능·운영 트레이드오프
| 범주 | 목적 | 대표 기술 | 장점 | 주의점 |
|---|---|---|---|---|
| **At-Rest**(디스크/스냅샷/백업) | 매체 탈취·스냅샷 유출 시 보호 | TDE(테이블스페이스/로그 암호화), OS/스토리지 암호화 | 앱 수정 최소 | 키 유출·운영 키순환이 핵심 위험 |
| **In-Transit**(네트워크) | 중간자·스니핑 대응 | **TLS(SSL)**, 계정의 `REQUIRE SSL`, `require_secure_transport` | 쉽고 표준 | 인증서 배포·회전 자동화 필요. 설정 미흡 시 다운그레이드 공격 위험. :contentReference[oaicite:0]{index=0} |
...
## 2. 저장 데이터 암호화(At-Rest)

### 2.1 MySQL의 TDE/데이터-앳-레스트 암호화
- MySQL **InnoDB Data-at-Rest Encryption**은 **마스터 키 → 테이블스페이스 키** 구조를 쓰며, **중앙 키관리(KMS) 연계 시 “MySQL Enterprise TDE”**라 칭한다. :contentReference[oaicite:1]{index=1}
- **키 저장소(Keyring/Component)**  
  - **Enterprise**: `keyring_okv`(KMIP/Oracle Key Vault 등), `keyring_aws`(AWS KMS 연동) 등. :contentReference[oaicite:2]{index=2}  
  - **8.4 LTS 변화**: 기존 `keyring_file`/`keyring_encrypted_file` 플러그인이 **제거**되고, **`component_keyring_encrypted_file`**로 대체. 마이그레이션 고려 필수. :contentReference[oaicite:3]{index=3}
...
> 운영 체크: **백업/복구 도구**가 암호화 테이블스페이스를 지원하는지 확인(Enterprise Backup는 지원). :contentReference[oaicite:4]{index=4}

---

### 2.2 컬럼/필드 수준 암호화(애플리케이션 계층 권장)
...
### 2.3 로그/임시/스냅샷과 백업 암호화
- **Redo/Undo/Temp** 로그도 암호화 옵션 제공(위 my.cnf 예시).  
- **백업**:  
  - Enterprise 환경: **암호화 테이블스페이스 인지** 백업(Enterprise Backup). :contentReference[oaicite:5]{index=5}  
...
## 3. 전송 데이터 암호화(In-Transit) — TLS & 강제 보안 연결
- **`require_secure_transport=ON`** 설정 시, 서버는 **TLS/Unix-Socket/Shared-Memory**만 허용(비보안 TCP 거부). :contentReference[oaicite:6]{index=6}  
...
```
> 운영 배포서는 **클라이언트 연결 실패 플랜**(드라이버, 워크벤치, ETL 등 갱신)을 포함. RHEL 가이드도 서버/클라 TLS 활성화를 권고. :contentReference[oaicite:7]{index=7}

---

## 4. 사용 중 데이터(In-Use) — 최소 보존과 노출면 축소
...
## 5. 키관리(KMS/HSM) — 수명주기·역할분리·감사
- **키수명주기**: 생성→활성→회전→폐기.  
- **역할 분리**: DBA≠KMS 관리자.  
- **MySQL 연계**:  
  - **KMIP/OKV**: `keyring_okv`로 중앙관리. :contentReference[oaicite:8]{index=8}  
  - **AWS KMS**: `keyring_aws`로 키 생성·저장 위임. :contentReference[oaicite:9]{index=9}  
- **8.4 이행 유의**: 플러그인 → **Component 기반 키링** 전환 점검. :contentReference[oaicite:10]{index=10}

---

## 6. 실습: “암호화 테이블 + TLS + 권한 최소화” 빠른 구축
1) **Keyring(Component) + InnoDB 암호화**: 위 2.1 예제.  
2) **TLS 강제**: `require_secure_transport=ON` 설정(3장). :contentReference[oaicite:11]{index=11}  
3) **계정 생성 + SSL 강제 + 암호정책**:
```sql
-- 패스워드 정책(컴포넌트) 활성/튜닝
...
```
> `validate_password`는 **컴포넌트**로 제공(8.0.4+), 시스템 변수로 정책을 세밀 조정 가능. :contentReference[oaicite:12]{index=12}

---

## 7. MySQL 취약점 점검(최신) — 체크리스트 & 실행 예제

### 7.1 인벤토리/버전
- `mysql --version`으로 구동 버전 식별, **8.4 LTS** 채택 여부와 릴리즈 노트로 **비호환/변경** 확인(특히 키링). :contentReference[oaicite:13]{index=13}
...
```
- **기본 인증 플러그인**: `caching_sha2_password` 사용 권장(드라이버 호환성 확인). :contentReference[oaicite:14]{index=14}  
- **패스워드 정책**: `validate_password` 컴포넌트로 길이/복잡도/사용자명 포함여부 등을 강제. :contentReference[oaicite:15]{index=15}

### 7.3 권한 최소화(Dynamic Privileges 포함)
...
require_secure_transport=ON
```
> **보안 연결 강제**를 통해 평문 접속 금지. :contentReference[oaicite:16]{index=16}
...
```
- `LOAD DATA LOCAL INFILE`는 **클라이언트 측 파일 유출** 위험이 있으므로 기본 **OFF** 권장. :contentReference[oaicite:17]{index=17}  
- `secure_file_priv`로 파일 입출력을 **특정 디렉토리로 제한**. :contentReference[oaicite:18]{index=18}
...
### 7.6 로깅/감사(Audit)
- **엔터프라이즈**: `audit_log` 플러그인 기반 **MySQL Enterprise Audit**로 접속/쿼리 활동 감사(정책 기반). :contentReference[oaicite:19]{index=19}  
- **커뮤니티**: 일반 로그는 성능 비용 높음 → **필요 시 한시적** 활성화. MariaDB `server_audit` 대안 검토(제품 혼동 주의).

### 7.7 백업·복구
- **암호화 백업**(Enterprise Backup) 또는 **외부 암호화**(GPG/볼륨 단). 복구 테스트는 분리된 환경에서 **주기적** 수행. :contentReference[oaicite:20]{index=20}
...
```
> `LOCAL INFILE`은 웹서버-DB 패턴에서 **웹서버 파일 유출** 트릭이 가능, 불가피 시 전용 디렉토리+권한 최소화. :contentReference[oaicite:21]{index=21}

---

## 9. MySQL 보안 베이스라인(my.cnf 예시)
```ini
[mysqld]
# 네트워크
...
- **In-Transit**: `require_secure_transport`, TLS 버전/암호군, 사용자 `REQUIRE SSL` 비율. :contentReference[oaicite:22]{index=22}  
- **계정/정책**: 플러그인(`caching_sha2_password`), `validate_password` 정책 값, 만료/락. :contentReference[oaicite:23]{index=23}  
- **설정 취약점**: `local_infile`, `secure_file_priv` 상태와 보완계획. :contentReference[oaicite:24]{index=24}  
- **감사/로깅**: Enterprise Audit/대체 수단 여부. :contentReference[oaicite:25]{index=25}  
...
## 참고(핵심 변경·권고 출처)
- **8.4 LTS/변경 사항**: 8.4 릴리스 노트(키링 관련 변경 포함). :contentReference[oaicite:26]{index=26}  
- **TDE/Keyring/KMS**: InnoDB Data-at-Rest Encryption, Enterprise TDE 설명, `keyring_okv`/`keyring_aws`. :contentReference[oaicite:27]{index=27}  
- **TLS 강제/설정**: `require_secure_transport`와 암호화 연결 가이드. :contentReference[oaicite:28]{index=28}  
- **LOCAL INFILE 보안**: 보안 고려사항(공식 매뉴얼/사고 사례 논의). :contentReference[oaicite:29]{index=29}  
- **Enterprise Audit**: 정책 기반 감사 플러그인. :contentReference[oaicite:30]{index=30}  
- **암호정책 컴포넌트**: `validate_password`(컴포넌트) 설치·튜닝. :contentReference[oaicite:31]{index=31}

---

### 마무리 체크리스트(요약)
- [ ] **At-Rest**: 테이블스페이스/Redo/Undo 암호화, 8.4 **Component Keyring** 적용·문서화. :contentReference[oaicite:32]{index=32}  
- [ ] **In-Transit**: `require_secure_transport=ON`, 계정 `REQUIRE SSL`, TLS 1.2/1.3만 허용. :contentReference[oaicite:33]{index=33}  
- [ ] **계정/정책**: `caching_sha2_password` 기본, `validate_password` 가동(길이≥14, 복잡도). :contentReference[oaicite:34]{index=34}  
- [ ] **설정 취약점**: `local_infile=OFF`, `secure_file_priv` 특정 디렉터리. :contentReference[oaicite:35]{index=35}  
- [ ] **감사/로깅**: Enterprise Audit 정책/보존주기, 일반/슬로우 로그 튜닝. :contentReference[oaicite:36]{index=36}  
- [ ] **백업**: 암호화 인지 백업·복구 테스트(주기). :contentReference[oaicite:37]{index=37}
