...
### 3.4 `log4j2.xml` 예시(콘솔 + 롤링 파일 + MDC)
...
## 5. 브리지(Bridge) — 혼재된 로깅을 SLF4J로 단일화
...
### 5.2 Gradle 예시(모두 SLF4J로 모으고 Logback으로 출력)
```kotlin
dependencies {
...
### 6.2 `logback-spring.xml` (콘솔 + 롤링 파일 + MDC)
```xml
<configuration>
...
### 7.1 Logback + logstash-logback-encoder
```kotlin
...
### 10.1 메시지·레벨 검증(Logback Test Appender)
