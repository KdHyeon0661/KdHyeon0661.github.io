...
### 메모리 보호키(MPK, x86)** / **메모리 태깅(MTE, ARMv8.5+)**
...
### 동일 물리 페이지를 다르게 매핑(읽기전용/읽기쓰기 분리)
...
### MPK(pkey)로 초저비용 권한 전환(하드웨어/커널 지원 필요)
