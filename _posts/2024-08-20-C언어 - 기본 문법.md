...
## 2. 상수와 매크로 — const / #define / enum / inline
...
## 3. `static` — 저장 기간과 연결(링크)를 동시에 이해

### 3.1 저장 기간 (storage duration)
...
## 5. 형 변환(Type Conversion) — 정수 승격과 통상 산술 변환
...
### 12.2 `static inline` (권장)

```c
static inline int max_int(int a, int b){ return a > b ? a : b; }
```

### 12.3 `_Generic` (C11)으로 타입별 선택
