...
# 4. 스칼라 서브쿼리 **캐싱 효과**(Scalar Subquery Caching)
...
# 5. “두 개 이상 값을 한 번에” 가져오고 싶을 때

스칼라 서브쿼리는 **1컬럼**만 반환 가능하므로, **여러 컬럼**을 붙이려면 다음 중 하나를 선택합니다.

## 5.1 **인라인 뷰(코릴레이티드)** + 조인 (권장, 가장 직관)
```sql
...
## 5.2 **LATERAL / CROSS APPLY / OUTER APPLY** (Oracle 12c+)
...
## 5.3 **JSON/집계(Function)로 묶어서 한 컬럼로 반환 후 파싱** (응용)
...
## 7.1 “VIP·GOLD 고객 100명에 대해: 최근 주문일, 최근 주문금액, 90일 주문수·합계”를 붙여라
...
### (3) 다중 스칼라 서브쿼리(가독성 좋지만, 재작성/캐시 의존)
```sql
SELECT c.cust_id, c.name,
