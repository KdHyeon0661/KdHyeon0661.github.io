...
3) **CBO가 조인으로 재작성(Scalar Subquery Unnesting/Decorrelate)**
   - Oracle 옵티마이저는 스칼라 서브쿼리를 **조인으로 풀어 최적화 공간을 넓히는 변환**을 수행할 수 있다.
   - 따라서 “표현식”으로 쓰되, 내부적으로는 조인처럼 최적화되는 경우가 많다.

4) **Scalar Subquery Caching**
   - 상관 스칼라 서브쿼리가 **같은 입력 키를 반복**해서 받을 때, Oracle 실행 엔진이 결과를 캐시해 재사용한다.
...
- Oracle은 이를 **조인으로 언네스팅**해 최적화할 수 있다.
...
- Oracle은 이 패턴을 **SEMI JOIN/ANTI JOIN으로 재작성**할 수 있다.
...
- `PUSH_SUBQ` / `NO_PUSH_SUBQ`
  ...
  - `PUSH_SUBQ`는 보통 `NO_UNNEST`와 세트로 쓰인다.
...
결과를 **쿼리 실행 컨텍스트 내 캐시**에 저장해 재사용한다.
...
Oracle 12c부터 **LATERAL 인라인 뷰**, `CROSS APPLY`, `OUTER APPLY`가 지원된다.
이는 “왼쪽 행 1건당 오른쪽에서 여러 컬럼을 **동시에 계산**”하는 데 최적이다.

#### CROSS APPLY: 매칭 없으면 왼쪽 제거
...
### 자동 재작성에 의존하지 말아야 하는 이유

- Oracle이 다중 스칼라를 **항상 조인으로 합쳐주지** 않는다.
- 통계/버전/힌트/쿼리 블록 구조에 따라 달라진다.
...
### Top-1은 “Stopkey + 인덱스 역순”으로

- `(key, order_dt)` 인덱스에서
  - `ORDER BY order_dt DESC FETCH FIRST 1 ROW`
  를 쓰면 대개 최적이다.

### NDV가 큰 경우는 조인/적재로 구조 변경
...
- 진짜 필요할 때만, 그리고 **특정 쿼리 블록(qb_name)**에 한정해 사용.
...
- 언네스팅(조인화)이 발생해 “캐싱이 필요한 구조 자체가 사라진” 경우도 있다.

### Q3. 다중 컬럼을 스칼라로 못 받나?

- 표준 스칼라 서브쿼리는 불가.
- **APPLY/LATERAL 또는 인라인 뷰 조인으로 설계를 바꿔라.**

### Q4. 언네스팅을 강제로 막아야 할 때는?

- 조인화가 오히려 비용을 키운다는 것이 **실측으로 확인된 특수 케이스**에서만.
- 그때는 `NO_UNNEST`를 **해당 서브쿼리에만 국지 적용**하라.
...
- 스칼라 서브쿼리는 “행당 단일 값”을 붙이는 강력한 표현식이며,
  ...
  **반복 키**에서는 **Scalar Subquery Caching**으로 반복 실행을 줄이기도 한다.
- **Top-1/최근 1건**은
  `ORDER BY ... DESC FETCH FIRST 1 ROW` + 인덱스 역순 스캔(Stopkey)으로 OLTP에서 최강 패턴이다.
- 여러 값을 한 번에 붙여야 한다면
  **LATERAL/CROSS|OUTER APPLY**나 **상관 인라인 뷰 조인**이 성능·플랜 안정성에서 가장 안전하다.
