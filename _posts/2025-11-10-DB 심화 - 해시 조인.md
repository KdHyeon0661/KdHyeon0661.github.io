...
- `IOSTATS` : 버퍼/디스크 I/O.  
- `MEMSTATS` : 해시/소트 workarea의 **PGA 사용량·TEMP 스필 여부**를 보여준다. `MEMSTATS`는 내부적으로 `V$SQL_WORKAREA` 요약을 끌어온다. :contentReference[oaicite:0]{index=0}  
...
- **DW/OLAP, 스타 조인 계열**  
  - 병렬 해시 조인 + **Bloom Filter(JOIN FILTER)** 로 프로브 집합을 크게 줄일 수 있을 때. :contentReference[oaicite:1]{index=1}  

### 2.2 불리한 상황(해시 조인을 피해야 할 때)
...
이 구분은 `V$SQL_WORKAREA_HISTOGRAM` / `V$SYSSTAT`에서도 관측된다. :contentReference[oaicite:2]{index=2}  

### 3.1 왜 스필이 느린가?

- 해시 조인 스필은 **빌드와 프로브 양쪽을 파티셔닝(Grace)** 해서 TEMP에 쓰고,  
  파티션별로 다시 빌드-프로브를 반복한다.  
- 결국 “큰 집합을 1번만 훑는” 장점이 줄고, **디스크 왕복과 재스캔**이 생긴다. :contentReference[oaicite:3]{index=3}  

---

## 4) 두 가지 실제 알고리즘: One-pass vs Hybrid/Multi-pass

### 4.1 One-pass(=Optimal) 해시 조인
...
### 4.2 Hybrid / Multi-pass(Grace) 해시 조인

- 빌드가 PGA를 초과하면 **해시 파티셔닝**을 수행한다.  
- 파티션 중 **일부는 메모리에 남겨 In-Memory로 처리(하이브리드)**,  
  나머지는 TEMP에 스필.  
- 파티션이 다시 커지면 **재분할(멀티패스)** 이 일어날 수 있다. :contentReference[oaicite:4]{index=4}  
...
- `USE_HASH(o)` 는 “**o를 직전 row-source와 해시 조인하라**”는 의미로,  
  해시 조인 선택 자체를 강제한다. :contentReference[oaicite:5]{index=5}  

### 6.3 빌드/프로브 교체: SWAP_JOIN_INPUTS / NO_SWAP_JOIN_INPUTS

**중요 포인트**:  
- 해시 조인은 **조인 순서와 독립적으로** CBO가 build/probe를 바꾸는 “input swap”을 고려한다.  
- `USE_HASH` 만으로는 **빌드가 확정되지 않을 수 있다.** :contentReference[oaicite:6]{index=6}  
...
- 프로브 측 스큐는 상대적으로 덜 치명적이고,  
  “빌드 스큐가 성능을 좌우”한다는 점이 실무적 핵심이다. :contentReference[oaicite:7]{index=7}  
...
- 실행계획에 `JOIN FILTER` 로 표시된다. :contentReference[oaicite:8]{index=8}  

### 8.1 Bloom Filter가 특히 강력한 조건
...
- `TempSpc` 가 크면 **One-pass/Multipass 스필 가능성**이 크다. :contentReference[oaicite:9]{index=9}  
- HASH JOIN 단계의 A-Rows가 E-Rows보다 훨씬 크면  
  **빌드 선택/조인 순서/선택도 추정 오판**을 의심.

### 10.2 시스템 차원 통계로 보는 방법
...
- `optimal / onepass / multipass` 분포를 보면  
  인스턴스 전체의 해시·소트 스필 빈도를 가늠할 수 있다. :contentReference[oaicite:10]{index=10}  
...
### 12.1 시나리오 A — 해시 조인이 맞는 전형적 DW 조인
...
- 해시 + 병렬 + Bloom Filter 조합이 잘 먹는 구간이다. :contentReference[oaicite:11]{index=11}  

### 12.2 시나리오 B — 빌드가 커져 스필이 나는 경우
...
### 12.3 시나리오 C — NL이 더 나은데 해시가 잡히는 회귀
...
- 23ai/Exadata 최신 스택에서는  
  **대형 테이블 해시 조인 가속(스캔/필터/전송 최소화)** 같은 하드웨어·스토리지 레벨 최적화가 더 강화되었다. :contentReference[oaicite:12]{index=12}  
- 따라서 최신 환경일수록  
...
- (운영) TEMP 부족은 해시 조인 실패의 전형적 원인이다. :contentReference[oaicite:13]{index=13}  
...
- [ ] `USE_HASH`만으로 빌드가 확정되지 않을 수 있으니  
      필요하면 `SWAP_JOIN_INPUTS/NO_SWAP_JOIN_INPUTS`로 보정했는가? :contentReference[oaicite:14]{index=14}  
- [ ] `MEMSTATS`에서 **TempSpc=0(Optimal)** 을 목표로 했는가?  
- [ ] 스큐/상관관계 컬럼에 **히스토그램/확장 통계**가 반영되었는가?  
- [ ] 병렬 환경이라면 **JOIN FILTER(Bloom)** 가 생성·활용되는가? :contentReference[oaicite:15]{index=15}  
