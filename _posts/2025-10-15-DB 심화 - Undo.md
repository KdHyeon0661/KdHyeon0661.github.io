...
## 한눈 개요 — “트랜잭션은 Undo 세그먼트의 **슬롯**을 잡고, 블록의 **ITL** 을 통해 **행 잠금**을 건다”
...
## 트랜잭션 테이블 슬롯(Transaction Table Slot) — XID의 집

### XID(usn.slot.sqn)과 슬롯 할당
...
## 데이터 블록 헤더의 **ITL 슬롯** — “이 블록은 지금 누가 만지니?”

### ITL(Interested Transaction List)란?
...
### ITL 부족과 `enq: TX - allocate ITL entry`
...
## 일관 읽기(Consistent Read): **Transaction Slot → Undo Chain → CR 블록 재구성**
...
### ORA-01555(snapshot too old)와 Undo 보존
...
## **실습** — ITL 부족과 Row Lock Byte 동작을 체감해 보기

> 주의: **테스트 전용** 환경에서만 실행하세요.

### 실습 준비: 작은 행·좁은 블록에 동시 갱신 몰아넣기
...
## **블록 덤프** 로 ITL/Row Lock Byte 확인하기(선택)
...
## **일관 읽기(Consistent Read)** 를 ITL/슬롯 관점으로 다시 보기
...
## Undo/ITL/Row Lock 관련 **대기 이벤트** 요약
...
## 추가 실습 — “Row Lock Byte ↔ ITL” 체감용 미니 시나리오
