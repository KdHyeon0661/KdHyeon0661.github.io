...
## 프로세스가 가진 것(논리적 메모리 지도 + 커널 자료구조)

### 주소 공간 레이아웃(일반적 x86-64, ASLR 단순화)
...
## 생성과 실행 이미지 교체: `fork`/`execve`/`posix_spawn`/`clone3`

### `fork()` + `execve()`(POSIX 정석)
...
## 종료와 수거: 좀비/고아, `wait*` API, 종료 코드 규약
...
## 스케줄링: CFS/RT/Deadline, 우선순위, niceness, affinity
...
## 프로세스 보안 컨텍스트: UID/GID, Capabilities, LSM, Namespaces
...
## 메모리: 힙/스택, `brk` vs `mmap`, COW, THP, NUMA
...
## IPC: 파이프·소켓·공유메모리·시그널·메시지큐·futex
...
### 소켓(로컬/네트워크), 메시지큐, `futex`(사용자 공간 락 원자 대기) 등은 상황별 선택.
...
## 에러 처리와 부분 I/O·타임아웃 패턴(시스템 콜 실전)
...
## 관찰·추적·디버깅: /proc, lsof, strace, perf, eBPF, 코어덤프
...
## 실습: 부모-자식 파이프라인 + 종료 코드/시그널 처리 종합
