...
## 코드 스플리팅과 지연 로드(라우팅·피드·관리자 페이지 분할)
...
## 해시·캐시 전략 — 롱 캐시(immutable)와 캐시 무효화
...
## CSS 파이프라인 — CSS/SCSS/PostCSS, 모듈, 추출
...
## 압축·최적화(Terser/ESBuild/SWC, 이미지 최적화)
...
## 테스트 환경과 번들러 — Vitest/Jest/Playwright 연계
...
## “한눈에 비교” — Webpack vs Vite 요약표(실전 관점)
