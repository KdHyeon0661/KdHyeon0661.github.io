...
## 7. Collectors 심화(Downstream/불변/요약/teeing)
...
### 7.7 teeing (JDK 12+): 두 결과를 동시에 결합
```java
...
### 12.1 체크 예외를 Optional/Result로 래핑
```java
...
## 13. 커스텀 Collector & Spliterator

### 13.1 Collector 직접 구현(마지막 N개만 유지)
```java
...
### 13.2 Spliterator로 균등 청크 분할 (병렬 친화)
```java
...
### 15.6 안정적 정렬 + 중복 제거 (Comparator + TreeSet)
```java
