...
### 2.3 dentry/inode/page cache(요약)
...
## 3. 하드 링크(하드링크, `ln`): **같은 아이노드**의 또 다른 이름
...
## 4. 소프트 링크(심볼릭 링크, `ln -s`): **경로 문자열**을 담은 별도 inode
...
## 11. xattr/ACL/chattr: 아이노드 확장 기능

### 11.1 xattr(확장 속성) & ACL
```bash
# xattr
...
## 12. 시간 스탬프 최적화: atime/relatime/noatime
...
### 19.1 inode 번호로 파일 찾기
```bash
ls -li
find . -xdev -inum <INO>
```

### 19.2 깨진 심링크 일괄 제거(주의)
```bash
find . -xtype l -print
# 검토 후
find . -xtype l -delete
...
## 20. 코드 예제: `stat/lstat/readlink` 한 번에 보기(C)
