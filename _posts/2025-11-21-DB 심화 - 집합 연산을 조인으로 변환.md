...
## 3) UNION  →  FULL OUTER JOIN(+ COALESCE) / 한 번 스캔 최적화
...
## 6) 고급: 중복(멀티셋)까지 보전( INTERSECT ALL / EXCEPT ALL 유사 )
