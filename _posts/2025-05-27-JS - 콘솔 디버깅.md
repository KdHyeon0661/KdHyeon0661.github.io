...
## 구조 관찰: `console.dir()` / `console.dirxml()` / DOM
...
## 그룹핑으로 맥락 유지: `group` / `groupCollapsed` / `groupEnd`
...
## 시간 측정: `time` / `timeLog` / `timeEnd` 와 Performance API
...
## 콜스택/흐름 추적: `trace()` / `debugger` / 예외 흐름
...
## 브라우저 DevTools 전용 유틸(표준 console 외)
...
### 이벤트 홍수 제어(throttle/debounce + 샘플링)
