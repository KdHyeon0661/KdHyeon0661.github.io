...
## 2. `/etc/security/limits.conf` — PAM을 통한 영구 정책
...
### 3.4 `loginctl enable-linger` — 로그아웃 뒤에도 사용자 서비스 유지
```bash
...
### 5.4 프로젝트 쿼터(대량 디렉터리 단위, xfs/ext4 신기능)
...
### 6.1 “limits.conf를 바꿨는데 서비스에 NOFILE이 그대로”
