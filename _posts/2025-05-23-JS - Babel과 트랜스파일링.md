...
## 5. preset-env + Browserslist: “필요한 것만” 변환/폴리필

### 5.1 Browserslist로 타깃 정의
`package.json`
```json
{
  "browserslist": [
...
### 5.2 Polyfill 전략(core-js) — `useBuiltIns`
`@babel/preset-env` 옵션:
...
## 7. @babel/plugin-transform-runtime: 헬퍼/레게너레이터 최적화
...
## 14. Babel + 폴리필 동작 확인 (usage vs entry)
