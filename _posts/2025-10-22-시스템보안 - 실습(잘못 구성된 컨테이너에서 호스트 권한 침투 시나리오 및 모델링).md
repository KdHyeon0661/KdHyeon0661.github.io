...
category: 시스템보안
---
# 실습: 잘못 구성된 컨테이너에서 “호스트 권한 침투 시나리오” **모델링** (공격 재현 없이 방어 관찰)
...
# 과권한 예시(Observability 목적): privileged + hostPID + cap-add
...
## 랩 2 — Kubernetes에서 “위험 Pod” **거부/감사** 흐름

### 위험 Pod 매니페스트(관찰용; 실제 적용 금지)
...
## Falco/EBPF로 **행동 탐지** (mount/ptrace/socket 접근)
...
# 방어: Pod Security, Seccomp Profile, AppArmor/SELinux, Notary / cosign
...
## AppArmor / SELinux — **경로·권한 MAC**

### AppArmor (Debian/Ubuntu 등)
...
### Cluster Admission — 서명 강제(Gatekeeper 예시)
...
## Kyverno — 보안 가드레일(간단 예)

### Privileged 금지 + Cap Drop + ReadOnly RootFS 강제
