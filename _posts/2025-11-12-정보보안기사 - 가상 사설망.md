...
## 1. VPN 개념과 분류 — 어디에서 무엇을 “싸고” “인증”하는가
### 1.1 계층별 분류
| 계층 | 대표 | 특징/장점 | 유의점 |
|---|---|---|---|
...
## 3. SSL/TLS 기반 VPN(“SSL VPN”) — OpenVPN·DTLS 1.3
...
## 5. MTU/오버헤드 직관 — 왜 “1420/1380” 같은 숫자가 나오나
### 5.1 대략적 오버헤드(IPv4, NAT-T 가정)
...
### 7.1 원격접속 — IKEv2(EAP-TLS) with strongSwan
**(A) 게이트웨이: strongSwan)**  
`/etc/ipsec.conf`
```conf
config setup
...
### 7.3 원격접속 — OpenVPN(UDP, TLS 1.3 우선)
**서버: `server.conf`**
```conf
port 1194
proto udp
...
### 7.4 사이트간(Site-to-Site) — Cisco IOS(IPsec/IKEv2)
```ios
...
### 7.5 Windows 원격접속 — PowerShell로 IKEv2 프로파일
```powershell
# 관리자 PowerShell
...
## 8. 운영·관측(Observability) — 로그·메트릭·패킷
### 8.1 핵심 로그 포인트
