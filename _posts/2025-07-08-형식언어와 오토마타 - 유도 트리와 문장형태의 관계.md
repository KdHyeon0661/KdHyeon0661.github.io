...
### 1.3 유도 트리 (Derivation/Parse Tree)
- 루트: \(S\).
...
## 3. 기본 예제: \(S \to aSb \mid \varepsilon\)
...
## 5. (증명 감각) 유도 트리 ↔ 문장형태, 두 방향 정리 스케치

### 5.1 트리에서 문장형태 열(sequence) 만들기
...
## 7. 실습 코드 ② — 좌측/우측 유도의 **문장형태 시뮬레이터** (S→aSb|ε)
...
## 9. 고급: 핸들(handle), 단순 구문구(phrase), LR 파싱 감각
