...
## 권한과 네트워킹(IAM, Lake Formation, VPC)

### IAM Role (Redshift → S3/Glue 접근)
...
### **Partition Projection**(파티션 메타 없이 프루닝)
...
## 머티리얼라이즈: UNLOAD/CTAS/외부 테이블 변환

### 외부 → 내부로 스냅샷(고속 BI 필요 구간)
...
## IaC 스니펫(CloudFormation/Terraform)

### CloudFormation(외부 스키마는 SQL로 생성)
...
## 고급: Federated Query/Athena/LF와의 공존
