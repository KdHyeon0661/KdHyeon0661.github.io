...
## 4. 부분집합 구성법(Subet Construction): NFA → DFA
...
### 5.1 포함 ①: $$\mathcal L(N)\subseteq \mathcal L(D)$$
귀납(접두사 길이)에 의해 위 불변식을 보이면, $$w$$ 읽은 뒤 DFA 상태에 **최종상태가 포함** ⇔ NFA가 어떤 경로로든 최종상태 도달.

### 5.2 포함 ②: $$\mathcal L(D)\subseteq \mathcal L(N)$$
...
## 7. 예제 2 — 정규식 \((a\mid b)^\*abb\) 의 NFA → DFA

**언어**: “`abb`로 끝나는 문자열”.

### 7.1 NFA(ε-없음) 한 가지 구성
```text
