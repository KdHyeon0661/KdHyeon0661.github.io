...
## 컬렉션/이터러블 생성·검사 관련 자주 쓰는 추가 빌트인
...
## 문자열 포맷 심화 (`format`, `str.format`, f-string)
...
## 종합 예제: 내장함수로 “CSV → 집계 → 정렬 → 보고서 문자열” 만들기
...
# 파일 읽기 + 2) 파싱(map) + 3) 필터(filter) + 4) 집계(sum) + 5) 정렬(sorted)
