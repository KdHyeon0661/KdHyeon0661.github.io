...
## 0. 들어가기 전에 — 언제 패턴으로 리팩토링할까?

### 0.1 대표 Code Smell ↔ 패턴 매핑
...
### 1.3 Strategy 선택 로직 외부화 — Registry/Factory/DI
...
## 2. 전역 인스턴스 남용 → Singleton 대신 DI로 치환 (필요 최소의 Singleton)
...
### 2.3 정말 전역 1개가 필요한 경우의 안전한 Singleton
...
## 5. 직렬 필터링/검증 로직 → Chain of Responsibility
...
## 6. 공통 횡단 관심사 로깅/캐싱/권한 → Decorator/Proxy
...
## 7. Undo/Redo가 필요한 편집기·도면 도구 → Command + Memento
...
## 9. 안정된 구조(AST/문서)에 연산 추가 → Visitor
