...
### 3.3 SNI·OCSP 스테이플링·Must-Staple
...
## 4. 핸드셰이크 흐름(개념도) & 레코드 계산
### 4.1 TLS 1.3 핸드셰이크 요약
...
## 5. 서버 소프트웨어별 “안전한 기본값” 템플릿
### 5.1 NGINX(HTTP/1.1·2·3)
```nginx
...
## 6. 클라이언트/애플리케이션 코드 보안 설정 예시
### 6.1 Python (requests + mTLS)
```python
import requests
...
### 6.2 Go (http.Client TLS 1.3 강제, Root CA 고정)
```go
package main
...
## 12. 운영 시나리오: “레거시 TLS 폐지 + 자동 발급/갱신 + 가시성”
### 목표
...
### 13.2 OpenSSL로 Must-Staple 확인(참고)
```bash
# 인증서의 TLS Feature(OID 1.3.6.1.5.5.7.1.24) 확인
