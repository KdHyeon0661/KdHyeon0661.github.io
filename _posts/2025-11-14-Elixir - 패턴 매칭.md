...
category: Elixir
---
# 엘릭서 2장: `=` 는 할당이 아니라 “매칭”이다 — 패턴 매칭, 언더스코어, 핀 연산자, 등호를 다시 보는 법

## _2.1 할당문: 당신이 생각하는 할당문이 아니다

많은 언어에서 `=` 는 “**왼쪽 변수에 오른쪽 값을 대입**”하는 연산자다.  
하지만 **엘릭서에서 `=` 는 “패턴과 값의 매칭(match)” 연산자**다. 공식 가이드에서도 `=` 를 **match operator**라고 부른다.:contentReference[oaicite:1]{index=1}
...
### 2.1.3 방향성: “오른쪽을 평가하고, 왼쪽은 검증(패턴)”만 한다
...
- `=` 는 “**같도록 강제하는 제약**”이고,
- `==`, `===` 는 “**이미 있는 두 값을 검사**하는 비교”이다.:contentReference[oaicite:2]{index=2}

### 2.1.5 어디까지가 “대입처럼” 보이고, 어디부터 다른가
...
이 “거꾸로 된” 형태가 허용되는 이유는, 엘릭서가 `=` 를 **양방향 대입이 아니라 “일치 제약”으로 정의**했기 때문이다.:contentReference[oaicite:3]{index=3}

---

## _2.2 더 복잡한 매칭

패턴 매칭은 **튜플, 리스트, 맵, 구조체, 바이너리/비트스트링** 등 거의 모든 자료구조에 적용된다. 이는 엘릭서의 가장 강력한 특징 중 하나로, Elixir School, 다양한 유럽·미국 개발자 블로그에서도 “핵심 언어 기능”으로 항상 가장 먼저 소개된다.:contentReference[oaicite:4]{index=4}

### 2.2.1 리스트와 튜플: 중첩 패턴으로 구조 한 번에 풀어내기
...
### 2.2.2 맵 패턴 — 키 기반 구조 제약

맵에서 패턴 매칭은 “**필수 키들을 강제하면서 값은 꺼내기**”에 매우 자주 쓰인다.:contentReference[oaicite:5]{index=5}
...
맵의 **키 자리에 변수**를 쓰고 싶을 때는 **핀 연산자 `^`**로 현재 값을 고정해야 한다.:contentReference[oaicite:6]{index=6}
...
### 2.2.4 바이너리/비트스트링 패턴 — 프로토콜 파서의 핵심

바이너리/비트스트링 패턴은 **네트워크 패킷, 파일 포맷, 문자열** 등을 처리할 때 성능·표현력 양쪽 모두에서 강력하다. 공식 문서와 여러 유럽/미국 블로그에서도 “효율적인 데이터 처리의 핵심 도구”로 소개된다.:contentReference[oaicite:7]{index=7}  
...
유럽/미국 개발자 문서에서도 강조하듯, 바이너리 패턴은 **바이트 단위**로 동작하기 때문에, `"ü"` 같은 멀티바이트 문자를 “글자 단위”로 나누고 싶다면 `String.next_grapheme/1` 같은 고수준 API를 써야 한다.:contentReference[oaicite:8]{index=8}

### 2.2.5 함수 헤드, `case`, `with` 에서의 패턴
...
같은 곳에서도 일관되게 사용된다.:contentReference[oaicite:9]{index=9}  
...
엘릭서에서 **하나의 패턴(매칭 식) 안에서** 같은 변수 이름이 두 번 등장하면, 두 번째부터는 “**기존 값과 일치해야 하는 제약**”으로 해석된다.:contentReference[oaicite:10]{index=10}  

### 2.4.1 같은 이름 두 번 쓰기: “같아야 함” 강제
...
### 2.4.2 함수 헤드에서 “같은 두 값만 받는 함수” 만들기
...
### 2.4.3 핀 연산자 `^` — 이미 바인딩된 값을 “고정”해서 매칭

`^var` 는 “**이 변수의 현재 값을 패턴에 고정하라**”는 뜻이다.  
공식 가이드에서도 “다시 바인딩하지 않고, 현재 값을 기준으로 매칭하고 싶을 때 쓰라”고 설명한다.:contentReference[oaicite:11]{index=11}  
...
### 2.4.4 “매칭의 경계”: 다른 매칭에서는 같은 이름이 다시 바인딩된다
...
이 관점은 Prolog/Erlang 계열 언어에서 가져온 것으로, Elixir 공식 문서와 여러 해외 튜토리얼에서도 반복해서 강조한다.:contentReference[oaicite:12]{index=12}  

### 2.5.1 수학적 관점: 방정식/일치 제약으로서의 `=`
...
이런 “계약이 코드에 직접 드러나는 스타일”은 미국·유럽 쪽 Elixir/BEAM 커뮤니티에서 **가독성과 신뢰성을 동시에 높이는 패턴**으로 많이 권장된다.:contentReference[oaicite:13]{index=13}  

---

## _2.6 실전 레시피: 패턴 매칭으로 바꾸면 좋아지는 코드들

### 2.6.1 API 응답 디코딩 — 성공/실패 분기
...
### 2.6.2 연속 이벤트에서 “같은 사용자 두 번 연속” 찾기
...
### 2.6.5 `GenServer` 콜 패턴 — 계약이 분기 로직을 정의
...
2. 바이너리 패턴 매칭은, 공식 문서와 여러 성능 분석 글에서 설명하듯이 **크게 두 가지 장점**이 있다.:contentReference[oaicite:14]{index=14}  
