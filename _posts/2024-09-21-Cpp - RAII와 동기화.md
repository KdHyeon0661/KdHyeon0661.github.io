...
## `std::lock_guard` vs `std::unique_lock` — 무엇을 언제 쓰나

### `std::lock_guard` — 가장 간단한 RAII 락
...
## 락 태그: `defer_lock`, `adopt_lock`, `try_to_lock`
...
## **다중 락과 데드락 방지** — `std::scoped_lock` / `std::lock`

### 가장 간단한 해법: `std::scoped_lock` (C++17)
...
### `std::lock` + `unique_lock(defer_lock)`
...
## 조건 변수와 RAII — 스푸리어스 웨이크업까지 안전하게

### 기본 패턴 (`wait`의 **predicate** 필수)
...
## 타임드 락: `timed_mutex` / `recursive_timed_mutex`
...
## 읽기-쓰기 락: `std::shared_mutex` / `shared_lock`
...
## 스레드 안전한 **초기화 1회 보장**: `std::call_once`
