...
#### 1.1.1 클라우드 서비스 계층 (IaaS/PaaS/SaaS)
...
## 4. IAM 기초 — 사용자/그룹/역할/정책

### 4.1 최소 권한 정책 예제(JSON)
...
## 8. 데이터베이스 — RDS/Aurora/DynamoDB/Redshift

### 8.1 RDS (MySQL/PostgreSQL 등)
...
## 9. 서버리스 — Lambda / API Gateway / EventBridge / SQS / SNS / Step Functions

### 9.1 Python Lambda 핸들러 예시
...
## 10. 컨테이너 — ECS/Fargate, EKS

### 10.1 ECS Fargate 서비스 빠른 개념

- EC2 관리 없이 **Fargate**로 컨테이너 실행
- ALB 대상 그룹과 연동, Auto-healing

#### 10.2 Task Definition 스니펫(JSON)
...
## 11. 모니터링/로깅 — CloudWatch, X-Ray, CloudTrail
...
## 12. 보안 — KMS/Secrets Manager/WAF/Shield
...
## 13. 배포 — CI/CD (CodePipeline·CodeBuild·GitHub Actions)

### 13.1 정적 사이트: S3 + CloudFront + OAC
...
## 15. 가용성·내결함성(HA/DR) — RTO/RPO, 다중 AZ/리전

- **RTO**(복구시간목표): 서비스 복구까지 허용 시간
- **RPO**(복구시점목표): 허용 가능한 데이터 손실 시간

### 15.1 가용성 계산(독립 가정 단순화)
...
## 17. 실전 랩 ① — “블로그 글 API” (API GW + Lambda + DynamoDB)
...
## 18. 실전 랩 ② — “컨테이너 웹앱” (ECS Fargate + ALB)
...
## 19. 실전 랩 ③ — “정적 블로그” (S3 + CloudFront + OAC + Route 53)
...
### 22.2 Node.js v3 SDK DynamoDB Put
