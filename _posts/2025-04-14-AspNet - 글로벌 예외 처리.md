...
## 3. 운영 환경에서의 전략 비교: Redirect vs Re-Execute vs Inline 처리

### 3.1 Razor/MVC 중심(HTML UI) — 경로 재실행(Re-Execute)
...
### 3.2 API 중심(JSON) — Inline 미들웨어 구성
...
## 4. `UseDeveloperExceptionPage()` — 개발용 상세 페이지
...
## 5. 상태 코드 페이지(404/403 등) — StatusCodePages
...
## 9. Minimal API에서의 간결한 매핑(.NET 7/8)
