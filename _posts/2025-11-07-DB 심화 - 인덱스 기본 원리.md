...
## 1) **인덱스 사용 불가/범위 스캔 불가**의 본질: SARGability
...
## 2) **컬럼 가공**(함수/연산) 때문에 인덱스가 안 붙는 경우

### 2.1 날짜 컬럼에 `TRUNC`(컬럼 쪽)
**나쁜 예**
```sql
...
### 2.2 문자열 컬럼에 `UPPER/LOWER/INITCAP`(컬럼 쪽)
**나쁜 예**
```sql
SELECT * FROM idx_demo
...
## 4) **인덱스 사용이 어려운 조건 유형**과 리라이트

### 4.1 `NOT EQUAL (!=, <>), NOT LIKE, NOT IN`
...
## 5) **FBI(Function-Based Index)**로도 풀 수 있는가?
...
## 7) **묵시적 형변환** 상세 사례 & 드라이버 레벨 체크

### 7.1 JDBC/ODP.NET/cx_Oracle 바인드 타입 일치
