...
기존 글은 **사전 준비 → kubeadm init → 네트워크 플러그인 → 워커 조인** 흐름이 잘 잡혀 있다. 이 글에서는 여기에 **시스템 튜닝(커널/방화벽/cgroup)**, **containerd 표준 설정**, **공식 APT 서명 방식(apt-key 미사용)**, **kubeadm 구성 파일 방식(init/join)**, **운영/업그레이드/리셋/토큰 재발급**, **자주 겪는 문제 해결**을 보강한다. 대상은 Ubuntu 20.04/22.04/24.04를 가정한다.
