...
## 5. 미들웨어 스타일 — `next()` 콜백, 여러 처리자 모두 실행(필터형)
...
### 13.2 C# — DelegatingHandler 스타일(HTTP 파이프라인 유사)
...
## 14. 실제 시나리오 1 — API 요청 파이프라인(권한→레이트리밋→캐시→핸들러)
