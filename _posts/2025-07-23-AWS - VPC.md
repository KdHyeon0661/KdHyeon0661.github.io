...
## 2. 라우팅·게이트웨이: IGW/NAT/엔드포인트/프라이빗 링크
...
### 2.3 게이트웨이 엔드포인트 vs 인터페이스 엔드포인트
...
## 4. 표준 3-Tier VPC 레퍼런스(멀티 AZ, HA NAT, 엔드포인트)
...
## 6. IaC — Terraform/CloudFormation 스니펫

### 6.1 Terraform: VPC + 2AZ 서브넷 + IGW + NAT + 기본 SG
...
### 6.2 CloudFormation(YAML) — S3 Gateway VPCE + Route
...
## 7. 하이브리드: VPN, Direct Connect, TGW, 피어링
...
## 8. 네임서비스·프라이빗 DNS: Route 53 Resolver/PHZ
...
## 9. 관측/운영: VPC Flow Logs, CloudWatch, 트러블슈팅
