...
### 2.1 포함/미포함 재귀(초안 확장판: 출력 대신 콜백·yield 지원)
...
### 2.2 visited 배열 방식(초안 보강: 재할당 최소화)
...
### 2.5 그레이 코드(한 비트만 바뀌며 순회) — 상태 전이 비용 최소화
...
### 2.6 사전식 **k-조합**(조합 길이를 정해 탐색) — start 인덱스
...
### 3.3 카테고리/속성 제약 — 미리 계산·진행 중 누적판정
...
### 4.1 1182 부분수열의 합 — 공집합 제외/Meet-in-the-Middle(대형 N)
...
### 4.2 1759 암호 만들기 — 조합 + 제약, 프루닝 포함(초안 보강)
...
## 6. 최적화/카운팅 변형 — 부분집합의 “값”을 다루는 문제들

### 6.1 “합 ≤ T의 최대합” — 브랜치&바운드(상한 컷)
...
### 6.2 카운팅: “합이 특정 값” 외에도 모듈러·카테고리 등
...
## 12. 통합 예제 — “조건 다수”를 한 번에 (합 == S, 길이 ≤ L, 금지 원소 집합 F)
