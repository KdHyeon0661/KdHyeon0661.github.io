...
## 3. `enumerate`, `zip`, `range` — 실전 3대장

### 3.1 enumerate: 인덱스와 값 동시
```python
names = ["kim", "lee", "park"]
...
## 4. for-else: **break 없이** 정상 종료 시 else 실행
...
### 5.3 pass: 자리표시(문법상 필요하지만 할 일 없음)
```python
for _ in range(3):
    pass
```
...
## 7. 언패킹(Unpacking) — `*` 활용 (확장/할당/인수)

### 7.1 print/함수 호출에서 언패킹
```python
lst = [1, 2, 3]
print(lst)     # [1, 2, 3]
...
### 7.3 확장 할당(extended iterable unpacking)
```python
...
### 8.1 `range(len(seq))` 대신 `enumerate` 사용
```python
seq = ["a", "b", "c"]
...
### 8.2 여러 컬렉션 동시 순회는 `zip`
```python
ids = [101, 102]
names = ["kim", "lee", "park"]  # 더 길다
for i, name in zip(ids, names):
    print(i, name)
# 파이썬 3.10+: zip(ids, names, strict=True) → 길이 불일치 시 ValueError
```
...
## 9. 고급: `iter(callable, sentinel)` — 센티넬 기반 루프
...
## 10. itertools 주요 구성요소(스트림 처리 강화)
