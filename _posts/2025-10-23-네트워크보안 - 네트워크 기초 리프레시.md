...
# 패킷 캡처(루프백 인터페이스, OS별 이름 확인: lo/lo0/Loopback Pseudo-Interface 등)

sudo tcpdump -i lo -nn -vvv -c 10 'tcp port 8080' -w http_lo.pcap
# Wireshark로 http_lo.pcap 열어 캡슐화/역캡슐화 관찰

```

#### 캡슐화 크기 감(IPv4/TCP 기본 헤더 크기)
...
## MTU/MSS/프래그먼테이션, 라우팅 vs 스위칭, ARP/ND, NAT 유형

### MTU, MSS, 프래그먼테이션의 상관관계
...
### ARP/ND (IPv4/IPv6 이웃 탐색), GARP, 이슈 포인트
...
# SNAT/MASQUERADE 규칙(호스트에서 iptables 사용 시 네임스페이스 경계 고려)
...
## 성능·지연·손실·재전송의 상호작용(지표 해석 기초)

### 지표 파악의 기본: RTT, 대역폭, BDP
...
### 재전송, Duplicate ACK, RTO, Fast Retransmit
