...
### 1.1 선택도(Selectivity)와 카디널리티(Cardinality)
...
### 1.2 드라이빙 테이블(Outer)과 인너 테이블(Inner)
...
## 2. 옵티마이저가 조인 순서를 고르는 방식

### 2.1 탐색 공간: 조인 그래프와 조인 트리
...
### 2.3 옵티마이저는 실행 중 조인 순서를 바꾸지 못한다

Adaptive Plans/Statistics Feedback이 있어도  
**일단 선택한 조인 순서 자체는 실행 중에 바뀌지 않는다.**  
Adaptive는 주로 **조인 메소드(예: NL ↔ HJ)** 를 갈림길로 열어두는 수준이다. :contentReference[oaicite:1]{index=1}
...
- 필요 시 `SWAP_JOIN_INPUTS`로 Build/Probe를 뒤집을 수 있다. :contentReference[oaicite:2]{index=2}
- 조인 순서와 Build 방향은 별개로 조정될 수 있음을 기억.

### 3.3 Sort Merge Join (SMJ)
...
## 5. 시나리오 1 — 화면 Top-N: “정렬 흡수 + Stopkey”는 순서가 전부다

### 5.1 요구
“KOR/VIP 고객의 최근 주문 Top-50의 상세 조회”

### 5.2 좋은 순서: CUST → ORD → OI (NL + 정렬 흡수)
...
## 6. 시나리오 2 — 대량 집계 보고서: HJ는 “빌드 입력을 작게 만드는 순서”가 승부처

### 6.1 요구
“2024 하반기 주문의 지역×카테고리 매출”

### 6.2 좋은 순서: PROD(카테고리 필터) → OI → ORD(기간 필터) → CUST
...
## 7. 시나리오 3 — 비등치/기간 매핑: SMJ에서 순서는 “정렬량을 줄이느냐”로 결정
...
### 7.2 좋은 순서: ORD → PROMO (ORD에서 기간 프루닝/정렬 대체)
...
## 8. 옵티마이저가 “나쁜 순서”를 고르는 대표 원인

### 8.1 통계 부정확(히스토그램/확장통계 부족)
...
### 8.3 뷰 머지/서브쿼리 Unnesting으로 순서가 바뀜
...
## 9. 조인 순서를 “직접 통제”하는 힌트들

Oracle 문서에서 조인 순서 힌트와, 그와 함께 조인 메소드 힌트를 권한다. :contentReference[oaicite:4]{index=4}

### 9.1 핵심 힌트

- `LEADING(a b c ...)` : 지정 순서로 조인(첫 테이블이 드라이빙) :contentReference[oaicite:5]{index=5}
- `ORDERED` : FROM에 쓴 순서대로 조인 :contentReference[oaicite:6]{index=6}
- `USE_NL(t) / USE_HASH(t) / USE_MERGE(t)` : 특정 테이블과의 조인 메소드 강제
- `SWAP_JOIN_INPUTS(t)` : 해시 조인에서 Build/Probe 뒤집기 :contentReference[oaicite:7]{index=7}
...
2. **조인 메소드 힌트(USE_*)를 같이 줘야 의도대로 고정**  
   (Oracle이 조인 순서 힌트는 따르되 메소드만 바꾸는 경우를 막기 위해) :contentReference[oaicite:8]{index=8}
...
## 12. “드라이빙 테이블 선정” 실전 규칙

### 12.1 Top-N/페이징/OLTP 화면
