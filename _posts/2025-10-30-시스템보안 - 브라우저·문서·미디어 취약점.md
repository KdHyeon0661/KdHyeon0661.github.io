...
category: 시스템보안
---
# 13. 브라우저·문서·미디어 취약점(시스템 관점)

## 13.1 렌더러/샌드박스/IPC 경계

### 13.1.1 현대 브라우저 프로세스 모델(개요)
...
### 13.1.2 샌드박스 경계(Windows/macOS/Linux)
...
# 제한: socket(AF_INET/AF_INET6) outbound 금지(렌더러), AF_UNIX만(브로커 IPC)
```
...
### 13.1.6 방어: Site Isolation / JIT 제한 / 메모리 보호
...
### 13.2.1 폰트 파서(OTF/TTF/WOFF/WOFF2)
...
### 13.2.2 이미지 파서(PNG/JPEG/WebP/AVIF/ICO…)
...
### 13.2.3 미디어 코덱(H.264/H.265/VP9/AV1/MP3/AAC)
...
### 13.2.4 GPU/드라이버 경유 영향(브라우저에서 시스템으로)
...
### 13.2.5 문서 형식(PDF/Office)의 임베디드 미디어/폰트/스크립트
...
### 13.2.6 미니 랩: 안전 디코더 스켈레톤 + Fuzz + Sanitizer

#### (1) 안전 스켈레톤: “간단 이미지 헤더” 파서
...
### 13.2.8 사고 대응(IR) 시나리오 요점(브라우저/미디어)
