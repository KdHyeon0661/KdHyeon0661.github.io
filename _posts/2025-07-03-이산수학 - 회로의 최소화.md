...
### 4.2 Don’t-Care 예 (4-변수)
- 1: \(m(1,3,7,15)\), X: \(d(5,13)\) → X를 포함해 4개/8개 군집을 만들면 리터럴 제거가 급증.

---

## 5) 퀘인-맥클러스키(Quine-McCluskey, QMC): 5~8변수 소형 해법
...
### 5.1 파이썬 구현 (SOP + Don’t-Care 지원, 소규모용)
```python
...
## 10) 실전 예제: 진리표 → 최소화 → 검증 → Verilog/NAND 매핑
