...
## 2. 공격 기법 분류 — 위협 모델과 방어 포인트

### 2.1 Volumetric/반사·증폭(Reflection/Amplification)
...
### 4.2 플로우(NetFlow/IPFIX) 기반(스케일 탐지)
...
### 5.2 방화벽/라우터 레이트 제한(예시: nftables)
...
### 5.3 SYN Cookies/SYN Proxy/커널 튜닝
...
### 6.1 Nginx — Rate/Conn/Timeout

```nginx
# nginx.conf (발췌)
# 1. IP별 요청 속도 제한(토큰 버킷)
