...
category: DB 심화
---
# **방지**·스칼라 서브쿼리 **캐싱**·`DETERMINISTIC`/`RESULT_CACHE` **활용**·복잡 함수 로직 **슬림화**로 **함수 호출 최소화**하기
...
## `DETERMINISTIC` vs `RESULT_CACHE` — 캐싱의 의미와 올바른 활용

### `DETERMINISTIC`의 의미 (캐시 **아님**)
...
### `RESULT_CACHE` vs 조인/서브쿼리 선택 가이드
...
## **복잡한 함수 로직** 슬림화: N+1 제거·배치화·SQL화

### 나쁜 예(재귀 SQL·자율 트랜잭션·외부 자원)
...
## 요약 코드 모음 (Before → After)

### 상태 라벨 표시 (페이지 + 뷰 머지 방지 + RESULT_CACHE)
