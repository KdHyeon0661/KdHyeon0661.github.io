...
## 0) 무엇이 Directory Traversal(=Path Traversal)인가?

**Directory Traversal(경로 조작)**은 사용자 입력에 포함된 경로 요소(`../`, `..\`, 절대경로, 이중 인코딩 등)를 악용해 **애플리케이션이 의도한 루트 밖의 파일**을 읽거나 쓰게 만드는 취약점입니다. 대표적으로 `/etc/passwd`, 앱의 `application.yml`, 데이터베이스 크리덴셜, 소스 코드, 키 파일 등이 노출됩니다. 또한 아카이브 해제(Zip)·업로드 처리기 등에서도 동일 원리로 **임의 위치 쓰기**가 발생합니다(Zip Slip). :contentReference[oaicite:0]{index=0}

> 결과적으로 공격자는 **민감 정보 열람 → 추가 침투(크리덴셜/키 재사용, RCE 체인)**로 이어갈 수 있습니다. 실무 가이드와 예제 랩은 PortSwigger/OWASP 문서를 참조하세요. :contentReference[oaicite:1]{index=1}
...
- **Zip 해제/아카이브 처리**: ZIP 항목 이름이 `../../app.war` 등인 경우 해제 시 상위 디렉터리로 탈출(Zip Slip) :contentReference[oaicite:2]{index=2}

---

## 2) 공격 시나리오(예제 중심)

### 2.1 “로그 보기” 기능에서 시스템 파일 유출
...
→ 이후 SSH/DB 접속·서버 내 평문 키 파일 검색 등 2차 침투로 확장. :contentReference[oaicite:3]{index=3}

---

### 2.2 “이미지 미리보기”의 경로 조작
...
### 2.3 Zip Slip(아카이브 해제 시 Path Traversal)
...
이 취약점은 “**Zip Slip**”으로 대중화된 아카이브 기반 경로조작입니다. 다수 프로젝트에 영향을 준 전례가 있으니 반드시 방어 패턴을 적용하세요. :contentReference[oaicite:4]{index=4}
...
6) **다운로드 전용 헤더 & MIME 스니핑 방지**: `Content-Disposition: attachment`, `X-Content-Type-Options: nosniff`로 브라우저 렌더링 억제(콘텐츠 하이재킹 연계 차단) :contentReference[oaicite:5]{index=5}

> **정리**: “**ID→서버가 아는 경로**”가 최선입니다. 불가피하게 경로를 받는다면 **정규화→루트 내부 검증**을 필수로 적용하세요. :contentReference[oaicite:6]{index=6}

---

## 4) 취약 → 안전: 언어/프레임워크별 레시피

### 4.1 Node.js(Express) — 안전한 파일 다운로드
...
### 4.3 Java(Spring Boot) — Java NIO `normalize/resolve`
...
```
- Java NIO는 `Paths.get(...).resolve(...).normalize()` 조합으로 **`..` 제거 + 절대화**를 제공하므로 이를 활용하세요. (실무 팁/모범 사례는 Java NIO 보안 글 참고) :contentReference[oaicite:7]{index=7}

---

### 4.4 PHP — `realpath` + 프리픽스 체크
...
```
- 핵심: **항목 이름 검사 + `resolve().normalize()` + `startsWith(dest)`**. 이 문제는 2018년 Snyk가 **Zip Slip**으로 정리해 다수 프로젝트에 경고한 바 있습니다. :contentReference[oaicite:8]{index=8}  
- Android/백엔드 가이드도 **동일한 원리**를 권장합니다. :contentReference[oaicite:9]{index=9}

---

## 6) 운영 환경 방어(서버/프록시/스토리지)

- **다운로드 전용 헤더**: 모든 다운로드 응답에 `Content-Disposition: attachment` + `X-Content-Type-Options: nosniff` → 브라우저 **렌더링 시도 억제**(특히 PDF/HTML/SVG). :contentReference[oaicite:10]{index=10}  
...
PortSwigger 아카데미의 랩(우회 기법, 필터 회피)을 통해 자동화 테스트에 바꾸기 좋습니다. :contentReference[oaicite:11]{index=11}

---

## 8) 실제 사고로 이어지는 흐름(케이스 스터디 요약)

1) **로그/이미지 다운로드 기능**에 Traversal → **서버 설정/소스/크리덴셜** 유출  
2) 유출된 **DB/클라우드 키**로 2차 접근 → **데이터 대량 유출**  
3) 업로드/Zip 해제 기능의 Zip Slip로 **임의 파일 쓰기 → RCE/웹셸**  
→ Path Traversal은 “정보 유출”에서 끝나지 않고 **RCE까지 확장**될 수 있습니다. :contentReference[oaicite:12]{index=12}
...
- **Spring**: `Resource`/`ResourceUtils`로 직접 경로를 받지 말 것, `Paths`/`normalize`/`toRealPath` 조합으로 루트 내부만 허용(문자 제한 포함). Java NIO 보안 팁 참고. :contentReference[oaicite:13]{index=13}  
...
- **OWASP**: Path Traversal 개요(예/우회/방어), Cheat Sheet Series. :contentReference[oaicite:14]{index=14}  
- **PortSwigger Web Security Academy**: Path Traversal 개념·랩·후속 공격. :contentReference[oaicite:15]{index=15}  
- **Zip Slip**: Snyk 리서치/깃허브 PoC, Android/일반 가이드. :contentReference[oaicite:16]{index=16}  
- **MDN**: `X-Content-Type-Options: nosniff`·MIME 검증(브라우저 스니핑 억제). :contentReference[oaicite:17]{index=17}  
- **Java NIO 모범 사례**(경로 정규화/검사). :contentReference[oaicite:18]{index=18}
