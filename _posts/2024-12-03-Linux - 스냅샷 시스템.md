...
스냅샷은 “현재 시점의 **메타데이터(블록 맵)**를 얼려서 **블록을 공유**”하는 기술이다. 데이터 변경이 발생하면 **원본 블록을 보존**한 채 **새 블록에 쓰기**(Copy-on-Write) 하므로, 스냅샷 직후에는 추가 공간을 거의 차지하지 않는다.

### 핵심 직관

- 스냅샷 생성 시: **메타데이터만 복제** → 즉시/저비용
- 이후 변경: **변경된 블록만** 새로 할당 → 스냅샷과 현재본(라이브)이 **공간을 공유**
- 스냅샷 삭제: 해당 스냅샷만 참조하던 블록이 있으면 **그 블록만 회수**

### 간단한 비용 모델(직관식)

스냅샷 이후 총 추가 사용 공간을 \(S\), 변경 비율을 \(p\), 총 데이터 크기를 \(D\)라 하면,
$$
S \approx p \cdot D
$$
스냅샷이 많을수록 **중복 참조 구간**이 늘어나며, 프루닝(오래된 스냅샷 삭제) 시 회수되는 공간은 **유일 참조 블록**의 합으로 근사할 수 있다.

---

## 🌲 2. Btrfs — 서브볼륨, 스냅샷, CoW 운영

Btrfs는 **서브볼륨(subvolume)** 단위로 스냅샷을 만든다. 루트에 여러 서브볼륨을 두고 마운트 단위로 관리하는 레이아웃이 정석이다.
...
DB/VM 이미지 등 **랜덤 쓰기 대파일**에 CoW가 오버헤드일 수 있다. 파일 생성 **이전**에 NOCOW 속성 부여:
```bash
...
```
> NOCOW는 Btrfs의 체크섬/스크럽 이점 일부 상쇄. 정말 필요한 워크로드에만 사용.

### 공간 유지/회수 도구

```bash
# 균형 재배치(프래그/공간 파편화 대응) — 부분적/프로필별 권장
...
```
- **balance**는 과하면 성능/내구성에 지장. 작은 단위로 점진 수행.
- **scrub**는 정기 수행(예: 월 1회) 권장 → 조기 오류 감지/복구.
...
```
- Btrfs는 네이티브 증분(`-p <parent>`). WAN전송 시 압축 스트림(`zstdmt`) 파이프 조합 가능.

---

## 💽 3. ZFS — 데이터셋/풀, 스냅샷/클론, 전송

ZFS는 **풀(zpool)** 위에 **데이터셋(dataset)**을 만든다. 스냅샷은 `dataset@name`, 클론은 `dataset/clone`으로 표현된다.
...
- **ZIL/SLOG**: 동기쓰기 가속 장치(저지연 SSD). DB/VM 동기 로그에 효과.
...
- **CoW**: 변경 블록만 추가 저장 → 스냅샷 비용↓
- **압축**: zstd/lz4 등으로 저장 효율↑, IO 감소 → 실제 체감 속도↑인 경우 다수
...
- **ZFS dedup**: 동일한 블록 해시로 **1회만 저장**.
  - 메모리/CPU 오버헤드 큼(해시 테이블). **백업 리포지토리/아카이브**처럼 진짜 중복이 많은 워크로드에만.
- **Btrfs dedup**: 온라인/오프라인 도구로 유저랜드 수준(예: `duperemove`, `beesd`). 운영 난이도 고려.
...
1) **스냅샷 ≠ 백업**
   - 같은 장비/풀에만 있다면 **장치 고장/실수/랜섬웨어**에 취약.
   - **오프사이트/오프라인**(다른 서버/스토리지)에 **증분 전송**을 반드시 추가.

2) **데프래그/밸런스 남용 금지**
   - CoW 파일시스템은 메타데이터 업데이트가 빈번. 비용 큰 작업은 **부분/계획적 수행**.

3) **ZFS dedup는 신중히**
   - 메모리 및 성능 비용이 큼. **아카이브/이미지 저장소** 등 중복 블록이 압도적으로 많은 곳에만.

4) **NOCOW 사용은 최소화**
   - 무결성/체크섬 이점을 잃는다. DB/VM 등 필요한 대상만 사전에 지정.

5) **부트 통합**
   - 루트 서브볼륨 전환 시 GRUB/호출자에게 `subvol=` 파라미터가 정확히 전달되어야 한다.
...
D' \approx r \cdot D
$$
`zstd`는 일반적으로 텍스트/로그/소스코드에서 \(r \ll 1\) (고압축), 영상/이미지(이미 압축 포맷)는 \(r \approx 1\).
...
$$
(\(\epsilon\)은 메타데이터/헤더 오버헤드). **짧은 주기 스냅샷**은 \(\Delta\)를 작게 유지 → 백업 창구/대역폭 안정.

### 블록 크기와 랜덤 IO

레코드사이즈(또는 Btrfs 내부 청크/노드)의 최적화는 워크로드별 **랜덤 vs 순차** 패턴과 관련.
- 순차 대파일: 큰 recordsize(예: ZFS 1M) → 대역폭 효율↑
- DB/로그: 작은 recordsize(예: 16K/8K) → 부분 업데이트 효율↑
...
- **스냅샷은 “시간의 북마크”**이며, CoW 덕분에 **빠르고 경제적**이다.
- **Btrfs**는 서브볼륨 기반으로 **데스크탑/서버 모두**에서 유연하고, **ZFS**는 강력한 **무결성/복제/튜닝** 무기를 제공한다.
- 실전은 **레이아웃 설계 → 마운트/압축 → 스냅샷 스케줄 → 증분 전송 → 프루닝 → 정기 스크럽**의 루틴을 **자동화**하는 일이다.
- 마지막으로, 스냅샷은 로컬 사고에 취약하므로 **오프사이트 증분 백업**을 반드시 같이 운영하라.
