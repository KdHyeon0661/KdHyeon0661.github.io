...
## 2. Heap 상세 — Young/Old, TLAB, 압축 참조, 객체 레이아웃

### 2.1 세대(Generation)와 수집 개요
...
### 2.3 객체 메모리 레이아웃(HotSpot 64-bit, 기본 가정)
...
### 2.4 Compressed OOPs & Class Pointers
...
## 3. Method Area / Metaspace / Constant Pool
...
## 5. Java Stack, Frame, PC Register
...
## 6. 클래스 로더 서브시스템 — 위임 모델과 라이프사이클

### 6.1 위임(Parent-Delegation) 모델
...
## 7. 실행 엔진 — 인터프리터, JIT, Deoptimization, Escape Analysis
...
## 8. GC/안전지점(Safepoint) — 매우 중요한 실행 제어
...
### 10.2 Metaspace OOM(동적 클래스 로딩 누수)
...
## 11. 문자열 상수풀, interning, deduplication
...
## 12. Java Memory Model(JMM) 핵심 — 가시성과 재배치
...
## 13. 네이티브/오프힙 — DirectByteBuffer, JNI, NIO
...
### 18.1 런타임 메모리/GC 상태를 프로그래매틱하게 확인
