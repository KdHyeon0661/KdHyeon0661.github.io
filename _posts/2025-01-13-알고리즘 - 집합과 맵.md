...
### 1.3 자료형 주의: **키는 불변(immutable)이여야 함**
...
### 2.2 카운팅 최강 콤보: `collections.Counter` / `defaultdict(int)`
...
## 3. 당신의 초안: 대표 문제 5종 — 풀이를 “완전” 보강

### 3.1 숫자 카드 (BOJ 10815) — Set 탐색
...
### 3.3 회사에 있는 사람 (BOJ 7785) — Set 시뮬레이션
...
### 3.4 걸그룹 마스터 (BOJ 16165) — 양방향 매핑(dict↔dict)
...
### 3.5 서로 다른 부분 문자열 개수 (BOJ 11478) — Set + 문자열
...
## 4. 실전 패턴: Set/Map으로 푸는 고전 과제들

### 4.1 두 수의 합(2-Sum) — Map으로 보완 검사
...
### 4.3 부분합으로 어떤 합이 되는 부분배열 개수 — Map 누적
...
### 4.4 윈도우에 포함된 서로 다른 문자 수 — Set/Map 혼용
...
### 4.5 그래프 인접 리스트 — Map of Set/List
...
### 6.2 사용자 정의 객체를 키로 — `__hash__`, `__eq__`
...
## 8. 문제 템플릿 모음 — 바로 가져다 쓰기

### 8.1 문자열 집합(BOJ 14425) — Set
...
### 8.2 나는야 포켓몬 마스터(BOJ 1620) — 양방향 맵
...
### 8.3 패션왕 신해빈(BOJ 9375) — 카테고리별 조합 카운트(Map)
...
## 9. “정렬 없는 탐색”을 더 강하게: 집합 연산으로 사고 전환
...
## 12. 심화: OrderedDict / 삽입 순서 / 정렬된 맵이 필요할 때
...
## 14. 종합 예제: “서로 다른 문자 K개 이하의 최장 부분문자열” + 출력 복원
