...
### 2.2 구성(Instantaneous Description, ID)
한 순간의 기계 상태를
$$
...
## 7. 결정 불가능성: 정지 문제(Halting), Rice의 정리, 대각선
...
### 7.2 \(A_{TM}=\{\langle M,w\rangle\mid M \text{이 } w \text{를 수용}\}\)은 결정 불가능
...
## 10. 예제 ③: 단항 덧셈 \(1^m\#1^n \mapsto 1^{m+n}\)
...
delta = {}
# scanL: 왼쪽에서 미표식(0/1)을 찾는다. 없으면(모두 X/_) 수용.
delta.update({
    ('scanL','X'): ('scanL','X','R'),
...
## 14. 예제 ④: 단항 덧셈 \(1^m\#1^n \mapsto 1^{m+n}\) TM
...
## 16. 환원(Reduction)과 부정결정성 증명 스케치
