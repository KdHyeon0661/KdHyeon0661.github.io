...
B-Tree 인덱스는 **키 순으로 저장/유지되므로 자연 정렬 순서 자체가 존재**하고, 옵티마이저가 그 순서를 그대로 활용하면 소트 연산을 완전히 제거할 수 있다.

---

## 1) 정렬 대체의 기본 원리(충분조건)

### ORDER BY 대체의 충분조건(직관)
...
### IFFS(Index Fast Full Scan)는 왜 안 되는가?

`INDEX FAST FULL SCAN`은 인덱스를 **테이블 풀스캔처럼 멀티블록/병렬/랜덤 방식으로 읽는다.**
그래서 **인덱스의 키 순서를 “그대로” 보장하지 않는다.**
...
- 인덱스가 이미 내림차순으로 정렬되어 있으므로 **추가 소트가 불필요**.
- 만약 옵티마이저가 IFFS를 선택하면 정렬이 보장되지 않으므로 소트가 다시 생긴다.

### 복합 정렬 제거(여러 컬럼/혼합 방향)
...
### 키셋(Seek) 페이지네이션으로 OFFSET 정렬 비용 제거
...
GROUP BY가 **순서 유지 입력을 받아 NOSORT가 성립**한다.
...
### GROUP BY ELIMINATION(그룹 연산 자체 제거)

GROUP BY 대상이 **PK/유니크**이고 **NOT NULL**이면
“그룹을 만들 이유가 없다”고 판단해 GROUP BY 자체를 제거할 수 있다.
...
**문제**: `INDEX FAST FULL SCAN`은 순서를 보장하지 않아 ORDER BY 제거가 불가.
...
- `INDEX FAST FULL SCAN`이 있으면 ORDER BY 제거 실패 신호.
