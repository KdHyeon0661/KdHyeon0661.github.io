...
## 1. Linux Capabilities — 루트 권한을 조각내어 최소화

### 1.1 Capabilities 핵심 개념
...
### 1.3 현재 프로세스·바이너리 Capabilities 확인/부여
```bash
# 현재 셸의 capability 상태
capsh --print
...
### 2.3 C 예제: libseccomp로 간결 허용 목록 구성
```c
...
## 3. ptrace — 디버깅의 칼날을 둔화시키는 YAMA 정책
...
## 5. 실전 시나리오 — 패턴으로 익히는 최소 권한

### 5.1 저권한 웹서버(80/443) 바인드
```bash
# (A) 파일 capability 방식
...
### 5.2 백엔드 작업자: 쓰기 금지 FS + 제한된 syscall
```bash
...
### 5.3 데이터 수집기: raw 소켓 금지, ptrace 완전 봉쇄
```bash
...
## 11. 실습 패키지 — 최소 샌드박스 샘플

### 11.1 샌드박스 바이너리(파일 cap + seccomp)
```bash
# 1. 필요한 syscall 수집
strace -f -o trace.log ./mytool --once

# 2. seccomp 프로파일(런타임 기본) + 파일 capability 부여
sudo setcap cap_net_bind_service+ep /usr/local/bin/mytool

# 3. systemd 유닛에서 추가 보호
