...
## 1. 핵심 개념 — “모든 것은 파일이며 FD로 식별된다”

### 1.1 파일 디스크립터(File Descriptor, FD)
...
## 2. 필수 시스템 콜 — 안전한 사용 패턴까지

### 2.1 `open/openat` — 열기(경쟁·보안 안전)
```c
#include <fcntl.h>
...
### 2.2 `read` / `write` — 부분 I/O·EINTR/EAGAIN
```c
#include <unistd.h>
...
### 2.6 Scatter/Gather — `readv/writev`
...
### 2.7 고성능 편의 — `sendfile/splice/copy_file_range`
...
## 3. 블로킹·논블로킹·다중화(io multiplexing)

### 3.1 `select/poll/ppoll` 기본 패턴
```c
#include <poll.h>
...
## 5. 버퍼 캐시·Direct I/O·동기화

### 5.1 커널 버퍼 캐시
- 일반 `read/write`는 **페이지 캐시**를 거친다(읽기 재사용/쓰기 병합).

### 5.2 `O_DIRECT` — 페이지 캐시 우회
- 장점: **이중 캐싱 회피**, 예측적 성능.
...
## 8. 메모리 매핑 I/O — `mmap/munmap/mprotect`
...
### 10.1 안전한 파일 복사 — `sendfile` 버전(커널 제로-카피)
```c
...
## 12. UNIX I/O vs. stdio(`fopen/fread/fwrite`) — 언제 무엇을?
