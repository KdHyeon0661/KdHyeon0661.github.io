...
> 모든 소스는 ``` 코드블록 ```으로 감싸고, 구성파일(.env, Dockerfile, compose 등)은 그대로 사용 가능하도록 작성한다.
...
### 4.2 `conf/nginx-site.conf` (server 블록)
...
### 5.1 `docker/app.Dockerfile` (멀티스테이지)
...
### 6.1 `scripts/wait-for.sh` (간단 포트대기)
...
### 8.3 `docker-compose.prod.yml` (프로덕션 오버레이)
...
## 9. 마이그레이션(Alembic/Flask-Migrate)
...
### 10.1 Nginx 컨테이너에서 Certbot (간편 예)
...
## 14. 배포 파이프라인(CI/CD) 예시

### 14.1 GitHub Actions (요약)
