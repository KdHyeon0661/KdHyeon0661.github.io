...
### 1.3 보존장·경로독립성(Gradient Theorem)
\(\mathbf F=\nabla \phi\) (보존장)이면
$$
...
### 5.1 선적분 \(\int_C \mathbf F\cdot d\mathbf r\)
1) 경로 매개화 \(\mathbf r(t)\).  
2) \(\mathbf r'(t)\) 계산.  
...
### 5.2 면적분 \(\iint_S \mathbf F\cdot d\mathbf S\)
...
## 6. 파이썬 코드 예제 (심볼릭/수치 검증)

> 실행 환경이 있으면 그대로 복붙해 확인해보세요.

### 6.1 SymPy로 스토크스 정리 상징 검증
```python
import sympy as sp
...
# Line integral over unit circle r(t) = (cos t, sin t, 0)
...
# Parametrize disk by (u,v) in polar: u=r, v=theta
...
### 6.2 NumPy로 수치 적분(원둘레 선적분 vs 포물면 캡 면적분)
```python
import numpy as np
...
### Q4. 면적분: \( \mathbf F=(x,y,1) \), \(S:\ z=1-x^2-y^2,\ x^2+y^2\le 1 \), 상향  
