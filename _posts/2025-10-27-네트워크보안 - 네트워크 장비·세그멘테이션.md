...
## 10.1 L2 보안: Port Security, DAI, DHCP Snooping
...
### 10.1.2 Port Security (예: Cisco IOS-XE 스타일)
...
### 10.1.3 DHCP Snooping (스푸핑/사칭 방지)
...
### 10.1.4 Dynamic ARP Inspection (DAI)
...
### 10.1.6 Open vSwitch/리눅스 브리지(대안)
...
## 10.2 L3/L4 보안: ACL/uRPF/RTBH/FlowSpec
...
### 10.2.2 uRPF (Unicast Reverse Path Forwarding)
- 목적: **소스 IP 스푸핑** 차단.  
- 모드:
...
### 10.2.3 RTBH (Remotely Triggered Black Hole)
...
### 10.3.3 eBPF 기반 마이크로세그 (K8s: Cilium 예)
...
### 10.3.4 eBPF(XDP) 미세 예제 — 단순 L4 차단(학습용)
...
**기능 테스트**
```bash
# 1. User → App 443 OK (mTLS 필요)
curl -vk --cert user.p12 https://zta.corp.example/

# 2. App → DB 3306 OK, 기타 포트 실패
