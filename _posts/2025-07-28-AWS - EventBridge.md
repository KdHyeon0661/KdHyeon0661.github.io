...
#### 8.1.3 Lambda 타겟(결제) — 타입 안정 처리(스키마 반영 가정)
...
### 8.2 API Destinations로 외부 웹훅 호출
...
### 8.3 Pipes: SQS → 변환Lambda → Step Functions
...
### 16.2 컨슈머(Lambda, Python) — InputTransformer로 정규화된 입력 가정
