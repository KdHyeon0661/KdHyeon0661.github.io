...
## 1. 준비: 실습 스키마/데이터/통계 (선택도·스큐·조인·변환 시연용)
...
### 3.2 쿼리 변환(Query Transformation)
...
### 3.5 코스트 계산 & 최저 비용 플랜 선택
- 각 후보마다 cost를 산출  
- **가장 낮은 cost** 플랜이 최종 선택된다.

### 3.6 실행 중 적응(Adaptive)
버전별로 차이는 있지만, 기본적으로:
- 실행 중 관측된 통계/피드백으로  
...
## 4. RBO vs CBO (왜 CBO가 표준인가)

### 4.1 RBO (Rule-Based Optimizer)
- 통계 없이 미리 정해진 우선순위 규칙으로 플랜 선택  
- 데이터 분포/스큐/파티션/병렬/시스템 성능을 반영 못함  
- 현대 Oracle에서는 사실상 폐기

### 4.2 CBO (Cost-Based Optimizer)
- 통계 기반으로 실제 비용을 모델링  
- 데이터/시스템 변화에 대응  
...
### 6.5 확장 통계(Extended Stats)
- 컬럼 그룹: 상관관계 반영  
- 표현식 통계: 함수/가상 컬럼의 분포 반영

---

## 7. 옵티마이저 모드(ALL_ROWS vs FIRST_ROWS_n)
...
## 8. 액세스 경로(Access Path) 전부 파헤치기

### 8.1 Table Full Scan (FTS)
- 테이블 블록을 연속적으로 읽는다.
...
### 8.5 Index Fast Full Scan (IFFS, FFS)
...
## 9. 조인 방법(NL / Hash / Sort-Merge) 완전 비교

### 9.1 Nested Loops Join (NL)
...
### 9.4 조인 순서가 왜 그렇게 중요한가?
- 조인 방법은 “입력 카디널리티”에 민감하다.
- 순서가 바뀌면 각 단계 입력량이 달라져 **코스트가 전혀 다른 세계**가 된다.

---

## 10. 히스토그램/스큐가 플랜을 바꾸는 실제 흐름

### 10.1 BRAND 스큐: 인기값 vs 희소값
...
### 10.2 REGION 스큐: CUST 쪽 필터가 조인 순서를 바꾼다
...
## 11. 모드가 실제 플랜을 바꾸는 대표 시나리오 (Top-N)
...
## 13. Query Transformation(변환) 핵심 8가지 + 실전 예

> 변환은 “옵티마이저가 더 좋은 플랜을 찾도록 SQL을 펼치는 과정”이다.

### 13.1 View Merging (뷰 머지)
...
### 13.2 Predicate Pushdown
필터를 가능한 하단(베이스 테이블/파티션/인덱스)으로 내린다.

### 13.3 Predicate Transitivity(조건절 이행)
...
### 13.4 Subquery Unnesting
서브쿼리를 조인으로 풀어 조인 순서 탐색이 가능해짐.

### 13.5 OR Expansion (USE_CONCAT)
...
### 13.6 Aggregate Pushdown / Group By Placement
집계를 더 아래로 내려 조인 입력량을 줄이는 변환

### 13.7 Join Elimination
FK/PK와 NOT NULL 제약이 엄격할 때 불필요 조인을 제거

### 13.8 Star Transformation
스타 스키마에서 차원 필터를 팩트로 강하게 전파

---

## 14. 적응형 기능(Adaptive) 해부

### 14.1 Dynamic Sampling
...
### 14.2 Adaptive Cursor Sharing(ACS)
...
### 14.3 Statistics Feedback(카디널리티 피드백)
반복 실행 중 실제 행수를 관측해 다음 하드파싱의 카디널리티를 보정.

---

## 15. 플랜이 흔들릴 때(운영 문제) — 원인/진단/해결 로드맵

### 15.1 가장 흔한 원인 TOP 10
1) 통계 노후/오염  
...
### 17.1 시나리오 A — 인기값에서 인덱스 플랜이 붙어 폭망
...
### 17.2 시나리오 B — 결합 조건 상관관계로 조인순서 뒤집힘
...
### 17.3 시나리오 C — Top-N 화면이 느림(정렬 과다)
