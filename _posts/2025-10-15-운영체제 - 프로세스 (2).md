...
이 장은 **IPC의 핵심 개념(3.4)**, **공유 메모리 기반 IPC(3.5)**, **메시지 패싱 기반 IPC(3.6)** 를 **실행 가능한 예제 코드**와 함께 상세히 설명한다.  
주요 대상 환경은 리눅스/유닉스 계열이며, 일부 예제는 관리자 권한이 필요할 수 있다.

---

## 3.4 Interprocess Communication (개요)
...
### 3.5.1 POSIX 공유 메모리 + 세마포어 (단일 생산자/소비자)
...
### 3.5.3 메모리 매핑 파일(`mmap`)을 이용한 Zero-Copy 파이프라인
...
### 3.6.2 UNIX 도메인 소켓 (로컬 IPC / 파일 경로 기반)
...
#### 파일 디스크립터(FD) 전송(요약: `sendmsg/recvmsg` + `SCM_RIGHTS`)
- 관리 프로세스가 **열린 소켓/파일**을 워커에게 넘겨 핸드오버

---

### 3.6.3 TCP/UDP 소켓 (프로세스/호스트 간)
