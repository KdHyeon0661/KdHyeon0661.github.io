...
## 1) 위협 모델(Threat Landscape) — 어디로, 무엇이, 어떻게 침해되는가
### 1.1 공격 표면 요약
| 계층 | 대표 위협 | 발생 맥락 | 영향 |
|---|---|---|---|
...
## 3) 네트워크/호스트 통제
### 3.1 방화벽·세그멘테이션 규칙(예: 리스너만 앱 서브넷에서 접근)
...
## 6) 안전한 쿼리/프로시저
### 6.1 SQL Injection 방지 — **준비문(Prepared Statement)** 강제
```python
# psycopg2 예
...
### 8.5 Oracle Unified Auditing(요지)
```sql
