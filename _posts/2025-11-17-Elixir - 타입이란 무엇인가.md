...
### 9.1.1 런타임 타입 vs 타입 명세(Typespec)
...
### 9.1.4 가드와 패턴 매칭은 “런타임 정제(Refinement)”
...
### 9.1.5 @type / @opaque / @typep — 공개/캡슐화/모듈 내부
...
#### 1) no match of right hand side value / no_return 관련
...
### 9.1.13 no_return/none — 종료/예외 함수
...
### 9.1.15 실전 미니 가이드: “타입이 문서이자 테스트다”

이제 실제 코드 설계 흐름에 타입을 녹이는 패턴을 보자.

#### A) API 경계: 외부(JSON) → 내부(Struct)
