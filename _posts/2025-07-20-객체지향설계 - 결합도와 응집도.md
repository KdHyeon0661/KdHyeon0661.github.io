...
### 시간적 결합 제거 → 명시적 라이프사이클

- `init()`/`start()`/`stop()`을 **상태 모델**로 강제, 타입으로 제약

### 응집도 상승 → 기능별 모듈/클래스 분해
