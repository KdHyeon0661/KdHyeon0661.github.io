...
### 3.3 예제(범위 조회를 I2에서, 직전에 I1이 읽어서 따끈한 상태)
```sql
...
## 4. **읽기/쓰기 전송(Read/Write Transfer)**
...
## 5. **쓰기/쓰기 전송(Write/Write Transfer)**
...
### 5.3 예제(동일 블록을 두 인스턴스가 번갈아 UPDATE)
```sql
...
## 6. **쓰기/읽기 전송(Write/Read Transfer)**
