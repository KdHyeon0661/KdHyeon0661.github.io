...
Oracle 12c 이후 히스토그램은 **Frequency / Top-Frequency / Hybrid** 중심으로 자동 선택된다.
...
### 값 스큐(편중)가 있을 때 — 히스토그램 필요

스큐 컬럼에서 균등 가정은 치명적이다.

Oracle은 **스큐가 의심되는 컬럼(사용 빈도/분포 기반)** 에 자동 히스토그램을 생성한다.
...
- **Hybrid**: 12c+의 기본, 구간+빈도 혼합형으로 넓은 NDV에서도 스큐를 포착
...
이건 Oracle이 공식적으로 밝히는 인덱스 저장 규칙과一致한다.

### NULL도 인덱스를 타게 하려면: FBI
...
### 확장 통계(Extended Statistics, Column Group)

Oracle은 같은 테이블 내 컬럼 간 상관을 **컬럼 그룹 통계**로 교정한다.
...
- 이후 `region='KOR' AND tier='VIP'`의 결합 선택도를
  “실제 동시 빈도”에 가깝게 추정한다.
- **조인 순서/조인 방식 선택에 직결**되는 개선점이다.

### 확장 통계는 “같은 테이블 내부”만 가능

Oracle은 **테이블 간 컬럼 상관**을 직접 통계로 저장하지 않는다.
따라서 다중 테이블 상관은 주로
...
- 히스토그램의 **누적 분포(CDF)** 로 구간 비율을 산출한다.
...
를 이용해 근사한다.
(정확한 수식/기본값은 버전·조건 유형별로 다르므로, **실측 플랜이 정답**이라는 원칙을 유지하라.)
...
현대 연구에서도 **조인 카디널리티 추정이 플랜 품질의 핵심 병목**임을 지적한다.
...
이 영역은 **히스토그램 + 확장 통계(테이블 내부) + 동적 통계**가 함께 필요하다.

---

## 9) 동적 통계(Dynamic Statistics, 구 Dynamic Sampling)

통계가 없거나 부정확할 때, Oracle은 **하드 파스 중 샘플링**으로 카디널리티를 보정한다.
...
기본 레벨(대개 2)은 **“통계 없음/부족”일 때만 제한적으로 발동**한다.
...
2. 그래도 오판이 남는 특정 SQL에만 동적 통계가 쓰이도록 유도/허용한다.
...
다음 실행에서 **피드백을 활용해 재최적화**하는 기능을 제공해 왔다(버전별 이름/범위 변화).
...
실무 사례에서도 ascending keys에서 오판이 흔하다고 보고된다.
...
- **최후수단**이다.
  통계/SQL 구조로 근본 해결해야 플랜이 장기적으로 안정된다.
...
이 우선순위는 Oracle Optimizer 팀의 권고(통계 품질 우선, 동적 통계는 보조)와 일치한다.

---

## 16) 자주 나오는 함정 패턴과 처방

### NVL/함수로 컬럼 가공 → 카디널리티 붕괴
...
- [ ] **스큐 컬럼**은 히스토그램 없으면 반드시 오판한다.
- [ ] **상관 컬럼**은 확장 통계(컬럼 그룹)로 교정하라.
- [ ] **범위/시계열/파티션**은 partition/증분/실시간 통계까지 점검하라.
- [ ] 통계가 충분한데도 오판이 남으면 **동적 통계**를 보조로 쓴다.
- [ ] **CARDINALITY 힌트는 최후수단**이고, 장기적으론 통계/SQL이 정답이다.
- [ ] 조인 카디널리티는 가장 많이 틀린다. **필터가 조인 키 NDV를 바꾸는지** 항상 의심하라.
...
AUTO 샘플 크기를 유지해야 최신 히스토그램(Top-Frequency/Hybrid)이 제대로 생성된다는 점은 Oracle이 공식 권고한다.
