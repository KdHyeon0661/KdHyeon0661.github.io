...
#### 예) Reservoir Sampling (스트리밍 무작위 k개)
...
#### 예) 병렬 Prefix-Sum(스캔) 개요

```text
# 업스윕(트리로 부분합), 다운스윕(누적) — 스팬 O(log n), 작업 O(n)
```

### 5.2 Work–Depth 설계 체크
- 분할 비율 균형?
...
## 7) 케이스 스터디 — 모델·설계의 실제 효과

### 7.1 초대형 그래프 BFS/SSSP (메모리 밖)
