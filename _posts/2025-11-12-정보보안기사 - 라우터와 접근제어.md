...
## 3. Wildcard(IPv4)와 Prefix(IPv6) — 정확히 매칭하기
...
## 7. Cisco IOS/XE — IPv4 Extended iACL 실전
### 7.1 토폴로지
```
...
### 9.2 PBR(Policy-Based Routing) 트래픽 정의
```cisco
...
## 11. Juniper JunOS — Firewall Filter(=ACL) 기본
### 11.1 인프라 보호(루프백 lo0 수신 보호)
```junos
...
## 12. Linux Router — nftables로 iACL/관리면 보호
```bash
...
## 13. 시간/객체/시퀀스 — 운영 편의 기능
### 13.1 time-range (Cisco 예)
```cisco
...
## 17. 실습 시나리오 — “엣지 iACL + 관리/제어 보호” 종합
...
### 20.2 JunOS: firewall filter 카운트/폴리싱
```junos
