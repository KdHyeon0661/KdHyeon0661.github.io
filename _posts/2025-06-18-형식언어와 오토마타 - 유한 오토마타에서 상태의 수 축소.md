...
## 2. 최소화의 큰 흐름(Partition Refinement 관점)
...
## 5. 표 채움(Table-Filling) 방식(부록용 요약)
...
### 6.4 최종 파티션 & 최소 DFA 상태

- \(P=\{\,[A,B],\ [C,E],\ [D],\ [F]\,\}\)
  (여기서 \([X]\)는 집합 \(X\)의 병합 상태를 뜻함)
- 상태 수: **6 → 4**

### 6.5 최소 DFA 전이(대표 전이로 구성)
...
## 8. Python 구현(실행 가능): Hopcroft / Moore / 도달성 / 동등성 테스트
