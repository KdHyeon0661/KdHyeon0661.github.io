...
---
# 4.1.3 Block Coding & 4.1.4 Scrambling — 확장본

## 0) 들어가며 — 왜 Block Coding·Scrambling을 쓰는가
...
### 3.4 간단 러닝 디스패리티 흐름(예시 코드)
> 실제 8b/10b 테이블은 길고 복잡하므로, 아래는 **RD 선택 로직 스켈레톤** 예시입니다.

```python
# 가짜(예시) 테이블: 실제 표준 테이블과 다릅니다!
...
## 4) Scrambling — 비트 통계 평탄화·런 분해·CDR 개선
...
### 5.3 HDB3 (유럽 E1) — 4개의 `0` 치환 + DC 균형 고려
...
## 6) 블록코딩 vs 스크램블 vs 치환(AMI+HDB3/B8ZS) — 언제 무엇을 택하나
