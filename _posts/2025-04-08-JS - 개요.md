...
### 3.5 함수가 일급 객체(First-class Function)
...
### 3.6 비동기 모델: 이벤트 루프, 태스크 큐

#### 3.6.1 콜백 → Promise → async/await
...
## 5. 모듈 시스템: ES Modules vs CommonJS

### 5.1 ES Modules (브라우저/Node 최신)
...
## 8. 비동기 심화: 경쟁·제한·재시도 패턴

### 8.1 동시성 제한(Concurrency Limiter)
...
### 13.3 데이터 파이프라인: map/filter/reduce
...
### 13.4 fetch + AbortController(요청 취소)
