...
### `db file sequential read` (단일 블록 I/O)
...
### `db file scattered read` (멀티블록 I/O, 테이블/인덱스 풀)
...
### `direct path read/write`, `direct path read temp`
...
### `enq: TX - row lock contention` (행 잠금)
...
### `library cache: lock/pin`, `cursor: mutex S/X`, `row cache mutex`
...
### `gc cr/current block busy`, `gc buffer busy acquire/release`
