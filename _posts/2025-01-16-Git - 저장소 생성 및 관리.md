...
## GitHub에서 저장소(Repository) 만들기 — 기본 절차(기존 내용 보강)
...
## 로컬에서 Git 저장소 초기화 및 원격 연결 — 가장 단순한 성공 경로
...
### 시나리오 A: GitHub에 **빈 저장소**(README 없음) → 로컬에서 최초 푸시
...
### 시나리오 B: GitHub에서 **README를 생성**한 경우(초보자 주의 포인트)

- 원격에 **이미 커밋(README)** 이 있으므로, 로컬에서 바로 `push` 하면 **거부**됩니다.
- 해결 절차(안전):
```bash
# 로컬에 내용이 이미 있다면, 먼저 원격의 초기 커밋을 당겨옵니다.
...
## 로컬에서 먼저 만들고 GitHub로 올리기 — “빈 원격” 기준 권장 절차
...
## Sparse/Partial Clone, 대용량 파일(LFS)

### Sparse checkout(일부 경로만)
...
## GitHub CLI(gh) — 터미널에서 저장소 생성부터 PR까지
...
## 간단 CI 예제(GitHub Actions) — 푸시/PR 시 빌드·테스트
...
### “GitHub에서 README 만들지 않았다(빈 원격)” → 로컬에서 올리기
...
### “GitHub에서 README가 이미 있다” → 로컬이 뒤섞이지 않게 시작
...
## 고급 트러블슈팅(원인 추적 예시 포함)

### “푸시가 거부된다(non-fast-forward)”
...
## 권장 커밋 메시지 포맷(Conventional Commits)
