...
## 1. DCL 핵심 명령 — GRANT / REVOKE (+ DENY in SQL Server)
...
## 3. 공통 시나리오: RBAC 설계 → GRANT/REVOKE 배포
...
## 4. 행·열·객체 단위: “필요한 만큼만” 주는 법

### 4.1 열(Column) 단위 권한 (엔진별 지원/제약)
...
### 4.2 행(Row) 단위 권한 — Row-Level Security (RLS)
...
## 5. WITH GRANT OPTION / ADMIN OPTION — 권한 확산 제어
...
### 7.2 Oracle/SQL Server/MySQL 대안
...
## 8. 보안 패턴: VIEW/PROC/OWNERSHIP CHAIN/EXECUTE AS

### 8.1 VIEW로 최소 공개 + 데이터 마스킹
...
## 12. 실전 레시피 모음 (Cookbook)

### 12.1 “리포트 전용 읽기 역할” 만들기
...
### 12.2 “애플리케이션 쓰기 최소 권한 (INSERT/UPDATE만)”
