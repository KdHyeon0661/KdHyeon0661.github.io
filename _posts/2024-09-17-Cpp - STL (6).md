...
## `insert` vs `emplace` vs **`try_emplace` / `emplace_hint`**
...
## 탐색: `find` / `count` / `lower_bound` / `equal_range` (정렬/정확 차이)
...
## 커스텀 정렬/비교 — “엄격 약한 순서(strict weak ordering)” 지키기
...
### 투명 비교자 — **heterogeneous lookup** (C++14~)
...
## 컨테이너 조합 패턴 — Group-by / Index / Multi-Map
...
## 해시 컨테이너 운영술 — `reserve`, 로드팩터, `rehash`
...
## 알고리즘 선택 — `sort/stable_sort`, `partial_sort`, `nth_element`
