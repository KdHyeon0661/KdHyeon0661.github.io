...
### 이산시간 \( \mathbf x_{k+1}=A\mathbf x_k \)
...
### 구조진동: \(K\mathbf u = \omega^2 M \mathbf u\)
...
## 종합 예제: PCA → 투영 → 그래프 분할 → 동역학 안정성

```python
import numpy as np

# 랜덤 3D 데이터 생성(두 개의 뭉치)
...
# 2D 공간의 k-최근접 이웃 그래프(간단: 완전그래프 대신 거리 임계)
