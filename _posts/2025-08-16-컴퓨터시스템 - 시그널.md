...
category: 컴퓨터시스템
---
# — 생성·전달·처리·마스킹·리얼타임·스레드·파일디스크립터(signalfd)까지 완전 정리
...
## 큰 그림: 시그널의 생애주기 (Generation → Queuing → Delivery → Disposition)
...
## 안전한 설치: `sigaction` 중심, `signal`은 지양
...
## 핸들러에서 “하면 안 되는 것” — Async-signal-safety
...
## 시그널 마스크/블로킹 — `pthread_sigmask`, `sigsuspend`, `pselect`
...
## 대체 시그널 스택 — `sigaltstack` + `SA_ONSTACK`
...
## 타이머와 시그널 — `alarm`, `setitimer`, `timer_create`
...
## `signalfd` — 시그널을 “파일디스크립터”로 읽기 (Linux 전용)
...
## 시그널의 “동기/비동기”와 `siginfo_t::si_code`
...
## Job Control & 터미널 관련 (포그라운드/백그라운드)
