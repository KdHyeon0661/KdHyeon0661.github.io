...
category: 시스템보안
---
# 8.4 실습: 랩 환경에서 “티켓 덤프/재사용” **모델** 만들기 → 탐지 로그 분석
...
## 8.4.2 “정상” Kerberos 흐름 발생 → 기준선 수집
...
## 8.4.3 “의심 패턴”을 **안전하게** 모의하여 탐지 규칙 검증
...
## 8.4.4 네트워크 관측(선택) — pcap/Zeek로 KRB5 트랜잭션만 보기
...
# 8.5 방어: PAM/SSO 하드닝, 2FA/U2F, Secret Rotation, LAPS/SCM

> 요점: “재사용 가능한 비밀(해시/티켓/토큰)”의 **가치와 수명**을 줄이고, **발급·사용·보관**의 모든 지점에 **강제 제약**을 둡니다.

---

## 8.5.1 PAM/SSO 하드닝 (Linux 중심 + SSO 일반 원칙)
...
## 8.5.3 Secret Rotation (Vault/Cloud KMS/SM) — 설계 & 예시
...
## 8.5.4 LAPS / “SCM(Services)” 하드닝 — Windows 운영 핵심
