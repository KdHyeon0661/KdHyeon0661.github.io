...
# (필수) 러너/SDK 추가 (템플릿에 이미 포함되어 있으면 생략)
...
## xUnit 기본: AAA 패턴과 `[Fact]`/`[Theory]`

### AAA(Arrange-Act-Assert)와 명명 규칙
...
## 데이터 주도 테스트 심화: `MemberData`, `ClassData`, `TheoryData`
...
## 픽스처: **IClassFixture**, **ICollectionFixture** (비용 큰 리소스 공유)

### IClassFixture — 클래스 단위 공유
...
### HTTP 테스트 — `HttpMessageHandler` 스텁
...
### 테스트 (Moq + StubHandler + FixedClock)
