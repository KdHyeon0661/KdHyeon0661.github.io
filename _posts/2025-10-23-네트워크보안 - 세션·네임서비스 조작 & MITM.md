...
## 5.1 세션 하이재킹 개념과 전제조건(쿠키·HSTS·H2/H3 고려)

### 5.1.1 무엇이 “세션 하이재킹”인가
...
### 5.1.3 HSTS와 HTTP/2, HTTP/3(QUIC) 고려
...
## 5.2 DNS 스푸핑/캐시 포이즈닝, Split-Horizon·DoH/DoT와 정책
...
## 5.3 IPv6 특화: ND 스푸핑, SLAAC 악용, RA 가짜 주입
...
## 5.4 실습: 합법 샌드박스에서 MITM→탐지 룰·차단 전략 수립

> **실제 MITM 공격 수행 금지.** 대신 **pcap 재생·합성 이벤트**로 **탐지→차단** 체인을 검증.

### 5.4.1 랩 아키텍처(컨테이너/네임스페이스)
```
...
### 5.4.2 합성 이벤트 생성기(“MITM 의심 상황”을 공격 없이 시뮬레이션)
...
### 5.4.3 pcap 재생으로 세션 보안 헤더·쿠키 검증(무해)
...
```zeek
# 5.2.4 예제를 활용. distinct answers > N within T => notice
```
...
### 5.4.6 회귀(Regression) 테스트 자동화(개념)
```bash
# 1. 합성 이벤트 생성 → 테스트 인제스트
