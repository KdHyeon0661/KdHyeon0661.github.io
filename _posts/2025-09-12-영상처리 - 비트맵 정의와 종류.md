...
### 비트맵 정의와 종류 (Raster vs. Vector, DDB vs. DIB)
...
### DIB의 좌표계: Bottom-Up vs Top-Down
...
### 비트맵 정보 헤더 (`BITMAPINFOHEADER` / V4 / V5)
...
### 그레이스케일 BMP(8bpp, BI_RGB)의 구조 보기
...
### 안전한 BMP 로더 구현 (BI_RGB / 8/24/32bpp, 16bpp 일부 지원)
...
#### 범용 로더(파일 → 내부 Top-Down BGRX32 버퍼)
...
#### `SetDIBitsToDevice` / `StretchDIBits` 로 그리기
