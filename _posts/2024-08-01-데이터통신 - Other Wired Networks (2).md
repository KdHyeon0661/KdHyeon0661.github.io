...
- **SONET(Synchronous Optical Network)**  
  ...
  - 상위 계층(전통 PDH: T1/E1/DS3, ATM, IP, Ethernet 등)을 **투명하게 싣고 장거리 전송**하는 역할:contentReference[oaicite:1]{index=1}  

- 국제적으로는 거의 동일한 개념의 **SDH(Synchronous Digital Hierarchy)**가 ITU-T에서 표준화  
  - SONET: **STS-N / OC-N** (미국식)  
  - SDH: **STM-N** (유럽·국제)  
  - 예: STS-3/OC-3 ≒ STM-1 = 155.52 Mbps:contentReference[oaicite:2]{index=2}  
...
- 상위에는
  ...
  - 지금은 **IP/MPLS, Carrier Ethernet, OTN** 등이 올라탐:contentReference[oaicite:3]{index=3}  

#### 3) SONET 계층의 큰 방향

SONET은 다음을 목표로 설계되었습니다.:contentReference[oaicite:4]{index=4}  
...
SONET에는 **“기술 계층(Photonic~Path)”**이 존재합니다. OSI 계층과는 다르지만, 기능을 구분하는 데 유용합니다.:contentReference[oaicite:5]{index=5}  
...
SONET의 기본 신호는 **STS-1**입니다. 그 구조를 정확하게 이해하는 것이 SONET의 핵심입니다.:contentReference[oaicite:6]{index=6}  
...
#### 2) SPE(Synchronous Payload Envelope)
...
- STS-1 payload 용량으로는
  ...
  - 또는 **21 × 2.048 Mbps(E1) 수준의 신호**까지 수용 가능하도록 설계되었습니다.:contentReference[oaicite:7]{index=7}  
...
실제로는 표준이 다음과 같이 정의되어 있습니다.:contentReference[oaicite:8]{index=8}  
...
**STS-N**은 **N개의 STS-1을 byte-interleaving**해서 만들며, 프레임 주기는 그대로 125 µs입니다.:contentReference[oaicite:9]{index=9}  
...
STS-1은 DS0(64 kbps) 음성 채널 기준으로 **672 채널**까지 실을 수 있습니다.:contentReference[oaicite:10]{index=10}  
...
SONET은 단순한 포인트 투 포인트 링크뿐 아니라, **복구·용량 확장에 최적화된 링(topology)** 구조로 많이 사용되었습니다.:contentReference[oaicite:11]{index=11}  
...
### 14.3.6 Virtual Tributaries (VT)

사용자 질문에 나온 “visual tributaries”는 사실 **Virtual Tributaries(VT)**를 말합니다. SONET에서 **PDH 계층(DS1, E1 등)을 수용하기 위한 작은 컨테이너**입니다.:contentReference[oaicite:12]{index=12}  
...
GL 통신 기술 개요에 따르면, STS-1의 payload에는 **7개의 VT 그룹**이 있으며, 각 VT 그룹은 여러 VT1.5/VT2/VT3/VT6 조합으로 구성될 수 있습니다.:contentReference[oaicite:13]{index=13}  
...
정확한 값은 SONET 포인터·정렬 오버헤드 등을 포함해 정의되어 있지만, 개념상 “해당 PDH 용량을 조금 여유 있게 담는 작은 슬롯” 정도로 이해하면 충분합니다.:contentReference[oaicite:14]{index=14}  
...
이 구성이 교과서에서 자주 언급되는 **“STS-1은 DS1 28개를 싣는다”**라는 사실과 연결됩니다.:contentReference[oaicite:15]{index=15}  

#### 4) 코드 예제: VT로 음성 채널 수 계산
...
### 14.3 마무리: SONET의 현재 위치

- SONET/SDH는 한때 **전 세계 통신 사업자 백본의 표준**이었지만,  
- 현재는 **IP/MPLS + Carrier Ethernet + OTN**이 백본을 지배하고 있고, SONET/SDH는
  - 레거시 TDM 서비스,
  - 일부 전용회선,
  - 혹은 OTN/이더넷망의 하부 전송계층으로만 남는 추세입니다.:contentReference[oaicite:16]{index=16}  

그래도 **프레임 구조·STS 계층·VT·링 보호** 개념은 여전히 시험과 설계에서 자주 등장하므로, 논리 구조를 정확히 이해하는 것이 중요합니다.

---

## 14.4 ATM (Asynchronous Transfer Mode)

이제 SONET 위에서 **“셀 스위칭”**을 했던 대표 기술, **ATM**을 살펴보겠습니다.

### 14.4.1 Design Goals

ATM은 1980년대 말 **B-ISDN(Broadband ISDN)**을 위해 설계된 기술로, 목표는 대략 다음과 같았습니다.:contentReference[oaicite:17]{index=17}  
...
- 여기에 헤더 5바이트를 붙여 **53바이트 셀**로 표준화:contentReference[oaicite:18]{index=18}  
...
ATM은 설계 단계부터 다양한 트래픽 클래스를 정의했습니다.:contentReference[oaicite:19]{index=19}  
...
이렇게 모든 것을 ATM으로 통합해 관리하는 것이 목표였고, 실제로 1990년대~2000년대 초반까지 많은 사업자 백본이 ATM을 사용했습니다.:contentReference[oaicite:20]{index=20}  

---

### 14.4.2 Problems (현실적인 문제점)

이론적으로는 매우 “깨끗한” 설계였지만, 실전에서는 여러 문제를 드러내며 결국 **Ethernet/IP/MPLS에 밀려났습니다.**:contentReference[oaicite:21]{index=21}  
...
- 동시에 **IP/MPLS가 QoS·트래픽 엔지니어링**을 점차 잘 지원하면서  
  - “굳이 ATM까지 써야 할 이유가 줄어든 것”이 큰 이유 중 하나였습니다.:contentReference[oaicite:22]{index=22}  
...
그래도 ATM의 아키텍처 자체는 시험·이론에서 중요한 포인트입니다.:contentReference[oaicite:23]{index=23}  
...
UNI(사용자–네트워크 인터페이스) 셀 헤더 구조:​:contentReference[oaicite:24]{index=24}  
...
#### 3) Virtual Path / Virtual Channel

ATM은 **두 단계의 가상 회선**을 사용합니다.:contentReference[oaicite:25]{index=25}  
...
#### 4) ATM 서비스 클래스와 트래픽 계약

각 VC는 생성 시 다음과 같은 파라미터를 네트워크와 협상합니다.:contentReference[oaicite:26]{index=26}  
...
현실 네트워크에서는 다음과 같은 구조가 흔했습니다.:contentReference[oaicite:27]{index=27}  
