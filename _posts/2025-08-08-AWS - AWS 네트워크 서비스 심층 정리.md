...
### 1.3 예제: S3 Gateway Endpoint + 버킷 정책 + EC2 IAM Role
```json
...
## 4. **CloudFront (CDN)** — 캐싱·엣지 제어·WAF·서명 URL
...
## 5. **Content Acceleration** — S3 Transfer Acceleration & 엣지 활용

### 5.1 S3 Transfer Acceleration
...
## 8. **GWLB (L3)** — IDS/IPS/방화벽의 투명 스케일아웃
...
## 11. 실습 구성 레시피 — “엣지 차단 + OIDC + 프라이빗 백엔드”

### 11.1 S3 정적 오리진 + CloudFront(OAC) + WAF
```hcl
