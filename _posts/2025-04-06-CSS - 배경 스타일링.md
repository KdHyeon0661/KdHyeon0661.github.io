...
## 2. `background-image` — 이미지/그라디언트/멀티 레이어
...
### 2.1 레티나/가변 해상도: `image-set()` (지원 브라우저 우선)
```css
.logo {
...
## 6. `background-attachment` — 스크롤 동작
...
## 7. `background-origin` & `background-clip` — 배경의 기준과 잘림

### 7.1 origin: **위치·반복 계산의 기준 박스**
```css
...
## 8. `background-blend-mode` — 레이어 혼합
...
## 9. `background` 단축(shorthand) — 레이어별 선언
...
## 14. 수학 메모 — `cover/contain`의 스케일 결정
