...
## 1. Amdahl의 법칙 — 직렬 구간이 만드는 단단한 천장
...
### 1.4 Karp–Flatt 메트릭(ε) — “직렬+오버헤드”의 합성 관측치
...
## 2. Gustafson–Barsis 법칙 — 문제를 키워 코어를 채운다(약한 스케일링)
...
## 4. 코드로 보는 차이 — 계산 병렬화 vs 연결 동시성

### 4.1 병렬성(계산 가속) — OpenMP 루프 병렬화 (C)
...
### 4.2 동시성(응답성/처리량) — epoll 기반 에코 서버 요지 (C/Linux)
