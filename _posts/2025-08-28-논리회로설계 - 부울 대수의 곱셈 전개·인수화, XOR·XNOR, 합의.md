...
## 배타적 OR(XOR)과 등가(XNOR) — 정의, 등가식, 성질
...
### 문제: \(F=(A+B)(C+D)+AB\) 를 SOP로 전개 후 최소화
...
### 문제: \(F=\overline{A}B + A\overline{B} + \overline{C}D\) 를 XOR/XNOR로 표현
...
### 문제: \(F=XY+\overline{X}Z+YZ\) 에 합의 정리 적용
...
### 문제: \(F=A+BC\) 의 보수 \(\overline{F}\) 를 드모르간으로, 그리고 POS로
