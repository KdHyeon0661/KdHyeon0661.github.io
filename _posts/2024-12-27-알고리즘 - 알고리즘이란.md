...
## 0. 형식적 관점과 전제(RAM/Word-RAM/비트복잡도)
...
## 2. 성능 평가 — Big-O의 정확한 뜻과 실전 비용 모델
...
### 7.1 이분 탐색, 병합정렬, 퀵정렬 — 앞서 다룸

### 7.2 선택 알고리즘(Quickselect) — 평균 \(O(n)\)
...
### 7.3 Karatsuba 곱셈(큰 정수) — \(O(n^{\log_2 3})\approx O(n^{1.585})\)

아이디어: 큰 정수를 절반으로 나눈 뒤 3번의 재귀 곱과 합성으로 덧셈/시프트를 줄인다.

---

## 8. 탐욕(Greedy) — 교환 논증으로 정당화

### 8.1 인터벌 스케줄링(최대 비겹침 개수)
