...
### 1.3 다중 핸들러: `OnPostSave`, `OnPostDelete`
...
## 2. 모델 바인딩 & 검증 — `[BindProperty]`, 이름 규칙, 오버포스팅 방어
...
## 3. `Page()` vs `RedirectToPage()` — 언제, 왜 쓰나
...
### 7.1 AJAX 제출(fetch) → 서버 검증 → JSON/Partial 반환
...
## 10. 통합 예제 — 제품 등록: 선택/태그/파일/PRG/TempData
