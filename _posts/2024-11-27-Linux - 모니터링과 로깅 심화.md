...
## 왜 MAC이 필요한가? DAC → MAC으로의 전환

리눅스 시스템에서 전통적인 접근 제어 방식인 **DAC(Discretionary Access Control)**은 소유자, 퍼미션, 그룹을 기반으로 합니다. 이 방식은 `rwx` 권한, `chmod`, `setuid` 등을 포함하지만, 루트 프로세스가 공격받으면 **시스템 전역으로 권한이 확장될 위험**이 있습니다.

이에 대비하여 **MAC(Mandatory Access Control)**은 "허용된 것만 가능하다(deny by default)"는 원칙을 따릅니다. 이 방식은 프로세스, 파일, 포트, IPC, 네임스페이스 등에 **라벨(정책)**을 부여하여 **루트 권한도 제약**할 수 있습니다.

MAC의 핵심 철학:
- 최소 권한 원칙(Least Privilege)
- 허용 목록 기반 접근(Allow-listing)
- 정책과 로깅을 통한 **가시성 확보 + 재발 방지**

---

## SELinux 핵심 개념 이해 (정책 타입·컨텍스트·booleans)
...
- **Enforcing**: 정책 위반 차단 + 로그 기록
- **Permissive**: 차단하지 않고 위반만 로그 기록(튜닝 단계에 유리)
...
- **Targeted**(기본): 네트워크 서비스 등 주요 데몬만 강제 적용
- **Strict**: 시스템 전체 강제 적용(학습/연구용)
...
- 보안성 유지와 운영 편의 사이의 균형을 **제어 가능한 스위치**로 분리

---

## SELinux 파일/포트 라벨링: semanage·restorecon·chcon

### 파일 컨텍스트 지정 (영구 vs 일시)

**영구 규칙 등록 → 실제 파일 라벨 복원**이 권장되는 방법입니다.

```bash
# fcontext 규칙 정의(영구)
sudo semanage fcontext -a -t httpd_sys_content_t '/srv/www(/.*)?'

# 실제 파일에 라벨 적용(restorecon)
...
```bash
# HTTP 데몬이 8081/tcp도 사용하도록 허용
sudo semanage port -a -t http_port_t -p tcp 8081
sudo semanage port -l | grep http_port_t
```

- **잘못된 타입**에 포트를 추가하면 허용되지 않습니다
- RHEL/Fedora/CentOS: `policycoreutils-python(-utils)` 패키지 필요할 수 있음

---

## SELinux 로그 해석 루틴: ausearch → audit2why → audit2allow
...
> **원칙**: 허용 룰은 **최소화**해야 합니다. 라벨링/boolean으로 해결 가능한지 먼저 확인하고, 정말 필요한 경우에만 policy 모듈을 추가하세요.

---

## 서비스별 SELinux 튜닝 예시(현업 빈도 높은 것만)

### NGINX/Apache: 웹 루트 변경 + 외부 네트워크 연결
...
### MariaDB/MySQL: 데이터 디렉토리 이동

```bash
sudo semanage fcontext -a -t mysqld_db_t '/data/mysql(/.*)?'
sudo restorecon -Rv /data/mysql
# systemctl 재시작 후 에러 시 audit 로그→audit2why 확인
```

### Docker/Podman: 컨테이너 볼륨 마운트 라벨

- RHEL/Fedora/Alma/Rocky 계열 컨테이너 런타임은 SELinux 연동이 강력합니다
...
라벨이 자동으로 컨테이너 도메인(`container_t`)에 맞춰져 **호스트-컨테이너 간 경계**가 확보됩니다.

---

## SELinux 트러블슈팅 체크리스트(현장용)
...
- **enforce**: 차단 + 로그 기록
- **complain**: 차단하지 않고 로그만 기록 (튜닝 단계)
- Ubuntu는 기본 활성, Debian은 선택적
...
## 정책 검증/회귀 방지: Dev → Stage → Prod

1. **Permissive/complain** 모드로 학습 + 로그 수집
...
- SELinux Enforcing에서 서비스가 급하게 막히면:
  ...
  3) 부팅 실패/무한 대기: **GRUB에서** `enforcing=0` 또는 `selinux=0` 임시로 설정
...
### SELinux — NGINX가 `/opt/site`를 루트로 쓰고 8443 포트로 서비스
...
### AppArmor — `/usr/local/bin/api`를 빠르게 감싸기
...
## 실무 팁: "정책이 아니라 라벨 문제"라는 사실

운영에서 발생하는 문제의 80%는 **정책 자체의 결함**보다 **라벨 불일치** 때문입니다:
- SELinux: **`semanage fcontext` → `restorecon`** 루틴을 숙달하세요
- AppArmor: 배포/업그레이드로 **바이너리 경로가 변경되면 프로파일 경로도 수정**하세요
...
리눅스 시스템 보안을 강화하기 위한 SELinux와 AppArmor는 각각의 장점과 적합한 사용 사례가 있습니다. 효과적으로 활용하기 위해 다음 원칙들을 기억하세요:

**적절한 도구 선택**:
- RHEL/Fedora/Alma/Rocky 계열: SELinux의 강력한 Type Enforcement 모델 활용
- Ubuntu/Debian 계열: AppArmor의 직관적인 경로 기반 프로파일로 시작

**체계적인 문제 해결 접근법**:
1. **로그 분석 우선**: `ausearch`나 `aa-logprof`로 문제 원인 파악
2. **최소 권한 원칙**: 필요한 최소한의 권한만 허용
3. **라벨 일관성 유지**: 파일과 포트의 적절한 라벨링 유지
4. **테스트와 검증**: Permissive/complain 모드로 테스트 후 Enforcing으로 전환

**실무 중심의 운영 철학**:
- "무조건 끄기"보다는 "운영 루틴에 통합하기"
- 정책을 **코드처럼 버전 관리**하고 **테스트 파이프라인**에 포함
- **로그 해석 → 최소 보완 → 재검증 → 강제 모드** 순환 프로세스 구축

SELinux와 AppArmor는 처음에는 복잡해 보일 수 있지만, 기본 개념과 핵심 명령어를 이해하면 효과적으로 활용할 수 있습니다. 이러한 강제 접근 제어 시스템은 시스템 보안을 크게 향상시키고, 침해 사고 시 피해 범위를 제한하는 데 중요한 역할을 합니다. 각 환경의 요구사항에 맞게 적절히 조정하고 점진적으로 통합하는 것이 장기적인 성공의 열쇠입니다.