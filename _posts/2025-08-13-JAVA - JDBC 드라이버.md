...
## 7. `DataSource` & 연결 풀 (HikariCP 권장)

### 7.1 왜 `DriverManager` 대신 `DataSource`?
- 재사용 가능한 **풀링 커넥션** 제공
- JNDI/컨테이너 연동 용이, 장애/재연결 관리

### 7.2 HikariCP 예시 (순수 자바)
```java
// build.gradle.kts
...
## 8. `CallableStatement` — 프로시저/함수 호출
...
## 11. 안전한 매핑(DAO 스타일) — 수동 RowMapper
