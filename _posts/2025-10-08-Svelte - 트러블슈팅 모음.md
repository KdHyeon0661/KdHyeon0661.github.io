...
category: Svelte
---
# 19. 트러블슈팅 모음 — **현장에서 자주 맞닥뜨리는 함정과 해법 총정리**
...
### 19.1.2 구조분해 후 **원본과 단절**된 변수에만 변경한 경우
...
### 19.1.5 `bind:this`로 DOM/컴포넌트 참조 후 **타이밍** 문제
...
### 19.2.2 CSR 네비게이션에서 `load`가 **재실행되지 않음** (캐시/`depends` 누락)
...
### 19.3.2 모듈 임포트 자체가 브라우저 전용인 경우 (예: `maplibre-gl`, `three`)
...
### 19.3.3 `+page.server.ts`에서 클라이언트 API 사용
...
### 19.4.4 서버 액션에서 **예외를 그냥 던짐** → HTML 응답 깨짐/CSR 전용에서 곤란
