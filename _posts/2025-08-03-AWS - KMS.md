...
## 1) 암호화 방식 총정리 (SSE vs CSE vs Envelope)
...
## 5) 실습: SSE-KMS, CSE, 비밀 암호화

### 5.1 S3 버킷 기본 암호화(SSE-KMS) 설정 (CLI)
...
### 5.3 클라이언트 사이드(Envelope) — Python 예제(boto3 + cryptography)
...
## 6) 권한 모델: 키 정책, IAM 정책, Grants, 암호화 컨텍스트

### 6.1 키 정책(Key Policy) 기본형
...
### 6.3 암호화 컨텍스트(Encryption Context)
...
### 10.1 CloudFormation — CMK + 별칭 + 키 정책
...
## 14) 확장 예제: Node.js 서버에서 SSE-KMS 업로드
