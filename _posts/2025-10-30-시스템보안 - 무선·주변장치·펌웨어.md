...
## BIOS/UEFI, Secure Boot 체인

### UEFI 부팅 체인 개요(Verified Boot vs Measured Boot)
...
### 키 계층(PK/KEK/db/dbx)과 SBAT(서브-컴포넌트 폐기)
...
### 실무 점검 예제(Windows / Linux)

#### Windows: Secure Boot/TPM 상태 확인 (PowerShell)
...
#### TPM 이벤트 로그 요약(PCR 0~7) — Python(교육용 간이 파서)
...
### 실습(방어형): 장치 식별·차단·로깅 예제

#### Linux — **USBGuard** 정책(허용 목록)
...
# 정책 편집: 명시적으로 필요한 장치만 allow, 나머지 block
# 키보드 1대만 허용, 저장장치 전부 차단
...
#### Windows — **장치 설치 제한(GPO/레지스트리)**

```powershell
# PowerShell (관리자): 특정 하드웨어 ID만 허용, 그 외 차단 예시
# 클래스/인스턴스 ID 파악: Get-PnpDevice -PresentOnly
# 정책 적용 (예: Allow installation of devices that match any of these device IDs)
...
#### Windows — WMI 구독으로 **USB 삽입 이벤트 로깅(C#)**
...
### Sysmon(Windows) — 장치 드라이버 로드/파일 생성 모니터링(발췌)
...
### Linux — `auditd`로 USB 서브시스템 이벤트(개념)
...
### Windows — Secure Boot 비활성 탐지(로그인 스크립트)
