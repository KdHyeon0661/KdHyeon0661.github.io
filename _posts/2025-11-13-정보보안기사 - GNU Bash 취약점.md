...
### 3.1 내 시스템의 Bash 버전/패치 여부
```bash
# 배포판에서 제공하는 보안 백포트가 있어 "버전 숫자만"으로 판단 금지
bash --version
# 예) patchlevel, release string, distro changelog 확인
...
```bash
# Nginx access.log에서 User-Agent 필드 내 '() {' 탐지
grep -E '\(\)\s*\{' /var/log/nginx/access.log
```

### 4.2 Elastic(ES|Kibana) KQL 예시
```text
...
## 6) 안전 실습: 방어 규칙/헌팅 자동화

### 6.1 HTTP 로그에서 의심 헤더 추출(파이썬; 무해)
```python
import re, sys
pat = re.compile(r"\(\)\s*\{")
