...
### 1.3 `:is()`/`:where()` — 선택자 **묶기**와 specificity 제어
...
## 2) 반응형의 재정의: 컨테이너 쿼리 · 서브그리드 · 네스팅

### 2.1 컨테이너 쿼리 — “화면이 아니라 **컴포넌트** 크기에 반응”
`@container`는 **요소 자신의 컨테이너 크기**에 따라 스타일을 바꿉니다. 레이아웃 조합이 많은 대규모 UI에서 **미디어쿼리의 단점(전역 뷰포트 의존)**을 해소합니다.  
DevTools가 검사/디버깅도 지원합니다. :contentReference[oaicite:2]{index=2}
...
### 2.3 CSS 네스팅 — SCSS 없이 **공식 CSS 문법**으로
...
## 3) 인터랙션 & 모션: 스크롤 타임라인, 성능 친화 애니메이션

### 3.1 `@scroll-timeline` / `animation-timeline`
...
- **설문 트렌드**: 컨테이너 쿼리·네스팅·`:has()`가 **실사용 곡선**에 진입(2024~25 지표에서 채택률 급증 보고). :contentReference[oaicite:6]{index=6}
...
- 컨테이너 쿼리 — 크롬/에지/사파리/파폭에서 개발자도구 지원 포함해 현실 적용 가능(검사·디버그 가이드 참고). :contentReference[oaicite:11]{index=11}
...
### 7.1 폼 필드 상태(값·에러)에 따른 부모 하이라이트 — `:has()`
...
### 7.3 기사 레이아웃의 헤더·본문 라인 정렬 — `subgrid`
...
- Chrome DevTools: **컨테이너 쿼리 디버깅** 가이드. :contentReference[oaicite:16]{index=16}  
- Apple/WebKit 블로그: 최신 CSS 기능 채택 소식(사파리).   
- State of CSS 2024: 채택률과 체감 트렌드 요약. :contentReference[oaicite:18]{index=18}
