...
category: 영상처리
---
# 6) **라이선스/배포**: libjpeg-turbo(“BSD-style”) 라이선스 고지 & DLL 배포 정책
...
## 3) **THIRD_PARTY_NOTICES.txt** 예시(우리 앱 관점)
...
## 5) **CMake 패키징**(정적/동적, 라이선스 동봉)

### 5.1 vcpkg/패키지 사용 가정 — 타깃 설정
```cmake
# CMakeLists.txt
...
## 14) (선택) 런타임 **라이선스 뷰어** 다이얼로그 코드 스케치
