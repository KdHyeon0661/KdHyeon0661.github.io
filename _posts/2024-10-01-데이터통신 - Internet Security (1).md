...
---
# 32.1 네트워크 계층 보안 — IPsec, IKE, VPN 완전 정리
...
**AES-GCM, SHA-2, IKEv2** 중심으로 설명한다. :contentReference[oaicite:0]{index=0}

---

## 1. 두 가지 모델(모드): Transport Mode vs Tunnel Mode
...
### 1.1 Transport Mode — end-to-end 보호
...
즉, **원래 IP 헤더는 평문으로 남고**, 그 뒤의 부분만 인증/암호화 대상이 된다. :contentReference[oaicite:1]{index=1}
...
이 구조는 RFC 4301의 IPsec 아키텍처에서 정의된다. :contentReference[oaicite:2]{index=2}
...
## 2. 두 가지 보안 프로토콜: AH와 ESP

IPsec은 두 가지 핵심 보안 프로토콜을 정의한다.

1. **AH (Authentication Header)**  
2. **ESP (Encapsulating Security Payload)** :contentReference[oaicite:3]{index=3}

오늘날 실제 VPN/트래픽 보호에서는 **거의 항상 ESP만 사용**하고,  
AH는 특수한 상황(헤더 인증만 필요할 때)에만 고려된다.

### 2.1 AH — 인증 전용(헤더+페이로드)
...
인증 데이터는 HMAC-SHA-256 같은 MAC으로 계산된다. :contentReference[oaicite:4]{index=4}

#### 2.1.1 AH가 거의 안 쓰이는 이유
...
### 2.2 ESP — 암호화 + 인증(실전 주력)

ESP(Encapsulating Security Payload)는  
**기밀성 + 무결성 + 출처 인증 + 재전송 방지**를 한 번에 제공한다. :contentReference[oaicite:5]{index=5}
...
**AES-GCM + SHA-2 + IKEv2** 조합을 기본 권장으로 보고 있다. :contentReference[oaicite:6]{index=6}

#### 2.2.2 ESP 헤더/트레일러 구조 (간략)
...
## 3. IPsec이 제공하는 서비스(Services Provided by IPsec)

IPsec은 네트워크 계층에서 다음과 같은 보안 서비스를 제공한다. :contentReference[oaicite:7]{index=7}
...
### 3.2 무결성(Integrity) 및 출처 인증(Data Origin Authentication)
...
### 3.3 재전송 공격 방지(Anti-Replay)

각 ESP 패킷에는 **Sequence Number**가 있고, 수신 측은  
sliding window(예: 32 또는 64, ESN 사용 시 64bit) 로  
“이미 본 번호”를 거부한다. :contentReference[oaicite:8]{index=8}

### 3.4 접근 제어/트래픽 선택(Access Control & Traffic Selector)
...
### 3.5 제한적인 트래픽 흐름 기밀성(Traffic Flow Confidentiality)
...
그 역할을 하는 것이 **IKE(Internet Key Exchange)** 이다. :contentReference[oaicite:9]{index=9}

오늘날 새로 설계하는 환경에서는 사실상  
**IKEv2(RFC 7296, 및 업데이트 RFC 8247/8983 등)** 만 사용한다고 보면 된다. :contentReference[oaicite:10]{index=10}
...
- IKEv2
  ...
  - NIST, NSA 등에서 **IKEv2 + 현대 암호(AES-GCM, SHA-2)** 사용을 권고 :contentReference[oaicite:11]{index=11}
...
- `esp=` 파라미터로 IPsec SA 암호 스위트를 제안하는 구조다. :contentReference[oaicite:12]{index=12}

---

## 6. Virtual Private Network (VPN) — IPsec의 대표 응용

IPsec의 대표적인 활용이 바로 **VPN(Virtual Private Network)** 이다.  
NIST SP 800-77 Rev.1은 IPsec 기반 VPN 설계와 운영에 대한  
대표적인 가이드 문서다. :contentReference[oaicite:13]{index=13}
...
### 6.2 대표적인 IPsec VPN 아키텍처

NIST는 주로 세 가지 아키텍처를 구분한다. :contentReference[oaicite:14]{index=14}
...
### 6.3 간단 시나리오: Site-to-Site VPN vs Remote Access VPN
...
### 6.4 IPsec VPN vs TLS VPN (간단 비교)
