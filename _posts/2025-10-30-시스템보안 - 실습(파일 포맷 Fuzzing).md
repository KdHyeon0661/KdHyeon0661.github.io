...
## 샌드박스 베이스라인(리눅스: Docker + seccomp/AppArmor)

### `Dockerfile` (슬림 이미지 + 도구)
...
## Windows Sandbox(.wsb)로 “디코더 퍼징” 감싸기
...
# 방어: 파일 무해화(CDR), 앱 격리(Windows Sandbox, AppContainer)
...
## AppContainer(Windows)로 앱 격리 실행(개념)
...
## 리눅스 측 앱 격리 — systemd-run + Bubblewrap/Firejail(개념)

### bubblewrap 샌드박스 실행 예
