...
## 공변/반공변(Variance) — 컬렉션/델리게이트에서의 다형성
...
## 타입 검사 — `is` / 패턴 매칭 / `as` / `typeof` / `GetType`
