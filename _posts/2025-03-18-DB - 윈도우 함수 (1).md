...
## 3. 프레임 절(ROWS / RANGE / GROUPS) 완전 해설
...
## 4. 윈도우 함수와 조건부 집계(필터링)

### 4.1 조건부 누적(ANSI FILTER vs CASE)
...
## 5. Top-N per Group (부분 순위) — 페이징/추천/상위 추출
...
## 6. 중복 제거/대표 행 선택(Dedup/Latest Per Key)
...
## 11. 다중 윈도우 공유(WINDOW 절, PostgreSQL)
...
### 12.5 부서 평균 이상 급여자만(윈도우→외부 WHERE)
```sql
WITH enriched AS (
  SELECT e.*,
