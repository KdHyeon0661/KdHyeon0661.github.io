...
SNMP MIB, X.509 인증서, 많은 텔코 프로토콜이 **ASN.1로 데이터 구조를 정의하고, X.690 계열 인코딩 규칙**으로 전송한다.:contentReference[oaicite:0]{index=0}  
...
**ASN.1은 “데이터 구조를 기술하는 언어”**다. 프로토콜 메시지의 구조를 문법적으로 정의해 놓으면, 양 끝단(에이전트/매니저, 클라이언트/서버)이 같은 정의를 공유하기만 하면 서로 다른 CPU/언어/OS라도 데이터를 주고받을 수 있다.:contentReference[oaicite:1]{index=1}  

예:

- **SNMP**: SMIv2가 ASN.1 매크로를 사용해 MIB 테이블을 정의하고, 전송은 ASN.1 + BER.:contentReference[oaicite:2]{index=2}  
- **X.509 인증서(RFC 5280)**: 인증서 구조, 확장 필드(subjectAltName 등)를 ASN.1 타입으로 정의하고 DER로 인코딩.:contentReference[oaicite:3]{index=3}  
- **텔코·무선(3GPP/5G)**: 시그널링 메시지 구조를 ASN.1로 정의하고, PER(특히 unaligned)을 사용해 매우 압축된 이진 형식으로 전송.:contentReference[oaicite:4]{index=4}  

즉, ASN.1은 **“추상 문법”**이고, X.690/X.691/X.693/X.697 등은 **“전송 문법(인코딩 규칙)”**이다.:contentReference[oaicite:5]{index=5}  

---

## 2. Language Basics — 언어 기초
...
- `SNMPv2-SMI`, `SNMPv2-TC` 등이 IETF MIB 모듈 이름(RFC 3418 등).:contentReference[oaicite:6]{index=6}  
- X.509도 `PKIX1Explicit88`, `PKIX1Implicit88` 같은 모듈로 정의.:contentReference[oaicite:7]{index=7}  
...
ASN.1 값은 인코딩 시 **Tag-Length-Value(TLV)** 또는 **Identifier-Length-Contents(ILC)** 구조로 표현된다. 태그는 크게 네 클래스.:contentReference[oaicite:8]{index=8}  
...
#### 2.3.1 IMPLICIT vs EXPLICIT vs AUTOMATIC
...
`AUTOMATIC TAGS`를 쓰면, `Person` 안의 `name`과 `age`는 자동으로 `[0]`, `[1]` 같은 context-specific 태그를 가진다(정확한 규칙은 X.680 기준).:contentReference[oaicite:9]{index=9}  

### 2.4 제약(Constraints)

ASN.1은 단순 타입에 **제약**을 붙여 값 범위를 제한할 수 있다.:contentReference[oaicite:10]{index=10}  
...
PER 같은 압축 인코딩 규칙(PER)은 이 제약을 활용해 비트를 줄인다. 범위가 좁을수록 필요한 비트 수가 줄어든다.:contentReference[oaicite:11]{index=11}  

### 2.5 Network Management와 ASN.1 예시(간단한 MIB)
...
X.680은 여러 **원시(primitive)**·**구성(constructed)** 타입과 태그 번호를 정의한다.:contentReference[oaicite:12]{index=12}  

### 3.1 Primitive vs Constructed
...
BER/DER에서는 이 구분이 인코딩의 P/C 비트에 반영된다(Identifier octet에서).:contentReference[oaicite:13]{index=13}  

### 3.2 주요 Universal Primitive 타입
...
- BER에서 `FALSE` → 내용 0x00, `TRUE` → 0xFF(또는 다른 non-zero; DER은 0xFF로 고정).:contentReference[oaicite:14]{index=14}  
...
- 2의 보수, big-endian, 최소 바이트 수로 표현(DER 기준).:contentReference[oaicite:15]{index=15}  
...
- PER, XER, JER 등에서 ENUM 이름 그대로 또는 숫자로 인코딩 규칙이 달라질 수 있다.:contentReference[oaicite:16]{index=16}  

#### 3.2.4 OCTET STRING / BIT STRING
...
- `OCTET STRING`: 바이트 배열
- `BIT STRING`: 비트 단위 필드. SNMP의 `TruthValue`, `RowStatus` 같은 것들이 이런 타입과 매핑된다(매크로 레벨에서).:contentReference[oaicite:17]{index=17}  

#### 3.2.5 OBJECT IDENTIFIER

OID는 ASN.1에서 **전 세계적으로 유일한 네임스페이스**를 만든다.:contentReference[oaicite:18]{index=18}  
...
X.690의 규칙에 따라 첫 두 개 `a1.a2`는 다음과 같이 결합된다.:contentReference[oaicite:19]{index=19}  
...
- **DEFAULT** 값은 실제 값이 default와 같으면 인코딩 생략(DER 조건 하에서).:contentReference[oaicite:20]{index=20}  

#### 3.3.2 SET / SET OF

- **SET**: 순서의 의미가 없고, DER에서는 필드 인코딩 순서를 태그 값 기준으로 정렬해야 한다.:contentReference[oaicite:21]{index=21}  
...
## 4. Encoding — BER/DER/CER, PER, XER, JER

X.690/X.691/X.693/X.697은 ASN.1 타입을 실제 바이트 또는 XML/JSON으로 인코딩하는 규칙을 정의한다.:contentReference[oaicite:22]{index=22}  

### 4.1 BER의 기본 구조: TLV

**BER(Basic Encoding Rules)** 는 각 요소를 `Identifier(또는 Tag) + Length + Contents`로 인코딩한다.:contentReference[oaicite:23]{index=23}  
...
X.690에서 이 구조 예시를 그대로 제시한다.:contentReference[oaicite:24]{index=24}  

### 4.3 Constructed 타입 인코딩
...
### 4.4 DER와 CER — Canonical 규칙

X.690은 **BER**, **CER(Canonical Encoding Rules)**, **DER(Distinguished Encoding Rules)** 를 규정한다.:contentReference[oaicite:25]{index=25}  
...
- **DER**:
  ...
  - X.509 인증서, CRL에서 필수.:contentReference[oaicite:26]{index=26}  
- **CER**:
  - 마찬가지로 canonical이지만, 긴 문자열 등 일부 경우에 **indefinite length**를 허용하는 변종.

### 4.5 PER — Packed Encoding Rules

X.691/PER는 **"비트를 최대한 아끼는" 인코딩 규칙**이다.:contentReference[oaicite:27]{index=27}  
...
### 4.6 XER(XML Encoding Rules)와 JER(JSON Encoding Rules)

X.693/X.697은 ASN.1 값을 XML/JSON으로 표현하기 위한 규칙이다.:contentReference[oaicite:28]{index=28}  
...
### 4.7 실전 예제: X.509 Subject 인코딩 스케치
...
이처럼 ASN.1 타입 정의 → DER 인코딩 → 인증서 바이너리라는 흐름으로 이어진다.:contentReference[oaicite:29]{index=29}  
...
### 5.1 Python(pyasn1)을 이용한 단순 디코딩 예(개념)
...
- 또는 pyasn1, go-asn1 같은 라이브러리를 이용해 동적으로 디코딩.:contentReference[oaicite:30]{index=30}  
...
- SNMP, X.509, 텔코 시그널링, 많은 네트워크 관리/보안 프로토콜에서 쓰이는 **표준 데이터 정의 언어**이며,:contentReference[oaicite:31]{index=31}  
- X.690/691/693/697 계열 인코딩 규칙(BER, DER, PER, XER, JER 등)과 결합되어
  - 효율적인 이진 전송(예: PER, BER/DER)
  - 사람이 읽기 좋은 XML/JSON 표현(XER, JER)을 동시에 제공한다.:contentReference[oaicite:32]{index=32}  
