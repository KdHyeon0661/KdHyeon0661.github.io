...
# 2) 등치 이행(Equality Transitivity) — 가장 기본이자 강력한 형태
...
# 3) 범위 이행(Range Transitivity) — 등치 조인을 매개로 범위를 전파
...
# 4) 조인 조건 이행(Join Predicate Pushdown) — 전이성 & Bloom
...
## Join Filter(Bloom) — 해시 조인의 스캔 차단
