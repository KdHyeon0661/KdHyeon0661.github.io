...
## 1. 내부 메커니즘: “동치 클래스(Equivalence Class)와 전이 폐쇄”
...
## 2. 가장 중요한 결과 형태: access predicate vs filter predicate

### 2.1 access predicate가 되면 “물리 경로가 바뀐다”
- **access**: 인덱스/파티션/조인 입력을 **“들어가기 전에”** 줄이는 조건  
- **filter**: 들어온 결과를 **“나중에”** 거르는 조건

조건절 이행의 **진짜 성공 조건은 “이행된 조건이 access로 내려갔는가”** 다.

### 2.2 Predicate Information에서 보는 법
...
### 3.2 상수 이행(Equality-to-Constant)
```sql
...
## 4. 범위 이행(Range Transitivity)

### 4.1 날짜 차원 → 사실 테이블(파티션 프루닝)
```sql
...
## 5. 해시 조인과 조건절 이행: Bloom / Join Filter와 결합

### 5.1 조인이 “필터 생성기”가 되는 이유
해시 조인에서는
...
## 6. 조건절 이행이 막히는 대표 8가지 (그리고 실전 해법)
...
## 7. 이행이 성능을 뒤집는 실전 시나리오

### 7.1 스타 스키마: 차원 조건만으로 사실 파티션을 줄이기
```sql
...
### 7.2 OLTP NL 조인: 바깥 키로 안쪽 인덱스 1방
```sql
SELECT /*+ USE_NL(s) LEADING(c s) */
       s.*
FROM   d_customer c
