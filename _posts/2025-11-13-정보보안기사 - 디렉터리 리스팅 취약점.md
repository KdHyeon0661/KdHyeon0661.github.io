...
## 재현(실습) — “취약 → 악용 → 패치” 한 번에 보기

### Docker로 빠르게 취약 환경 띄우기 (Nginx + autoindex)
...
## 테스트 자동화(파이프라인 품질 게이트)

### 간단 E2E(Playwright/Puppeteer 예시)
...
### Nginx 설정 유닛 테스트(Testcontainers 예시, 개념)
...
## 부록 — 민감 파일/디렉터리 패턴 모음(화이트리스트 차단에 활용)
