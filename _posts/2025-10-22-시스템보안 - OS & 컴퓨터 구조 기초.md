...
category: 시스템보안
---
# 1. OS & 컴퓨터 구조 기초 — 시스템 해킹을 위한 “바닥” 이해
...
## 1.1 프로세스 / 스레드 / 스케줄러 / 메모리 관리 개요

### 1.1.1 프로세스와 스레드 — “자원” vs “실행 흐름”
...
### 1.1.2 스케줄러(Scheduler) — “누구를 언제 CPU에 태울까”
...
### 1.1.3 메모리 관리 — 힙/스택/맵 영역, 할당자, 페이지
...
#### (Linux) `/proc/<pid>/maps`로 메모리 지도 훑기
```bash
./demo & pid=$!
...
## 1.2 유저/커널 경계, 시스템 콜(System Call)과 ABI

### 1.2.1 유저/커널 경계 — Ring3 ↔ Ring0
...
## 1.3 가상 메모리: 페이지 테이블, ASLR, NX, PIE
...
## 1.4 파일 / 디바이스 / 드라이버 / 인터럽트 개념 정리

### 1.4.1 VFS(가상 파일 시스템) — “모든 것은 파일이다”
...
## 1.5 보안 기능 지도: SELinux/AppArmor, Windows Integrity/UAC, Code Signing

### 1.5.1 리눅스 MAC: SELinux vs AppArmor
- **SELinux**: 레이블 기반(주체/객체 컨텍스트), 정책은 **Type Enforcement(TE)** 중심. 강력하지만 학습곡선 높음.
- **AppArmor**: 경로 기반 프로필, 비교적 단순/가볍고 배포 친화.

#### (SELinux) 현재 모드/컨텍스트 보기
```bash
...
#### (AppArmor) 최소 프로필 예시 (curl 일부만 허용, 교육용)
```
...
### 1.5.2 Windows Integrity Level / UAC
...
#### (Windows) RunAsInvoker(UAC 우회 패턴 연구는 금지; 개념만)
- **주의**: 실서비스 우회는 정책 위반/불법 가능. 연구는 **자체 VM**에서.

### 1.5.3 코드 서명(Code Signing)
