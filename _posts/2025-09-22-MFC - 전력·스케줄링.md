...
## 큰 그림: 전력 모델과 우리가 만질 수 있는 레버(레시피 먼저)
...
## 백그라운드 모드·우선순위·Power Throttling
...
### Delivery Optimization(대용량/조직환경)
...
### 푸시 vs 폴링, Connected Standby에서의 웨이크
...
## 절전/재개: Modern Standby(S0ix) 알림, 유지 요청, 사용 금지 패턴
...
### 유지 요청(PowerSetRequest) — 필요할 때만, 짧게
...
## ETW/측정: “좋아 보이지만 실제로는 배터리 먹는” 함정을 찾자
