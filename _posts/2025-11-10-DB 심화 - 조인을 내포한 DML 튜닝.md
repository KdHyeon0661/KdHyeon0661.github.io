...
### 2.2 기본 예제 — “주문이 존재하는 ELEC 상품만 3% 인상”
...
#### 2.3.2 DISTINCT를 사용하는 방식(가능하지만 주의)
...
### 2.4 Updatable Join View로 DELETE 하기
...
### 2.6 Updatable Join View 튜닝 체크리스트
...
### 3.2 기본 예제 — STG_PRICE로 PROD 가격 동기화
...
### 3.3 MERGE의 가장 흔한 실패: 복수 매칭(ORA-30926)
...
### 3.5 MERGE 성능을 좌우하는 요소들

#### 3.5.1 USING 서브쿼리의 “빌드 입력 크기”
...
### 4.2 INSERT ALL — 모든 조건을 만족하면 모두 적재

#### 4.2.1 기본: 일자·카테고리와 일자·브랜드 요약 동시 적재
...
### 4.3 INSERT FIRST — 첫 번째 조건만 적재(우선순위 분기)
...
### 4.4 사전 집계 + Multi-Insert (대량에서 매우 중요)
...
### 6.1 “매칭된 것만 UPDATE, 미매칭은 무시” — MERGE를 UPDATE 전용으로
...
### 6.2 “미매칭만 INSERT, 매칭은 건드리지 않음” — INSERT 전용 MERGE
...
### 6.3 “키별 최종 상태만 반영” — USING에서 Top-1 per key 만들고 MERGE
...
### 6.4 “대상 소량, 내부 대량” — APPLY로 Top-1만 읽어 UPDATE
