...
## 1) .NET Concurrent Collections 한눈에
...
## 3) ConcurrentDictionary<TKey, TValue> — 키-값의 왕도
...
## 4) ConcurrentBag<T> / ConcurrentStack<T>
...
## 5) BlockingCollection<T> — 생산자-소비자와 Backpressure
...
## 7) 스레드 안전 초기화: `Lazy<T>` / `LazyThreadSafetyMode`
...
## 9) ReaderWriterLockSlim / SemaphoreSlim / AsyncLocal
...
## 11) 파티셔닝과 PLINQ, Parallel.ForEachAsync
...
## 12) 메모리 모델 핵심: `volatile`, `Thread.MemoryBarrier`, `Volatile.Read/Write`
...
## 14) 실전: 고성능 카운터/집계기 설계

### 14.1 단순 카운터 — `Interlocked`
```csharp
class FastCounter
{
    private long _value;
...
### 14.2 키-값 집계 — `ConcurrentDictionary` + 불변 값
```csharp
...
## 17) 종합 예제 — 로그 파이프라인(생산자-소비자 + backpressure + 종료)
