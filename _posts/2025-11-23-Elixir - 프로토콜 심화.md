...
#### `BitString` 하나로 “바이너리와 비트스트링” 모두를 잡는 이유
...
#### `Map` 구현이 plain map에만 적용되는 이유(중요)
...
#### “고아 구현(orphan impl)”이 실제로 터지는 시나리오
