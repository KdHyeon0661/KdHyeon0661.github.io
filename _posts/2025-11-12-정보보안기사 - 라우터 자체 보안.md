...
## 4. Cisco IOS/XE 하드닝 예제

### 4.1 관리 접근(SSHv2·VTY ACL·로그인 보호)
```cisco
! 도메인/호스트명/암호정책
...
### 4.2 AAA(RADIUS/TACACS+) + 로컬 fallback
```cisco
aaa new-model
...
### 4.5 iACL(인프라 ACL) — 라우터 자신으로 향하는 트래픽 최소화
```cisco
...
### 4.6 CoPP(Control Plane Policing)
```cisco
...
### 5.3 루프백/lo0 제어(Receive Protect = iACL 개념)
```junos
...
## 6. Linux Router/FRR 하드닝 예제

### 6.1 OS 커널 보안/라우팅 관련 sysctl
```bash
...
## 9. 인프라 ACL(iACL) — 데이터플레인에서 라우터 보호
...
## 12. 가시성/모니터링 — Syslog·NetFlow/IPFIX
...
## 17. 점검 스크립트(운영팀 도우미; Cisco IOS-XE 예시)
```cisco
! 간이 점검: 관리면/제어면/로그 상태
show ip ssh
