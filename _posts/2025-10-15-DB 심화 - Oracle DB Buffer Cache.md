...
**논리 I/O**는 **버퍼 캐시**에서의 블록 접근, **물리 I/O**는 디스크에서 블록을 읽어오는 작업입니다. 캐시 히트율은 참고 지표일 뿐, **상위 대기 이벤트/경합** 기반으로 병목을 판단해야 합니다.
...
Oracle 데이터베이스 버퍼 캐시는 단순한 블록 저장소가 아니라, **Cache Buffer Chains(CBC)** 해시 체인과 **터치 카운트 기반 LRU**라는 복합 정책을 통해 효율성을 극대화하는 정교한 메커니즘입니다. 핵심 성능 이슈는 대부분 특정 블록에 대한 과도한 논리적 I/O에서 비롯된 **CBC 래치 경합**으로 나타나며, 이는 **Reverse Key Index, 해시 파티셔닝, 샤딩 카운터** 등의 설계 기법으로 접근을 분산시켜 해결할 수 있습니다.

동시에 **KEEP/RECYCLE 버퍼 풀**을 활용한 워킹셋 관리와 **direct path read**를 고려한 대량 스캔 전략은 캐시 오염을 방지하고 핵심 데이터의 메모리 상주율을 보장합니다. 실제 운영에서는 캐시 히트율보다는 **v$latch**, **v$session**, **AWR/ASH** 보고서를 통해 **`buffer busy waits`**, **`latch: cache buffers chains`**, **`free buffer waits`** 같은 구체적인 대기 이벤트를 관찰하고, 그 근본 원인인 데이터 접근 패턴과 스키마 설계를 개선하는 것이 성능 튜닝의 올바른 접근법입니다.