...
category: DB 심화
---
# Oracle **Response Time Analysis(응답시간 분석) 방법론**과 **OWI(Oracle Wait Interface)**
...
## 5. 실전 SQL: **RTA 프로파일** 빠르게 뽑기

### 5.1 구간별(예: 최근 10분) **Top Wait Class/Event**
...
### 5.3 특정 SQL의 **Wait 타임라인(초단위 프로파일)**
...
### 8.3 `enq: TX - row lock contention` — FK 미인덱스
...
### 8.4 RAC `gc current block busy` — 글로벌 핫블록
...
## 9. **RTA 대시보드**: 10분 만에 만드는 SQL

### 9.1 Wait Class 프로파일(최근 15분)
```sql
SELECT wait_class,
       COUNT(*) AS samples,
...
### 14.1 시스템 수준 RTA 스냅샷(최근 15분, Top 12 이벤트)
```sql
SELECT wait_class, event, COUNT(*) AS samples,
       ROUND(100*RATIO_TO_REPORT(COUNT(*)) OVER (),1) AS pct
FROM   v$active_session_history
