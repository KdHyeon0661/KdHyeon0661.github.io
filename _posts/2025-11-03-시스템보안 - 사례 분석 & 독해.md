...
## 19.1 유명 CVE 타임라인 재구성(초기 접근→권한 상승→영속화)

> 사례 A·B·C는 **역사적으로 널리 알려진 공격군**이며, 본문은 **공격 목적의 세부 절차**가 아닌 **“독해·재구성 틀”**을 제공합니다.

### 19.1.1 사례 A: Log4Shell 계열(원격 입력 → JNDI 문자열 처리 → 원치 않는 로딩)
...
### 19.1.2 사례 B: ProxyLogon/ProxyShell 계열(메일 서버 전단 취약점 연계 체인)
...
### 19.1.3 사례 C: EternalBlue 계열(SMB 취약 경로 → 내부 확산)
...
### 19.2.3 로그 시그널 규칙(예시: 애플리케이션 경계값 에러)
...
## 19.3 실습: “한 건 완전 복기” — IOC/공격 그래프/방어 개선안 도출
...
### 19.3.4 공격 그래프(자산·신호·경로) 생성(무해 관계 그래프)
