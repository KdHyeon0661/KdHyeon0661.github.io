...
```yaml
# Suricata 룰 스니펫 — 서버에서 외부로 비정상 셸 인터프리터 유사 UA/TLS 핑거프린트 탐지(구체 값은 예시)
...
#### 윈도우 — PowerShell(무해한 조사 스니펫)

```powershell
# 자식으로 생성된 쉘/스크립트 엔진 나열
...
#### 윈도우 — 드라이버 서명 이슈 탐색(개념 PowerShell)
