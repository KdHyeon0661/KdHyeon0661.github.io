...
### 단일 리전, 다중 환경(Dev → Stg → Prod, 수동 승인 포함)
...
## CodeBuild ― `buildspec.yml` 실전 템플릿

### Node.js + 테스트 + 캐시 + SBOM + 리포트
...
### Docker-in-Docker(이미지 빌드 + 취약점 스캔 + ECR 푸시)
...
## CodeDeploy ― 대상별 배포 전략

### EC2 In-place (appspec + 스크립트)
...
### EC2 Blue/Green + CloudWatch 알람 기반 롤백
...
### ECS Blue/Green (CodeDeploy + ALB 대상 그룹 교체)
...
## CodePipeline ― 파이프라인 정의(IaC)

### CloudFormation(요약형, Source→Build→Stg→Approval→Prod)
...
## 데이터베이스 마이그레이션(릴리스 내 통합)

### Build 단계에서 마이그레이션 패키징
...
## GitHub Actions ↔ CodePipeline 연계(옵션)
