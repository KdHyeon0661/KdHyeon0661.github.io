...
## 인증 스택 강화 — Kerberos 우선, NTLM 축소/차단
...
```powershell
# 권한 부여 예: LAPS 읽기 권한 그룹에 특정 OU 내 컴퓨터 암호/만료 읽기
...
## LSA/LSASS 보호, Credential Guard/WDigest/RC4

```powershell
# LSA 보호(LSASS 보호: RunAsPPL)
...
# WDigest 비활성화(평문 캐시 금지)

Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\WDigest' -Name 'UseLogonCredential' -Type DWord -Value 0

# RC4 사용 축소(도메인/클라이언트 정책으로 AES 우선)
...
## 취약 구성 탐지 — 스캐닝 스크립트 묶음

### 로컬·도메인 취약 신호

```powershell
# 로컬 Administrators 그룹 덤프
...
# AD: Password never expires / PreAuth disabled / Unconstrained Delegation
