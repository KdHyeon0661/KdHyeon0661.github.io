...
### 2.2 더 좋은 길: IAM Identity Center(SSO) 도입(팀/다계정 권장)
...
## 8. CloudTrail/Config/GuardDuty/Security Hub 즉시 활성화

**감사/보호는 만든 날부터 켠다.** 사고 후 켜면 로그가 없다.

### 8.1 CloudTrail(관리 이벤트 로깅)
- 조직/단일 계정 선택
...
### 8.3 GuardDuty(위협 탐지) · Security Hub(보안 기준 통합)
- 한 번 클릭으로 리전별 활성화. Organizations로 멀티계정 집계 권장.

---

## 9. 비용 관리: 예산/알림/Cost Explorer/태깅

### 9.1 비용 예산(Budgets) 알림(이메일)
...
### 9.2 Cost Explorer로 서비스/리전/태그별 분석
...
## 11. 실전 예제 1: 첫 EC2(무료 티어) + 보안그룹 + SSH
...
## 12. 실전 예제 2: 정적 웹 호스팅(S3 + CloudFront + OAC)
...
### 12.2 OAC로 프라이빗 버킷 + CloudFront만 접근
...
## 14. IaC(선택): CloudFormation/Terraform/CDK 스켈레톤

### 14.1 CloudFormation: S3 버킷(버전/암호화)
```yaml
