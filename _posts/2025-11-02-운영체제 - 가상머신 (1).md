...
### 18.1.2 가상화의 기본 이론 (Popek & Goldberg)
...
### 18.1.8 (심화) /dev/kvm 사용자 공간에서 VCPU 실행 최소 예(개념)
...
### 18.3.4 라이브 마이그레이션(Pre-copy) 알고리즘 스케치
...
### 18.3.5 리소스 과할당(오버커밋)과 Ballooning
...
### 18.3.6 Libvirt를 이용해 VM 정의/시작(파이썬 예)
...
### 18.3.9 클라우드-초기화(cloud-init)로 VM 부팅 시 사용자 설정

```yaml
# user-data (cloud-init) — 최초 부팅에서 계정/키/패키지 구성
#cloud-config
users:
  - name: dev
    ssh_authorized_keys:
