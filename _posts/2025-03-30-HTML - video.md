...
category: HTML
---
# HTML5 `<video>` 태그 완벽 가이드 (심화 확장판)

초안의 흐름(기본 → 속성 → `<source>`/포맷 → 자막 → JS 제어 → UX/접근성 → 반응형)을 유지하면서, **코덱/컨테이너/브라우저 호환**, **모바일 자동재생 정책**, **자막·챕터·오디오 설명 트랙**, **CORS/보안**, **커스텀 컨트롤**, **Picture-in-Picture/전체화면/Media Session**, **HLS/DASH(MSE)**, **성능 최적화**까지 확장했다. 예제는 모두 복붙 후 실행 가능하도록 최소한의 문맥을 포함한다.
