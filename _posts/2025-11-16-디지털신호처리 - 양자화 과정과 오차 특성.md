...
## 1) 양자화란 무엇인가 — 연속값을 유한 비트로

### 1.1 “연속값 → 격자(grid)”로 매핑
...
## 2) 균일 양자화기: 미드트레드 vs 미드라이즈

### 2.1 미드트레드(mid-tread) 양자화기
...
## 6) 양자화 오차의 스펙트럼: 잡음 vs 왜곡

### 6.1 “랜덤” 입력 vs “결정론적” 입력
...
## 9) 노이즈 셰이핑(Noise Shaping)과 ΣΔ(Delta-Sigma)
...
## 11) 미드트레드 vs 미드라이즈: 작은 DC 바이어스 관찰
