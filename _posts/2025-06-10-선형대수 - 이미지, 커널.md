...
### 4.2 기본정리(FTLA, Fundamental Theorem of Linear Algebra)
- \(\mathrm{Col}(A)^\perp=\mathrm{Null}(A^\top)\).
- \(\mathrm{Row}(A)^\perp=\mathrm{Null}(A)\).
- \(\dim\mathrm{Col}(A)=\dim\mathrm{Row}(A)=\mathrm{rank}(A)\).

### 4.3 제1同型정리(First Isomorphism Theorem)
선형사상 \(T:V\to W\):
$$
...
### 7.1 선형회귀 \(A\mathbf x\approx\mathbf b\)
