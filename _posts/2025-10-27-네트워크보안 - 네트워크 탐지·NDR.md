...
## 17.1 NIDS/NIPS/NDR, 시그니처 vs 이상탐지
...
## 17.2 Suricata/Zeek로 L7 가시성 확보

### 17.2.1 센서 준비(리눅스, 멀티큐 NIC)
```bash
# 1) Suricata 설치(예: Ubuntu)
...
# 3) PF_RING/AF_PACKET 멀티큐 사용 권장(고속)
...
### 17.2.2 Suricata 최소 설정 (eve.json)
```yaml
...
## 17.3 TLS 핑거프린트·DNS 이상·C2 행위

### 17.3.1 TLS 지문 (JA3/JA4)
...
### 17.3.2 DNS 이상 (DGA/NXDOMAIN/TTL/엔트로피)
- **지표**:  
...
### 17.4.3 Filebeat로 Zeek/Suricata 수집
```yaml
# filebeat.yml (요지)
filebeat.inputs:
- type: filestream
...
### 17.4.5 Elasticsearch 인덱스·탐지(예시 KQL/SPL)
...
### 17.4.6 알림/자동화(Watcher or ElastAlert/Rule Engine)
- 조건:  
