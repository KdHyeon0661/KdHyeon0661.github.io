...
## 트레이 아이콘 (Shell_NotifyIcon) — 추가/수정/제거/클릭/컨텍스트 메뉴/풍선
...
## 트레이 알림 심화 — 상태표시(오버레이), 다중 인스턴스, Explorer 재시작
...
## 작업 표시줄 통합 (Windows 7+) — 점프 리스트, 진행률, 오버레이, 썸네일 버튼
...
## Windows 토스트 알림 (현대식 알림 센터) — 개요와 간단 경로
...
## 통합 데모 스니펫 — “작업 앱” (트레이 + 풍선 + 진행률 + 점프리스트)
