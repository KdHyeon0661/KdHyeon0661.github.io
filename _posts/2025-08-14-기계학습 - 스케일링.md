...
### Unit-norm Scaling ( \(L_1/L_2/L_\infty\) 정규화 )
...
### Quantile Transformer (순위-가우시안/균등화)
...
## 수치·최적화 관점: 조건수·사전조건화·정규화와의 상호작용

### 사전조건화(Preconditioning)로서의 스케일링
...
### ColumnTransformer + 파이프라인 (수치/범주 혼합)
...
### 희소행렬 처리 (MaxAbs 또는 표준화 with_mean=False)
