...
## 1.4 EC2에 Python/Flask/Boto3 설치
...
## 3.3 VPC 엔드포인트(S3 Gateway)

프라이빗 서브넷의 EC2가 NAT/인터넷 없이 S3에 접근하게 하여 보안/비용 개선.

---

# 4. 브라우저 직업로드: Pre-signed URL/POST 모범 예제

EC2를 경유하지 않고 **클라이언트가 S3에 직접 업로드**하는 방식.

## 4.1 서버에서 Pre-signed URL 생성(Flask)
...
## 6.2 전송 가속(Transfer Acceleration)
...
# 부록 B) 서버리스 백엔드로 Pre-signed URL 발급(AWS Lambda)
