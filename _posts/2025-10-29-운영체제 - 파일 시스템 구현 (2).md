...
### 14.4.2 Contiguous Allocation (연속 할당)
...
### 14.4.3 Linked Allocation (연결 리스트, FAT 포함)
...
### 14.4.4 Indexed Allocation (인덱스 블록 / 아이노드 다중 간접)
...
### 14.4.5 Extent-Based Allocation (현대 주류)
...
### 14.5.4 Buddy Allocator (2의 거듭제곱 단위)
...
### 14.5.5 트리 기반(범위 트리, B+Tree, RBTree)
...
### 14.6.3 시뮬레이터: 정책별 조각화·I/O 시간 비교

```python
# fs_sim.py — 연속/best-fit vs 랜덤(비연속) vs extent-best-fit 비교(개념 시뮬)
import random
...
### 14.6.4 운영 실습: `fallocate`/`posix_fadvise`/`fio`
