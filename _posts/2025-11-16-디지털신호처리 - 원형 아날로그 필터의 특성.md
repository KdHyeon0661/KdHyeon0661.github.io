...
## Butterworth 원형 아날로그 필터

### 크기 응답 특성 (maximally flat)
...
### GNU Octave: 아날로그 Butterworth 원형 필터 응답
...
## Chebyshev I 원형 아날로그 필터 (통과대역 등리플)

### Chebyshev 다항식과 크기 응답
...
### GNU Octave: 아날로그 Chebyshev I 원형 필터 응답
...
### GNU Octave: 아날로그 Chebyshev II 예제
...
## Elliptic(=Cauer) 원형 필터 (통과·저지대역 모두 등리플)
...
### GNU Octave: Bessel 아날로그 응답 (직접 구현 예)
...
## 원형 아날로그 필터의 주파수 변환 (LP↔HP↔BP↔BS)
...
## 원형 아날로그 필터 → 디지털 IIR (bilinear 변환)
...
## GNU Octave: Butterworth 아날로그 프로토타입 → 디지털 IIR 설계 예
...
### Octave 코드 (직접 prewarping + bilinear 구현)
