...
category: DB 심화
---
# PL/SQL 함수 호출 부하 해소 방안 — **페이지(부분범위) 처리** + **DECODE/CASE로 함수 대체**
...
## 1. 문제 패턴: 함수로 인한 부하 (비교용 ‘나쁜’ 예)

### 1.1 PL/SQL 함수(내부 재귀 SQL 포함)
...
## 3. 해법 B — **PL/SQL 함수 제거: DECODE/CASE(순수 SQL) 또는 조인으로 대체**

### 3.1 단순 매핑: **DECODE/CASE**로 즉시 변환
...
## 4. 두 해법의 결합: “페이지 처리 + 함수 제거(=DECODE/CASE/조인)”
...
## 5. SARG(인덱스 활용성) 유지 팁 — 함수 제거와 함께 반드시 점검
...
## 6. BEFORE → AFTER 비교(개요)

### 6.1 BEFORE (함수 + OFFSET)
...
## 8. “룩업이 자주 바뀌어요” — CASE 하드코딩 대신 **조인/SQL 매크로**
