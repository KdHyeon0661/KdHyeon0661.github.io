...
## 2. 동시성 제어(Concurrency Control)가 왜 필요한가?
...
## 3. 격리 수준(Isolation Levels) — ANSI와 제품별 매핑
...
## 5. 스케줄과 직렬가능성(Serializability)

### 5.1 충돌 직렬가능성(Conflict Serializability)
...
## 6. 핵심 알고리즘 — 2PL / 타임스탬프 순서 / OCC

### 6.1 2PL(2-Phase Locking)
...
### 6.2 타임스탬프 순서(Time-Stamp Ordering, TSO)
...
## 8. 이상(Anomalies) — 예제와 방지책

### 8.1 Dirty Read (Oracle RC에선 발생하지 않음)
- **설명**: T1의 미커밋 값을 T2가 읽음 → T1 롤백시 T2는 유령값.
- **방지**: RC 이상. Oracle은 기본적으로 **Undo 기반 읽기**로 **불허**.

### 8.2 Non-Repeatable Read (RC에서 가능)
```sql
-- 세션 A (RC)
...
### 8.3 Phantom Read (Oracle RC에서 가능)
```sql
-- 세션 A
...
### 8.5 Write Skew (Snapshot/Serializable 계열에서 전형)
- 두 트랜잭션이 **서로 다른 행**을 바꾸지만 **전역 제약** 위반을 초래.
- **방지**: **DB 제약**으로 전역 조건 강제, **락 테이블**, 애플리케이션 재시도.

---

## 9. Oracle 실무 예제 — RC/Serializable/SELECT FOR UPDATE
...
### 9.3 Serializable의 충돌(ORA-08177)
```sql
-- 세션 A
...
## 10. 락과 인덱스/범위 — Next-Key, Gap, Predicate
