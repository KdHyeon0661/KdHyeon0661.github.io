...
### 1.1 요소(Element) 선언 규칙 (반드시 알아야 할 제약)
...
### 6.3 `TYPE_USE`로 정밀 타입 애너테이션 (런타임 체크 예)
```java
import java.lang.annotation.*;
...
## 7. 애너테이션 프로세싱(컴파일 타임) — `@Retention(SOURCE)` 활용
...
## 9. 고급 설계 — enum/클래스 요소, 중첩 애너테이션, 기본값 상수화
...
### 11.2 `@JsonField` — 필드 이름 매핑(간단 직렬화)
```java
import java.lang.annotation.*;
import java.lang.reflect.Field;
import java.util.*;
...
## 12. 테스트 프레임워크 흉내 — `@Test`, `@BeforeEach`, `@AfterEach`
