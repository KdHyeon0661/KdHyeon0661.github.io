...
#### 예) `<~>`: “Map-like transform” (키-값 변환 DSL)
...
### 22.7.1-추가) “연산자 매크로는 자동 import되지 않는다”
...
### 22.7.1-추가) 연산자 매크로도 `bind_quoted`가 기본
...
### 결합성/우선순위(associativity/precedence) 실전 팁 (초안 포함)
...
# 출력이 " (1 + 2) ~> (& &1 * 3) " 형태인지 확인하고, 필요하면 괄호 보강
...
### 22.7.2-추가) Elixir 연산자 우선순위 “감각 지도”
...
### 22.7.2-추가) 결합성(Associativity)도 확인하라
...
### 가드 친화 매크로(연산자 포함) — `defguard/defguardp` (초안 포함)
...
### 22.7.4-추가) defguard가 허용하는 것/허용하지 않는 것
