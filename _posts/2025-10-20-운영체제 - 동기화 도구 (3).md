...
### 6.7.3 Java 내장 모니터 (synchronized + wait/notify)

#### 6.7.3.1 Bounded Buffer (Mesa 의미론)
```java
...
#### 6.7.3.2 Readers–Writers (Writer 우선)
```java
...
### 6.7.4 C++ 모니터 패턴 (mutex + condition_variable)

#### 6.7.4.1 Monitor 클래스로 캡슐화
```cpp
...
#### 6.7.4.2 조건변수 2개로 명확한 의도 표현
- `cv_not_empty` / `cv_not_full` 를 분리해 **불필요한 깨어남**을 줄인다(성능/전력).

---

### 6.7.5 Go: 모니터 없는 CSP 스타일 (참고)
...
### 6.7.7 Dining Philosophers — 모니터로 교착·기아 모두 방지
...
### 6.8.1 안전(Safety) vs 활성(Liveness)
...
## 6.9 Evaluation (정확성 & 성능 평가)

### 6.9.1 정확성 평가: 불변식·정형기법·테스트
