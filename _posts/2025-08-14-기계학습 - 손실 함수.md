...
## 3. 분류(Classification) 손실

### 3.1 이진 BCE (확률/로짓 별표기 주의)
- **확률 기반**(불안정할 수 있음):
$$
...
### 3.4 Focal Loss(클래스 불균형/하드 예제 강조)
- 이진:
$$
...
### 13.1 NumPy — 안정 BCEWithLogits / Softmax-CE / Huber / Quantile
```python
import numpy as np
...
### 13.2 PyTorch — BCEWithLogits / CE(안정) / Focal / Dice
```python
import torch
