...
## **근거성(Groundedness/Faithfulness)** 체크리스트

**목표**: 답변의 모든 주장/사실이 **제공된 컨텍스트에서 정당화**되는가? (환각 방지)

### 인용 기반 정량화(골드 evidence가 있을 때)
...
### NLI 기반 자동 판정(골드 evidence가 없을 때)
...
## **지시 따르기(Instruction-Following)** 체크리스트
