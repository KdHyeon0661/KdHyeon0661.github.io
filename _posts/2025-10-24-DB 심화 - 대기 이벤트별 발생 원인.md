...
## `db file sequential read` (랜덤 블록 읽기: 인덱스→ROWID)
...
## `db file scattered read` (멀티블록 읽기: FTS/범위 스캔)
...
## `direct path read temp` / `direct path write temp` (TEMP 스필)
...
## `enq: TX - row lock contention` (행 잠금 충돌)
...
## `buffer busy waits` / `read by other session` (핫블록/블록 경합)
...
## `library cache: lock/pin` (커서/오브젝트 정의 경합)
...
## `gc cr block busy` / `gc current block busy` (글로벌 캐시 경합)
...
## `SQL*Net more data to client/from client`
...
## `ON CPU` (대기 아님, CPU 경합 또는 긴 CPU 작업)
...
# 대기 이벤트 → 실행계획 분석: **케이스 스터디**

## [User I/O] `direct path read temp`가 1위
...
## [Concurrency] `enq: TX - row lock contention`
...
## 최근 15분 **Wait Class/Event → Top SQL**
...
## **Temp 사용 상위 라인** 빠르게 찾기 (SQL Monitor가 없을 때)
