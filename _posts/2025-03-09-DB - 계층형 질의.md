...
## 1. 전개 방향 — 순방향(부모→자식) vs 역방향(자식→부모)
...
## 3. 순환(CYCLE) 처리 — `NOCYCLE`, `CONNECT_BY_ISCYCLE`, SQL 표준의 방법
...
### 4.3 계층 번호(Outline Numbering: 1, 1.1, 1.1.1 …)
...
## 11. 모델링 대안 — Adjacency vs Materialized Path vs Nested Set vs Closure
...
### 11.2 Closure Table 예(권한/집계, 다루기 쉬움)
...
## 12. 실전 시나리오 ① — “조직도: 루트→자식, 형제 정렬, 경로, 리프만”
...
## 15. 보너스 — SQL 표준의 CYCLE 구문(지원 DB 한정)
