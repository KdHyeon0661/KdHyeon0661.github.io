...
## Unquoted Service Path(경로 인용 누락) — 권한 상승 고전 취약점
...
## 파일/폴더 ACL 하드닝(바이너리·작업 디렉터리·업데이트 폴더)
...
## 프로세스/이미지 취약성 완화(EMET 대체: IFEO Mitigations)
...
## 배포 템플릿(예제) — 안전한 신규 서비스 등록

```powershell
# 서비스 등록(Own Process), 인용 포함
...
# 실행 계정 최소화(LocalService), Service SID 활성화
