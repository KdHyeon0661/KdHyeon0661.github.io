...
## 7) 실전: 가변 벡터(Vector) — `realloc` 성장 전략
...
## 8) `calloc` vs `malloc+memset` 성능/의미
...
## 11) 정렬/특수 할당: `aligned_alloc`, `posix_memalign`
...
## 18) 종합 예제: 안전한 입력 → 동적 누적 → 크기 확장 → 정렬/출력 → 정리
