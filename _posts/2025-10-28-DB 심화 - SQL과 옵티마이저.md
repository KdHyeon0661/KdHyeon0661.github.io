...
## 2. 통계(Statistics)와 히스토그램

### 2.1 통계 수집(테이블/인덱스/컬럼)
```sql
-- 기본: 최신화 + 인덱스까지
...
## 8. 적응형 플랜(Adaptive Plans)과 동적 샘플링

### 8.1 Adaptive Plans(12c+)
- 실행 중 관측에 따라 **NL↔Hash** 선택을 바꾸거나, 분기에 따른 플랜 경로를 **런타임 선택**.
- `Note`에서 **"adaptive plan"** 표시 확인.

### 8.2 Dynamic Sampling/Automatic Degree
