...
### 서버 핸들러(화이트리스트, secure_filename, MIME 스니핑)
...
### 업로드 후 파이프라인(바이러스 스캔 → 리사이즈/메타 → 영구 저장)
...
## 바이러스 스캔(ClamAV/clamd) 통합

### Docker로 클램AV 데몬 띄우기(예)
...
## 대용량 다운로드 — 스트리밍/Range/X-Accel-Redirect/서명 URL
...
## 파일 보안/품질: MIME 스니핑, EXIF/메타, 이미지 리사이즈
