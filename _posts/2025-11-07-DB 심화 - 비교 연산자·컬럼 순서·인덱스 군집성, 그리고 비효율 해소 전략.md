...
# 1. 비교 연산자 & 컬럼 순서가 만드는 **인덱스 레코드의 군집성**

## 1.1 군집성(Clustering) 직관
...
# 4. **INDEX SKIP SCAN**으로 선행 비등치/누락 문제 보완
...
## 4.1 예제: 인덱스 `(cust_id, order_dt)`만 있을 때, `cust_id` 조건이 없음
```sql
