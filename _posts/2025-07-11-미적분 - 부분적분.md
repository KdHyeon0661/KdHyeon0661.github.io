...
## 감소식(감소 관계) 만들기 — 일반 패턴을 한 번에

### \( \displaystyle I_n=\int x^n e^{ax}\,dx \) (다항×지수)
...
### \( \displaystyle S_n=\int x^n\sin bx\,dx,\quad C_n=\int x^n\cos bx\,dx \)
...
### \( \displaystyle J_n=\int (\ln x)^n\,dx\ (x>0) \)
...
### \( \displaystyle \int x^m(\ln x)^n\,dx\ (m\neq -1) \)
...
### \( \int_{0}^{\infty} e^{-ax}\cos bx\,dx \) (\(a>0\))
