...
### 늦은 이벤트(Allowed Lateness)와 사이드아웃
...
## 상태(State) — RocksDB, Value/List/Map/Reducing/Aggregating
...
## Exactly-once 처리 — 체크포인트, 싱크 트랜잭션

### 체크포인트(Checkpoint) & 세이브포인트(Savepoint)
...
### S3/Redshift/OpenSearch 등 싱크의 exactly-once
...
## 스트림 결합(Join) — 윈도우 조인/인터벌 조인/유저정의
...
### 인터벌 조인(Interval Join) — 두 스트림의 시간 관계
...
## CEP(Complex Event Processing) — 패턴 탐지
...
## Flink SQL / Table API — 선언형 스트리밍

### Kinesis + JSON 테이블 예(카탈로그·DDL)
...
## 직렬화/스키마 — Avro/Protobuf/JSON 선택 기준
...
## AWS에서 Flink: Kinesis Data Analytics for Apache Flink
...
## 테스트 전략 — MiniCluster, Golden Test, Watermark 시뮬레이션
...
## 모니터링·알람 — CloudWatch + Flink Metrics
