...
category: AWS
---
# AWS S3를 이용한 정적 웹 사이트 호스팅 완전 정복 (재작성 · 실전 확장판)
...
## 3) (단순 경로) S3 Website 엔드포인트를 직접 사용하려면
...
## 4) (권장 경로) CloudFront + OAC + HTTPS 구성
...
### 5.2 S3 Website 엔드포인트를 쓰는 경우(HTTP만)  
...
## 7) CI/CD 자동 배포(GitHub Actions 예시)

### 7.1 예시 워크플로우: 빌드 → S3 업로드 → CF 무효화
```yaml
...
### 9.1 Terraform (요약 스니펫: S3 + CF + OAC)
...
### 10.4 `error.html` (S3 Website 모드용)
```html
<!doctype html>
<html lang="ko">
