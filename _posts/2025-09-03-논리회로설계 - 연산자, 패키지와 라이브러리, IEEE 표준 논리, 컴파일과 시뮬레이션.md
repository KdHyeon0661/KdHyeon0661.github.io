...
### 2.4 (VHDL-2008) **컨텍스트(Context)** — 헤더 간소화
```vhdl
context ctx is
  library ieee;
  use ieee.std_logic_1164.all;
...
### 3.1 `std_logic_1164` — 9값 로직/해상(Resolution)
- **9값**: `U, X, 0, 1, Z, W, L, H, -`
...
## 4. **컴파일(Analyze) & 시뮬레이션(Elaborate/Run)**
...
### 5.4 3상태는 **I/O**에서만 (내부는 MUX로 합성)
- 현대 FPGA 내부는 대개 **참 3상태**를 지원하지 않으며, 동작상 **멀티플렉서**로 대체됩니다.

### 5.5 엔티티 직접 인스턴스 vs 컴포넌트
```vhdl
-- 직접 인스턴스 (권장)
...
## 7. 심화 주제 — 델타 사이클/변수 vs 신호/관성·수송(짧은 예제)
...
### 8.2 `std_match`로 프레임 헤더 검사
```vhdl
hdr_ok <= '1' when std_match(hdr, "1010----") else '0';
```

### 8.3 테스트벤치(전수검사 예시, GHDL/Questa 공용)
```vhdl
-- tb_add8.vhd
library ieee; use ieee.std_logic_1164.all; use ieee.numeric_std.all;
entity tb is end;
