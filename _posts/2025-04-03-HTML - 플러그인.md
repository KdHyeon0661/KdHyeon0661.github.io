...
## 내부 아키텍처 — NPAPI / ActiveX / PPAPI 개요
...
## HTML에서 플러그인 삽입 — `<object>`/`<embed>`/`<applet>`
...
### Flash/Unity 2D 게임 → Canvas/WebGL
...
## 레거시 페이지 방어 — 가급적 빠른 전환, 그전까지의 최소 안전장치
...
## 실전 레거시 → 표준 변환 “리팩토링 샘플”

### Flash 오디오 플레이어 → `<audio>` + 커스텀 UI
