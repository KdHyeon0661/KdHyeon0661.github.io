...
> - `+NOTE`로 **Outer→Inner 변환, Join Elimination, Predicate Move-Around** 같은 CQT 흔적 확인.
...
## Outer Join의 논리: “NULL 공급 측”과 의미 보존

### 용어

- **Preserved side (보존측)**: 결과에서 **항상 유지되는 쪽**
- **Null-supplying side (NULL 공급측)**: 매칭이 없을 때 **NULL로 채워지는 쪽**
- LEFT OUTER의 preserved side는 **왼쪽**, RIGHT OUTER는 **오른쪽**, FULL OUTER는 **양쪽**.
...
## ON vs WHERE: Outer Join 성능/의미를 갈라놓는 1순위 규칙

Outer Join에서 **WHERE절은 조인 이후 필터**, ON절은 **조인 조건 자체**입니다.
특히 **NULL 공급측 컬럼에 대한 WHERE 필터는 NULL 행을 제거**해 조인이 **사실상 INNER로 바뀌는** 것이 핵심 함정입니다.

### “Null-rejecting predicate”가 INNER화를 만든다
...
이때는 **CBO가 Outer→Inner 변환을 수행**할 수 있으며 `+NOTE`에 흔적이 남습니다.

---

## Oracle CBO가 Outer Join에 대해 수행하는 주요 변환(CQT)
...
`USE_NL` 계열 힌트가 NL 메소드를 강제하며, 대상 테이블을 inner로 삼는 의미입니다.
...
### 예제 1 — VIP 고객의 최근 주문 10건, 배송 없으면 표시
...
Oracle 튜닝 가이드에 Hash Outer Join의 전형적 플랜이 설명돼 있습니다.
...
- `SWAP_JOIN_INPUTS(t)` 로 특정 해시 조인의 빌드/프로브를 교체 가능.
...
### 재작성: FULL OUTER = LEFT OUTER ∪ RIGHT ANTI
...
## 최신(23ai/21c 계열)에서의 OUTER Join 관련 변화 포인트

최근 릴리스에서는 **벡터화/인메모리 조인 프레임워크가 OUTER/FULL까지 확장**되면서,
대용량 분석 워크로드에서 Hash/Hybrid OUTER가 더 빨라질 수 있습니다.
따라서 “예전보다 HJ OUTER가 더 잘 선택되거나 더 효율적으로 동작”하는 케이스가 증가하는 추세입니다.
...
2) **프레디킷 위치**
   - NULL 공급측의 null-rejecting predicate는 **ON절**로.
...
### FULL OUTER에서 WHERE 추가 후 행이 사라진다
...
- **ON vs WHERE**가 의미/성능의 1순위 분기점이며, NULL 공급측 조건을 WHERE에 두면 **암묵적 INNER화**가 발생한다.
- **NL OUTER**는 Top-N/부분범위/조기 종료에,
  **SMJ OUTER**는 대량/정렬/비등치 OUTER에,
  **HJ OUTER**는 대량 스캔/집계에 강하다.
