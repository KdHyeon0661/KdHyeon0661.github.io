...
## 3. 누수 **원인 패턴 12종** (문제 ↔ 수정)

> 각 패턴은 “문제 코드 → 원인 → 수정 코드/대안” 순으로 제시합니다.

### 3.1 `static` 컬렉션 무제한 성장
```java
// 문제
...
### 4.3 JFR(Java Flight Recorder) — 할당 핫스팟 파악(JDK 11+)
```bash
...
### 4.4 MAT/YourKit/JProfiler/VisualVM
...
### 6.1 `ThreadLocal` & InheritableThreadLocal
