...
## 1. ICMPv6 공통 형식과 큰 그림

### 1.1 ICMPv6 메시지의 두 클래스
...
### 2.1 Destination Unreachable (Type 1)
...
#### 2.1.3 예제 시나리오 1 — 방화벽에 의해 차단된 UDP 포트
...
#### 2.1.4 예제 시나리오 2 — 포트 미열림 (Port Unreachable)
...
#### 2.2.3 예제 시나리오 — 1500 → 1280 MTU 다운 링크
...
### 3.1 Echo Request (Type 128) / Echo Reply (Type 129)
...
### 4.1 Router Solicitation (Type 133)
...
### 4.2 Router Advertisement (Type 134)
...
### 4.3 Neighbor Solicitation (Type 135) / Neighbor Advertisement (Type 136)

이 둘은 **ARP의 IPv6 버전**이라 볼 수 있다.

#### 4.3.1 Neighbor Solicitation(NS)
...
#### 4.3.2 Neighbor Advertisement(NA)
...
#### 4.3.4 예제 2 — Duplicate Address Detection(DAD)
...
## 5. Group Membership Messages (MLD — Multicast Listener Discovery)
...
### 5.2 MLDv1 메시지 (RFC 2710)

#### 5.2.1 Multicast Listener Query (Type 130)
...
#### 5.2.2 Multicast Listener Report (Type 131)

- 호스트가 “나는 이 그룹에 가입하고 싶다/유지하고 있다” 를 라우터에게 알려주는 메시지.
- IGMPv2의 Report와 동일한 역할.

#### 5.2.3 Multicast Listener Done (Type 132)
...
## 6. 종합 예제 — 하나의 링크에서 ICMPv6가 하는 일들
...
## 8. 간단한 코드 예시 — Scapy로 ICMPv6 Echo + NS 만들기
