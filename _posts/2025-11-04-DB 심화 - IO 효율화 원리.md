...
### 1.1 SARGable(인덱스 사용 가능) 표현으로 바꾸기
...
### 1.2 **정렬 일치 + Stopkey(Top-N)** 로 앞부분만
- **나쁜 예**(정렬 후 OFFSET)
```sql
...
### 2.2 **확장 통계(Extended Stats)** — **상관관계** 반영
...
### 2.3 **클러스터링 팩터(Clustering Factor)** 개선
...
### 2.4 **시스템 통계(System Stats)** — IO/CPU/MBRC
...
## 6. 종합 시나리오: “최근 7일 VIP 고객 Top-K 주문 목록 + 합계”
