...
## ES14(2023 이후) — 참고용 제안/추가 표준

### (표준) **불변 배열 유틸**: `toSorted`/`toReversed`/`toSpliced`/`with`
...
### (표준) `findLast`/`findLastIndex`

```js
[1,3,2,5].findLast(x=>x%2===1); // 5
```

### (제안) `Symbol.dispose`/`using`(리소스 정리) — **스테이지 확인 필요**
### (지원 확대) **JSON Import** / Import Assertions
