...
### 1.1 `let`/`const` (블록 스코프, TDZ, 재선언 금지)
```js
{
...
### 1.2 화살표 함수(lexical `this`, `arguments` 없음, `new` 불가)
```js
const add = (a, b) => a + b;
const obj = {
  x: 10,
...
### 1.3 템플릿 리터럴(멀티라인, 인터폴레이션, 태그드 템플릿)
```js
...
### 1.5 기본 매개변수 / 나머지·전개(Spread/Rest)
```js
...
### 1.6 클래스(프로토타입 문법의 설탕, `extends`, `super`)
```js
class Person {
...
### 1.7 모듈(정적 `import`/`export`, 라이브 바인딩)
```js
// math.js
...
### 3.2 `Object.values` / `Object.entries` / `Object.getOwnPropertyDescriptors`
```js
...
### 3.4 `SharedArrayBuffer`/`Atomics` (저수준 동시성; 브라우저 보안 정책 영향)

### 3.5 trailing commas(함수 매개변수/호출 위치 허용)

---

## 4. ES9(2018)

### 4.1 **객체 Rest/Spread**(정식 명세)
```js
...
### 4.4 **Async Iteration**: `for await...of`
```js
...
### 5.3 `trimStart`/`trimEnd`, `Symbol.prototype.description`, `JSON.stringify` 안정화,  
...
### 6.4 `globalThis` (환경 중립 전역)
```js
globalThis.setTimeout === setTimeout; // true
```

### 6.5 **Dynamic `import()`**, `import.meta`
```js
...
### 6.7 `String.prototype.matchAll` (모든 매치 이터레이터)
```js
...
### 7.2 **Numeric Separators** (가독성)
```js
const billion = 1_000_000_000;
```

### 7.3 **`String.prototype.replaceAll`**
```js
"hello".replaceAll("l","-"); // "he--o"
```

### 7.4 **`Promise.any`** + `AggregateError`
```js
try {
...
### 7.5 `WeakRef` / `FinalizationRegistry` (고급 GC 제어 — 신중히 사용)

---

## 8. ES13(2022)

### 8.1 **Top-level `await`** (ESM 모듈 스코프)
```js
...
### 8.2 **`Object.hasOwn`** (명확한 소유 속성 검사)
```js
Object.hasOwn(obj, "key"); // hasOwnProperty 안전 대체
```

### 8.3 **클래스 필드/프라이빗**(인스턴스/정적, private 필드/메서드, 정적 블록)
```js
class Counter {
...
### 8.4 **RegExp Match Indices `/d` 플래그**(매치 인덱스 제공)
```js
const m = /foo/d.exec("..foo..");
m.indices[0]; // [2,5]
```

### 8.5 **`Error` 원인 옵션** (`cause`)
```js
try {
...
## 13. ES14(2023 이후) — 참고용 제안/추가 표준

### 13.1 (표준) **불변 배열 유틸**: `toSorted`/`toReversed`/`toSpliced`/`with`
```js
const a = [3,1,2];
...
### 13.2 (표준) `findLast`/`findLastIndex`
```js
[1,3,2,5].findLast(x=>x%2===1); // 5
```

### 13.3 (제안) `Symbol.dispose`/`using`(리소스 정리) — **스테이지 확인 필요**  
### 13.4 (지원 확대) **JSON Import** / Import Assertions

> 실무 적용 시 **타깃 런타임 버전**과 **트랜스파일/폴리필 전략**을 함께 설계하세요.

---

## 14. 요약 표 — ES6 이후 핵심 문법
