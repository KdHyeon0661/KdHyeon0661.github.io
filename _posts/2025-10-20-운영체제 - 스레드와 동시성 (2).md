...
### 4.4.2 C++17/20 `std::thread`, `std::mutex`, `std::condition_variable`, `std::future`
...
### 4.4.5 .NET — TPL(Task Parallel Library) & async/await
```csharp
// Program.cs
using System;
using System.Linq;
...
### 4.5.2 Grand Central Dispatch (GCD, macOS/iOS)
...
### 4.5.3 Intel TBB / Cilk / Fork-Join (개념)
...
## 4.6 Threading Issues (실무 이슈 총정리)

### 4.6.1 경쟁 조건(Race), 원자성, 메모리 가시성
...
### 4.6.5 스레드 취소(Thread Cancellation)
...
### 4.7.2 Windows (Win32 threads, SRWLOCK, IOCP)
...
### 4.7.3 macOS / iOS (pthreads + GCD + QoS)
...
### 4.7.6 .NET ThreadPool & Work-Stealing
