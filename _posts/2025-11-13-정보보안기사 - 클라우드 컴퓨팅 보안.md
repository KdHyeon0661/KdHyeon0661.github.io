...
## 1. 클라우드 보안의 기본 관점 — **공동 책임 모델(Shared Responsibility)**
...
## 2. 클라우드 분류 — **서비스 모델 / 배포 모델**

### 2.1 서비스 모델(IaaS / PaaS / SaaS)
...
## 7. 데이터 보호(At Rest / In Transit) — **최신 포인트**
...
## 8. 로깅·탐지·Posture(CSPM/CWPP/CNAPP)
...
## 9. 네트워크·세그멘테이션·원격접속 — **Zero Trust** 접목
...
## 10. **SECaaS(Security as a Service)** — 카테고리 & 적용
...
### 13.2 Azure — **Policy** 로 필수 태그/암호화/퍼블릭 IP 제한
```bash
...
### 13.3 GCP — **SCC** 활성화 + Organization Policy(외부 IP 제한 등)
```bash
ORG_ID=<org>
