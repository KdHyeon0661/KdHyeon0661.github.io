...
## 5. FD 수명, 복제, 상속 — `dup/dup2/dup3` & `O_CLOEXEC`
...
## 7. 데이터 영속성 — `fsync`/`fdatasync` & 원자적 교체

### 7.1 권장 패턴(전원 장애/크래시 안전)
1) **임시 파일**에 전체 쓰기  
...
## 9. `stdio`(fopen/fclose)와 UNIX I/O 혼용
...
### 12.2 로그 파일 열고 append(원자적 오프셋 갱신)
```c
