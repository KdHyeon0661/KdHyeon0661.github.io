...
## 4. 실습 1 — EMR Serverless로 Spark 배치 파이프라인

### 4.1 Serverless 애플리케이션 생성(콘솔/CLI)
...
### 4.2 PySpark 잡 코드(로우→실버 전처리 + 파티셔닝)
...
## 5. 실습 2 — EMR on EC2: 클러스터 생성부터 튜닝
...
### 5.3 EMRFS Consistent View 활성(선택)
...
## 6. 실습 3 — EMR on EKS: 쿠버네티스 표준으로 Spark
...
### 6.2 Spark 잡 제출(EMR Runtime for EKS)
...
## 7. 카탈로그/SQL/서빙 — Glue + Athena/Spectrum
...
## 8. 테이블 포맷(Hudi/Iceberg/Delta) — 변경 데이터/타임트래블
...
## 9. 스트리밍(실시간) — Kinesis/MSK → Structured Streaming
...
## 12. 오케스트레이션 — Step Functions / Airflow(MWAA)

### 12.1 Step Functions로 EMR Serverless 파이프라인
...
## 14. 예제 — 배치 ETL → 품질 점검 → Iceberg 머지 → Athena 서빙

### 14.1 PySpark(집계·차원 조인·SCD 업서트: Iceberg MERGE)
...
## 18. 참고 CDK/CloudFormation 스니펫(요약)

### 18.1 CDK(Typescript, EMR on EC2 기초)
