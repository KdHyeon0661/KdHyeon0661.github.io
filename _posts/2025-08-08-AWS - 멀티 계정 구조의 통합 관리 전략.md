...
## 조직 통제: Organizations + SCP + SSO

### Organizations: 핵심 통합 레버
...
## 보안 통합: 로그·탐지·분석·대응의 중앙화

### 조직 로그 표준 — CloudTrail/Config/Flow Logs 집약
...
### 탐지/평가 — GuardDuty / Security Hub / Macie / Inspector / Detective
...
## 네트워크 전략: 중앙 허브와 일관된 경계 보안

### 중앙 네트워크 계정(Hub) + Transit Gateway(Spoke)
...
### VPC 공유(Shared VPC) vs VPC 피어링 vs TGW
...
## 비용 전략: 통합 청구 → 태깅 표준 → 예산/이상탐지 → 구매 최적화

### 통합 청구(Consolidated Billing)
...
### OU별 강제 가드레일(SCP) – Sandbox에서 금지 예
...
### CloudTrail 조직 트레일 + S3 Object Lock 버킷 정책(요약)
