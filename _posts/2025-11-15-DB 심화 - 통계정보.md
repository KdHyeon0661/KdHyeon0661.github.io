...
잘못된 조인 순서/방법(예: NL vs HASH), 잘못된 스캔 경로(예: 인덱스 vs FTS)를 고른다. :contentReference[oaicite:0]{index=0}
...
Oracle 옵티마이저 통계는 크게 다음으로 나뉜다. :contentReference[oaicite:1]{index=1}
...
- **STATS_TYPE_LOCKED / STALE_STATS**: 잠금/오래됨 여부  
  → 옵티마이저는 이 값으로 **기본 카디널리티**를 만든다. :contentReference[oaicite:2]{index=2}
...
- 낮은 고정 샘플(예: 1% 고정)은  
  스큐/상관관계 컬럼을 못 잡아 **오판을 구조적으로 만든다.** :contentReference[oaicite:3]{index=3}

### 3.3 자동 통계(Autotask)와 STALE 판정

Oracle은 유지보수 윈도우에서 자동 통계를 돌리고,  
**기본 STALE 기준은 “테이블 변경량 약 10%”**이다.  
(버전·옵션에 따라 세부는 다르지만, 기본 정책은 이 ‘변경량 기반 stale’) :contentReference[oaicite:4]{index=4}
...
### 3.4 파티션 테이블의 증분 통계(Incremental Stats)

대규모 파티션 테이블에서는  
매번 전체를 재수집하는 대신

- 변경된 파티션만 수집
- 나머지는 **synopsis(시놉시스)**로 합성해 글로벌 통계를 만든다. :contentReference[oaicite:5]{index=5}
...
- **CLUSTERING_FACTOR(CF)**:  
  ...
  → **NL 조인에서 테이블 랜덤 액세스 비용을 좌우**한다. :contentReference[oaicite:6]{index=6}
...
**비슷한 선택도**로 계산해 버린다.  
→ 그래서 스캔 경로가 틀어진다. :contentReference[oaicite:7]{index=7}

### 5.3 히스토그램 타입(12c+ / 19c/21c/23ai/26)

Oracle은 버전이 올라오며  
Height-Balanced 중심에서 **Top-Frequency/Hybrid 중심으로 이동**했다. :contentReference[oaicite:8]{index=8}

| 타입 | 특징 | 언제 생성되는가 |
|---|---|---|
| FREQUENCY | NDV ≤ 버킷 수면 값별 정확 빈도 | AUTO 정책에서 “스큐 강하고 NDV 작음” |
| TOP-FREQUENCY | 최빈값(top-n)을 별도로 정확 기록 | **AUTO_SAMPLE_SIZE일 때만** 가능 :contentReference[oaicite:9]{index=9} |
| HYBRID | 최빈값은 정확 + 나머지는 구간근사 | **AUTO_SAMPLE_SIZE일 때만** 가능 :contentReference[oaicite:10]{index=10} |
| HEIGHT-BALANCED | 구간 균형 기반(구버전 잔재) | Hybrid/Top-Freq 비활성·부족 시 남을 수 있음 :contentReference[oaicite:11]{index=11} |
...
이 오판은 **조인 순서·방법을 뒤집는 1순위 원인**이다. :contentReference[oaicite:12]{index=12}
...
생성을 자동 유도할 수 있다. :contentReference[oaicite:13]{index=13}
...
- 옵티마이저가 “stale한 시점 사이의 공백”을 줄이도록 개선했다. :contentReference[oaicite:14]{index=14}
...
### 9.1 시스템 통계가 무엇을 모델링하나

CBO 비용 모델은 “블록 읽기 비용”과 “CPU 비용”으로 구성된다.  
시스템 통계는 이 비용을 **실제 서버/스토리지 성능에 맞게 보정**한다. :contentReference[oaicite:15]{index=15}
...
### 9.2 NOWORKLOAD vs WORKLOAD

- **NOWORKLOAD**: 랜덤 I/O 중심의 짧은 측정, 운영에서 가장 흔한 기본 선택.
- **WORKLOAD**: 실제 워크로드 구간을 대표해 측정(정밀하지만 운영 부담). :contentReference[oaicite:16]{index=16}
...
- **PUBLISH**: 바로 통계 게시할지 여부 :contentReference[oaicite:17]{index=17}

### 10.2 Pending Stats로 안전 검증
...
### 10.3 Export/Import/Restore/Lock
...
- [ ] 파티션 대형 테이블은 **INCREMENTAL + synopsis** 필수 :contentReference[oaicite:18]{index=18}  
- [ ] 대량 로드 직후는 실시간/수동 수집으로 공백 제거 :contentReference[oaicite:19]{index=19}  

### 13.2 히스토그램/확장통계
- [ ] 스큐 컬럼은 히스토그램  
- [ ] 상관관계는 확장통계  
- [ ] “불필요한 히스토그램 남발”은 플랜 변동성만 키움  
- [ ] SPD가 만든 자동 확장통계도 주기적으로 점검 :contentReference[oaicite:20]{index=20}  
...
CBO의 비용 모델이 현실과 맞는다. :contentReference[oaicite:21]{index=21}
