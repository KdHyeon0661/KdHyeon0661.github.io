...
category: CSS
---
# CSS 성능 최적화 팁 완전 정리 — 렌더링 파이프라인 관점의 실전 가이드

브라우저는 **DOM**과 **CSSOM**을 결합해 **렌더 트리**를 만들고, 이후 **레이아웃 → 페인트 → 컴포지트** 단계를 거쳐 화면을 그린다.  
CSS 최적화는 “파일을 작게” 만드는 것을 넘어, **레이아웃·페인트·컴포지트 비용을 줄이고, 초기 페인트를 앞당기며, 런타임 재계산을 최소화**하는 일이다.  
아래는 실무에서 바로 적용할 수 있는 체크리스트와 코드 스니펫을 **증상→원인→해결** 흐름으로 정리한 것이다.

---

## 0) 빠른 체크리스트

- [ ] 미사용 CSS 제거(Purge/coverage) & 배포용 Minify  
- [ ] **Critical CSS** 인라인 + 비차단 로딩  
- [ ] **선택자 간단화**, 깊은 후손 선택 회피  
- [ ] 레이아웃 변화를 **transform/opacity**로 대체  
- [ ] `content-visibility`, `contain`, `contain-intrinsic-size` 적용  
- [ ] 이미지/폰트 **용량·전송·표시 전략** 최적화(`image-set`, `font-display`)  
- [ ] **미디어 쿼리/브레이크포인트** 재사용(변수·mixin)  
- [ ] CSS 분할(라우트별) & `@layer` 로드 순서 통제  
- [ ] DevTools **Performance/Coverage/Rendering**로 병목 점검  
