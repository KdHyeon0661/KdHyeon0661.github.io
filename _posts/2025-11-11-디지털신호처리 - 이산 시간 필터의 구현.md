...
### 2.2 Direct Form II (DF-II) & 전치형(DF-II-T)
...
### 3.3 FIR 전치(Transposed) — 파이프라인 친화

입력 1 샘플마다 **짧은 조합 경로**(누산기 체인) → 실시간 하드웨어/고정소수점 구현에 유리.

---

### 3.4 폴리페이즈(Polyphase) — 멀티레이트 결합
...
### 4.4 격자-사다리(lattice-ladder)

격자(반사계수) + 사다리(탭) 결합으로 **영·극 동시 제어**. FIR/ARMA 구현에 활용.

---

## 5. 실전 이슈 — 안정성·정수화·상태·메모리
