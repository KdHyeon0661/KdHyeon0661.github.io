...
## 1. Program.cs — 환경별 파이프라인 + HTML/JSON 동시 처리
...
## 2. Razor Pages — 500/404/403 전용 페이지 설계

### 2.1 예외 페이지(500) — `Pages/Error.cshtml`
...
### 2.2 상태 코드 페이지(404/403) — `Pages/StatusCode.cshtml`
...
## 4. Web API — RFC7807 ProblemDetails(JSON) 표준화

### 4.1 전역 ProblemDetails(ASP.NET Core 8+)
...
## 7. SPA(React/Vue/Svelte) + 프록시(Nginx/IIS)에서의 에러
...
## 10. 상태 코드 페이지 전략 — ReExecute vs Redirects vs Text
