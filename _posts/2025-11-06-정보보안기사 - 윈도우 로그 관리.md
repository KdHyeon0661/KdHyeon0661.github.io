...
## 3. 고급 감사 정책(Advanced Audit Policy) — 골든 프로파일
...
## 6. Get-WinEvent — 고성능 쿼리 패턴

### 6.1 FilterHashtable (빠르고 간결)
```powershell
...
## 7. WEF/WEC(Windows Event Forwarding/Collector)
...
## 8. PowerShell 로깅 강화(스クリپ트/모듈/전사)
...
## 9. Defender/방화벽/AppLocker/Code Integrity 로그
...
### 14.1 로컬/도메인 관리자 그룹 변경 탐지
```powershell
# Administrators 그룹(로컬) 멤버 추가/제거(4732/4733)
$since=(Get-Date).AddDays(-1)
...
## 15. SIEM로 보내기 — Winlogbeat/Fluent Bit 예시(개념)
