...
**값/구간별 선택도를 현실적으로 계산**하게 해 준다. Oracle은 `FREQUENCY`, `TOP-FREQUENCY`, `HYBRID`, `HEIGHT BALANCED(legacy)` 히스토그램을 지원하며, 분포/NDV/버킷 수/샘플링 방식에 따라 자동 선택한다. :contentReference[oaicite:0]{index=0}

---

## 2. 히스토그램이 사용되는 통계 체인(그림)
...
## 4. 히스토그램 유형 4가지와 선택 기준

Oracle은 컬럼의 NDV, 버킷 수 `n`(기본 254), 샘플링 방식(AUTO vs 수동)을 보고 히스토그램 유형을 결정한다. :contentReference[oaicite:1]{index=1}

### 4.1 Frequency 히스토그램

- **조건**: NDV ≤ n 이면 값마다 1버킷을 배정할 수 있어 **도수분포(FREQUENCY)** 가 생성된다. :contentReference[oaicite:2]{index=2}
- **특징**
...
### 4.2 Top-Frequency 히스토그램

Frequency의 변형. NDV가 꽤 크지만 “상위 인기값”이 대부분의 행을 차지하면,  
그 인기값들만 정확히 기록하고 나머지는 density로 근사한다. :contentReference[oaicite:3]{index=3}

- **생성 조건(공식 요지)** :contentReference[oaicite:4]{index=4}  
  1) NDV > n  
...
### 4.3 Hybrid 히스토그램(12c+ 표준)

NDV가 n보다 큰데, 상위 n값이 테이블 대부분을 먹지는 않는 경우  
Top-Freq 대신 **Hybrid**가 생성된다. :contentReference[oaicite:5]{index=5}

- **생성 조건(공식 요지)** :contentReference[oaicite:6]{index=6}  
  1) NDV > n  
...
### 4.4 Height-Balanced 히스토그램(legacy)

- 11g까지 NDV>n이면 기본으로 생성되던 방식.
- 각 버킷이 비슷한 “행 수(높이)”를 가지도록 경계를 잡는다. :contentReference[oaicite:7]{index=7}
- 12c 이후 AUTO_SAMPLE_SIZE로 새로 만드는 히스토그램은 Top-Freq/Hybrid가 되고  
  기존 Height-Balanced는 **업그레이드 후 통계 재수집 시 교체**된다. :contentReference[oaicite:8]{index=8}

---

## 5. Oracle이 히스토그램 유형을 고르는 로직(결정 테이블)
...
## 6. 내부 저장 구조: USER_TAB_COL_STATISTICS & USER_HISTOGRAMS

### 6.1 컬럼 통계에서 보는 히스토그램 메타
...
- `HISTOGRAM`:
  - `FREQUENCY / TOP-FREQUENCY / HYBRID / HEIGHT BALANCED` 로 유형 표시 :contentReference[oaicite:9]{index=9}
- `NUM_BUCKETS`:
  - 실제 생성된 버킷 수  
  - 요청한 버킷과 다를 수 있음(SIZE AUTO/분포 기반 자동 조정)

### 6.2 USER_HISTOGRAMS 해석
...
## 7. 실습 1 — 스큐 카테고리 컬럼에 Top-Frequency/Frequency 만들기
...
- SIZE 254는 최대 버킷 한도(기본치)이며, Oracle이 분포를 보고  
  FREQUENCY / TOP-FREQUENCY / HYBRID 중 자동 선택한다. :contentReference[oaicite:10]{index=10}
...
## 8. 실습 2 — 연속형 컬럼에 Hybrid/Height-Balanced 만들기
...
AUTO_SAMPLE_SIZE라면 NDV가 큰 연속형 컬럼은 대개 Hybrid로 간다. :contentReference[oaicite:11]{index=11}
...
## 9. 선택도 계산의 핵심 수학(현업에서 반드시 필요한 부분)
...
- “비인기값”은 density 기반 보정이 섞인다(Top-Freq/Hybrid). :contentReference[oaicite:12]{index=12}
...
## 10. 바인드 변수 + 히스토그램: Bind Peeking과 ACS
...
## 11. 실전 함정 10가지(“왜 히스토그램이 있어도 틀리나?”)
...
9) **업그레이드 후 히스토그램 유형 교체**  
   - 11g Height-Balanced → 12c+ Hybrid/Top-Freq로 교체될 수 있음. :contentReference[oaicite:15]{index=15}
10) **파티션별 분포가 다른데 GLOBAL만 보고 판단**
    - 파티션 히스토그램/증분 통계/프루닝과 함께 봐야 한다.

---

## 12. 결합 선택도(상관관계)와 확장 통계
...
### 14.2 시나리오 B: 바인드 하나로 값군이 극단적으로 갈리는 리포트
...
- [ ] AUTO_SAMPLE_SIZE로 Top-Freq/Hybrid 품질 확보 :contentReference[oaicite:16]{index=16}  
