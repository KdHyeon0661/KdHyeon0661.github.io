...
가 갈리면서 실행 시간이 **수 배~수십 배** 달라집니다. 자동 모드에서는 이 Workarea 크기를 Oracle이 **전역 예산(PGA_AGGREGATE_TARGET)과 동시성**을 보고 매 순간 조정합니다. :contentReference[oaicite:0]{index=0}

---

## 1) AUTO vs MANUAL: “무엇이 바뀌는가”를 먼저 확실히 하자

### 1.1 AUTO(Automatic PGA Memory Management)의 정의

- `WORKAREA_SIZE_POLICY=AUTO`일 때
- Oracle은 **PGA_AGGREGATE_TARGET 안에서 전역 상한(Global Memory Bound, GMB)** 을 계산하고,  
  각 Sort/Hash Workarea에 **Grant**를 부여합니다. :contentReference[oaicite:1]{index=1}
- 이때 **SORT_AREA_SIZE, HASH_AREA_SIZE는 무시**됩니다. :contentReference[oaicite:2]{index=2}
...
- Workarea 크기는 **SORT_AREA_SIZE(정렬), HASH_AREA_SIZE(해시)** 로 고정됩니다. :contentReference[oaicite:3]{index=3}
- 전환의 내부 조건은 “**PGA_AGGREGATE_TARGET=0**”과 동일한 효과입니다. :contentReference[oaicite:4]{index=4}
...
AUTO는 “전역 bound”에서 이를 스로틀하지만, MANUAL은 못 합니다. :contentReference[oaicite:5]{index=5}

### 2.3 “큰 값이면 항상 좋은가?” — 아니다
...
### 2.4 그래서 Oracle이 AUTO를 기본값으로 강하게 권장한다
...
이 “전역 상한 기반 자동 조절”이 AUTO의 정수입니다. :contentReference[oaicite:6]{index=6}
...
그 안에서 GMB를 계산합니다. :contentReference[oaicite:7]{index=7}
...
`V$PGA_TARGET_ADVICE`와 `V$PGA_TARGET_ADVICE_HISTOGRAM`입니다. :contentReference[oaicite:8]{index=8}
...
- `workarea executions - onepass/multipass` 비율이 높다  
  ...
  → PGA 타깃 상향 후보 :contentReference[oaicite:9]{index=9}
- `over allocation count`가 자주 증가한다  
  → 동시 Workarea가 예산을 빼앗아 급격한 축소/스필이 발생했다는 신호  
  → 타깃 상향 또는 동시성/PQ 조절 후보 :contentReference[oaicite:10]{index=10}
- `global memory bound`가 지나치게 낮다  
  → “지금 이 순간 Workarea 경쟁이 심하다”는 뜻  
  → 동시성/쿼리 구조/대상 Workarea 줄이기가 먼저일 수도 있음. :contentReference[oaicite:11]{index=11}
...
- Advice 뷰는 “**최근 워크로드**를 기반으로 재계산”되고,  
  인스턴스 재기동이나 타깃 변경 시 히스토그램이 리셋될 수 있습니다. :contentReference[oaicite:12]{index=12}  
  → 반드시 **같은 조건**에서 반복 측정.

#### (3) 하드 캡(PGA_AGGREGATE_LIMIT) 설정

`PGA_AGGREGATE_LIMIT`는 전체 PGA의 **하드 상한**으로,
타깃을 넘는 폭주(스필 능력 초과, 비정상 메모리 누수)를 OS 레벨 전에 차단합니다. :contentReference[oaicite:13]{index=13}
...
### 4.2 AUTO의 동작 핵심: “GMB 기반 Grant”

AUTO에서는 Oracle이 계산한 **Global Memory Bound(GMB)** 가
각 Workarea의 상한으로 작동합니다. :contentReference[oaicite:14]{index=14}

- Grant 산정 개념:
  $$\text{Grant} = \min(\text{ExpectedOptimalSize},\ \text{GMB})$$
- 동시성↑ → GMB↓ → Workarea가 줄어 **Onepass/Multi** 로 전환 :contentReference[oaicite:15]{index=15}
- 동시성↓ → GMB↑ → 같은 SQL이라도 더 큰 Grant를 받아 Optimal 비율이 늘어남

즉, AUTO는 “**전체를 살리기 위한 자동 스로틀**”을 내장한 모드입니다.

### 4.3 MANUAL의 동작 핵심: “고정 상한 + 디스크 스필”

MANUAL에서는

- Sort Workarea 상한 = SORT_AREA_SIZE
- Hash Workarea 상한 = HASH_AREA_SIZE
- 전역 상한/자동 축소가 없습니다. :contentReference[oaicite:16]{index=16}
...
활성 Workarea가 많아져 GMB가 줄었기 때문일 수 있습니다. :contentReference[oaicite:17]{index=17}  
...
### 6.2 케이스 A: MANUAL, 작은 SORT_AREA_SIZE
...
### 6.3 케이스 B: MANUAL, 큰 SORT_AREA_SIZE
...
- `global memory bound` 상승 :contentReference[oaicite:18]{index=18}
- Optimal 비율 증가  
...
### 8.2 하드 캡 + Resource Manager 병행

- `PGA_AGGREGATE_LIMIT` 하드 상한 설정 :contentReference[oaicite:19]{index=19}
- Consumer Group별 PGA/병렬도 제한  
  (12cR2+는 **프로세스 PGA 상한**도 설정 가능) :contentReference[oaicite:20]{index=20}

### 8.3 “최악 시나리오 총합”을 계산해 둔다
...
예산을 뚫고 성능을 보장하지 않습니다. :contentReference[oaicite:21]{index=21}  
따라서 성능 튜닝의 우선순위는
...
- **기본값은 AUTO + 합리적인 PGA_AGGREGATE_TARGET**  
  AUTO는 GMB를 통해 Workarea 크기를 동시성에 맞춰 자동 조절합니다. :contentReference[oaicite:22]{index=22}
- **MANUAL은 “특수 배치/튜닝 실험”에서만, 세션 단위로 제한적으로**  
  자동 분배를 끄면 SORT/HASH 상한이 고정되며, 동시성·PQ에서 곱셈 폭주가 생깁니다. :contentReference[oaicite:23]{index=23}
- PGA 타깃은 **Advice 뷰와 One/Multipass 비율을 근거로 증분 조정**하고,  
  하드 캡은 `PGA_AGGREGATE_LIMIT`로 잡아 폭주를 막습니다. :contentReference[oaicite:24]{index=24}
