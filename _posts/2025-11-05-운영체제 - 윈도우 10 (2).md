...
### 21.4.3 예제 — WTS API로 세션 나열·메시지 보내기(C/C++)
```cpp
...
### 21.6.2 예제 — Winsock TCP 에코 서버(간단, select)
```cpp
...
### 21.6.3 예제 — HttpListener(HTTP.sys) 최소 서버(C#)
```csharp
using System;
using System.Net;
using System.Text;
...
### 21.7.5 COM 기초 — File Open Dialog 사용(C++)
```cpp
...
### 21.7.9 보안: ACL·무결성·AppContainer
