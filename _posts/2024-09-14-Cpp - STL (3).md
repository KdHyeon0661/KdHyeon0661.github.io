...
## 1) `map` — 정렬된 Key→Value (기본 비교자: `std::less<Key>`)
...
### 1.5 이종(heterogeneous) 조회 — **비용↓, 임시 생성 없음**
...
### 1.6 노드 조작(C++17) — `extract/merge`로 **키 수정/컨테이너간 이동**
...
### 3.5 **이종(heterogeneous) 조회** in 해시 (C++20)
...
### 4.1 안전한 삭제 루프 (`map`/`set`/`unordered_map` 공통)
...
### 4.2 `insert_or_assign` vs `try_emplace` (C++17)
...
### 8.3 키 수정이 필요할 때 — `extract`로 안전하게
