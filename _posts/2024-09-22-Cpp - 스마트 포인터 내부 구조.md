...
## `shared_ptr` 내부 구조 ― “컨트롤 블록과 참조 카운트”
...
## `make_shared` / `allocate_shared`가 유리한 진짜 이유

### 단일 할당(one-shot allocation)
...
### `allocate_shared` — 커스텀 allocator와 결합
...
## 사용자 정의 Deleter, `get_deleter`, 배열/서브오브젝트
...
### **Aliasing constructor** — 서브오브젝트 공유
...
## `enable_shared_from_this`와의 상호작용
...
### deleter가 크면? (EBO/[[no_unique_address]])
...
### `shared_ptr`을 값으로 컨테이너에 많이 담을 때
...
## 통합 예제: 파일 디스크립터, 메시지 버퍼, aliasing, 콜백
