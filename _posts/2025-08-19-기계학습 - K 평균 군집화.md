...
### 2.1 목적 함수(Objective; Within-Cluster SSE)
$$
...
# 3. KMeans (K-Means++ 초기화, 여러 시도 중 최적 선택)
k = 5
...
### 11.3 Mini-Batch K-Means(대용량 근사)
```python
...
### 11.4 혼합형 데이터 대안(k-prototypes 아이디어 스케치)
> 실제 구현은 `kmodes` 패키지 참고. 여기선 개념적 흐름 예시.

```python
# 연속형 특징 L2, 범주형 특징 해밍거리(불일치 개수)에 가중 합:
# d(x, μ) = α * ||x_cont - μ_cont||_2^2 + (1-α) * Hamming(x_cat, mode_cat)
# μ_cont는 연속형 평균, mode_cat은 범주별 최빈값.
```

### 11.5 Numpy로 from-scratch(학습용; 기본형)
```python
import numpy as np
