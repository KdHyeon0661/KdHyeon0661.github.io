...
4) **Adaptive 계획/통계 피드백 개입 가능성** :contentReference[oaicite:0]{index=0}

### 1.1 SARGable vs Non-SARGable
...
### 1.3 NOT IN vs NOT EXISTS (3-valued logic)
...
### 1.4 OR 조건, IN-LIST, OR-Expansion
...
### 2.2 Clustering Factor(CF)

CF는 “인덱스 순서가 테이블 물리 순서와 얼마나 맞는가”를 나타내며,  
NL 조인에서 테이블 랜덤 액세스 비용을 결정하는 핵심 통계다. :contentReference[oaicite:1]{index=1}
...
### 2.5 Automatic Indexing(19c+)

Oracle 19c부터 **자동 인덱싱**이 도입되어  
워크로드를 보고 인덱스를 만들고, 효과 없으면 제거한다. :contentReference[oaicite:2]{index=2}
...
**원본 테이블을 읽지 않고 MV로 대체**할 수 있다. :contentReference[oaicite:3]{index=3}
...
제약은 단순 무결성 체크가 아니라  
옵티마이저에게 **“논리적 사실”을 알려주는 통계급 정보**다. :contentReference[oaicite:4]{index=4}
...
통계는 CBO 의사결정의 입력이다.  
자세한 내용은 앞선 통계 가이드와 동일 원리다. :contentReference[oaicite:5]{index=5}

### 7.1 AUTO_SAMPLE_SIZE가 중요한 이유

AUTO_SAMPLE_SIZE일 때만  
**Top-Frequency/Hybrid 히스토그램이 생성**될 수 있다. :contentReference[oaicite:6]{index=6}

### 7.2 확장 통계(Extended Stats)

컬럼 상관관계를 반영해 결합 선택도를 교정한다. :contentReference[oaicite:7]{index=7}
...
### 7.3 Dynamic Statistics / OPTIMIZER_DYNAMIC_SAMPLING

통계가 부족하거나 복잡한 조건일 때  
파스 시점에 샘플을 떠서 선택도를 보정한다. :contentReference[oaicite:8]{index=8}
...
### 7.4 Real-Time Statistics & High-Frequency Stats Gathering (19c+)
...
가 도입되었다. :contentReference[oaicite:9]{index=9}
...
남은 단계의 결정을 바꿀 수 있다. :contentReference[oaicite:10]{index=10}
...
```
이 파라미터를 끄면  
SPD/조인 피드백/적응 샘플링 일부가 비활성화된다. :contentReference[oaicite:11]{index=11}  
...
힌트는 “성향”이 아니라 **강제**다. :contentReference[oaicite:12]{index=12}
...
“좋은 플랜을 **고정**하거나,  
나쁜 플랜을 **자동 회피**”하는 메커니즘. :contentReference[oaicite:13]{index=13}

### 10.1 SQL Plan Baseline(SPM)
...
### 10.4 Real-Time SPM (23ai)

23ai에서는 플랜 회귀를 실시간 감지해  
**자동으로 baseline을 만들고 회귀를 복구**하는 RT-SPM이 강화됐다. :contentReference[oaicite:14]{index=14}
...
사용자 의도 없이도 병렬 플랜이 나올 수 있다. :contentReference[oaicite:15]{index=15}
...
| `optimizer_dynamic_sampling` | Dynamic Statistics 수준 | 과하면 파스 비용↑ :contentReference[oaicite:16]{index=16} |
| `optimizer_features_enable` | 버전별 옵티마이저 기능 묶음 | 회귀 대응용. 상시 고정은 지양 :contentReference[oaicite:17]{index=17} |
| `cursor_sharing` | EXACT / FORCE | 바인드 치환으로 선택도/플랜 흔들림 가능 :contentReference[oaicite:18]{index=18} |
...
이 달라진다. :contentReference[oaicite:19]{index=19}

업그레이드 시
- 테스트 없이 OFE를 구버전으로 고정하는 건 최후수단이며,
- **SPM/Baseline으로 핵심 SQL을 보호**하면서 점진 전환하는 게 정석이다. :contentReference[oaicite:20]{index=20}
