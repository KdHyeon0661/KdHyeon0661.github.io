...
## 1.2 OR → UNION ALL, IN-LIST, OR-Expansion
```sql
...
# 2. **인덱스**: 구조, 클러스터링 팩터, 함수기반 인덱스, Skip Scan
## 2.1 기본/결합/커버링
```sql
...
## 2.4 Index Skip Scan (선행 컬럼이 등치가 아닐 때)
```sql
...
# 5. **MV(Materialized View) & Query Rewrite**
## 5.1 MV로 집계/조인 결과 캐시
```sql
-- 월/카테고리 집계를 MV로
...
# 6. **제약(Constraint)**: 카디널리티 추정/조인 제거/전개
## 6.1 NOT NULL/UNIQUE/PK → **선택도/전개**
- `col IS NOT NULL`은 불필요(컬럼이 NOT NULL이면 필터 제거).
- PK/UK는 **유일성** → **카디널리티 1** 가정 → **Index Unique Scan** 선호.

## 6.2 외래키(FK)로 **조인 제거(Join Elimination)**
...
## 6.3 체크 제약(Check) → 범위 추정 도움
- 값 범위를 명확히 하면 선택도 추정 정확도 ↑.

---

# 7. **옵티마이저 힌트**: 탐색공간 제어 도구
...
# 10. **DBMS 버전/에디션/종류**: 기능/코스트모델/적응성 차이
- **버전 차이**:  
