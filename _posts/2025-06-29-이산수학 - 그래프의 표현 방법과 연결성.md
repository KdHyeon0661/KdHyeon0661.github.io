...
### 1.4 희소 행렬 포맷(CSR/CSC/COO) — 대용량/수치계산·GPU 친화
...
## 6) Union-Find(Disjoint Set Union, DSU) — 오프라인 연결성/동적 합치기
...
## 8) (무방향) 단절점(articulation) & 브리지(bridge)
...
## 10) 트랜지티브 클로저(도달가능성 전체) — 행렬/반복 BFS

### 10.1 플로이드–워셜(작은 \(n\), 밀집)
