...
category: DB 심화
---
# 트랜잭션 수준, Dirty/Non-Repeatable/Phantom Read까지 (Oracle 중심·타 DB 비교)
...
## Statement-Level vs Transaction-Level (Oracle 관점)
...
## Dirty/Non-Repeatable/Phantom: 요약표(제품 매핑)
...
트랜잭션 수준 읽기 일관성(TLRC)은 데이터 일관성 요구사항을 가장 강력하게 보장하는 격리 수준입니다. 이는 트랜잭션이 시작된 시점의 데이터 스냅샷을 트랜잭션 전체에 걸쳐 일관되게 유지함으로써, 같은 데이터에 대한 반복 조회 시에도 결과가 변하지 않음을 보장합니다. Oracle에서는 SERIALIZABLE 격리 수준으로, PostgreSQL에서는 REPEATABLE READ나 SERIALIZABLE로 구현되며, InnoDB에서는 기본 REPEATABLE READ로 제공됩니다.

그러나 이러한 강력한 일관성 보장은 비용을 수반합니다. Oracle의 SERIALIZABLE 모드에서는 경쟁적 수정이 발생할 경우 ORA-08177 오류와 함께 트랜잭션이 실패하므로, 애플리케이션에 재시도 메커니즘을 구축해야 합니다. 장시간 실행되는 트랜잭션의 경우 충분한 Undo 공간과 적절한 보존 정책이 필요하며, 그렇지 않으면 ORA-01555 "snapshot too old" 오류가 발생할 수 있습니다.

실제 설계에서는 완벽한 TLRC가 모든 트랜잭션에 필요한 것은 아닙니다. 대부분의 OLTP 작업에서는 Oracle의 기본 Read Committed(SLRC) 수준으로 충분하며, 이는 성능과 일관성 사이의 합리적인 균형을 제공합니다. TLRC는 재무 보고서 생성, 복잡한 배치 처리, 또는 특정 비즈니스 규칙에 따라 반드시 트랜잭션 전체의 일관성이 요구되는 핵심 구간에 선택적으로 적용해야 합니다.

각 데이터베이스 제품의 구현 차이(Oracle의 충돌 감지 vs InnoDB의 사전 차단 vs PostgreSQL의 SSI)를 이해하고, 시스템의 특성과 비즈니스 요구사항에 맞는 격리 수준을 선택하는 것이 중요합니다. 결국 트랜잭션 격리 수준의 선택은 단순한 기술적 결정이 아니라, 데이터 정확성, 시스템 성능, 개발 복잡도, 그리고 비즈니스 리스크 사이의 전략적 균형을 찾는 과정입니다.