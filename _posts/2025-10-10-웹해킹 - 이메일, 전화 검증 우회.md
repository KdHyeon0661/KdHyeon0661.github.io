...
## 4. Node.js/Express + Postgres/Prisma (예제)

### 4.1 발급(전송) — **멱등 + 최신만 유효 + 쿨다운**
...
## 5. Redis(또는 캐시)로 구현하는 **속도 제한 & 레이스 방지**

### 5.1 재전송 쿨다운(키: `otp:issue:{purpose}:{dest}`)
