...
### 3.4 “a 다음 b면 X로 치환, 아니면 원문” — 지연 설계 패턴
...
## 4) Mealy ↔ Moore 변환(장단·상태 수·지연)

### 4.1 Moore → Mealy(출력 지연 제거)
...
### 4.2 Mealy → Moore(출력 안정화·1-사이클 지연)
...
## 5) 최소화(Partition Refinement) — 정의·알고리즘·복잡도
...
## 7) 일반화: **문자열 출력**·**최종 출력**(후속, subsequential)
...
## 9) 구현 포인트 — 테이블·성능·코드 아키텍처

### 9.1 테이블 기반 DMealy 구현 스켈레톤(Python)
