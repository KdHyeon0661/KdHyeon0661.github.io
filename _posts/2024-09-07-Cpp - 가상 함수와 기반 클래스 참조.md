...
## `virtual`·`override`·`final`·공변 반환·가상 소멸자
...
### ref-qualifier로 lvalue/rvalue 구분
...
## 기반 클래스 참조(super 대용) — 범위 지정과 은닉 해제
...
## NVI(Non-Virtual Interface): 생성/소멸 가상호출 함정 회피
...
## 실전 예제 1 — 다형 플러그인 + super 호출 + 테스트 friend
...
## 실전 예제 2 — `this` ref-qualifier로 체이닝 안전화
