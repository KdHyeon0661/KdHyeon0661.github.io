...
## 0. 큰 그림: “AI 보안” 위협 지형(Threat Landscape)
...
### 2.2 프라이버시 공격 — 멤버십/모델 반전/속성 추론

#### 2.2.1 멤버십 추론(MIA) — 손실 임계 기반 간단 공격
...
#### 2.2.2 (개념) 모델 반전/속성 추론
**대응**: **DP-SGD**, **공개 가능한 통계 최소화**, **민감 특성 비학습/분리**, **API 출력 제한**(Top-k/확률 비공개).

---

### 2.3 데이터 중독/백도어 공격과 방어

#### 2.3.1 백도어(Trigger) 삽입 예시(데이터 변조)
```python
...
## 3. 차등프라이버시(DP-SGD) — PyTorch(표준 기능)로 구현
...
### 4.2 실험 추적/메타데이터 서버(예: MLflow/W&B 대체)
...
## 7. 실전 시나리오 & PyTorch 예제

### 7.1 분류기(CIFAR-10 가정) — 적대적 훈련 + 평가
```python
...
## 8. 운영 표준(Policies) — 실전에 바로 쓰는 규정 초안
