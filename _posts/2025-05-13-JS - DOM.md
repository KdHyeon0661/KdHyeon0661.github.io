...
### 3.1 `textContent` / `innerText` / `innerHTML`
```js
...
### 6.2 `DocumentFragment`로 배치 삽입(성능)
```js
...
### 8.1 고급 순회기: `TreeWalker`/`NodeIterator`
```js
...
### 10.2 `IntersectionObserver`로 지연 로딩
```js
