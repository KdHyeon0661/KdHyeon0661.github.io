...
## 5. finally 대체: `using` / `await using`

### 5.1 동기 `using` (C# 8 이전/이후 모두)
...
### 5.3 비동기 `await using` (IAsyncDisposable)
...
## 8. **ExceptionDispatchInfo** — 스택 보존 재던지기
...
## 9. 비동기/병렬: async/await, Task, AggregateException

### 9.1 async/await — 예외 전파
...
### 9.2 Task/Parallel — AggregateException
...
### 9.3 취소: OperationCanceledException
