...
### 7.1 \(\int \dfrac{dx}{(x^2+a^2)^n}\) 감소식
\[
\int \frac{dx}{(x^2+a^2)^n}=
\frac{x}{2(n-1)a^2(x^2+a^2)^{n-1}}
...
### 7.2 \(\int \dfrac{Ax+B}{(x^2+px+q)^n}\) 일반 해법
...
## 10. 코드 예시(심볼릭 분해/적분) — Python/SymPy
...
# 3. 혼합형 예제: (x^3+2x^2+3x+4)/((x-1)^2*(x^2+1))
