...
## 5.1 함수와 패턴 매칭 (Functions with Pattern Matching)
...
### 5.1.4 인자 패턴 vs 본문 if/else — 스타일 비교
...
## 5.2 함수는 하나, 본문은 여러 개 (Multi-clause Functions)
...
## 5.3 함수를 반환하는 함수 (Functions that Return Functions)
...
## 5.4 함수를 인자로 전달하기 (Passing Functions as Arguments)
...
### 5.4.3 `Task.async_stream/3` — 병렬 처리 대상 함수를 전달
...
### 5.4.4 함수 vs MFA (Module-Function-Arity)
...
## 5.5 함수는 엘릭서의 핵심이다 (Why Functions are the Core)
...
### 5.5.6 종합 예제: 미니 라우터 + 파이프라인 + 전략 주입
