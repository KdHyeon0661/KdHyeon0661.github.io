...
category: Django
---
# 읽기 모델(Elasticsearch) 설계 · 다중 리더/리더리스 이벤트 스트리밍 · 사가 설계(오케스트레이션 vs 코레오그래피) 비교 (Django 5.x 기준)
