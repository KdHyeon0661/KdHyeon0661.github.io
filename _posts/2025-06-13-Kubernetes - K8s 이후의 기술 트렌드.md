...
## K3s — Rancher가 만든 초경량 CNCF 호환 K8s
...
## MicroK8s — Canonical의 개발·AI/ML 친화 경량 K8s
...
### GitHub Actions로 이미지 빌드→K3s 배포(요약)
...
### K3s + Nginx Ingress + Longhorn + HPA
...
### MicroK8s에서 Kubeflow(한줄)

```bash
microk8s enable kubeflow    # 설치 시간 다소 소요, 대용량 디스크 권장
```

### KubeEdge 장치 제어(개념 YAML 스니펫)
