...
## 1) 파일 열기: `open(path, mode, encoding, newline, buffering, errors, ...)`
...
## 2) 파일 쓰기: `write() / writelines() / print(..., file=f)`
...
### 2.1 권장: `with` 컨텍스트 (자동 close/플러시/예외에도 안전)
```python
...
## 3) 파일 읽기: `read / readline / readlines / for line in f`
...
## 5) 안전한 파일 처리: `with`와 예외/flush/fsync
...
### 7.2 `iter(callable, sentinel)` 패턴 (EOF 센티넬)
```python
...
## 8) JSON/CSV/피클 등 실전 포맷

### 8.1 JSON (사람/언어 친화, 텍스트)
```python
import json
...
### 8.3 Pickle (파이썬 객체 직렬화, **동형성/성능**, 단 보안 주의)
```python
import pickle
...
## 9) 압축/아카이브: `gzip`, `bz2`, `zipfile`, `tarfile`
...
## 10) 메모리 맵 파일: `mmap` (랜덤액세스/대용량 슬라이싱)
...
## 12) 원자적(Atomic) 저장 패턴 (중요 파일 안전 기록)
...
## 13) 권한/보안: `os.open`/모드/umask/경로 검증

### 13.1 권한 제어로 파일 열기 (저수준 FD)
```python
import os, stat
...
## 14) 경로/파일 작업: `pathlib` (현대적/가독성 높은 경로 API)
...
## 16) `io` 모듈 계층: Text/Buffered/Raw & 메모리 파일
...
## 21) 실전 미니 프로젝트: 로그 회전(rotate) + 압축 + 안전 저장
