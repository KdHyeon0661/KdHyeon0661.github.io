...
## 1. 기본형: 단일 경로(One-path) 편도 연쇄법칙

### 1.1 스칼라 출력, 중간이 스칼라 1개
...
### 1.2 스칼라 출력, 중간이 벡터 (다중 입력이 한 출력으로 모임)
...
## 2. 계산 그래프 관점의 다중 경로(Multiple-path) 연쇄법칙
...
## 6. 좌표변환에서의 연쇄법칙 (극·구면·일반 변환)

### 6.1 극좌표 \((r,\theta)\)에서 \(f(x,y)\)
...
### 6.2 구면좌표 \((\rho,\phi,\theta)\)에서 \(f(x,y,z)\)
...
### 7.1 하나의 제약: \(F(x,y)=0\)가 \(y=y(x)\)를 규정
...
### 7.2 다변수 암시적 함수: \(F(x,y,z)=0\)에서 \(z=z(x,y)\)
...
## 8. 재료미분(Material derivative): 이동하는 관찰자
...
## 9. 확률·최적화의 전형적 합성: 로스/링크/정규화

### 9.1 로지스틱 회귀의 \(\partial L/\partial w\)
...
### 10.1 \(\phi(u_1,\dots,u_m)\)와 \(u_j(x)\)의 경우
...
## 11. 지수·로그·삼각 함수의 편도 연쇄 예제

### 11.1 \(f(x,y)=\exp(x\sin y)\)

- \(u=x\sin y\).
- \(f_x = e^u \cdot \sin y\).
- \(f_y = e^u \cdot x\cos y\).

### 11.2 \(f(x,y,z)=\ln(x^2+y^2+z^2)\)

- \(u=x^2+y^2+z^2\).
- \(f_{x} = \dfrac{2x}{u}\), \(f_{y}=\dfrac{2y}{u}\), \(f_{z}=\dfrac{2z}{u}\).

### 11.3 \(f(r,\theta)=\cos(r^2\theta)\), \(x=r\cos\theta,\ y=r\sin\theta\)와의 결합
...
## 12. 실전: 편도 연쇄를 파이썬으로 확인 (심볼릭/수치)

### 12.1 SymPy로 편도 연쇄 확인
...
## 13. 역전파/자동미분에서의 편도 연쇄

### 13.1 스칼라 로스 \(L\)에 대한 파라미터 \(\theta\)
