...
## ABI(이진 인터페이스) 설계 — C 함수 테이블 + 버전 협상
...
## DLL 로딩 안전 가이드(하이재킹 방지/검색 경로 제어)
