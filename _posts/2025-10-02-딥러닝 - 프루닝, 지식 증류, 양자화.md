...
#### Unstructured L1 Magnitude (layer-wise / global)
...
#### Structured LN-structured (필터/채널 제거)
...
### 커스텀 구조화 프루닝 파이프라인(Conv 채널 제거 후 그래프 수정)
...
### 글로벌 스케줄(Iterative Pruning + Fine-tune)
...
#### Dynamic PTQ(가중치만 양자화; LSTM/Linear에 효과)
...
#### Static PTQ(FX Graph Mode; Conv에 효과, 캘리브레이션 필요)
...
# Conv-BN-ReLU fuse (fx 기반 모델에 따라 auto/수동)
# # 예시(구버전 API)
...
### Distillation: ResNet-Teacher → SmallCNN-Student
