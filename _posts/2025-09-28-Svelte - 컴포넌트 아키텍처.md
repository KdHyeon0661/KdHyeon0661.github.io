...
### 입력 컴포넌트: `bind:value` 지원 + `change/input` 이벤트
...
### 메뉴/선택 그룹 설계 (액션과 섞지 말고 **컨텍스트**로 연결)
