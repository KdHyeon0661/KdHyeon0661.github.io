...
### 고정 IN 목록 + sentinel 값 (정수/숫자 키일 때)
...
## IN-list 항목이 **가변**이고 “최대 경우 수가 **아주 많은**” 경우

> **상황**: 선택된 ID/코드가 **수십~수천 개**.
> **목표**: 텍스트 정적, 커서 공유 유지, **대량 키 필터**를 효율적으로.

### **스키마 타입 + 배열 바인드** (정적 SQL 유지)
...
## **체크 조건** 적용이 가변적인 경우 (옵셔널 필터) — 정적 SQL로
...
### `UNION ALL` 가드 분기 (텍스트는 1문장, 분기 플랜 명확)
...
## **연산자**(=, <, <=, BETWEEN, LIKE …)가 **바뀌는** 경우 — 정적 SQL로
