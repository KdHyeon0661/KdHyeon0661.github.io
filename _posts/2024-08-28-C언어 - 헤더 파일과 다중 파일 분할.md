...
## 3) 헤더 가드 vs `#pragma once`

### 3.1 Include Guard(전통/이식성 최고)
...
## 4) 헤더 위생(Header hygiene) — 반드시 지킬 것
...
### 10.2 MSVC (Developer Command Prompt)
```bat
...
## 12) 라이브러리로 쪼개기 — 정적(.a/.lib) vs 동적(.so/.dll)
...
## 15) 예제 프로젝트 2 — 모듈 3개와 순환 회피, 단위 테스트
...
## 16) 테스트 가능한 API와 링크 에러 예방 체크리스트
