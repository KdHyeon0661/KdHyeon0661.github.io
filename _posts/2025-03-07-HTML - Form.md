...
## 4. HTML5 Constraint Validation — 속성·CSS·JS API
...
### 4.4 FormData API와 `formdata` 이벤트
...
## 8. 모바일/국제화 UX — inputmode·enterkeyhint·자동완성 토큰
...
## 9. 다단계(멀티스텝) 폼 — `fieldset`/`progress`/포커스 관리
...
## 10. 버튼별 다른 전송 — `formaction/formmethod/formenctype/formnovalidate`
...
## 11. 검색/로그인/회원가입 — 실전 스니펫

### 11.1 검색 폼(GET, 북마크 가능)
...
### 13.1 `application/x-www-form-urlencoded` 예
```
...
### 13.2 `multipart/form-data` 경계 기반 예(개념)
```
...
### 13.3 fetch로 JSON 전송(PE: Progressive Enhancement)
```js
const fd = new FormData(form);
...
## 15. 종합 예제 — 회원정보 수정(파일·유효성·접근성·PE)
