...
### 7.1 파이썬: GIL이 있어도 안전하다고 단정 못 한다
...
### 7.3 자바: 정적 내부 클래스(Initialization-on-demand holder)
```java
public class Singleton {
    private Singleton() {}
...
### 8.1 자바 직렬화: `readResolve()`로 단일성 보존
```java
...
## 15. 리팩토링 경로 — “나쁜 전역”에서 “관리되는 싱글턴/DI”로
