...
### 1.3 Rootless Docker(선택) — 데몬 자체를 비루트로
```bash
# 설치 스크립트(예: Ubuntu)
...
## 4. 낮은 포트(80/443) 바인딩, 루트 없이 하는 법

### 4.1 `setcap`으로 바이너리만 권한 부여
```dockerfile
FROM caddy:2.8.4-builder AS builder
# ... (빌드)

FROM caddy:2.8.4
# 1024 이하 포트 바인딩 권한 부여
...
## 6. LSM(AppArmor/SELinux) 적용

### 6.1 AppArmor(Ubuntu 등)
```bash
...
### 6.2 SELinux(RHEL/Alma 등) — 볼륨 레이블
```bash
# :Z(독점), :z(공유) 레이블 옵션
