...
### 디스크 아티팩트 지도 (Windows / Linux / macOS)
...
# 최근 실행 흔적(ShimCache/Amcache는 전문 도구 권장), 이벤트 개수 요약
...
### 메모리 아티팩트 지도 (Windows 중심, Linux 병행)
...
## 타임라인/유입 경로 분석, 휘발성 데이터 수집(메모리 덤프)

### 타임라인(슈퍼 타임라인) — Plaso(Log2Timeline) → Timesketch
...
# 슈퍼 타임라인 생성 (수 시간 걸릴 수 있음, 랩은 샘플로)
...
### 네트워크 타임라인(Zeek/TShark) — 유출·C2 힌트
...
### IR 의사결정: 봉쇄(Containment) ↔ 증거보존 균형
