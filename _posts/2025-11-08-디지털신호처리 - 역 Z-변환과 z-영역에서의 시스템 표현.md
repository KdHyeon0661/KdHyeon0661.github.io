...
## 2) 역 Z-변환의 네 가지 주력 기법

### 2.1 거듭제곱 전개(power series)
\(|z|>|a|\)이면
$$
...
## 4) 예제로 익히는 역 Z-변환 (ROC별 대비)

### 4.1 예제 A — 우측 ROC: 서로 다른 두 단일극
$$
...
### 4.2 예제 B — 같은 \(X(z)\), **양측 ROC** (\(0.3<|z|<0.8\))
...
## 7) 구현(실현) 구조: Direct Form·SOS·FIR·상태공간

### 7.1 IIR Direct Form I/II
