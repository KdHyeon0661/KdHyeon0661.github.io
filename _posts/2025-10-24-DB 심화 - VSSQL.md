...
## 큰 그림: `V$SQL`은 “Child 커서별 실행/리소스 통계”
...
### 평균 단가 보기(Elapsed/Exec, Gets/Row 등)
...
## 실행계획 연결: `DBMS_XPLAN` & `V$SQL_PLAN`
### Child 커서의 **실제 라인 통계**(마지막 실행 기준)
...
### 왜 Child가 분기되었나? — `V$SQL_SHARED_CURSOR`
...
## Adaptive Cursor Sharing(ACS) & 바인드 피킹
...
## 플랜 변동 감시(Plan Hash 기반)

### 동일 SQL_ID, Child별 플랜 라벨 비교
