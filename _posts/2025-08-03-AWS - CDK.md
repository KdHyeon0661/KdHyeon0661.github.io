...
## 실습 ① S3 → Lambda → DynamoDB (이벤트 드리븐 파이프라인)
...
## 교차 스택(Cross-Stack) 참조와 경계

### Stack Props로 전달(느슨하게)
...
## 자산 번들 & 빌드 (NodejsFunction / PythonFunction / Docker)

### NodejsFunction (esbuild 번들)
...
## 고급 통합: API Gateway + Lambda + DynamoDB + Cognito
...
## CI/CD: **CDK Pipelines** (CodePipeline 통합)
...
## 실습 ② VPC + ALB + ECS + Route53 + ACM(HTTPS)
...
## Terraform/CloudFormation 사용자 이행 팁
