...
웹에서 사용자의 클릭, 입력, 포커스 이동, 네트워크 수신 등 **모든 상호작용은 “이벤트(Event)”**로 모델링됩니다. 이 글은 기초 문법에서 시작해 **전파(캡처링/버블링)**, **이벤트 위임**, **성능/보안 고려**, **키보드/포인터 심화**, **커스텀 이벤트**, **Shadow DOM/`composedPath`**, **자동 해제(`AbortController`)**까지 실전에 바로 쓰는 패턴을 체계적으로 정리합니다.

---

## 0) 큰 그림: 이벤트 루프와 이벤트 타이밍 한 줄 요약
...
## 4) 이벤트 위임(Event Delegation) — 동적 DOM의 정석
...
## 7) 포인터/마우스/터치 — 현대 브라우저 표준은 Pointer Events
...
## 8) 스크롤/제스처 성능 — `passive`와 스로틀/디바운스

### 8.1 `passive: true`로 레이아웃 차단 방지
...
### 10.2 독립 `EventTarget`로 미니 이벤트 버스
