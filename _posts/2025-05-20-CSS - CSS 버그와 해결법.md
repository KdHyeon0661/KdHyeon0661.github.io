...
category: CSS
---
# 자주 겪는 CSS 버그와 해결법 정리 — 증상→원인→해결 패턴, 브라우저별 주의점, 디버깅 레시피

CSS는 **상속(cascade)·레이아웃(Formatting Context)·스택 컨텍스트(Stacking Context)**가 한 번만 어긋나도 의도와 다른 화면을 만든다.  
아래는 실무에서 반복적으로 마주치는 버그를 **증상 → 원인 → 해결법** 포맷으로 정리하고, 재현 코드와 디버깅 체크리스트까지 함께 제공한다.  
필요한 곳엔 **대체 구현(폴백)** 과 **브라우저별 주의 사항**을 덧붙였다.
