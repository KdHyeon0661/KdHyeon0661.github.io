...
### 3.2 다중행 — IN / EXISTS / ANY(=SOME) / ALL
...
## 4. FROM 절 — 인라인 뷰 / 파생 테이블 / LATERAL/APPLY
...
### 4.2 LATERAL (PostgreSQL) / CROSS APPLY (SQL Server)
...
## 6. 상관 서브쿼리 → 조인(반상관화) 리라이팅

### 6.1 상관 서브쿼리(행마다 평균 계산) — 느리다
...
### 8.1 “모든 카테고리를 산 고객” — 관계 나눗셈(Division)
...
### 8.2 그룹 내 Top-N (부분 순위) — 상관 서브쿼리 vs 윈도
...
## 9. NOT IN vs NOT EXISTS — NULL의 함정
...
### 14.1 “평균 이상 급여자 + 부서 평균/회사 평균 동시 표기”
...
### 14.3 “각 부서 Top-1(최고 연봉) 한 명 + 동률 처리”
