...
### 고급: `INotifyDataErrorInfo`(비동기 검증/여러 오류 메시지)
...
### 오류 표시 스타일 — `Styles/Validation.axaml`
...
## 디바운스(Throttle)로 실시간 입력 반응을 제어

검색창처럼 입력이 자주 바뀌는 UI는 **디바운스**로 과도한 처리(네트워크/쿼리)를 줄인다.

### ViewModel — `ThrottledSearchViewModel.cs`
...
### View — `ThrottledSearchView.axaml`
