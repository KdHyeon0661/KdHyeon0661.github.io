---
layout: post
title: 파이썬 심화 - 문자열과 텍스트 (2)
date: 2025-11-26 18:25:23 +0900
category: 파이썬 심화
---
# 문자열과 텍스트 (2)

## 정규 표현식의 고급 활용

### 탐욕적 매칭과 최단 매칭 이해하기

정규 표현식에서 수량자(`*`, `+`, `?`, `{n,m}`)는 기본적으로 **탐욕적(greedy)**으로 동작합니다. 즉, 가능한 한 가장 긴 문자열을 찾으려 합니다. 반면 **비탐욕적(non-greedy)** 매칭은 가능한 한 가장 짧은 문자열을 찾습니다.

```python
import re

# HTML 형식의 텍스트 예시
html_content = """
<div class="header">메인 헤더</div>
<div class="content">주요 내용 영역</div>
<div class="footer">푸터 정보</div>
"""

print("=== 탐욕적 매칭 vs 최단 매칭 ===")
print("원본 텍스트:")
print(html_content)

# 탐욕적 매칭: 가장 긴 매칭을 찾음
greedy_pattern = r'<div.*>.*</div>'
greedy_match = re.search(greedy_pattern, html_content, re.DOTALL)
print("\n1. 탐욕적 매칭 결과:")
if greedy_match:
    print(f"매칭된 문자열: {greedy_match.group()[:50]}...")
    print(f"매칭 길이: {len(greedy_match.group())}자")

# 비탐욕적 매칭: 가장 짧은 매칭을 찾음
non_greedy_pattern = r'<div.*?>.*?</div>'
non_greedy_matches = re.findall(non_greedy_pattern, html_content, re.DOTALL)
print("\n2. 비탐욕적 매칭 결과:")
for i, match in enumerate(non_greedy_matches, 1):
    print(f"  {i}. {match[:40]}...")
    print(f"     길이: {len(match)}자")

# 실제 활용 예제: HTML 태그 내용 추출
def extract_div_contents(html_text):
    """모든 div 태그의 내용 추출"""
    pattern = r'<div[^>]*>(.*?)</div>'
    contents = re.findall(pattern, html_text, re.DOTALL)
    
    # 내용 정제 (공백 제거)
    cleaned_contents = [content.strip() for content in contents]
    return cleaned_contents

print("\n3. div 태그 내용 추출:")
contents = extract_div_contents(html_content)
for i, content in enumerate(contents, 1):
    print(f"  div {i}: '{content}'")
```

### 다양한 수량자의 비탐욕적 버전

```python
# 다양한 비탐욕적 수량자 예제
text = "aaa bbbb ccccc dddddd eeeeeee"

print("=== 다양한 수량자 비교 ===")

# 1. * (0회 이상) vs *? (0회 이상, 최소)
matches_greedy = re.findall(r'a*', 'aaaa')
matches_non_greedy = re.findall(r'a*?', 'aaaa')
print(f"a* (탐욕적): {matches_greedy}")
print(f"a*? (비탐욕적): {matches_non_greedy}")

# 2. + (1회 이상) vs +? (1회 이상, 최소)
test_text = "<tag>내용1</tag><tag>내용2</tag>"
greedy_tags = re.findall(r'<tag>.*</tag>', test_text)
non_greedy_tags = re.findall(r'<tag>.*?</tag>', test_text)
print(f"\n<tag>.*</tag>: {greedy_tags}")
print(f"<tag>.*?</tag>: {non_greedy_tags}")

# 3. 복잡한 패턴에서의 활용
complex_text = "start{middle1}end start{middle2}{middle3}end"
print(f"\n원본 텍스트: {complex_text}")

# 중괄호 안의 내용 추출
greedy_content = re.findall(r'start\{.*\}end', complex_text)
non_greedy_content = re.findall(r'start\{.*?\}end', complex_text)
print(f"탐욕적 패턴 결과: {greedy_content}")
print(f"비탐욕적 패턴 결과: {non_greedy_content}")
```

## 다중 라인 모드와 DOTALL 모드

정규 표현식에서 줄바꿈 문자(`\n`)를 처리하는 두 가지 중요한 플래그가 있습니다: `re.MULTILINE`과 `re.DOTALL`

```python
def demonstrate_multiline_patterns():
    """다중 라인 텍스트 처리 데모"""
    
    # 멀티라인 텍스트 예시
    log_data = """[INFO] 2024-01-03 10:00:00 - Application started
[ERROR] 2024-01-03 10:05:23 - Database connection failed
    Error details: Connection timeout
    Retrying in 5 seconds...
[WARNING] 2024-01-03 10:05:30 - High memory usage detected
[INFO] 2024-01-03 10:10:00 - Operation completed successfully"""
    
    print("=== 다중 라인 텍스트 처리 ===")
    print("원본 로그:")
    print(log_data)
    
    # 1. 기본 모드 (^와 $는 문자열의 시작과 끝만 매칭)
    print("\n1. 기본 모드 (^와 $의 동작):")
    basic_matches = re.findall(r'^\[.*\]', log_data)
    print(f"  '^\\[.*\\]' 패턴 매칭: {basic_matches}")
    print(f"  결과 수: {len(basic_matches)}개")
    
    # 2. MULTILINE 모드 (^와 $가 각 줄의 시작과 끝에 매칭)
    print("\n2. MULTILINE 모드:")
    multiline_matches = re.findall(r'^\[.*\]', log_data, re.MULTILINE)
    print(f"  '^\\[.*\\]' 패턴 매칭: {multiline_matches}")
    print(f"  결과 수: {len(multiline_matches)}개")
    
    # 3. DOTALL 모드 (.이 개행문자도 포함)
    print("\n3. DOTALL 모드 활용:")
    # ERROR 로그 전체 추출 (여러 줄에 걸친)
    error_pattern = r'\[ERROR\].*?Retrying in \d+ seconds\.\.\.'
    error_match = re.search(error_pattern, log_data, re.DOTALL)
    
    if error_match:
        print("  ERROR 로그 전체:")
        print(f"  {error_match.group()}")
    
    # 4. MULTILINE과 DOTALL 조합
    print("\n4. MULTILINE + DOTALL 조합:")
    combined_pattern = r'^\[(ERROR|WARNING)\].*?(?=^\[|\Z)'
    warnings_and_errors = re.findall(combined_pattern, log_data, 
                                     re.MULTILINE | re.DOTALL)
    
    print("  모든 경고와 에러:")
    for i, item in enumerate(warnings_and_errors, 1):
        print(f"  {i}. {item.strip()[:50]}...")
    
    # 5. 실전 예제: 로그 레벨별 분석
    print("\n5. 로그 레벨별 통계:")
    
    def analyze_log_levels(log_text):
        """로그 레벨별 통계 생성"""
        pattern = r'^\[(\w+)\]'
        levels = re.findall(pattern, log_text, re.MULTILINE)
        
        stats = {}
        for level in levels:
            stats[level] = stats.get(level, 0) + 1
        
        return stats
    
    level_stats = analyze_log_levels(log_data)
    for level, count in level_stats.items():
        print(f"  {level}: {count}건")

demonstrate_multiline_patterns()
```

## 유니코드와 정규 표현식

파이썬 3에서 문자열은 기본적으로 유니코드이지만, 정규 표현식의 `\w`, `\d`, `\s` 같은 문자 클래스는 기본적으로 ASCII 문자만 인식합니다.

```python
def unicode_regex_demo():
    """유니코드 정규 표현식 데모"""
    
    # 다국어 텍스트 예시
    multilingual_text = """
    안녕하세요! Hello! こんにちは！ Bonjour!
    숫자: 123, 四五六, ७८९
    이모지: 👍 🎉 🌍
    특수문자: @#$%^&*
    """
    
    print("=== 유니코드 정규 표현식 ===")
    print("다국어 텍스트:")
    print(multilingual_text)
    
    # 1. 기본 모드 (ASCII만 인식)
    print("\n1. 기본 모드 (ASCII):")
    ascii_words = re.findall(r'\w+', multilingual_text)
    print(f"  단어 찾기 (\\w+): {ascii_words}")
    print(f"  찾은 단어 수: {len(ascii_words)}")
    
    # 2. 유니코드 모드
    print("\n2. 유니코드 모드:")
    unicode_words = re.findall(r'\w+', multilingual_text, re.UNICODE)
    print(f"  단어 찾기 (\\w+ with UNICODE): {unicode_words}")
    print(f"  찾은 단어 수: {len(unicode_words)}")
    
    # 3. 다양한 언어의 문자 매칭
    print("\n3. 언어별 문자 매칭:")
    
    # 한글만 추출
    korean_pattern = r'[가-힣]+'
    korean_words = re.findall(korean_pattern, multilingual_text)
    print(f"  한글 단어: {korean_words}")
    
    # 히라가나만 추출
    hiragana_pattern = r'[ぁ-ゔ]+'
    hiragana_words = re.findall(hiragana_pattern, multilingual_text)
    print(f"  히라가나: {hiragana_words}")
    
    # 한자 추출
    kanji_pattern = r'[一-龥]+'
    kanji_words = re.findall(kanji_pattern, multilingual_text)
    print(f"  한자: {kanji_words}")
    
    # 4. 유니코드 속성 사용
    print("\n4. 유니코드 속성 사용:")
    
    # 모든 문자 찾기 (유니코드 카테고리 포함)
    def analyze_unicode_chars(text):
        """텍스트의 유니코드 문자 분석"""
        results = {
            'letters': [],
            'numbers': [],
            'punctuation': [],
            'symbols': [],
            'emojis': []
        }
        
        for char in text:
            category = unicodedata.category(char)
            
            if category.startswith('L'):  # Letter
                results['letters'].append(char)
            elif category.startswith('N'):  # Number
                results['numbers'].append(char)
            elif category.startswith('P'):  # Punctuation
                results['punctuation'].append(char)
            elif category.startswith('S'):  # Symbol
                results['symbols'].append(char)
            elif unicodedata.name(char, '').startswith('EMOJI'):
                results['emojis'].append(char)
        
        return results
    
    analysis = analyze_unicode_chars(multilingual_text)
    print(f"  문자 수: {len(analysis['letters'])}")
    print(f"  숫자 수: {len(analysis['numbers'])}")
    print(f"  이모지 수: {len(analysis['emojis'])}")
    print(f"  이모지 목록: {analysis['emojis']}")

unicode_regex_demo()
```

## 유니코드 정규화 심화

유니코드에서 동일한 문자라도 다른 코드 포인트로 표현될 수 있습니다. 이를 통일하는 과정을 정규화(normalization)라고 합니다.

```python
def unicode_normalization_demo():
    """유니코드 정규화 상세 데모"""
    
    print("=== 유니코드 정규화 ===")
    
    # 1. 다양한 'é' 표현
    print("\n1. 다양한 'é' 표현:")
    
    # 표현 1: 단일 문자 (LATIN SMALL LETTER E WITH ACUTE)
    e_single = 'é'  # U+00E9
    print(f"  단일 문자 'é':")
    print(f"    유니코드: U+{ord(e_single):04X}")
    print(f"    길이: {len(e_single)}")
    print(f"    이름: {unicodedata.name(e_single)}")
    
    # 표현 2: 조합 문자 (e + combining acute accent)
    e_combined = 'e\u0301'  # U+0065 + U+0301
    print(f"\n  조합 문자 'e\\u0301':")
    print(f"    유니코드: U+{ord(e_combined[0]):04X} + U+{ord(e_combined[1]):04X}")
    print(f"    길이: {len(e_combined)}")
    print(f"    e 이름: {unicodedata.name(e_combined[0])}")
    print(f"    액센트 이름: {unicodedata.name(e_combined[1])}")
    
    print(f"\n  두 표현 비교:")
    print(f"    동일한가? {e_single == e_combined}")
    print(f"    시각적으로 동일한가? 'é' == 'é'? {'예' if e_single == e_combined else '아니오'}")
    
    # 2. 정규화 비교
    print("\n2. 정규화 형식 비교:")
    
    # NFC (Normalization Form C): 결합된 형태
    nfc_single = unicodedata.normalize('NFC', e_single)
    nfc_combined = unicodedata.normalize('NFC', e_combined)
    print(f"  NFC 정규화:")
    print(f"    'é' -> {repr(nfc_single)} (길이: {len(nfc_single)})")
    print(f"    'e\\u0301' -> {repr(nfc_combined)} (길이: {len(nfc_combined)})")
    print(f"    동일한가? {nfc_single == nfc_combined}")
    
    # NFD (Normalization Form D): 분해된 형태
    nfd_single = unicodedata.normalize('NFD', e_single)
    nfd_combined = unicodedata.normalize('NFD', e_combined)
    print(f"\n  NFD 정규화:")
    print(f"    'é' -> {repr(nfd_single)} (길이: {len(nfd_single)})")
    print(f"    'e\\u0301' -> {repr(nfd_combined)} (길이: {len(nfd_combined)})")
    print(f"    동일한가? {nfd_single == nfd_combined}")
    
    # 3. 한글 정규화 예제
    print("\n3. 한글 정규화 예제:")
    
    # 한글 자소 분리/결합
    hangul_syllable = '한'  # U+D55C
    hangul_decomposed = unicodedata.normalize('NFD', hangul_syllable)
    
    print(f"  음절 '한':")
    print(f"    원본: {hangul_syllable} (U+{ord(hangul_syllable):04X})")
    print(f"    NFD: {repr(hangul_decomposed)}")
    print(f"    길이: {len(hangul_decomposed)}")
    
    # 자소별 분석
    for i, char in enumerate(hangul_decomposed):
        print(f"    자소 {i+1}: {char} (U+{ord(char):04X}, {unicodedata.name(char)})")
    
    # 4. 실전 활용: 텍스트 비교
    print("\n4. 텍스트 비교 실전 예제:")
    
    def safe_text_comparison(text1, text2):
        """정규화 후 텍스트 비교"""
        normalized1 = unicodedata.normalize('NFC', text1)
        normalized2 = unicodedata.normalize('NFC', text2)
        
        return normalized1 == normalized2
    
    test_cases = [
        ("café", "cafe\u0301"),  # café
        ("naïve", "nai\u0308ve"),  # naïve with diaeresis
        ("👨‍👩‍👧‍👦", "👨\u200d👩\u200d👧\u200d👦"),  # 가족 이모지
    ]
    
    for text1, text2 in test_cases:
        raw_comparison = text1 == text2
        safe_comparison = safe_text_comparison(text1, text2)
        print(f"  '{text1}' vs '{text2}':")
        print(f"    직접 비교: {'동일' if raw_comparison else '다름'}")
        print(f"    정규화 비교: {'동일' if safe_comparison else '다름'}")

unicode_normalization_demo()
```

## 텍스트 클리닝 고급 기법

```python
def advanced_text_cleaning():
    """고급 텍스트 클리닝 기법"""
    
    # 다양한 문제를 가진 텍스트 샘플
    dirty_text = """
    Hello,   World!  \t\n
    This is a SAMPLE text with   multiple   spaces.  
    
    Special Characters: @#$%^&*()_+
    Mixed ＣＡＳＥ and  full-width  characters.
    Extra    spaces    and		tabs.
    
    Line breaks\nand\r\nmultiple\rnewlines.
    
    HTML tags: <div>content</div> & <p>paragraph</p>
    URLs: https://example.com/path?query=string
    Emails: user@example.com, admin@test.co.kr
    """
    
    print("=== 고급 텍스트 클리닝 ===")
    print("원본 텍스트:")
    print(dirty_text)
    
    # 1. 기본 클리닝 파이프라인
    def basic_clean_pipeline(text):
        """기본적인 클리닝 파이프라인"""
        cleaned = text
        
        # 공백 정규화 (여러 공백을 하나로)
        cleaned = re.sub(r'\s+', ' ', cleaned)
        
        # 앞뒤 공백 제거
        cleaned = cleaned.strip()
        
        return cleaned
    
    print("\n1. 기본 클리닝 결과:")
    basic_cleaned = basic_clean_pipeline(dirty_text)
    print(basic_cleaned[:200] + "...")
    
    # 2. 고급 클리닝: 특정 패턴 제거
    def advanced_clean_pipeline(text, remove_patterns=None):
        """고급 클리닝 파이프라인"""
        if remove_patterns is None:
            remove_patterns = [
                r'<[^>]+>',  # HTML 태그
                r'https?://\S+',  # URLs
                r'\S+@\S+\.\S+',  # 이메일
                r'[^\w\s.,!?-]',  # 특수문자 (일부 제외)
            ]
        
        cleaned = text
        
        # 각 패턴 제거
        for pattern in remove_patterns:
            cleaned = re.sub(pattern, '', cleaned)
        
        # 공백 정규화
        cleaned = re.sub(r'\s+', ' ', cleaned)
        cleaned = cleaned.strip()
        
        return cleaned
    
    print("\n2. 고급 클리닝 (HTML, URL, 이메일, 특수문자 제거):")
    advanced_cleaned = advanced_clean_pipeline(dirty_text)
    print(advanced_cleaned)
    
    # 3. translate 메서드를 활용한 고속 문자 제거
    def clean_with_translate(text, chars_to_remove):
        """translate 메서드를 사용한 고속 클리닝"""
        # 변환 테이블 생성 (삭제할 문자 매핑)
        trans_table = str.maketrans('', '', chars_to_remove)
        
        # 변환 적용
        cleaned = text.translate(trans_table)
        
        # 공백 정규화
        cleaned = re.sub(r'\s+', ' ', cleaned)
        cleaned = cleaned.strip()
        
        return cleaned
    
    print("\n3. translate를 사용한 특수문자 제거:")
    special_chars = '@#$%^&*()_+<>'
    translated_cleaned = clean_with_translate(dirty_text, special_chars)
    print(translated_cleaned[:200] + "...")
    
    # 4. 유니코드 정규화 포함 클리닝
    def unicode_aware_cleaning(text):
        """유니코드 인식 클리닝"""
        # NFC 정규화
        normalized = unicodedata.normalize('NFC', text)
        
        # 전각 문자를 반각으로 변환
        def full_to_half(char):
            """전각 문자를 반각 문자로 변환"""
            code = ord(char)
            # 전각 알파벳, 숫자, 공백을 반각으로
            if 0xFF01 <= code <= 0xFF5E:  # 전각 문자 범위
                return chr(code - 0xFEE0)
            elif code == 0x3000:  # 전각 공백
                return ' '
            return char
        
        # 문자 변환 적용
        half_width = ''.join(full_to_half(c) for c in normalized)
        
        # 대소문자 정규화 (선택적)
        normalized_case = half_width.lower()  # 또는 .upper()
        
        return normalized_case
    
    print("\n4. 유니코드 인식 클리닝 (전각→반각, NFC 정규화):")
    mixed_case_text = "Ｍｉｘｅｄ ＣＡＳＥ and ｆｕｌｌ－ｗｉｄｔｈ  ＡＢＣ１２３"
    unicode_cleaned = unicode_aware_cleaning(mixed_case_text)
    print(f"원본: {mixed_case_text}")
    print(f"클리닝: {unicode_cleaned}")
    
    # 5. 실전 예제: 데이터 클리닝 클래스
    class TextCleaner:
        """다양한 클리닝 옵션을 제공하는 클래스"""
        
        def __init__(self):
            self.pipelines = {
                'basic': self._basic_pipeline,
                'html': self._html_pipeline,
                'url_email': self._url_email_pipeline,
                'unicode': self._unicode_pipeline,
            }
        
        def clean(self, text, pipeline='basic', **options):
            """지정된 파이프라인으로 텍스트 클리닝"""
            if pipeline not in self.pipelines:
                raise ValueError(f"지원하지 않는 파이프라인: {pipeline}")
            
            return self.pipelines[pipeline](text, **options)
        
        def _basic_pipeline(self, text, **kwargs):
            """기본 클리닝"""
            cleaned = re.sub(r'\s+', ' ', text)
            return cleaned.strip()
        
        def _html_pipeline(self, text, **kwargs):
            """HTML 태그 제거"""
            cleaned = re.sub(r'<[^>]+>', '', text)
            return self._basic_pipeline(cleaned)
        
        def _url_email_pipeline(self, text, **kwargs):
            """URL과 이메일 제거"""
            cleaned = re.sub(r'https?://\S+', '', text)
            cleaned = re.sub(r'\S+@\S+\.\S+', '', cleaned)
            return self._basic_pipeline(cleaned)
        
        def _unicode_pipeline(self, text, **kwargs):
            """유니코드 정규화"""
            normalized = unicodedata.normalize('NFC', text)
            return self._basic_pipeline(normalized)
    
    print("\n5. TextCleaner 클래스 사용 예제:")
    cleaner = TextCleaner()
    
    test_text = "  Hello   <b>World</b>!  Visit https://example.com  "
    
    print(f"원본: '{test_text}'")
    print(f"기본 클리닝: '{cleaner.clean(test_text, 'basic')}'")
    print(f"HTML 제거: '{cleaner.clean(test_text, 'html')}'")
    print(f"URL 제거: '{cleaner.clean(test_text, 'url_email')}'")

advanced_text_cleaning()
```

## 문자열 포맷팅과 정렬 심화

{% raw %}
```python
def advanced_string_formatting():
    """고급 문자열 포맷팅 기법"""
    
    print("=== 고급 문자열 포맷팅 ===")
    
    # 1. 다양한 정렬 옵션
    print("\n1. 문자열 정렬:")
    
    sample_text = "Python"
    
    # 기본 정렬 메서드
    print(f"원본: '{sample_text}'")
    print(f"좌측 정렬 (10자): '{sample_text.ljust(10, '-')}'")
    print(f"우측 정렬 (10자): '{sample_text.rjust(10, '*')}'")
    print(f"가운데 정렬 (10자): '{sample_text.center(10, '=')}'")
    
    # 2. format() 메서드 심화
    print("\n2. format() 메서드 심화:")
    
    # 위치 기반 인자
    print("위치 기반: {}".format("첫번째"))
    print("위치 기반: {} {}".format("첫번째", "두번째"))
    
    # 이름 기반 인자
    print("이름 기반: {name}은 {age}살".format(name="철수", age=30))
    
    # 인덱스와 이름 혼합
    print("혼합: {0}의 {item}".format("철수", item="사과"))
    
    # 3. f-string 고급 활용
    print("\n3. f-string 고급 활용:")
    
    name = "김파이썬"
    score = 95.5678
    count = 1234567
    
    # 다양한 포맷팅
    print(f"이름: {name:<15}")  # 좌측 정렬
    print(f"이름: {name:>15}")  # 우측 정렬
    print(f"이름: {name:^15}")  # 가운데 정렬
    
    print(f"\n점수: {score:.1f}")      # 소수점 1자리
    print(f"점수: {score:.2f}")      # 소수점 2자리
    print(f"점수: {score:8.2f}")     # 전체 8자리, 소수점 2자리
    
    print(f"\n개수: {count:,}")       # 천단위 구분
    print(f"개수: {count:12,}")     # 12자리 + 천단위 구분
    
    # 4. 날짜와 시간 포맷팅
    print("\n4. 날짜와 시간 포맷팅:")
    
    from datetime import datetime
    now = datetime.now()
    
    print(f"현재 시간: {now:%Y-%m-%d %H:%M:%S}")
    print(f"현재 시간: {now:%Y년 %m월 %d일 %A %p %I시 %M분}")
    print(f"타임스탬프: {now:%s}")
    
    # 5. 사용자 정의 포맷터
    print("\n5. 사용자 정의 포맷터:")
    
    class Product:
        def __init__(self, name, price, stock):
            self.name = name
            self.price = price
            self.stock = stock
        
        def __format__(self, format_spec):
            """사용자 정의 포맷 규칙"""
            if format_spec == 'short':
                return f"{self.name}"
            elif format_spec == 'detail':
                return f"{self.name}: ${self.price:.2f} ({self.stock}개)"
            elif format_spec.startswith('width='):
                # width=10 과 같은 형식 처리
                width = int(format_spec.split('=')[1])
                return f"{self.name:{width}}"
            else:
                # 기본 포맷
                return f"{self.name} - ${self.price}"
    
    product = Product("노트북", 1299.99, 15)
    print(f"기본: {product}")
    print(f"짧게: {product:short}")
    print(f"상세: {product:detail}")
    print(f"너비 10: '{product:width=10}'")
    
    # 6. 표 형식 출력
    print("\n6. 표 형식 데이터 출력:")
    
    data = [
        {"name": "Apple MacBook Pro", "price": 2499.99, "stock": 10, "category": "노트북"},
        {"name": "iPhone 15", "price": 1199.99, "stock": 25, "category": "스마트폰"},
        {"name": "iPad Air", "price": 899.99, "stock": 30, "category": "태블릿"},
        {"name": "Apple Watch", "price": 499.99, "stock": 50, "category": "워치"},
        {"name": "AirPods Pro", "price": 299.99, "stock": 100, "category": "이어폰"},
    ]
    
    # 헤더 출력
    header_format = "{:<25} {:>12} {:>8} {:>10}"
    row_format = "{:<25} {:>12,.2f} {:>8} {:>10}"
    
    print(header_format.format("상품명", "가격($)", "재고", "카테고리"))
    print("-" * 65)
    
    for item in data:
        print(row_format.format(
            item["name"][:24],  # 24자로 제한
            item["price"],
            item["stock"],
            item["category"]
        ))
    
    # 통계 추가
    print("-" * 65)
    total_value = sum(item["price"] * item["stock"] for item in data)
    total_stock = sum(item["stock"] for item in data)
    print(f"{'총계':<25} {'':>12} {total_stock:>8} {total_value:>10,.2f}")
    
    # 7. 동적 포맷팅
    print("\n7. 동적 포맷팅:")
    
    def create_formatter(column_specs):
        """동적으로 포맷터 생성"""
        format_parts = []
        for name, width, align in column_specs:
            if align == 'left':
                format_parts.append(f"{{{name}:<{width}}}")
            elif align == 'right':
                format_parts.append(f"{{{name}:>{width}}}")
            elif align == 'center':
                format_parts.append(f"{{{name}:^{width}}}")
        
        return " ".join(format_parts)
    
    # 동적 포맷터 생성
    columns = [
        ("name", 20, "left"),
        ("price", 10, "right"),
        ("stock", 8, "right"),
    ]
    
    formatter = create_formatter(columns)
    print("동적 생성된 포맷터:", formatter)
    
    # 사용 예제
    sample_items = [
        {"name": "상품A", "price": 1000, "stock": 50},
        {"name": "상품B", "price": 2500, "stock": 30},
        {"name": "상품C", "price": 500, "stock": 100},
    ]
    
    print("\n동적 포맷터로 출력:")
    for item in sample_items:
        print(formatter.format(**item))

advanced_string_formatting()
```
{% endraw %}

## 결론

파이썬의 문자열과 텍스트 처리 기능은 단순한 조작을 넘어서 다양한 고급 기법을 제공합니다. 정규 표현식의 비탐욕적 매칭을 활용하면 HTML 파싱이나 복잡한 패턴 추출 시 더 정확한 결과를 얻을 수 있으며, `re.MULTILINE`과 `re.DOTALL` 플래그를 조합하면 다중 라인 텍스트를 효과적으로 처리할 수 있습니다.

유니코드 처리는 현대적인 텍스트 처리에서 필수적인 요소입니다. NFC/NFD 정규화를 통해 다양한 표현의 동일한 문자를 통일하고, `re.UNICODE` 플래그를 사용하면 다국어 텍스트에서도 정규 표현식을 올바르게 적용할 수 있습니다. 특히 한국어, 일본어, 중국어와 같은 비라틴 문자를 다룰 때는 이러한 기능들이 중요합니다.

텍스트 클리닝은 데이터 전처리의 핵심 단계로, `re.sub()`의 패턴 기반 치환, `translate()`의 고속 문자 삭제, 유니코드 정규화 등을 상황에 맞게 조합해야 합니다. 복잡한 클리닝 로직은 클래스나 파이프라인으로 구조화하면 재사용성과 가독성을 높일 수 있습니다.

문자열 포맷팅과 정렬은 데이터 표현의 핵심입니다. f-string의 등장으로 파이썬 3.6 이상에서는 더욱 간결하고 강력한 포맷팅이 가능해졌으며, 동적 포맷팅이나 사용자 정의 `__format__` 메서드를 통해 특정 요구사항에 맞춘 유연한 출력을 구현할 수 있습니다.

이러한 고급 기법들을 마스터하면 실제 프로젝트에서 발생하는 다양한 텍스트 처리 문제를 효율적으로 해결할 수 있으며, 데이터 분석, 웹 스크래핑, 자연어 처리, 리포트 생성 등 다양한 분야에서 활용할 수 있습니다. 각 상황에 맞는 최적의 기법을 선택하고 조합하는 것이 중요합니다.