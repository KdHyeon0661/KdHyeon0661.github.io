...
## 3. AH vs ESP, Transport vs Tunnel 비교
...
## 4. NAT 환경에서의 동작 — AH vs ESP(UDP 4500, NAT-T)
...
### 5.2 PFS(Perfect Forward Secrecy)
- IKE 협상에서 **(EC)DH** 사용(예: ECP256, ECP384). 키 유출 시 과거 세션 보호

### 5.3 Anti-Replay(재전송 방지)
...
## 9. 설계 패턴 — Policy vs Route 기반, 원격/사이트/클라우드

### 9.1 Policy-based
- 선택자 기준으로 자동 보호  
- **장점**: 세밀한 트래픽 단위 보호. **단점**: 스케일·운영 난이도↑

### 9.2 Route-based(VTI/VTUN)
...
## 10. 실전 예제 1 — strongSwan(IKEv2, Tunnel, AES-GCM)
...
## 11. 실전 예제 2 — Linux XFRM 수동 구성(교육용, 정책 기반)
...
## 12. 실전 예제 3 — Cisco IOS(IKEv2 + ESP-GCM, Route-based)
...
## 13. 실전 예제 4 — Windows IKEv2(원격접속 프로파일)
...
### 16.1 NO_PROPOSAL_CHOSEN
- 현상: IKE 협상 실패  
- 조치: 양단 **ike/esp** 암호군 완전 일치 확인(순서·느낌표 강제 `!`), DH 그룹 정합

### 16.2 AUTHENTICATION_FAILED
