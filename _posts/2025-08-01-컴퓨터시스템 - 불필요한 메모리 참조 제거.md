...
## 2. 대표 패턴 — “불필요”가 생기는 순간들

### 2.1 같은 메모리 값을 반복해 읽는다
```c
// before
sum  += node->val;
prod *= node->val;
...
## 3. 실전 기법 — C 코드 변환(전/후)과 설명

### 3.1 스칼라 치환(SRA): 자주 쓰는 필드를 레지스터에
```c
// before
for (int i=0;i<n;i++)
...
### 3.2 누적(accumulator)은 메모리가 아니라 레지스터에 유지
```c
...
### 3.3 주소 계산/인덱싱 단순화(컴파일러가 최적화 추정하기 쉽게)
```c
// before
...
### 3.4 Dead Store to whole object: 전체 덮어쓸 버퍼의 사전 memset 제거
```c
// before
...
### 3.5 별칭 제거: `restrict`/`const`/속성으로 “안전”을 알려주기
```c
// before
...
### 3.6 store→load forwarding 실패 회피(같은 폭·정렬 유지)
```c
...
### 3.7 함수 경계 넘는 스칼라 유지: 인라인/`static`/LTO
```c
// header
...
## 4. 고급: RFO/NT-store/쓰기 병합과 “불필요 참조”의 미세조정
...
## 6. 컴파일러가 이미 잘하는 것 vs 우리가 도와야 하는 것
...
## 8. 대용량 루프 — 메모리 참조 최소화 묶음 레시피

### 8.1 블로킹 + 레지스터 누적(행렬-벡터)
```c
...
### 8.3 경계 분리(peeling)로 정렬 맞추기(벡터화/전방 성공률 ↑)
```c
