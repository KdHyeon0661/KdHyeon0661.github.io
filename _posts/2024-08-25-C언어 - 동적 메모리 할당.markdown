...
category: C
---
# 💾 C 언어 7편 – 동적 메모리 할당 (`malloc`, `free`)

이번 편에서는 C 언어의 강력한 기능 중 하나인 **동적 메모리 할당(Dynamic Memory Allocation)** 에 대해 알아봅니다. 정적 배열이 아닌, **프로그램 실행 중 필요한 만큼 메모리를 할당하거나 해제할 수 있는 기능**입니다.
...
- `malloc()` 또는 `calloc()`으로 할당한 메모리를 **반환(해제)**
