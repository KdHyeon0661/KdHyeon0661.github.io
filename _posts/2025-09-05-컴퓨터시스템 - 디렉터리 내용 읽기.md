...
## 재귀 순회(디렉터리 트리 걷기) — 안전 DFS (openat 기반)
...
## 정렬·필터링 — `scandir`, `alphasort/versionsort`, 스트리밍 정렬
...
## 보안 — TOCTTOU, 심링크, 경로 길이/인코딩, FD 유출
...
## 고급: Linux `openat2(2)`로 경로 정책 고정 (보안 샌드박스)
...
## 부록 — 한 화면 도구 만들기: `ls-lite` (정렬+필터+크기)
