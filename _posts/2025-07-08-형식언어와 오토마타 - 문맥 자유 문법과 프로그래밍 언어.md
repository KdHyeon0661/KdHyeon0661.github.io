...
## 5. LL(1) 친화 변환: 좌재귀 제거·좌측 공통 인자 제거
...
### 5.2 좌측 공통 인자 제거(Left Factoring)
```
A → αβ | αγ
```
공통 접두사 `α` 추출:
...
## 7. 파이썬으로 FIRST/FOLLOW 및 LL(1) 테이블 만들기 (실행용)
...
## 8. 미니 언어 설계: 문법(LL 친화), 렉서, 파서, AST, 인터프리터
...
### 8.2 구현(파이썬) — 렉서·재귀하강 파서·AST·간단 인터프리터
