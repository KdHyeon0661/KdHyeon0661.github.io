...
## IMM32(전통 IME) 기본기 — 커스텀 에디트에 조합 표시
...
## TSF(Text Services Framework) — 현대적(권장) 접근
...
## 재변환(Reconversion) & 재변환 가능한지 질의

### IMM32 — `WM_IME_REQUEST / IMR_RECONVERTSTRING`
...
## 그라페믹 클러스터(문자 경계) — 이동/삭제/선택의 기본 단위
...
## 유니코드 정규화(Normalization) — 비교/검색/저장 규칙
...
## 실전: DirectWrite 기반 커스텀 에디트 미니 샘플(요약)
