...
### 4.3 연산자 끼워넣기(BOJ 14888) — 브랜치&바운드 예시
...
## 5. N-Queen(BOJ 9663) — 대각선 제약의 O(1) 검사
...
## 7. 부분합/조합형 최적화 — 바운드로 큰 폭 가지치기

### 7.1 부분수열의 합: 조기 불가능성 컷
...
### 7.2 배낭 문제(0-1 Knapsack) — 백트래킹 + 상한 추정(그리디 분수)

- 분수 배낭으로 계산한 **낙관적 상한**(upper bound)이 전역 최선 이하이면 서브트리 컷.

---

## 8. 그래프 문제 백트래킹: 해밀턴 경로/색칠 문제

### 8.1 그래프 색칠(Chromatic Number) — 인접 제약
...
## 15. 종합 예제: “합이 정확히 S인 부분집합 중 원소 수가 최소” — 다중 제약, 바운드, 휴리스틱
