...
category: Elixir
---
# 11. 문자열, 바이너리, 차리스트 — UTF-8과 Unicode, Erlang 인터롭까지 완전 정리
...
## 11.2 ‘문자열’이라는 이름 — 바이너리·코드포인트·그래페임
...
### 11.2.5 iodata/iodata()로 성능 좋게 결합
...
### 11.3.4 charlist 기반 재귀 파서 예시(교육용)
...
### 11.4.1 CSV → 맵(헤더 보존) (대용량 안전, iodata 출력)
...
### 11.4.2 UTF-8 안전 슬라이스(바이트 인덱스 금지)
...
### 11.4.4 문자열 ↔ charlist Erlang 인터롭
...
### 11.4.5 로그 라인 파싱 + CSV 출력 (문자열 + iodata 조합)
