...
category: 정보보안기사
---
# SECTION 14 시스템 점검 도구 — 01. 취약점 점검(Nessus/Nikto) · 02. 무결성 점검(Tripwire) · 03. 루트킷 점검(chkrootkit)

## 개요 — “식별(Identify) → 보호(Protect) → 탐지(Detect) → 대응(Respond) → 복구(Recover)”
...
#### (D) 결과 활용 — XML/JSON 내보내기 & 요약 추출
Nessus는 `.nessus(XML)` 또는 JSON 내보내기를 지원한다(버전에 따라 API 키 필요).

```bash
# (개념) Nessus 결과를 XML(.nessus)로 저장했다고 가정
...
# 3. 특정 경로 루트(가상호스트)·SSL·SNI 환경
nikto -h app.example.com -root / -ssl

# 4. 출력 포맷/저장(CSV/JSON/HTML은 배포판에 따라 옵션 상이)
nikto -h app.example.com -o nikto_app.csv -Format csv

# 5. 튜닝(탐지 카테고리 선택) — 숫자는 카테고리 ID, 조합 가능
...
#### (E) Nmap과의 파이프라인 — “웹 포트 감지 → Nikto 연동”
```bash
# 1. Nmap으로 웹 포트가 열린 호스트 추출
nmap -p 80,443,8080,8443 10.10.30.0/24 --open -oG - | \
awk '/Ports:/{print $2}' > /tmp/webhosts.txt

# 2. Nikto 병렬 스캔(단, 속도/부하 주의)
