...
## 2. 키 생성 (CLI / CDK / CloudFormation)

### 2.1 CLI로 비대칭 서명 키 생성
...
### 2.2 CDK(TypeScript)로 생성 (권장 설정 포함)
...
## 4. 서명(Sign) — CLI / Python / Node / Java / Go
...
### 4.3 Python (boto3, DIGEST) — 해시를 직접 계산
...
## 5. 검증(Verify) — KMS / 로컬(OpenSSL, cryptography, WebCrypto)
...
### 5.6 브라우저 WebCrypto(오프라인 검증, ECDSA 예)
...
## 13. Quick Assets — 스니펫 모음

### 13.1 공개키 다운로드 & 검증 원라이너 (리눅스)
