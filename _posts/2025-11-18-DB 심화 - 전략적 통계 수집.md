...
## 2. 데이터 샘플링(estimate_percent) — 정확도 vs 비용의 균형
...
## 3. 히스토그램과 확장 통계 — 샘플링의 한계를 넘기는 핵심
...
## 4. 파티션 테이블 통계 — 증분 통계로 빠르고 안전하게

### 4.1 Granularity & Incremental 개념
...
### 4.2 표준 운영 플로우(Partition Exchange + Incremental)
...
## 5. 인덱스 통계 — cascade와 CF의 의미

### 5.1 왜 인덱스 통계가 플랜을 바꾸는가
...
## 6. 커서 Invalidation — 플랜 급변/하드파스를 막는 법
...
### 6.2 통계 수집 시 완충: NO_INVALIDATE + Pending
...
## 7. 자동 통계 수집(Auto Optimizer Stats Collection)
...
### 7.4 고빈도 자동 통계(High-frequency)와 실시간 통계
...
### 7.5 Direct-Path 로드의 Online Stats
...
## 8. Statistics Preferences — 정책을 코드로 고정하는 방법
...
## 10. 문제 재현 → 진단 → 개선 (핵심 실습 모음)

### 10.1 샘플링/히스토그램에 따른 플랜 차이
...
### 10.2 증분 통계와 Partition Exchange
...
### 10.3 인덱스 통계의 영향(CF/LEAF/BLEVEL)
