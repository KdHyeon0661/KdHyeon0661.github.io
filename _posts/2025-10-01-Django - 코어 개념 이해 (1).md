...
> 하나의 CBV 안에서는 `get/post/...` 등 **모든 핸들러를 async 또는 sync 중 하나로 통일**해야 한다. 혼합하면 `ImproperlyConfigured` 예외가 발생한다.

#### ORM과 async

- Django 5.2는 많은 ORM 메서드에 대해 `a` 접두어 버전(`aget`, `acreate`, `afirst`, `aall` 등)을 제공한다.
...
- **CSRF 보호**:
  - HTML 폼 POST → {% raw %}`{% csrf_token %}`{% endraw %} + `CsrfViewMiddleware` 유지.
