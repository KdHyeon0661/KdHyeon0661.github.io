...
category: Django
---
# — URL 라우팅, View(함수형/CBV/믹스인/제네릭), Template(DTL/Jinja2 비교) — Django 5.x 기준

## — `path()`, `re_path()`, `include()`, 네임드 URL, 리버스
...
- **템플릿 태그**: {% raw %}`{% url 'namespace:name' arg1 arg2 %}`{% endraw %} 또는 {% raw %}`{% url 'namespace:name' key=value %}`{% endraw %}
...
- **URL**
  ...
  - [ ] `include()` 모듈화
  - [ ] 컨버터 우선, `re_path` 최소화
  - [ ] 모든 링크/리디렉트는 **reverse**/{% raw %}`{% url %}`{% endraw %} 사용
- **View**
  - [ ] FBV/CBV 일관성(기능군 기준)
