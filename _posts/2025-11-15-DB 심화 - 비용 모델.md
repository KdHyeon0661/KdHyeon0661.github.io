...
- Cost 모델을 이해하는 이유는 결국 **“예상 vs 실제가 어디서 갈라지는지”** 찾기 위해서다.

---

## 1) Cost 모델 큰 그림

Oracle Optimizer는 통계에 기반해 다음을 수행한다.
...
AUX_STATS$/V$SYSSTAT 기반으로 다음을 사용한다.
...
> WORKLOAD는 `SREADTIM/MREADTIM/MBRC`를 직접 측정해 더 현실적이다.
...
### Index Range Scan + Table Rowid access
...
CF는 인덱스 순서와 테이블 물리 순서가 얼마나 맞는지의 지표다.

- CF ≈ **테이블 블록 수**면 “잘 정렬”
- CF ≈ **테이블 행 수**면 “매우 랜덤”

CF가 높으면 같은 행 수를 가져와도
**읽어야 할 테이블 블록 추정치가 증가** → Index 경로 Cost가 튀어 오른다.
...
그리고 이 CPU Time을 SBR 단위 Cost로 정규화한다.
...
- `spill_temp`는 Workarea 크기, 입력 크기(카디널리티×rowlen)에 따라 추정된다.
...
### Workarea가 부족할 때(Spill)

- TEMP로 run 파일 생성
- multi-pass merge 발생
- direct path read/write temp가 모델에 반영되며 Cost가 폭증한다.
...
### Dynamic Statistics(구 Dynamic Sampling)

통계가 없거나 오판 위험이 크면
Optimizer가 하드파스 중 샘플링으로 카디널리티를 보정한다.
...
### Adaptive Query Optimization / Statistics Feedback

실행 중 E-Rows≠A-Rows가 크면
다음 실행에서 **통계 피드백/재최적화**로 Cost를 수정한다.
...
- 같은 SQL도 시스템 통계에 따라
  **FFS/Index**, **HJ/NL** 선호가 바뀌는 것을 볼 수 있다.
...
- **SQL Plan Baseline(SPM)**: 허용 플랜 집합을 관리해 Cost 변동에도 플랜을 고정
