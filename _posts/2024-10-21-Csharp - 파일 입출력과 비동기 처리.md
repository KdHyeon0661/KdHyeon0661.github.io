...
### 인코딩 지정 (기본: UTF-8 with BOM 아님/.NET Core+는 UTF-8)
...
## StreamReader / StreamWriter — 스트림 기반 텍스트
...
### 임의 접근(Seek)과 길이(Length/SetLength)
...
## BinaryReader / BinaryWriter — 형태 있는 바이너리
...
## MemoryStream / Span / Pipelines (간단 포인터)

### MemoryStream — 메모리 상 임시 버퍼
...
### `Span<T>/Memory<T>` — 고성능 처리(파일 I/O에는 `Read(Span<byte>)` 오버로드 활용)
...
## 파일 잠금/접근/공유 — FileAccess/FileShare
...
## 메모리 매핑 — MemoryMappedFile (초대용량/랜덤 접근)
