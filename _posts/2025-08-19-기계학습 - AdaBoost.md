...
## 2. 알고리즘(AdaBoost.M1; 이진, \(y\in\{-1,+1\}\))
...
### 4.1 Real AdaBoost (confidence-rated)
...
### 4.2 다중클래스 AdaBoost — **SAMME / SAMME.R**
클래스 수 \(K\ge3\).
...
### 7.5 **from-scratch** (결정 stump + AdaBoost.M1; 교육용 미니멀)
```python
import numpy as np
