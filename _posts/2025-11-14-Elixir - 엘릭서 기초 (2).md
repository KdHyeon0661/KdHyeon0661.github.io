...
### 4.5.3 변수 키(동적 키)와 핀(^) — 2장에서 이어지는 규칙
...
### 4.5.4 깊은 갱신: `put_in/3`, `update_in/3`, `get_and_update_in/3`
...
### 4.5.5 `Map.merge/2` vs 커스텀 `deep_merge/2`
...
### 4.5.6 Keyword list vs Map — 언제 무엇을 쓸까
...
### 4.8.3 alias / import / require / use
...
### 4.9.2 `if/case/cond/with` — 재바인딩이 전파되는 블록
...
### 4.9.3 `fn`(익명 함수)와 컴프리헨션 `for` — 새 스코프 생성
...
### B) 예약 요청 처리 — 날짜 파싱 + 맵 검증 + `with` 스코프
