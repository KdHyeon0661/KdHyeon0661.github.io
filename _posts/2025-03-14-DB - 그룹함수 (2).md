...
## 1. ROLLUP — 계층형 소계와 총계를 한 번에

### 1.1 개념
`ROLLUP(a, b, c)`는 **(a,b,c) → (a,b) → (a) → ()** 순서로 **소계 → 상위 소계 → 총계**를 자동 생성한다.

### 1.2 기본 예제 (PostgreSQL/Oracle/SQL Server)
```sql
SELECT
  region,
  country,
...
## 3. GROUPING SETS — 필요한 요약 조합만 골라서
...
## 4. ROLLUP/CUBE/GS + HAVING/WHERE, NULL, “미정(UNKNOWN)” 전략
...
## 6. 백분율/기여도 계산 — **총계 대비 비율**을 ROLLUP과 결합
...
### 7.2 Oracle / SQL Server의 PIVOT 예제
...
### 7.3 PostgreSQL / MySQL의 PIVOT 대체(조건부 집계)
```sql
SELECT
  region,
...
## 8. UNPIVOT — 열→행 변환(정규화 복원)

### 8.1 개념
열에 흩어진 메트릭을 **행으로 녹여** “긴(long)” 형태로 만든다.

### 8.2 Oracle / SQL Server의 UNPIVOT
```sql
...
### 8.3 PostgreSQL / MySQL 대체: UNION ALL
```sql
...
## 9. PIVOT + 소계/총계 — **피벗 결과에 총계 열/행** 붙이기
...
## 10. Ordered-Set Aggregates & 백분위수 with GROUPING SETS
...
## 11. 윈도 함수 vs ROLLUP/CUBE — “축소 여부”와 “라벨/총계”
