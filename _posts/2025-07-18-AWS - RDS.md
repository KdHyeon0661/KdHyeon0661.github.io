...
1) RDS 개요와 필요한 이유  
2) 엔진별 특징(영구 스토리지·가용성 옵션 포함)  
3) 인스턴스 생성 플로우와 핵심 설정(서브넷 그룹, 파라미터 그룹, 옵션 그룹)  
4) 고가용성: Multi-AZ, Cross-Region, Aurora Global Database  
5) 백업: 자동 백업, PITR, 스냅샷 복제/공유  
6) 읽기 확장: Read Replica, Aurora Reader, RDS Proxy  
7) 보안: VPC, SG, 인증(IAM/SSL), KMS 암호화, Secrets Manager  
8) 성능: 인덱스/쿼리, 스토리지(IOPS/스루풋), 캐시/풀링, Performance Insights  
9) 비용 모델과 사이징, 예약/서버리스  
10) IaC 예제(CloudFormation/Terraform), 운영 스크립트, 모니터링/알람  
11) 실전 시나리오(WordPress, Django/Node 연결), 마이그레이션 경로  
12) 트러블슈팅·체크리스트
...
## 11. IaC: CloudFormation / Terraform

### 11.1 CloudFormation (MySQL, Multi-AZ, GP3)
```yaml
...
### 11.2 Terraform (PostgreSQL, Read Replica)
```hcl
