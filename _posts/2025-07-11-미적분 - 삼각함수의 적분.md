...
## 2) \(\sin^m x \cos^n x\) 형태 — **홀수/짝수 전략**

### 2.1 한쪽이 **홀수**일 때(예: \(m\) 홀수)
\(\sin\)이 홀수라 가정(\(m=2k+1\)):
$$
...
## 3) \(\tan^m x \sec^n x\), \(\cot^m x \csc^n x\)
...
### 5.1 \(\displaystyle \int \sin^n x\,dx\) ( \(n\ge 2\) )
부분적분(또는 항등식 조합)으로
$$
\int \sin^n x\,dx
...
### 5.2 \(\displaystyle \int \sec^n x\,dx\)
표준 감소식:
$$
\int \sec^n x\,dx
...
### 11.1 \(\displaystyle \int \frac{dx}{a+b\cos x}\)
...
### 11.2 \(\displaystyle \int \sqrt{a+b\cos x}\,dx\)
