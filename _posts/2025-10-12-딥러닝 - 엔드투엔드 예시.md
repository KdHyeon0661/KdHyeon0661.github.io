...
### 1.2 `common/amp_ckpt.py` — AMP, 체크포인팅, 조기종료, EMA
```python
# common/amp_ckpt.py
...
### 1.3 `common/meter.py` — 지표/혼동행렬/ECE
```python
# common/meter.py
import torch, math
...
### 1.4 `common/log.py` — JSONL/CSV 로거
```python
# common/log.py
...
# 프로젝트 A. 이미지 분류 (SmallCNN) — 데이터→학습→평가→서빙
...
# 프로젝트 C. 시계열 예측 + 이상탐지(LSTM) — 배치+실시간
