...
## 인덱스·액세스 경로 체크리스트 — “랜덤 방문과 전체 스캔을 최소화했는가?”
...
## 통계·옵티마이저 팩터 체크리스트 — “좋은 길을 선택할 근거를 주었는가?”
...
## 힌트 체크리스트 — “원인 교정 후에도 불확실하면 안전 레일”
...
## TEMP/정렬/해시 체크리스트 — “스필(direct path read/write temp)을 줄였는가?”
...
## Direct Path I/O 체크리스트 — “대량 작업은 Bypass로, OLTP는 캐시 재사용”
...
## 페이징/리스트 API 체크리스트 — “OFFSET 말고 Keyset”
...
## 파티셔닝 체크리스트 — “읽지 않을 파티션은 아예 건드리지 않는다”
...
## 애플리케이션·네트워크 체크리스트 — “DB 왕복/Fetch를 최소화했는가?”
...
## RAC/캐시 퓨전 체크리스트 — “전송 없는 읽기 비율을 높였는가?”
