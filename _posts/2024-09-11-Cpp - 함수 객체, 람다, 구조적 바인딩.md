...
### init-capture (C++14~) — 캡처 시 계산·이동 캡처
...
## 호출자 추상화: `std::function`, `std::invoke`, `std::bind_front`, 그리고 `std::bind`

### `std::function` — 다형 호출 래퍼(타입 지우개)
...
### `std::bind_front` (C++20) — 안전·간단한 인자 고정
