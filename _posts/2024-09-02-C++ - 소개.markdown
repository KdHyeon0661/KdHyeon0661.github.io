...
> C++에서는 **함수 포인터**뿐 아니라 **람다(lambda)** 나 **std::function** 도 사용 가능합니다.  
...
> C에서는 구조체 안에 멤버함수를 넣을 수 없다. 단, 함수 포인터를 사용하면 유사하게 사용이 가능합니다.
> C++에서는 `struct`도 클래스처럼 사용되며, 기본 접근 지정자만 `public`인 점을 제외하면 `class`와 거의 동일합니다.

### C에서 함수 포인터를 사용해 멤버함수를 사용
