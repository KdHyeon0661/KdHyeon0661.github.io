...
## 로컬에서 브랜치 생성과 전환 (switch vs checkout)
...
# GitHub에서 기본 브랜치(Default branch)를 main으로 변경(웹 설정)
# 필요 시 보호 규칙(branch protection)도 main으로 재설정
# 팀원들에게 pull/rebase 대상 변경 안내

```

---

## “detached HEAD”와 특정 커밋/태그에서 브랜치 만들기
...
## 브랜치 상태/차이/앞뒤(ahead/behind) 파악 도구
...
## 동시 작업을 위한 **worktree** (1 리포에서 다중 워킹트리)
...
## GitHub 보호 브랜치(웹 설정) — merge 없이도 알아야 하는 운영 팁
...
## 완전 재현 가능한 미니 시나리오 (merge 없음)

### “웹에서 브랜치 만들어 로컬로 가져다 쓰기”
...
### “브랜치 이름 바꾸기(main 전환 포함, merge 불개입)”
