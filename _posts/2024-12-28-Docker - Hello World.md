...
## 1.1 공통(Windows PowerShell / WSL2 / Linux)
```bash
docker version
```
...
# 5. 컨테이너/이미지 내부 관찰과 이해

## 5.1 컨테이너 로그·상태·세부 정보
```bash
...
# 8. 운영환경/회사망에서 흔한 이슈와 해결

## 8.1 프록시/사설 CA로 인한 pull 실패
증상:
```
...
# 9. Hello World를 Compose로 실행(로컬 배포 맛보기)
...
# 12. Hello World 이후 반드시 해볼 추가 과제
