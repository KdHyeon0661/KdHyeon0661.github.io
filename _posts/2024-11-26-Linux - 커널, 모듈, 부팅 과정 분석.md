...
## 1. 전체 부팅 타임라인(UEFI/BIOS 공통 사고 모델)
...
## 3. 커널 이미지·initramfs — 루트 마운트 전의 “작전 기지”

### 3.1 initrd/initramfs가 필요한 이유
...
## 4. 커널 커맨드라인(cmdline) — 한 줄로 바꾸는 부팅 성격
...
## 6. 커널 메시지/로그 — dmesg, journalctl -k
...
## 8. 커널 패닉·OOPS 대응 — kdump, kexec, SysRq
...
### 8.3 Magic SysRq(막힌 시스템에서 최후의 수단)
```bash
# 활성 여부
...
## 9. 보안·무결성 — Secure Boot, 모듈 서명, MOK

### 9.1 Secure Boot 활성 환경에서 외부 모듈
...
## 10. ARM(임베디드) 부팅 특이점 — Device Tree/UEFI 혼재
...
## 13. 커널 파라미터(런타임) — sysctl와 /etc/sysctl.d
