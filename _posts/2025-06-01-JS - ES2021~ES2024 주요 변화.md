...
최신 ECMAScript(ES) 규격은 매년 한 번씩 정식판(ECMA-262)으로 묶여 공개됩니다.  
이 글은 ES2021(ES12)부터 **ES2025(ECMA-262 16th edition)**까지 핵심 변경사항을 **예제 중심**으로 정리하고, 기존 글의 핵심을 뽑아 **설명·주의점·적용 팁**을 대폭 보강한 확장판입니다.

---

## 목차
1. [ES2021(ES12)](#es2021es12)
2. [ES2022(ES13)](#es2022es13)
3. [ES2023(ES14)](#es2023es14)
4. [ES2024(ES15)](#es2024es15)
5. [ES2025(ES16)](#es2025es16-가장-최근-정식판)
6. [마이그레이션 팁 & 체크리스트](#마이그레이션-팁--체크리스트)
7. [브라우저/런타임 호환성 메모](#브라우저런타임-호환성-메모)
8. [참고 링크](#참고-링크)
...
### 3) `WeakRef`, `FinalizationRegistry` — 약한 참조/소멸 콜백
```js
...
> 더 읽기(개념/레퍼런스): `Promise.any`, WeakRef, 논리 할당은 MDN에 잘 정리되어 있습니다. :contentReference[oaicite:0]{index=0}
...
### 3) `Object.hasOwn` — 안전한 own-prop 체크
```js
...
### 2) *Change-Array-by-Copy* 계열 (불변 메서드)
```js
const xs = [3,1,2];
...
> ES2024에는 개발자 체감도가 높은 **병렬/정규식/배열/버퍼** 관련 제안들이 다수 Stage 4(표준 반영)로 승격되었습니다. 종합 요약은 아래를 참고. :contentReference[oaicite:1]{index=1}
...
> 위 항목들은 ES2024 Stage 4 모음 페이지에서 확인 가능합니다. 각 항목은 런타임/브라우저에 따라 도입 시점이 다릅니다. :contentReference[oaicite:2]{index=2}
...
> 2025년판 ECMA-262(16th edition)가 공식 공개되었습니다. 핵심 기능은 아래 링크와 요약을 참고하세요. :contentReference[oaicite:3]{index=3}
...
- ES2025 기능 개요 페이지에도 포함. :contentReference[oaicite:4]{index=4}
...
- ES2025 하이라이트 항목 중 하나. :contentReference[oaicite:5]{index=5}

#### (b) **RegExp Modifiers**
- 정규식 **수준에서 플래그/옵션을 보다 유연하게** 지정하는 문법(런타임별 제공 방식에 유의).
- ES2025 항목 요약 참고. :contentReference[oaicite:6]{index=6}
...
- **JSON을 네이티브 모듈**로 다루고, import에 **속성(attribute)**을 부가하여  
  **로더/번들러가 의도를 명확히** 해석하도록 합니다. :contentReference[oaicite:7]{index=7}

### 4) **Iterator Helpers(동기)** — 반복자 유틸 표준화
```js
// 가독성 높은 파이프라이닝
const sum = [1,2,3,4]
  .values()                // 이터레이터
...
- ES2025 항목 요약에 *Sync Iterator helpers*로 수록. :contentReference[oaicite:8]{index=8}
...
```
- 콜백이 **즉시 예외를 던져도** Promise 거부로 변환 → **일관된 async 흐름**. :contentReference[oaicite:9]{index=9}

### 6) **Float16**(number/TypedArray 계열)
```js
...
```
- 16-bit 부동소수 지원으로 **메모리 밀도** 향상. (런타임 지원 상태 확인) :contentReference[oaicite:10]{index=10}
...
## 참고 링크
- ES2024 Stage 4(정리): *A close look at ECMAScript 2024* (항목별 요약) :contentReference[oaicite:11]{index=11}  
- ES2025 정식 공개(ECMA 공식 뉴스): **ECMAScript® 2025 Language Specification published** :contentReference[oaicite:12]{index=12}  
- ES2025 기능 하이라이트(요약 페이지): **ES2025: What's new**(Set 메서드, JSON 모듈, RegExp 확장, Iterator helpers 등) :contentReference[oaicite:13]{index=13}  
- 개별 기능/배경 설명(추가 읽기): Dr. Axel의 ECMAScript 시리즈(문법 변화 해설) :contentReference[oaicite:14]{index=14}  
- MDN JavaScript 가이드(메모리/WeakRef 등): 개념·API 레퍼런스 :contentReference[oaicite:15]{index=15}  
