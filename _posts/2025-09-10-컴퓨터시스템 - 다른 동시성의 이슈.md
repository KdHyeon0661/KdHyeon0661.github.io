...
## 정확성(Correctness) 이슈 — 메모리 모델과 동기화의 본질
...
### 원자성 위반(Atomicity Violation) — check-then-act
...
### 가시성(Visibility)·순서(Order) — **happens-before** 만들기
...
## 성능(Scalability/Performance) 이슈 — 락·캐시·스케줄러
...
## 신뢰성(Operational) 이슈 — 취소·시그널·시간·자원
