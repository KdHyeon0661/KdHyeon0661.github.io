...
## 1) 정확성(Correctness) 이슈 — 메모리 모델과 동기화의 본질
...
### 1.2 원자성 위반(Atomicity Violation) — check-then-act
...
### 1.3 가시성(Visibility)·순서(Order) — **happens-before** 만들기
...
## 3) 신뢰성(Operational) 이슈 — 취소·시그널·시간·자원
...
### 3.4 시간/타임아웃 — **단조시계(CLOCK_MONOTONIC)**
...
## 4) I/O·이벤트 기반 추가 이슈 — 리액터와 함께

### 4.1 Head-of-Line(HoL) 블로킹
...
## 9) 수학으로 보는 직관 — Amdahl · Little · 큐잉

### 9.1 Amdahl’s Law (병렬 가속 한계)
$$
...
## 10) 사례 ① — **경계 안전** 유한 큐(조건변수, 타임아웃, MONOTONIC)
...
## 11) 사례 ② — Lock-Free 스택 + **Hazard Pointer** 스케치
