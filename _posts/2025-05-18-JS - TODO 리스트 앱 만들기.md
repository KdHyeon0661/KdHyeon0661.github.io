...
이 문서는 기본 DOM 조작으로 시작해 **필터/편집/키보드 단축키/접근성/성능/데이터 영속화**까지 한 번에 담은 실전형 가이드입니다.  
아래 순서대로 구현하면 “작동하는 기본판 → 확장판(MVC 모듈화)”까지 완성할 수 있습니다.

---

## 목차

1) 기본판: HTML/CSS/JS 단일 파일 구현  
2) 확장판: 모듈화(MVC), 필터(전체/진행/완료), 편집, 단축키, 삭제 취소(Undo)  
3) 데이터 영속화: localStorage 스키마 버전, 스로틀 세이브, 탭 간 동기화  
4) 접근성/UX: a11y 속성, 포커스 이동, 라이브 리전, 터치/키보드 조작  
5) 성능/아키텍처: 이벤트 위임, 렌더 최소화, 가상 리스트 팁  
6) 테스트/디버깅/배포 체크리스트  
7) 확장 아이디어
