...
## 실전 도구: `std::advance`, `std::distance`, `std::next`, `std::prev`
...
### `move_iterator` — 이동 시멘틱으로 복사 비용 절감
...
### `insert_iterator` — `back_inserter`/`front_inserter`/`inserter`
...
## 무효화(Invalidation) 규칙 — “언제 반복자가 죽는가?”
