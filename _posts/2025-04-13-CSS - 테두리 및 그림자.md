...
### 3.3 링 효과(포커스/선택) — blur=0, spread 이용
```css
/* 테두리보다 바깥쪽 링을 그림자로 */
.is-focused {
  box-shadow:
...
## 4) 그라디언트/이미지/이중 보더 — 실전 패턴

### 4.1 두 줄 보더(안쪽 얇은 선 + 바깥 링)
```css
/* outline + border 조합 */
.token {
...
### 4.2 **그라디언트 보더** (background-clip 트릭)
```css
.badge {
...
### 4.3 `border-image` — 이미지/그라디언트 슬라이스
```css
.ribbon {
