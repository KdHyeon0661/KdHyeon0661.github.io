...
## 1. 모델 설계 — Aggregate/Value Object, 불변/가변 전략
...
### 1.2 Aggregate: `UserProfile` (가변 POCO + 단순 유효성)
...
### 3.2 DataService — 옵션, 마이그레이터, 검증 포함
...
## 5. ViewModel — 저장/불러오기 + 자동 저장(디바운스) + 검증
...
## 7. 외부 변경 감지(파일 Watcher)와 즉시 반영(선택)
