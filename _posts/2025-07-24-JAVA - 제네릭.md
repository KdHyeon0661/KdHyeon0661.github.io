...
### 6.2 varargs와 힙 오염(Heap Pollution)
```java
...
## 7. 타입 소거(Type Erasure) — 규칙, 제약, 브리지
...
### 9.2 메서드 타입 추론 & 타겟 타입(람다/메서드 레퍼런스)
```java
...
## 10. 스트림/컬렉션과 제네릭 시그니처

### 10.1 `Comparator`/`Collectors`의 제네릭
```java
...
### 10.2 Optional — `map`/`flatMap`의 타입 전파
```java
...
## 11. 실전 패턴 — 타입 안전 레지스트리/복사/타입 토큰

### 11.1 타입 안전 레지스트리: `Map<Class<T>, T>`
```java
class Registry {
...
## 12. 금지/주의 목록 — 제네릭 예외, raw type, instanceof, equals
