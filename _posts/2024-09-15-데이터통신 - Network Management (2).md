...
---
# SNMP — Managers, Agents, SMI, MIB, SNMP 프로토콜 완전 정리
...
SNMP는 여전히 IETF의 **Internet Standard Management Framework**의 핵심으로, RFC 3411/STD 62(아키텍처), RFC 1157(SNMPv1), RFC 341x 시리즈(SNMPv3), RFC 2578/STD 58(SMIv2) 등에 기반한다.
...
RFC 3411/STD 62는 “하나 이상의 **SNMP entity**가 command generator, notification receiver 애플리케이션을 포함할 때 이를 전통적으로 **manager**라고 했다”고 설명한다.
...
SNMP 아키텍처 문서는 “하나 이상의 command responder 및 notification originator 애플리케이션을 포함한 SNMP entity를 전통적으로 **agent**라 부른다”고 정의한다.
...
- 각 라우터: SNMP 에이전트 활성화, 표준 MIB-II(`ifTable`, `system` 등) + 벤더 MIB(CISCO-CPU, JUNIPER-MIB 등) 제공
- NMS: SNMP Manager 역할
...
여기서 `ifInOctets.1`은 `MIB-II::ifTable` 안 첫 번째 인터페이스의 수신 바이트 카운터를 의미한다.
...
2. **텍스트 규약(Textual Conventions)**
   ...
   동일한 표현이 여러 MIB에서 일관되게 사용되도록 한다.
...
4. **OBJECT-TYPE 매크로**
   ...
   각 객체는 `OBJECT-TYPE` 매크로로 정의하며, 다음 정보를 포함한다.
...
- Cisco: `CISCO-CPU-MIB`, `CISCO-IF-EXTENSION-MIB`, `CISCO-CVP-MIB` 등
...
| SNMPv3 | 현재 표준 | **USM**(User-based Security Model)과 **VACM**(View-based Access Control Model)로 **인증·암호화·접근통제** 제공  |

- RFC 1157은 SNMPv1의 기본 프레임워크를 정의한다.
- RFC 3411/STD 62 아키텍처와 RFC 3414 USM, RFC 3415 VACM이 SNMPv3를 구성한다.
