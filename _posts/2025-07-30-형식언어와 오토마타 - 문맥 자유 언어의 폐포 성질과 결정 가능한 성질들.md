...
## **결정 가능(Decidable)** 한 것들

### **멤버십(소속성)** \( w\stackrel{?}{\in}L(G) \) — **Decidable**
...
### **Emptiness(공백성)** \( L(G)=\varnothing? \) — **Decidable**
...
### **정규와의 교차 공백성** \( L(G)\cap R = \varnothing? \) — **Decidable**
...
### \( \mathbf{CFL}\cap\mathbf{REG} \) 닫힘 (PDA×DFA)
