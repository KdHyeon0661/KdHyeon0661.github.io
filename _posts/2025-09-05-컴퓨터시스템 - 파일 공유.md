...
## 2. 경쟁 조건과 **원자성** — 올바른 쓰기/읽기 패턴

### 2.1 `lseek+read/write`는 경쟁의 온상
```c
...
## 3. 잠금 — `flock`, POSIX `fcntl` 잠금, **OFD 잠금**, Mandatory 잠금
...
## 5. 파일 수명과 디렉터리 엔트리 — `unlink/rename`의 공유 의미
...
## 6. FD 관리 — `CLOEXEC`, 대량 FD 닫기, FD 전달
...
## 7. 네트워크 파일 공유 — NFS vs SMB **일관성·잠금·리스**
...
### 10.1 **오프셋 공유 버그 → `pread/pwrite`로 수정**
...
### 10.3 **Unix 도메인으로 FD 전달 (SCM_RIGHTS)**
