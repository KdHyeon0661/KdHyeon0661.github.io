...
## 버퍼링 모드와 flush 규칙 — **출력 사라짐**을 막는 법
...
## 대용량/성능 최적화 — setvbuf, unlocked, 64비트 오프셋
...
## 혼용·프로세스·스레드 — 미묘하지만 중요한 규칙

### stdio vs 저수준 I/O **혼용 금지**
...
### `scanf("%s", buf)`에서 폭 미지정

- 버퍼 오버런 위험. **폭 지정**(예 `%31s`) 사용.

### `printf(user_input)` 직접 사용

- 포맷 문자열 취약점. **항상 `"%s"`와 함께**.

### 같은 fd에 stdio + `write(2)`

- 버퍼 경합. **한 계층만** 사용.

---

## 성능 관점: stdio vs RIO/저수준
...
## 확장: POSIX `getline/getdelim`와 “레코드 경계” 처리
