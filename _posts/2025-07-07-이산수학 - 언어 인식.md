...
### 1.2 NFA/ε-NFA와 부분집합 구성(SubSet Construction)
...
### 1.3 정규식 → NFA(Thompson), NFA → DFA(부분집합), DFA 최소화(Hopcroft)
...
## 3. 문맥자유 언어 인식 — PDA & 파싱(CYK/Earley)

### 3.1 PDA 모델
- **스택** 보유 → 중첩 구조(`()` 중첩, `a^n b^n` 등) 인식 가능  
- 수용: **최종 상태** 또는 **빈 스택** (표현에 따라 동치)

### 3.2 CYK 알고리즘(Chomsky Normal Form)
...
## 4. 문맥민감 언어 & LBA, 그리고 튜링기계

### 4.1 LBA(Linear Bounded Automaton)
...
## 9. 코드 모음 — 정규/CFG 인식 실습

### 9.1 범용 DFA 최소 예 (위 §1.1 확장 처리)
```python
