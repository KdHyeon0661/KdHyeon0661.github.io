...
### 여러 값을 반환해야 할 때: **출력 인자(out param)** 또는 구조체 반환
...
## `const` / `restrict` / `volatile` — 함수 인자에 깃발 꽂기
...
### `restrict` (C99) — 별칭(aliasing) 없음 힌트로 최적화 유도
...
## 가변 인자(variadic) — `printf` 류 직접 만들기
