...
### 1.2 예제 — 애플리케이션 커서 캐시/배치 Fetch의 효과를 비용이 과소평가
```sql
...
### 3.2 재현 — 단일 컬럼 히스토그램은 OK, 조합은 MISS
```sql
...
### 4.3 Adaptive Cursor Sharing(ACS)로 완화… 하지만
...
### 5.2 재현 — REGION과 TIER의 상관, BRAND와 CATEGORY의 상관
```sql
EXPLAIN PLAN FOR
...
### 6.2 재현 — Full Scan이 항상 빠르다고 보지만, 캐시에 전부 있으면?
```sql
