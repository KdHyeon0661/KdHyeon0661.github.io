...
Oracle 개발 가이드도 “실제 앱의 연결/반복 패턴과 DB 비용의 차이를 별도 계측하라”는 관점을 강조한다.
...
Oracle 튜닝 가이드는 “통계 최신성/적합성이 카디널리티의 기반이며, 틀리면 플랜이 틀린다”고 명시한다.

### 재현: 데이터 분포 변경 후 통계 미수집
...
### “Full Scan이 항상 싸다 / 해시가 항상 싸다”는 평균치
...
하드웨어 교체/스토리지 변화 후 **재수집이 필요**하다.
또한 In-Memory/Join Filter(Bloom) 같은 엔진 최적화는
특정 워크로드에서 비용 예측이 더 어려워질 수 있다.
