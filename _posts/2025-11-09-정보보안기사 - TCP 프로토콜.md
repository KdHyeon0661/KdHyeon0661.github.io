...
## 혼잡 제어(cwnd) — 슬로 스타트, 혼잡 회피, Fast Retransmit/Recovery
...
### RTT 샘플링 & RTO 추정(Jacobson/Karels)
...
## 성능/튜닝(리눅스 중심, 일부 윈도우 포함)

### 리눅스 sysctl 주요 파라미터

```bash
# 혼잡 제어 알고리즘 확인/설정 (cubic 기본)
...
# 윈도우 스케일/타임스탬프/SACK (대부분 기본 사용)

sysctl -w net.ipv4.tcp_window_scaling=1
sysctl -w net.ipv4.tcp_timestamps=1
sysctl -w net.ipv4.tcp_sack=1

# SYN backlog / 재시도 / SYN쿠키
...
## 보안 — SYN Flood, RST Injection, 하이재킹(개요)
...
## 코드 예제 — Python/PowerShell

### Python: 간단 Echo 서버/클라이언트
