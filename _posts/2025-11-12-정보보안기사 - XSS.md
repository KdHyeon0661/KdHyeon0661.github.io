...
category: 정보보안기사
---
# 완전 정리 (유형·컨텍스트·현대 브라우저·프레임워크별 안전코딩·CSP/Trusted Types·테스트/모니터링·체크리스트·예상문제)
...
|---|---|---|---|
| **HTML 본문** | {% raw %}`<div>{{data}}</div>`{% endraw %} | HTML 이스케이프 (`&`, `<`, `>`, `"`, `'`) | `innerHTML`로 직접 삽입 |
| **HTML 속성 값** | {% raw %}`<img alt="{{data}}">`{% endraw %} | 속성 인코딩 + 따옴표로 감싸기 | 이벤트 속성(`on*`)·`srcdoc` |
| **URL 속성** | {% raw %}`<a href="{{url}}">`{% endraw %} | **프로토콜 화이트리스트**(`https:`, `mailto:` 등) | `javascript:`, `data:`(특히 `text/html`) |
| **JS 문자열 리터럴** | {% raw %}`<script>var x='{{data}}'</script>`{% endraw %} | JS 문자열 이스케이프 + `<script>` 내 삽입 지양 | `</script>` 중단, 템플릿 보간 |
| **CSS 컨텍스트** | {% raw %}`style="background:url({{u}})"`{% endraw %} | 인라인 CSS 지양, 클래스/토큰 화이트리스트 | `url()`에 `javascript:` |
...
- **Django/Jinja2**: 기본 **자동 이스케이프**. {% raw %}`{{ x }}`{% endraw %}는 안전, `|safe` 남용 금지.
...
- **Vue**: {% raw %}`{{ }}`{% endraw %}는 이스케이프. `v-html`은 **비이스케이프** → 제한된 신뢰 입력만.
