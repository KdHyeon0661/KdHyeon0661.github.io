...
### 기본 패턴: Child → Parent FK 조인인데 Parent를 안 씀
...
### EXISTS/SEMijoin 제거 — FK가 “존재성”을 증명
...
### “LEFT JOIN + IS NULL”은 제거 불가(의미가 필터)
...
Oracle 문서에서도 RELY/Novalidate 상태에 따라 옵티마이저가 제약을 **쿼리 변환/리라이트 판단에 활용**할 수 있음을 명확히 설명한다.
...
### 눈사람 스키마(snowflake)에서 차원 체인을 걷어낸다
...
- **제약 신뢰도(VALIDATE / RELY)가 곧 제거의 연료**다. Oracle 문서도 RELY 제약이 옵티마이저 변환/리라이트에 사용될 수 있음을 명시한다.
- **검증은 플랜 + 통계**로 한다:
