...
```elixir
%User{}
# ** (ArgumentError) the following keys must also be given when building struct User: [:id, :email]
```
...
```elixir
# %{u | nickname: "kim"} 
# ** (KeyError) key :nickname not found in: %User{...}
```
...
### 8.6.7 실전 미니 프로젝트: User + Session + Role 구조체
...
### 8.7.2 안전 갱신: `update_in/3`, `get_and_update_in/3`
...
### 8.7.3 리스트 내부 요소 선택: `Access.at/1`, `Access.filter/1`
...
## 8.9 큰 힘은 큰 유혹을 낳는다 — 강력 기능의 함정과 기준선
...
### 8.9.1 동적 아톰 생성 — 편하지만 위험한 메모리 누수
...
## 종합 실전: 설정 로더(Keyword→Map→Struct) + 규칙 엔진(MapSet) + 안전 갱신
...
### 8.10.2 로더 구현 (Keyword | Map → Struct)
