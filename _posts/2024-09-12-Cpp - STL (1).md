...
## 3. 컨테이너 한눈 분류 (노드 기반 vs 배열 기반)

### 3.1 순차 컨테이너 (Sequence)
...
## 5. 알고리즘(algorithm) — 컨테이너 독립 범용 도구
...
## 6. **반복자 무효화(Iterator Invalidation)** — 실전 오류 1순위
...
## 7. 예외 안전(Strong/Basic/No-throw) & 이동 시맨틱스
...
## 9. STL 사용 확장: 투명 비교자/이종 조회/커스텀 해시

### 9.1 투명 비교자(heterogeneous lookup)
...
## 10. `emplace` 계열과 `piecewise_construct`
...
## 12. `map`/`unordered_map` 핵심 API 패턴
...
## 14. 비소유 문자열/메모리 뷰: `string_view`, `span`
