...
## 3. 텐서 핵심 연산 (프레임워크 관점 포함)

### 3.1 수축(내적, contraction) — 축 줄이기
- 두 텐서의 일부 축을 합산하여 제거:
$$
...
### 3.2 모드-n 전개(unfold, matricization)
...
## 8. PyTorch: 핵심 연산 & 분해 코드

> 모든 코드는 **PyTorch** 기준(사용자 선호). GPU 사용 시 `to('cuda')`.

### 8.1 기본 연산: 외적·수축·모드-곱·전개
...
### 8.3 CP-ALS (3차, 간단 참조 구현 — 소규모/연구용)
