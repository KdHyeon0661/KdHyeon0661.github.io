...
초안의 구성(정의→문법→특징→브라우저 지원)을 유지하면서, **접근성(a11y)**, **검증/보안**, **서버 연동**, **프로그레시브 인행스먼트(PE)**, **국제화(i18n)**, **실전 패턴/반패턴**, **대안 API(WebAuthn/WebCrypto)**까지 보완했다. 모든 예제는 **동작하는 최소 코드**로, 복사-붙여넣기 후 바로 실험 가능하도록 구성했다.
