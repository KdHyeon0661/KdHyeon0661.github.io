...
## 작은 메시지 최적화: Coalescing vs `TCP_NODELAY`

### Nagle의 알고리즘 & Delayed ACK 상호작용
...
## Zero-copy 경로: `writev/sendmsg`, `sendfile` (+ 보너스)

### `writev/sendmsg` — **벡터화 전송** (헤더+바디 합치기)
...
## 실습: **파일 송신기** — `sendfile` vs `read+write` 비교
