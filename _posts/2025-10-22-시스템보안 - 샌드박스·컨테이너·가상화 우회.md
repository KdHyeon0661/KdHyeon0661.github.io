...
category: 시스템보안
---
# 7. 샌드박스·컨테이너·가상화 우회 (방어 중심 완전 정리)
...
## 7.1 네임스페이스 / cgroup / seccomp-bpf 이해

### 7.1.1 네임스페이스(Namespaces)
...
## 7.2 Docker/Containerd 보안: CapDrop, read-only FS, userns-remap

### 7.2.1 리눅스 Capabilities 최소화
...
### 7.2.2 read-only RootFS & Masked/Readonly Paths
...
### 7.2.3 userns-remap / rootless 컨테이너
...
### 7.2.4 AppArmor/SELinux & seccomp 결합
...
## 7.3 탈주 패턴: 마운트/소켓/도커 소켓, runc 사례(개념 분석)
...
### 7.3.2 Docker 소켓(`/var/run/docker.sock`) 노출
...
### 7.3.3 `--privileged`, 넓은 Cap, 호스트 네임스페이스 공유
...
### 7.3.5 런타임 탐지(Falco/EBPF) — “행동”을 본다
...
## 7.4 하이퍼바이저(VT-x/AMD-V)와 VM Escape 개요
