...
## 핵심 API — `stat/lstat/fstat` → `fstatat` → `statx`(Linux)
...
## `struct stat` 빠르게 훑기 + 모드 비트/특수 비트
...
## 실전 코드 ① — 안전하고 풍부한 `stat`(경로/FD/심링크 대응)
...
## 실전 코드 ② — `fstatat`(디렉터리 FD 기준, TOCTTOU 완화)
...
## 실전 코드 ③ — `statx`로 **btime**(생성 시각) 포함 정밀 질의(Linux)
...
## 실전 코드 ④ — **희소 파일** 탐색(`SEEK_HOLE/SEEK_DATA`) 예
