...
### 1.2 OR Expansion의 정의

Oracle 문서에서 OR expansion은 **최상위 OR(Disjunction)** 을 가진 쿼리 블록을 **UNION ALL 분기 형태**로 변환하는 CQT(비용기반 쿼리 변환)로 정의된다. :contentReference[oaicite:0]{index=0}
...
- 파티션 프루닝(access predicate)이 **명확**해진다. :contentReference[oaicite:1]{index=1}

---

## 2) OR Expansion의 종류: LORE(legacy) vs ORE(cost-based)

Oracle 내부 트레이스/코멘트에서 OR 확장은 두 계열로 언급된다. :contentReference[oaicite:2]{index=2}

### 2.1 LORE (Legacy OR-Expansion, “Concatenation”)

- 오래된 방식: **모든 분기에서 인덱스 접근이 가능해야** 잘 작동하는 경향  
- 옵티마이저 트레이스에서 **CONCATENATION / LORE** 로 표시  
- 실무적으로 `USE_CONCAT`로 유도하는 전통적 패턴

### 2.2 ORE (Cost-based OR-Expansion)

- 12cR2 이후 강화된 방식으로 알려져 있으며,  
  **분기마다 서로 다른 조인전략/액세스 경로**를 더 유연하게 허용한다. :contentReference[oaicite:3]{index=3}
...
어떤 SQL이 **예전에는 LORE로 잘 풀렸는데**, 새 버전에선 ORE를 선택하지 못해 **확장이 사라지고 플랜이 악화**되는 사례가 있다. :contentReference[oaicite:4]{index=4}
...
### 3.1 `USE_CONCAT` — OR Expansion 강제

`USE_CONCAT`는 OR(및 IN-list)을 **UNION ALL 기반 분기**로 강제로 재작성하게 하는 힌트다. :contentReference[oaicite:5]{index=5}
...
- 단, **IN-LIST iterator를 꺼버리고** OR을 전부 확장할 수 있으니(= 분기 수 폭발 위험) 주의. :contentReference[oaicite:6]{index=6}

### 3.2 `OR_EXPAND` / `NO_OR_EXPAND`

- `OR_EXPAND`는 **보다 공격적으로 OR expansion을 강제**하는 힌트로 소개된다.  
- 반대로 `NO_OR_EXPAND(@qb)`는 특정 쿼리 블록에서 OR 확장을 막는다. :contentReference[oaicite:7]{index=7}
...
- 자동 변환이 과도하거나 잘못될 때는 `NO_OR_EXPAND`로 차단하고 다른 전략을 택한다. :contentReference[oaicite:8]{index=8}

### 3.3 `NO_EXPAND` — OR/INLIST 확장 방지
...
**의미 보존(semantic equivalence)** 이 확실해야 하고, 비용도 유리해야 한다. :contentReference[oaicite:9]{index=9}

### 4.1 자동 확장이 가능한 전형 조건
...
- 업그레이드/패치 이후 특정 OR 변환이 **차단되는 회귀**(실무에서 `USE_CONCAT`로 복구) :contentReference[oaicite:10]{index=10}
...
Oracle Optimizer 팀 공식 글에서도 **인덱스/프루닝/조인 전략**의 분기 최적화가 핵심 이득으로 언급된다. :contentReference[oaicite:11]{index=11}
...
### 7.3 OR Expansion + 브랜치별 최적화(After)
...
> 공식 문서/블로그에서 말하는 OR expansion의 대표 장점이 그대로 구현된다. :contentReference[oaicite:12]{index=12}
...
Oracle은 같은 컬럼의 OR을 **IN-list iterator**로 처리할 수 있다. :contentReference[oaicite:13]{index=13}

### 9.1 IN-LIST iterator의 의미
...
- 옵티마이저는 같은 인덱스를 반복 스캔하는 **INLIST ITERATOR** 연산을 선택할 수 있다. :contentReference[oaicite:14]{index=14}

이건 사실상 **“자동 OR 확장”의 특수 케이스**다.

### 9.2 언제 IN-LIST가 더 낫나?
...
공식적으로 OR expansion은 **SARGABLE 회복**에 매우 유용하다. :contentReference[oaicite:15]{index=15}
...
OR expansion의 성패는 **분기별 선택도 추정 정확도**에 달린다. :contentReference[oaicite:16]{index=16}
...
19c 이후 ORE는 바인드 스큐에서도 꽤 잘 동작하는 사례가 보고된다. :contentReference[oaicite:17]{index=17}  
하지만 여전히 다음 문제는 존재한다.

### 14.1 Bind Peeking/Adaptive Cursor Sharing과의 결합
...
- 필요하면  
  ...
  - 반대로 `NO_OR_EXPAND`로 “항상” 막고  
    **SPM Baseline**으로 플랜을 안정화한다. :contentReference[oaicite:18]{index=18}
...
- 분기1은 p202502만  
- 분기2는 p202501만  
**각 분기 프루닝이 완벽히 분리**된다. :contentReference[oaicite:19]{index=19}

---

## 16) Bitmap OR vs OR Expansion vs INLIST — 최종 선택 표

Oracle Optimizer 팀은 **Bitmap OR, OR expansion, INLIST iterator**를 구분해 설명한다. :contentReference[oaicite:20]{index=20}
...
- [ ] OR이 같은 컬럼 값 나열인가?  
  → 먼저 IN-LIST/INLIST ITERATOR 가능성 검토. :contentReference[oaicite:21]{index=21}
- [ ] OR 분기 간 Overlap이 있는가?  
  → `AND NOT(cond_A)`로 상호배타화, 또는 `UNION` 사용.
- [ ] 자동 확장이 사라져 성능이 떨어졌는가(업그레이드/패치 후)?  
  → `USE_CONCAT`로 legacy 확장을 복구. :contentReference[oaicite:22]{index=22}
- [ ] 분기 수가 너무 많은가?  
  → 부분 확장/동적 SQL/IN-LIST 묶기 고려.
- [ ] 통계/히스토그램이 분기 선택도에 맞게 최신인가?  
  → 오판 방지 핵심. :contentReference[oaicite:23]{index=23}
- [ ] 바인드 스큐가 큰가?  
  → 실측 플랜으로 확인하고 SPM/힌트로 안정화. :contentReference[oaicite:24]{index=24}
...
- OR Expansion은 OR이 만든 족쇄를 풀고,  
  **분기별 최적화를 통해 인덱스/프루닝/조인전략을 동시에 살리는** 강력한 CQT다. :contentReference[oaicite:25]{index=25}
- 핵심은
  1) **분기별 최적 경로가 정말 다른가**,  
