...
## 1) Rolling Window 심화: ROWS / RANGE / GROUPS 정밀 사용

### 1.1 ROWS vs RANGE vs GROUPS — 선택 기준
...
### 1.2 시간 기반 Rolling — RANGE + INTERVAL
...
### 1.4 GROUPS 프레임 — 피어(동점) 그룹 단위 슬라이딩
...
## 2) CUME_DIST(), PERCENT_RANK() — 분포/백분위 분석
...
## 3) 퍼센타일(Percentile) — PERCENTILE_CONT / PERCENTILE_DISC
...
## 4) 프레임 배제(EXCLUDE) — CURRENT ROW / TIES / GROUP
...
## 5) 고급 집계: 가중 이동 평균, 윈저화(Winsorize), 이상치 완화

### 5.1 가중 이동 평균 (예: 최근이 더 중요)
...
### 5.2 윈저화(Winsorize) 이동 평균 — 극단값 완화
...
## 6) NTILE과 분위/백분위 레이어링

### 6.1 NTILE로 분위 레이블 만들기
...
### 6.2 NTILE vs PERCENTILE vs CUME_DIST
...
### 7.3 “월별 매출의 전년 동월 대비(YoY), 누적(YTD), 이동(MA) 동시 산출”
...
## 12) 종합 예제 — “상위 백분위별 KPI + 롤링·누적 지표 + 동점배제 평균”
