...
## 0) 개념 리마스터: Git은 무엇을 “되돌리는가” — 3계층 상태 모델
...
## 4) 부분 리셋(pathspec) — `git reset` 으로 특정 파일만 Index 동기화
...
## 5) 협업에서의 안전 수칙 — 언제 reset을 쓰고, 언제 쓰지 말까
...
## 8) rebase/merge/cherry-pick 와 reset의 조합
