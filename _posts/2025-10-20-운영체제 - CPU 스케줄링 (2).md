...
### 5.5.4 Per-CPU Runqueue + 주기적 Load Balancing 시뮬레이터
...
### 5.6.2 Fixed-Priority: Rate-Monotonic (RM)
...
### 5.6.3 Dynamic-Priority: Earliest-Deadline-First (EDF)

- **가장 임박한 데드라인** 우선
- 단일 프로세서에서 **필요충분조건**: $$ U \le 1 $$ (이상적인 조건 하)

### 5.6.4 정형 검증 — Response-Time Analysis(RTA) (RM)
...
### 5.6.5 EDF Demand-Bound Function(DBF)
...
### 5.6.7 POSIX 실시간 정책 적용(주의: root/권한 필요)
