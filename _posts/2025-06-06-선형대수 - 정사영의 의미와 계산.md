...
category: 선형대수
---
# 🎯 정사영(Projection) 완전 정복 — 정의 → 유도 → QR/SVD → PyTorch 실습
...
## 3. 원점을 지나지 않는 Affine 직선/평면 위 정사영

### 3.1 직선 \(\ell=\{\mathbf{p}_0+\alpha \mathbf{u}\}\)
$$
\boxed{\;\mathrm{proj}_{\ell}\mathbf{v}
=\mathbf{p}_0+\frac{(\mathbf{v}-\mathbf{p}_0)\cdot\mathbf{u}}{\mathbf{u}\cdot\mathbf{u}}\;\mathbf{u}\;}
$$

### 3.2 평면 \(\Pi\) (법선 \(\mathbf{n}\), 점 \(\mathbf{p}_0\)) 또는 평면식 \(\mathbf{n}\cdot\mathbf{x}=d\)
$$
