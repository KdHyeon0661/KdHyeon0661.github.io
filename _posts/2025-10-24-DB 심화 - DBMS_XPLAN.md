...
## **예상 실행계획 출력** — `EXPLAIN PLAN FOR` + `DBMS_XPLAN.DISPLAY`
...
## **캐싱된 커서의 실제 실행계획 출력** — `DBMS_XPLAN.DISPLAY_CURSOR`
