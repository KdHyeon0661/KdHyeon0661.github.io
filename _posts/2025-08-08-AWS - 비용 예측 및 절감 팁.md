...
## 1. 비용 **추적** — 데이터부터 곧바로 쌓자

### 1.1 Cost and Usage Report(CUR) → Athena/QuickSight
...
### 1.2 Cost Explorer API로 빠른 피벗 (boto3)
```python
...
### 1.4 Cost Anomaly Detection — 갑툭튀 제어
```bash
# 서비스 단위 이상징후 탐지
...
### 3.1 EC2 **스팟 인스턴스** (최대 ~90% 저렴, 중단 내성 필요)
...
### 3.2 **Reserved Instances(RI)** vs **Savings Plans(SP)**
...
### 3.5 **네트워크 비용** — “Egress·NAT·데이터경로” 줄이기
...
## 4. 자동화/가드레일 — “만들었으면 지워지게”

### 4.1 유휴자원 정리(예: 7일간 CPU<3% 인스턴스 중지)
```python
import boto3, datetime
...
## 6. 예시: 스팟+온디맨드 혼합 ASG로 CI 파이프라인 비용 60%↓ (시나리오)
...
# 4. Budgets 생성(월 1,000 USD 경보)
aws budgets create-budget ... (본문 예시 재사용)

# 5. S3 Lifecycle 적용(버킷 정책은 콘솔/CLI로 추가)
