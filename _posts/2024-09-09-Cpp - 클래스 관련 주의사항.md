...
## 다이아몬드(Diamond)와 가상 상속(virtual inheritance)
...
### 실수 #2 — 가상 상속에서 “누가 `Animal`을 초기화하는가?”
...
## Rule of Three/Five/Zero — 복사·이동·수명 규약

### 실수 #3 — 소유 리소스를 가진데 복사/대입/소멸 불일치
...
## 다형성에서 값 전달(by value) — 슬라이싱(slicing)
...
## 이름 숨김(name hiding)과 오버로드 세트 소실
...
## `override`/`final`/`explicit`를 잊는 실수

### `override` 누락 → 오타/시그니처 불일치 미검출
...
## 이동 연산의 `noexcept` 누락 — 성능 저하(컨테이너 재할당)
...
## 사례 통합: “하이브리드 위젯” — 잘못된 설계 vs 교정 설계
