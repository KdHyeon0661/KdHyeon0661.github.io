...
**폼(Form)**은 사용자 입력을 수집하고 서버와 상호작용하는 가장 보편적인 인터페이스입니다.  
이 글은 **HTML5 내장 검증(Constraint Validation API)** → **자바스크립트 커스텀 검증** → **비동기 서버 검증/전송(fetch)** → **접근성/UX/보안/성능/테스팅**까지, 실전에 필요한 전 과정을 한 번에 정리합니다.

---

## 0) 한눈에 보는 체크리스트

- [ ] 가능한 한 **HTML 속성으로 먼저** 검증(`required`, `type`, `min/max`, `pattern`, `step`)  
- [ ] 시각 피드백: `:invalid/:valid` CSS, **에러 메시지**를 `aria-describedby`로 연결  
- [ ] 제출 시: `form.checkValidity()` → 유효하면 **이중 제출 방지**(버튼 비활성/스피너)  
- [ ] **서버 검증**은 반드시 별도 수행(신뢰 경계)  
- [ ] 비동기 검증(중복 아이디 등)은 **디바운스 + AbortController**  
- [ ] 파일 업로드는 **크기/타입/갯수** 클라이언트에서 선검증  
- [ ] CSRF, XSS, Rate limit, `SameSite`/`HttpOnly` 쿠키, 입력 정규화/필터링 적용  
- [ ] 에러 요약(상단) + 필드 옆 인라인 에러 + 포커스 관리로 접근성 보장
...
## 2) Constraint Validation API — HTML만으로 되는 것들
...
## 4) 비동기(서버) 검증 — 중복 아이디, 초대 코드 등

### 4.1 디바운스 + 중복 요청 취소(AbortController)
...
## 5) 폼 전송 — `FormData` vs JSON, 파일 업로드

### 5.1 `FormData` (파일/멀티파트에 유리, 네이티브 폼과 친화)
...
### 5.3 더 나은 제출 트리거: `form.requestSubmit()`
...
## 9) 접근성(Accessibility) — 에러 전달과 포커스
...
## 11) 고급 이벤트 — `invalid`, `input`, `change`, `beforeinput`
...
## 13) 예제 — 실전 회원가입 폼(로컬+서버 검증, 파일 포함)
