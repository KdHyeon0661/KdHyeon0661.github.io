...
category: 시스템보안
---
# 10. 루트킷·EPP/EDR 우회 개론 (블루팀 관점)

## 10.1 유저/커널 루트킷 개념, 훅킹/인라인 패치/SSDT
...
### 10.1.2 블루팀 “관찰 실습” — 후킹/변조 징후 스캔(읽기 전용)

> 목적: “후킹이 있을 수 있는 흔적”을 **변경 없이 관찰**하고, **허용/차단 기준**을 설계.

#### (PowerShell) 프로세스 모듈 서명/경로 요약
```powershell
...
#### (PowerShell) 코드 무결성 및 AMSI/ETW 차단 시도 지표
```powershell
# 코드 무결성(CI) 관련 로그 채널/정책 상태 점검 (읽기 전용)
...
### 10.1.3 커널 드라이버·콜백 관찰 (변경 금지, 서명/상태만 점검)
...
## 10.2 안티-분석: Anti-VM / Anti-Debug / Timing trick (탐지·완화 관점)
...
### 10.2.2 “안전한 관찰 실습” — Anti-VM/Anti-Debug 시그널 헌팅

> 아래 스크립트는 **공격 코드가 아닌**, **분석 회피 흔적**을 **로그로 탐지**하기 위한 템플릿입니다.

#### (PowerShell) 의심 API Import/메모리 보호 변경 패턴 나열
```powershell
...
#### (Sysmon 규칙 스케치) – ETW/AMSI 관련 레지스트리/모듈 조작 경고
```xml
...
### 10.2.3 타이밍 기반 탐지 — “분석 회피 지연”의 이상치
...
## 10.3 EDR 탐지 표면: API 호출, 드라이버 통신, ETW/Sysmon 이벤트

### 10.3.1 EDR/EPP의 일반적 가시성 레이어
...
### 10.3.2 ETW와 Sysmon — 실전 헌팅 쿼리/보고서

#### (PowerShell) Sysmon 이벤트 빠른 요약
```powershell
# 최근 2시간 핵심 이벤트 요약
...
### 10.3.3 하드닝: WDAC/ASR/LSA 보호/드라이버 정책
...
#### (PowerShell) WDAC/드라이버 차단 상태 샘플 확인(개념)
```powershell
...
### B) “의심 DLL 사이드로딩” 관찰(업무 앱 경로 외 DLL)
```kusto
Sysmon
| where EventID == 7
