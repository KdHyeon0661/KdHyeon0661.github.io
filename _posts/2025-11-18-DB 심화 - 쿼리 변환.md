...
### 0.1 정의
**쿼리 변환(Query Transformation)** 은 옵티마이저가 **실행계획을 만들기 전에** SQL을 **논리적으로 동치(결과 동일)** 인 다른 형태로 자동 재작성하는 단계다. 변환된 SQL 위에서 조인 순서/접근 경로/조인 방식 등 비용 탐색이 이뤄진다. :contentReference[oaicite:0]{index=0}

### 0.2 목적
1) **카디널리티 추정(행 수, 선택도)과 비용 산정을 쉽게/정확하게** 만들고  
2) **더 저렴한 대안 플랜을 탐색할 수 있도록 후보군을 확장**하기 위함이다.  
즉, “같은 결과”를 내는 SQL을 여러 형태로 만들어 놓고 **CBO가 그중 최저 비용을 고르는 구조**다. :contentReference[oaicite:1]{index=1}

### 0.3 큰 그림에서의 위치
옵티마이저는 대체로  
**파싱 → (동치 SQL 생성) 쿼리 변환 → 비용 기반 탐색/선택 → Row Source Tree 생성**  
순으로 움직이며, 변환은 **조인 순서 탐색보다 앞단에서 후보 SQL을 증가**시키는 역할을 한다. :contentReference[oaicite:2]{index=2}

---

## 1. 옵티마이저 내부 흐름과 Query Block

### 1.1 Query Block(QB)란
SELECT / 서브쿼리 / 인라인뷰 / WITH(CTE) 등 **각 논리 단위**를 옵티마이저는 Query Block으로 취급한다.  
변환은 “SQL 전체”가 아니라 **QB 단위로 적용/차단/강제**되는 경우가 대부분이다. 그래서 **QB 네이밍이 제어의 출발점**이다. :contentReference[oaicite:3]{index=3}

### 1.2 QB_NAME으로 이름 붙이기
```sql
SELECT /*+ qb_name(main) */ ...
...
- 운영에서 “전체 변환 금지”는 위험하고, **문제 QB만 국지 제어**하는 게 표준적이다. :contentReference[oaicite:4]{index=4}

---

## 2. 변환 제어의 기본 도구: 힌트/파라미터/보안 모드

### 2.1 변환 제어 힌트 패밀리
오라클은 변환을 **힌트로 강제/차단** 할 수 있게 한다. 대표군은 다음과 같다. :contentReference[oaicite:5]{index=5}
...
> - 대부분은 `NO_MERGE`, `NO_UNNEST`, `NO_EXPAND`처럼 **정확히 목표 변환만 끊는 게 안전**하다. :contentReference[oaicite:6]{index=6}

### 2.2 Secure View Merging(권한 안전 뷰 머지)
뷰 머지는 **권한/보안 의미가 바뀔 수 있기 때문에**, 오라클은 “보안 안전 모드”에서만 머지를 허용한다.  
`optimizer_secure_view_merging` 파라미터가 이 동작을 제어한다. :contentReference[oaicite:7]{index=7}

---

## 3. 대표 쿼리 변환들 — 원리/예제/주의점/제어

여기서는 **당신이 이미 정리한 12개 변환**을 중심으로, 오라클 공식 변환 분류(추가 변환 포함)와 맞물리게 확장한다. :contentReference[oaicite:8]{index=8}

---

### 3.1 프레디킷 푸시다운 & 이동 (Predicate Pushdown / Move-Around / Transitivity)

#### 3.1.1 원리
- “필터는 가능한 한 **가장 아래로 내린다.**”  
- 조인 조건에서 **전이 프레디킷(Transitive Predicate)** 이 만들어질 수 있다.  
  예: `A=B` and `B=10` ⇒ `A=10` 을 추가해서 **접근 범위를 더 줄임**. :contentReference[oaicite:9]{index=9}

#### 3.1.2 외부조인(OUTER JOIN) 주의
외부조인에서는 NULL 보존 때문에 **전이/푸시가 결과를 바꿀 수 있어 제한된다.**  
NOT NULL 제약이 있거나 옵티마이저가 안전하다고 판단할 때만 이동한다. :contentReference[oaicite:10]{index=10}
...
- 반대로, 푸시가 **의미를 깨는 상황(외부조인·NULL·비결정 함수)** 이면  
  해당 QB에 `NO_QUERY_TRANSFORMATION` 또는 버전별 `NO_PUSH_PRED` 계열로 보호. :contentReference[oaicite:11]{index=11}

---

### 3.2 뷰 머지 (View Merging — Simple / Complex)
...
복합 머지는 **집계/분석/집합연산이 껴 있으면 제한**된다. :contentReference[oaicite:12]{index=12}

#### 3.2.2 Simple View Merge 예
```sql
SELECT /*+ qb_name(main) */
       s.sales_id, p.brand
...
#### 3.2.3 Complex View Merge가 막히는 대표 조건
...
- 머지 유도: `MERGE(@vs)`  
- 머지 차단: `NO_MERGE(@vs)`  
- 보안 이슈 시: `optimizer_secure_view_merging` 확인. :contentReference[oaicite:14]{index=14}

---

### 3.3 서브쿼리 언네스트 (Subquery Unnesting)

오라클은 언네스트를 통해 “서브쿼리 기반 논리”를 **조인 기반 논리로 환원**해  
더 많은 조인 전략(HJ/SMJ/NL/Bitmap/Star 등)을 열어준다. :contentReference[oaicite:15]{index=15}

#### 3.3.1 EXISTS / IN → SEMI JOIN
...
- 스칼라 캐시가 있어도, **외부 로우 수가 커지면** 조인+집계가 더 싸다. :contentReference[oaicite:18]{index=18}

#### 3.3.4 제어
- 강제: `UNNEST(@qb)`  
- 차단: `NO_UNNEST(@qb)`  
- 특히 NULL 의미(3값 논리)나 상관관계가 민감할 때는 차단이 필요. :contentReference[oaicite:19]{index=19}

---

### 3.4 OR-EXPAND (USE_CONCAT) / INLIST ITERATOR

#### 3.4.1 OR-EXPAND 원리
`OR` 조건은 한 번의 인덱스 경로로 커버하기 어렵다.  
옵티마이저는 이를 **`UNION ALL`로 분기**해 각 분기별 최적 경로(다른 인덱스)를 탐색한다.  
오라클에서 이 변환이 **OR Expansion**이며 힌트가 `USE_CONCAT`다. :contentReference[oaicite:20]{index=20}
...
#### 3.4.2 OR-EXPAND가 특히 좋은 상황
- 각 OR 분기에 **서로 다른 인덱스가 명확히 존재**하고  
- 분기 선택도가 높아 **각 분기에서 소량 인덱스 탐색**으로 끝나는 경우. :contentReference[oaicite:21]{index=21}

#### 3.4.3 주의: 분기 폭발
OR 조건 개수가 많으면 UNION ALL 분기가 기하급수로 늘어  
플랜 후보 탐색/실행이 폭발할 수 있다 → `NO_EXPAND`로 차단. :contentReference[oaicite:22]{index=22}

#### 3.4.4 INLIST ITERATOR
`col IN (a,b,c,...)` 는 **한 플랜에서 인덱스 반복 탐색**하는 INLIST ITERATOR로 처리될 수 있다.  
OR로 풀지 않아도 비슷한 이득을 본다. :contentReference[oaicite:23]{index=23}

---

### 3.5 조인 제거 (Join Elimination / Join Back Elimination)
...
### 3.6 집계 푸시다운 (Group-By Pushdown / Join-Group Transformation)

#### 3.6.1 원리
조인 후 집계를 하는 대신  
**사실 테이블에서 먼저 그룹핑/집계 → 축소된 결과를 차원과 조인**하도록 재작성한다.  
특히 스타/스노우플레이크에서 입력 폭을 급감시키는 핵심 변환이다. :contentReference[oaicite:27]{index=27}
...
#### 3.6.3 언제 유리한가
- 사실 테이블이 매우 크고  
- 그룹 키 카디널리티(예: prod_id)가 사실 행 수보다 훨씬 작을 때. :contentReference[oaicite:28]{index=28}

---

### 3.7 WITH(Subquery Factoring) 변환 — INLINE vs MATERIALIZE (Temp Table Transformation)

#### 3.7.1 원리
CTE는 두 가지 방식으로 처리된다. :contentReference[oaicite:29]{index=29}
...
### 3.8 스타 변환 (Star Transformation)

#### 3.8.1 원리
스타 스키마에서  
1) 여러 차원 테이블 조건으로 **ROWID 후보 집합**(보통 비트맵/선택도 높은 인덱스)을 만든 뒤  
2) 사실 테이블에 **한 번에 진입**하는 변환.  
차원 필터 선택도가 높을수록 압도적으로 유리하다. :contentReference[oaicite:31]{index=31}

#### 3.8.2 환경/힌트
- 파라미터: `star_transformation_enabled = TRUE`  
- 힌트: `STAR_TRANSFORMATION` :contentReference[oaicite:32]{index=32}
...
### 3.9 파티션 프루닝 & 파티션-와이즈 조인 (Partition Pruning / Pwise Join)

#### 3.9.1 프루닝 원리
파티션 키 조건을 가능한 한 아래로 내려  
**읽을 파티션 목록을 사전에 확정**하고 나머지는 건너뛴다.  
프레디킷 푸시/뷰 머지/언네스트와 결합되어 프루닝이 완성되는 경우가 많다. :contentReference[oaicite:33]{index=33}
...
**파티션끼리만 조인하도록 계획을 분해**해 로컬/병렬 효율을 높인다. :contentReference[oaicite:34]{index=34}

---

### 3.10 Top-N 최적화 — STOPKEY Pushdown

#### 3.10.1 원리
상위 N건만 필요하면 **전체 정렬/스캔을 하지 않고**  
소트와 카운트에서 **조기 종료(STOPKEY)** 한다.  
실행계획 연산자로 `SORT ORDER BY STOPKEY`, `COUNT STOPKEY`가 나타난다. :contentReference[oaicite:35]{index=35}
...
- 인덱스가 없으면 소트 단계가 `STOPKEY`와 결합되어 **N개 찾는 순간 멈춘다.** :contentReference[oaicite:36]{index=36}

---

### 3.11 표현식 단순화 / 상수 폴딩 / 조건 정규화

#### 3.11.1 원리
- 상수 계산을 선행: `amount > 10*2` → `amount > 20`  
- 동치 변환으로 인덱스 활용 가능성 확대  
- 서로 다른 표현을 “같은 형태”로 맞춰 **카디널리티 추정 오차를 줄임** :contentReference[oaicite:37]{index=37}
...
### 3.12 Materialized View(MV) Query Rewrite

#### 3.12.1 원리
MV가 기반 테이블 질의를 **동치로 대체**해 훨씬 작은 데이터에서 조회하도록 재작성한다.  
세션/시스템 파라미터와 MV 속성이 맞아야 하며, 무결성 모드는 다음이 핵심이다:  
`QUERY_REWRITE_ENABLED`, `QUERY_REWRITE_INTEGRITY`. :contentReference[oaicite:38]{index=38}
...
```
동일 패턴의 원본 질의는 MV로 자동 리라이트될 수 있다. :contentReference[oaicite:39]{index=39}

---

## 4. 변환이 **결과/의미를 바꿀 수 있는** 위험 지점

### 4.1 외부조인과 NULL 보존
- 외부조인에서 프레디킷 전이/푸시는 **NULL 생성 위치가 달라져 결과가 바뀔 수 있다.**  
- NOT NULL 제약이 있거나 옵티마이저가 안전하다고 입증할 때만 이동. :contentReference[oaicite:40]{index=40}

### 4.2 NOT IN vs NOT EXISTS
...
- 뷰 머지는 보안상 안전하지 않으면 제한되며,  
  secure view merging 모드에서만 일어난다. :contentReference[oaicite:43]{index=43}

---

## 5. 변환 진단/관찰/팩트체크

### 5.1 DISPLAY_CURSOR로 변환 노트 확인
```sql
SELECT * FROM TABLE(
...
- **NOTE 영역**: OR Expansion, View Merging, Star Transformation, Join Elimination 등 변환 적용 흔적이 찍힌다. :contentReference[oaicite:44]{index=44}  
- **Predicate Information**: `access`/`filter`가 원래 위치에서 이동했는지 확인. :contentReference[oaicite:45]{index=45}

### 5.2 E-Rows vs A-Rows
변환의 성패는 “이론”이 아니라  
**E-Rows(추정)와 A-Rows(실측)가 근접하는지**로 판단해야 한다.  
추정이 틀리면 변환도 틀린다. :contentReference[oaicite:46]{index=46}

### 5.3 10053 Trace
10053 trace는 **교육/최후 진단용**이다. 운영에서는 부하/보안/노이즈 때문에 필요 구간만 제한적으로 쓴다. :contentReference[oaicite:47]{index=47}

---

## 6. 실전 케이스 스터디

### 6.1 스칼라 서브쿼리 과다 호출 → 언네스트로 해결
```sql
-- 느린 패턴
SELECT p.prod_id,
       (SELECT SUM(amount)
...
- 효과: 외부 로우 수만큼 반복 호출되는 스칼라 실행을 제거. :contentReference[oaicite:48]{index=48}

### 6.2 OR 조건으로 인덱스 못 탐 → OR-EXPAND
```sql
SELECT /*+ USE_CONCAT */ *
FROM   F_SALES
...
```
- 분기별 인덱스 경로가 열리면 **랜덤 I/O가 급감**. :contentReference[oaicite:49]{index=49}
...
- 파티션 키가 함수로 감싸지면 프루닝이 약해진다.  
  예: `TRUNC(sales_dt)=:d`  
  → `sales_dt>=:d AND sales_dt<:d+1` 로 정규화 유도. :contentReference[oaicite:52]{index=52}  
- 키가 뷰/서브쿼리 안에 숨으면  
  **뷰 머지/언네스트가 선행되어야 프루닝이 활성화**될 수 있다. :contentReference[oaicite:53]{index=53}

### 6.5 Top-N이 느릴 때
- `SORT ORDER BY STOPKEY`가 보이면  
  Top-N 조기 종료가 작동 중. :contentReference[oaicite:54]{index=54}  
- 없다면  
  - 인덱스 정렬 경로가 없거나  
...
- 컬럼 그룹은 확장 통계로 결합 선택도를 보정. :contentReference[oaicite:55]{index=55}

### 7.2 변환을 활용해야 할 때
- 대량 스칼라 서브쿼리 → `UNNEST`  
- 복잡 OR, 분기별 인덱스 → `USE_CONCAT`  
- 다회 재사용 CTE → `MATERIALIZE`  
- 고선택도 차원 필터 → `STAR_TRANSFORMATION` :contentReference[oaicite:56]{index=56}

### 7.3 변환을 막아야 할 때
- 외부조인 NULL 의미 손상 우려  
- 뷰 내부 집계/윈도우의 결과 순서/중복 의미가 중요한 경우  
- OR-EXPAND 플랜 분기 폭발  
→ 대상 QB에 정확히 `NO_MERGE`, `NO_UNNEST`, `NO_EXPAND`로 **국지 차단**. :contentReference[oaicite:57]{index=57}

---

## 8. 부록 — 변환별 “한 줄 요약” 미니 레퍼런스
...
- 쿼리 변환은 **“같은 결과를 내는 SQL을 더 좋은 모양으로 바꿔서 CBO가 싸게 실행하도록 하는 단계”**다. :contentReference[oaicite:59]{index=59}  
- 변환은 **통계가 정확할수록 과감해지고**, 통계가 틀리면 변환도 틀린다. :contentReference[oaicite:60]{index=60}  
- 성능 문제를 만나면  
  1) **변환 적용 여부를 Note/Predicate로 확인**하고  
  2) **E-Rows vs A-Rows로 추정 품질을 검증**하며  
  3) 필요하면 **QB 단위로 국지 힌트 제어**하라. :contentReference[oaicite:61]{index=61}  
