...
### 실전 1: 결과와 로그를 함께 만들기 (map + side log)
...
### 실전 2: 에러 태그와 map 결합 (map + tagged tuple)
...
## 리스트를 줄여 하나의 값으로 만들기 (reduce/fold)
...
### 표준 파생: sum, product, count, any?/all?, join
...
## 리스트 모듈 실전 (List / Enum / Stream)

이제 표준 라이브러리 차원에서 **List / Enum / Stream** 이 어떤 역할을 하는지 정리한다.

---

### List vs Enum vs Stream — 역할 비교
...
## 리스트와 친해지기 (Idioms, Pitfalls, Exercises)
