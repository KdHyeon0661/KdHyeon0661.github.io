...
### e-폴딩 시간(e-folding time)

\(|k|^{-1}\): 값이 \(e\)배(성장) 또는 \(1/e\)배(감소) 되는 시간.

---

## 데이터에서 모델 파라미터 추정(로그 선형화)
...
## 수치해석: Euler, Heun, RK4 비교(간단 스니펫)

아날리틱 해가 있어도 **불균일 \(k(t)\)**, **잡음 포함 데이터 동화** 등에서는 수치적분이 필요하다.

```python
# y, y(0)=y0 의 수치해석 비교: Euler, Heun(개선 오일러), RK4
...
# 비교, 혹은 그래프(환경에 따라 matplotlib 사용)
