...
## AngularJS(2010~2016): 브라우저 MVC의 실험
...
## Angular(2+)(2016~): 엔터프라이즈급 풀스택 프론트
...
@Component({
  selector: 'app-hello',
  template: {% raw %}`<input [(ngModel)]="name"><p>Hello {{name}}</p>`{% endraw %}
})
...
## Svelte(2019~): 컴파일러가 만드는 런타임 없는 반응성
