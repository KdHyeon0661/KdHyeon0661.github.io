...
### **nonce 재사용** 케이스: 두 파일 XOR로 평문 관계 복구
...
### Windows DPAPI (CryptProtectData) — Python ctypes 예시
...
### macOS Keychain (CLI `security`) — 개념
...
### Linux/프리폼 — libsecret/gnome-keyring (개념)
...
### 토큰 버킷(로컬 API/해킹 관문 차단) — Python Flask 예
...
### .NET (Minimal API) — Rate Limiting 미들웨어
