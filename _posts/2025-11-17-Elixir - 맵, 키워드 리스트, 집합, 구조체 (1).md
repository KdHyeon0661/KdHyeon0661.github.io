...
### 8.2.4 키워드 리스트를 “데이터”로 쓰지 말아야 할 때
...
## 8.4 패턴 매칭하기, 맵 수정하기 (원리 편)

맵은 **부분 패턴 매칭**이 가능하다.  
즉, “맵에 이 키들이 반드시 있어야 한다”는 것을 패턴으로 선언할 수 있다.

### 8.4.1 부분 패턴 매칭과 필수 키
...
### 8.5.2 중첩 업데이트: `put_in/3`, `update_in/3`
...
## 8.8 실전 미니 프로젝트: 설정 파이프라인 (Keyword → Map → Struct)
...
#   - new(attrs) :: {:ok, %User{}} | {:error, reason}
#   - 이메일 정규식을 이용해 형식 확인
