...
## 1. 스칼라장에 대한 선적분: \(\displaystyle \int_C f\,ds\)
...
## 2. 벡터장에 대한 선적분: \(\displaystyle \int_C \mathbf{F}\cdot d\mathbf{r}\)
...
### 3.2 선적분의 기본정리(Fundamental Theorem for Line Integrals)
...
## 4. 평면형식 \( \displaystyle \int_C P\,dx + Q\,dy \) 과 Green 정리(미리보기)
...
## 7. 좌표계/특수 경로: 극좌표·파라메트릭

### 7.1 극좌표에서 \( \int_C Pdx+Qdy \)
일반적으로 \(x,y\) 의 함수로 계산하되, 경로가 원/부채꼴이면 **자연 매개화**로 단순화.

### 7.2 예제 I — 반지름 \(R\) 원호 \(\theta=\theta_1\to\theta_2\)
