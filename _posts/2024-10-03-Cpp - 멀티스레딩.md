...
### `std::unique_lock`: 유연함(지연 잠금, 해제/재잠금, CV 연계)
...
### 여러 락을 한 번에: `std::scoped_lock`/`std::lock`
...
### C++20 `std::osyncstream` (줄 단위 자동 동기화)
...
## 병렬화의 한계: Amdahl’s Law & False Sharing
...
## 스레드 취소/정지: C++20 `std::jthread` + `stop_token`
