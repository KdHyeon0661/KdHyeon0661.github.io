...
## `std::unique_ptr<T, Deleter>` — “단일 소유”의 표준
...
## `std::shared_ptr / std::weak_ptr` — 공유 소유와 순환 차단
...
## **예외 안전**: 강한 보장(Strong), 기본 보장(Basic)
...
## 고급: `unique_resource`(범용 RAII)와 `scope_exit`
...
## 문자열/버퍼/뷰: `string_view`/`span`/`optional`
