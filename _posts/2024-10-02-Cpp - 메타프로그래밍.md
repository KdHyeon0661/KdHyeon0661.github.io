...
### 1.4 범용 람다 파라미터(C++14): “템플릿 없는 템플릿”
...
### 2.2 `std::declval`과 조합: 의존 표현식의 타입 구하기
...
## 3) `decltype(auto)`: 값/참조/const **그대로** 반환
...
## 5) `constexpr` / `consteval` / `constinit`

### 5.1 `constexpr`: **컴파일 타임에도** 평가 가능
...
### 5.3 `constinit`(C++20): 정적 저장수명의 **정확 초기화** 보장
...
## 6) 메타프로그래밍: 템플릿 재귀 vs `if constexpr`
...
### 6.2 `constexpr` + `if constexpr` (C++17)
...
## 8) 레퍼런스 붕괴(Reference collapsing)와 전달 참조
...
### 8.2 전달 참조(Forwarding reference)
...
## 11) SFINAE 없는 오버로드 해석 트릭: `if constexpr`
