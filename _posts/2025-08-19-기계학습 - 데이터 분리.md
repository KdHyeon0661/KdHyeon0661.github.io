...
## 교차검증(Cross Validation) — 변동성 완화와 데이터 극대 활용
...
## 특수 상황별 분할법 — 누수 방지의 핵심

### 그룹이 있는 데이터(Grouped CV)
...
## 데이터 누수(Data Leakage) — 가장 흔한 실패 원인
...
## Train/Validation/Test — 3-Way 분리와 교차검증 결합
...
## 파이프라인 + ColumnTransformer — “전처리 누수 0” 실전
...
## 종합 예제 — 이진 분류 전체 워크플로(불균형 + 파이프라인 + Nested CV + 최종 Test)
