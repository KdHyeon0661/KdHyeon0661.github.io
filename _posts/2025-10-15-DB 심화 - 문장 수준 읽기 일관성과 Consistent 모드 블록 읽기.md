...
category: DB 심화
---
# Oracle의 **문장 수준 읽기 일관성(Statement-Level Read Consistency)** 과 **Consistent 모드 블록 읽기(CR Read)** (19c 기준)
...
결과적으로 세션 A의 SUM(val)은 **전부 0**으로 계산된다(문장 시작 시점 기준). 세션 B의 커밋은 **세션 A의 다음 문장부터** 보이게 된다.
...
1. **ORA-01555** 가 보이면 `UNDO_RETENTION` / Undo TS 용량 / DML·보고서 시간 분리 / 쿼리 튜닝을 점검하세요.
2. **consistent gets 과다** 시에는 스캔 폭/조인 순서/인덱스 설계/커버링 인덱스/SELECT 컬럼 축소를 고려해 보세요.
3. **read by other session** 이 빈번하면 인덱스/파티셔닝/캐시 정책(KEEP/RECYCLE)으로 히트율 개선을 검토하세요.
4. **buffer busy waits** 는 핫 블록/ITL 부족/쓰기경합의 신호로, 키 분산, INITRANS/PCTFREE, 샤딩 카운터 등이 해결책이 될 수 있습니다.
5. **지연 클린아웃 빈발** 시 해당 세그먼트의 커밋 직후 읽기/쓰기 패턴을 확인하고 ITL/헤더 여유, 경합 완화, 파티셔닝을 검토하세요.
6. **SELECT FOR UPDATE** 사용 구간은 Current read 증가로 경합/대기 양상이 변동하므로 필요한 곳에서만 사용하세요.
...
Oracle 데이터베이스의 강력한 **문장 수준 읽기 일관성**은 모든 SELECT 문에 대해 기본적으로 보장되는 핵심 기능입니다. 이 일관성은 단순한 개념이 아니라, **쿼리 시작 SCN**을 기준으로 **Undo 세그먼트**, **블록 헤더의 ITL**, **행 잠금 바이트**, **트랜잭션 슬롯**이 유기적으로 협업하는 정교한 **Consistent Read(CR)** 메커니즘으로 구현됩니다.

이 메커니즘 덕분에 장시간 실행되는 보고서 쿼리도 중간에 발생하는 다른 트랜잭션의 커밋으로부터 영향을 받지 않고, 시작 시점의 일관된 스냅샷을 유지할 수 있습니다. 그러나 이 같은 장점은 **Undo 공간 관리**와 **CR 블록 생성에 따른 오버헤드**라는 비용을 동반합니다. **ORA-01555 "snapshot too old"** 오류는 바로 이 시스템의 한계에서 발생하는 대표적인 현상입니다.

실제 성능 튜닝에서는 consistent gets 수치 자체보다, 그것을 유발하는 **비효율적인 쿼리 패턴**, **부족한 Undo 보존 시간**, **핫 블록에 의한 버퍼 경합**을 종합적으로 진단해야 합니다. 적절한 인덱스 설계, 파티셔닝, Undo 테이블스페이스 구성, 그리고 애플리케이션의 배치 작업 시간 조정을 통해 CR 메커니즘의 장점을 최대한 활용하면서도 그 잠재적 비용을 효과적으로 관리할 수 있습니다.

요약하면, 오라클의 Consistent Read는 데이터 일관성이라는 근본적인 요구사항을 충족시키는 동시에 고성능을 유지할 수 있도록 설계된 정교한 엔진입니다. 이를 이해하는 것은 오라클 데이터베이스의 동작을 깊이 있게 파악하고, 안정적이며 효율적인 애플리케이션을 설계하는 데 필수적인 토대가 됩니다.