---
layout: post
title: Svelte - 예제 미니 프로젝트 (4)
date: 2025-10-08 15:30:23 +0900
category: Svelte
---
# 예제 미니 프로젝트 (4) — **실시간(웹소켓 / SSE) 알림 보드**

> SvelteKit로 **실시간 알림 보드**를 0 → 1로 구축한다.  
> - **전송 방식**: WebSocket(양방향, 채널/존재 감지) + SSE(단방향, 서버리스 친화) 둘 다 제공  
> - **알림 모델**: 구독 토픽(채널), 읽음/안읽음, 뱃지 카운트, 중요도(priority)  
> - **확장성**: 멀티 인스턴스 대비 Redis Pub/Sub 연결(선택)  
> - **회복력**: 재접속, 하트비트, 백프레셔, 메시지 드롭 전략  
> - **보안**: 토큰 검증, 레이트 리미팅, 입력 검증(Zod), CORS/Origin, 권한(채널 접근)  
> - **클라이언트 UI**: 종(bell) 뱃지, 알림 피드, 필터, 토스트, 토픽(채널) 구독/해제  
> - **배포**: Node(WS) / 서버리스(SSE) / 에지 제약

---

## 0. 아키텍처 설계

### 선택지 비교: **WebSocket vs SSE**
| 항목 | WebSocket | SSE (Server-Sent Events) |
|---|---|---|
| 방향성 | 양방향 | 서버→클라이언트 단방향 |
| 브라우저 지원 | 광범위 | 매우 광범위 |
| 서버 환경 | 노드/장기연결 지원 필요 | 서버리스/에지에서도 쉬움 |
| 메시지 크기/빈도 | 빈번/상호작용 O | 알림/브로드캐스트 최적 |
| 부하 분산 | 스테이트풀(연결 유지) | 상대적으로 단순 |
| 사용 예 | 존재감지, 타이핑, 확인 응답 | 뉴스/알림 feed, 로그 스트림 |

> 이 가이드는 **두 가지를 모두 구현**한다. 환경에 맞춰 선택/혼용 가능(예: 알림은 SSE, 상호작용은 WS).
...
### 알림(Notification) 스키마
- `id: string` — 고유 ID(ulid/cuid)  
- `topic: string` — 채널/주제(예: `"sales"`, `"system"`, `"user:123"`)  
- `title: string` — 제목  
- `body: string` — 메시지  
- `priority: 'low'|'normal'|'high'` — 우선순위  
- `ts: number` — Unix ms (정렬/중복 제거 기준)  
- `meta?: Record<string, any>` — 링크/아이콘/CTA 등  
- `read?: boolean` — 읽음 상태(클라/서버 양쪽 관리 가능)

### 메시지 타입: **서버→클라**
```ts
type ServerEvent =
  | { type: 'push'; data: Notification }
  | { type: 'bulk'; data: Notification[] }      // 초기 동기화
  | { type: 'ack'; ids: string[] }              // 서버 처리 확인
  | { type: 'presence'; topic: string; count: number }  // 구독자수
  | { type: 'pong' }                             // 하트비트 응답
...
type ClientEvent =
  ...
  | { type: 'read'; ids: string[] }              // 읽음 확인
  | { type: 'ping' }                             // 하트비트
  | { type: 'emit'; data: Notification };        // 고급: 클라→서버 생성(권한 필요)
```
...
src/
  lib/
    schema/
      notif.ts              # Zod 스키마
    server/
      bus.ts                # InMemoryBus / RedisBus(선택)
      auth.ts               # 토큰 검증
      rate.ts               # 레이트 리미트/안티 스팸
      ws.ts                 # WS 핸들러(노드)
      sse.ts                # SSE 브로드캐스트 유틸
    stores/
      notif.ts              # 클라 스토어(알림/구독/상태)
    components/
  ...
  routes/
    api/
      notify/+server.ts     # REST로 알림 발행
      ws/+server.ts         # WebSocket 업그레이드 엔드포인트 (adapter-node)
      sse/+server.ts        # SSE 스트림
      history/+server.ts    # 페이지네이션(오프라인/초기 로드)
```

> **WS는 `adapter-node`/장기연결 제공 환경**에서만 동작. 서버리스/에지는 SSE를 사용.
...
## 4. 서버 이벤트 버스(InMemory / Redis)

### 4.1 InMemory 버스(단일 인스턴스용)
```ts
...
### 4.2 (선택) Redis Pub/Sub 스텁
```ts
// src/lib/server/bus-redis.ts (선택)
...
> 멀티 인스턴스에서 **publish**만 Redis로 보내고, 각 인스턴스에서 **subscribe**로 수신하면 스케일아웃 가능.
...
export async function requireToken(event: RequestEvent): Promise<{ uid: string, topics: string[] }> {
  // 예시: 쿠키의 세션에서 토큰 확인(또는 Authorization 헤더 Bearer)
  ...
  const uid = m[1];
  // 권한/주제 제한 로직(조직/역할 기반) — 데모로 모든 topic 허용
  return { uid, topics: [] };
}
```
...
export const POST: RequestHandler = async (event) => {
  try {
    ...
    // 권한 체크: uid가 topic에 발행할 권한이 있는지 검증(생략/데모)
...
> 프로덕션에서는 발행 시 DB에도 기록(내역/리플레이/오프라인 회복). 여기선 메모리+히스토리 API로 대체.

---

## 7. 히스토리(초기 로드/오프라인 동기화) — `/api/history`
...
export const GET: RequestHandler = async (event) => {
  ...
  const p = z.object({
    ...
    cursor: z.string().optional()  // ts 기준(문자열), 이하 과거 탐색
  ...
  let rows = ring.slice().reverse(); // 최신→과거
...
> **발행** 시 `appendHistory()`도 호출하도록 연결(WS/SSE에서 같이 처리; 예제에서 WS/SSE 구간에 포함).
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      for (const t of topics) {
        ...
        // 존재감지(옵션): 구독자 수를 주기적으로 보낼 수도 있음 — 단순화
      }
...
// 발행 시 history 적재 연결(REST/WS에서 호출하는 예시)
// 실제로는 /api/notify, WS 핸들러에서 appendHistory(n) 호출
```

---

## 9. WebSocket 핸들러 — `/api/ws` (Node 어댑터)

> **전제**: `adapter-node` 사용. `ws` 패키지 설치.
```bash
pnpm add ws
```

### 9.1 SvelteKit에서 WS 업그레이드
- `src/routes/api/ws/+server.ts`에서 **`upgrade`**를 직접 처리.
- Node 서버로 배포 시, 생성된 `build/index.js` 내부의 `server`를 가져다 커스텀 서버를 구성할 수도 있다.  
  여기서는 **라우트 레벨 업그레이드** 예제를 사용.
...
const listeners = new WeakMap<WebSocket, Set<string>>(); // 소켓별 구독 토픽
...
export const GET: RequestHandler = async (event) => {
  ...
  // @ts-ignore Node 타입
  const { socket, response } = DenoOrNodeCompatibleUpgrade(event); // 아래 헬퍼 참고
  if (!wss) {
...
/** Node/Deno 호환 업그레이드 헬퍼 — 여기선 Node만 사용(간소화) */
...
> 구현 단순화를 위해 일부 플랫폼 별 난점을 추상화했다. **adapter-node** 환경에서 `request.raw` 접근이 가능한지 확인하고 필요 시 `hooks.server.ts`에서 커스텀 서버를 구성해도 된다.
...
export function connectSSE(topics: string[], token: string) {
  ...
  function open() {
    ...
    // 토큰은 쿠키/헤더가 권장. SSE는 커스텀 헤더 불가 → 쿠키 기반 인증 추천
...
export function connectWS(topics: string[], token: string) {
  ...
  function open() {
    ws = new WebSocket(`${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.host}/api/ws`, ['json']); // subprotocol 예시
    ws.onopen = () => {
    ...
    ws.onmessage = (ev) => {
      ...
      // ack/pong/error/presence 등 추가 처리
    };
    ws.onclose = () => {
      connected.set('error');
      clearInterval(hb);
...
<button class="bell" on:click={onClick} aria-label="알림">
  🔔
  {#if $unread > 0}
...
<span class="status" aria-live="polite">
  {#if $connected==='open'}● 실시간{:else if $connected==='connecting'}… 연결중{:else}○ 오프라인{/if}
</span>
...
<ul class="feed">
  {#each $feed as n (n.id)}
    <li class:unread={!n.read}>
      ...
      <p>{n.body}</p>
      {#if n.meta?.href}
        <a href={n.meta.href} target="_blank" rel="noreferrer">자세히 보기 →</a>
      {/if}
    </li>
  {/each}
</ul>
<button on:click={() => onRead($feed.filter(x=>!x.read).map(x=>x.id))}>모두 읽음 처리</button>
...
## 14. 페이지 구성(+page.svelte) — 선택한 전송 방식으로 연결

```svelte
<!-- src/routes/+page.svelte (데모 홈) -->
<script lang="ts">
  ...
  function handleRead(ids: string[]) {
    // WS면 ws로 보내고, SSE면 REST로 보내는 식으로 구현(데모에선 스토어만)
    markRead(ids);
  }
</script>
...
<header class="bar">
  ...
  <NotificationBell on:click={() => { /* 피드 토글 등 */ }} />
</header>
...
<section class="send">
  ...
  <form on:submit|preventDefault={async (e: SubmitEvent) => {
    ...
    const body = {
      id: `n_${now}`,
      topic,
      title: String(fd.get('title') ?? '제목'),
      body: String(fd.get('body') ?? '본문'),
      priority: 'normal',
      ts: now,
...
## 15. WS/SSE에서 히스토리 적재 연결

> **중요**: 실전에서 **발행 시** 영속 저장(예: DB, 큐)을 해두면 클라이언트 초기 진입 시 `/api/history`로 **최근 N개**를 받아 즉시 피드를 채울 수 있다. SSE/WS 연결 직후 `bulk` 이벤트로 초기 세트를 내려도 된다.

- 위 `/api/notify`에서 `bus.publish(n)` **직후** `appendHistory(n)` 호출을 추가하거나,  
- WS ‘emit’ 처리 시 이미 `appendHistory` 호출.

---

## 16. 백프레셔 & 메시지 드롭 정책

- **WebSocket**: `ws.bufferedAmount` 가 임계치(예: 1MB)를 넘으면 **낮은 priority** 메시지를 드롭하거나, **가장 오래된** 큐를 제거한다.
- **SSE**: HTTP 스트림이라 자연 백프레셔가 걸리지만, 전송 속도가 매우 느린 클라이언트는 **타임아웃으로 연결 종료** 후 재연결 시도하게 한다.
- **서버**: 토픽별 브로드캐스트 시 개별 소켓의 전송 상황을 고려(지나치게 느린 소켓 정리).
...
1) **인증/인가**  
   - 토큰(JWT/세션)에서 **주체(uid)**와 **권한(roles)** 파생  
   - 토픽 네이밍 규칙(예: `user:<uid>` 채널은 **자기 자신만** 접근) 강제  
   - 퍼블릭 토픽(`system`)은 읽기만 허용, 발행은 관리자만

2) **레이트 리미팅**  
   - `/api/notify` (발행) IP/uid 단위 제한  
   - WebSocket `emit`/`subscribe`에 액션별 별도 제한

3) **입력 검증(Zod)**  
   - 모든 외부 입력은 Zod로 구문·길이 제한  
   - HTML/Markdown 렌더링 시 XSS 방지: **텍스트만** 표시, 링크 rel 속성

4) **CORS/Origin**  
   - 동일 출처 권장. 필요 시 CORS 화이트리스트  
   - WebSocket은 `Origin` 검사, SSE는 쿠키 자격증명 사용 시 `SameSite` 정책 확인
...
- **Redis Pub/Sub**  
  - 각 인스턴스에서 로컬 연결을 관리  
  - 발행 시 Redis에 publish → 모든 인스턴스가 수신하여 **자신에게 붙은** 클라이언트에게 브로드캐스트  
- **Presence(존재감지)**  
  - 토픽별 카운트는 로컬 + Redis `INCR/DECR` 또는 `SET`로 **전역 합산**  
  - 주기적으로 조정(유실 대비)
...
- **하트비트**  
  - WS: ping/pong 20~30초 간격. **미응답** 시 종료  
  - SSE: 20초마다 `:` 주석 라인 또는 dummy event  
- **재연결**: 지수 백오프(최대 20초), **Jitter** 추가 추천  
- **메시지 순서**: 단일 토픽 내에서는 `ts` 기준 정렬로 정합성 유지  
- **리플레이**: `Last-Event-ID`(SSE) 활용 시 유실 복원 가능(여기선 단순화)

---

## 20. 성능

- **JSON 크기** 최소화(필드명 짧게/불필요 데이터 제거)  
- **토픽 분리**: 고빈도와 저빈도 채널 분리 → 저빈도 구독자에게 고빈도 스팸 회피  
- **배치 전송**: 밀리초 단위로 쏟아지는 이벤트는 **10~100ms** 윈도우로 묶어 `bulk`로 전송  
- **GC 압력**: 대형 메시지는 **링크**만 보내고, 상세는 REST로 가져오기

---

## 21. 테스트 시나리오

1. **초기 연결**(SSE/WS): `hello` 수신, 상태 `open` 전환  
2. `/api/notify` 호출 → 피드에 즉시 표시(해당 토픽 구독자만)  
3. **읽음 처리**: 버튼 클릭 → badge 0  
4. **재접속**: 서버 강제 재시작/네트워크 차단 → 재연결 동작  
5. **레이트 리미트**: 1분 60회 이상 발행 → 429  
6. **권한**: `user:other` 토픽 발행/구독 시도 → 차단

---

## 22. 배포 가이드

- **SSE 전용(서버리스/에지)**: Vercel/Netlify/Cloudflare에서 동작 쉬움  
- **WebSocket(노드 런타임)**: `adapter-node` + PM2/Docker. L4(로드밸런서)에서 **스티키 세션** 필요  
- **멀티 리전**: Redis Global/Upstash, 또는 지역별 클러스터 + Edge 라우팅  
- **프록시**: Nginx/Haproxy에서 `upgrade` 헤더 전달 설정
...
> 모니터링 시 위 지표를 로그 기반으로 산출해 대시보드에 표시한다.

---

## 24. 요약 & 체크리스트

- [x] **전송 레이어**: WS(양방향) + SSE(단방향) 둘 다 구현  
- [x] **버스**: InMemory/Redis로 브로드캐스트  
- [x] **API**: `/api/notify` 발행, `/api/history` 초기 로드  
- [x] **클라 스토어/UI**: 뱃지/피드/읽음/연결상태  
- [x] **회복력**: 재연결, 하트비트, 백프레셔  
- [x] **보안**: 토큰, 권한, 레이트 리미트, 검증  
- [x] **배포**: 서버리스(SSE)/노드(WS) 선택
...
1. `pnpm add ws zod`  
2. `lib/schema/notif.ts`, `lib/server/bus.ts`, `lib/server/auth.ts` 생성  
3. `routes/api/notify/+server.ts`, `routes/api/history/+server.ts` 생성  
4. `routes/api/sse/+server.ts` (SSE), `routes/api/ws/+server.ts` (WS) 생성  
5. 클라: `stores/notif.ts`, `client/sse.ts`, `client/ws.ts`  
