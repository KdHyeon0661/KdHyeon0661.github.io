...
## 4.2 Docker API 호출 예시(unix socket)
```bash
...
## 8.1 로컬 풀스택(Compose) — 이미지·컨테이너·네트워크·볼륨 종합
```yaml
# docker-compose.yaml
services:
  web:
...
## 8.2 레지스트리 고정(digest pinning) 배포
```bash
