...
- {% raw %}`transition:name={{ 옵션 }}`{% endraw %} 형태로 **등장/퇴장** 모두에 적용된다.
...
## 애니메이션(animate:) & FLIP — “재배치”를 자연스럽게
...
- {% raw %}`animate:flip={{ duration: f => Math.sqrt(f) * 400 }}`{% endraw %}처럼 **인자**로 duration/easing 제어 가능(`f`는 거리 비율).

### 상태 변화 애니메이션(키프레임·CSS 변수)
...
## `svelte/transition` 고급 — crossfade로 두 리스트간 이동
...
## `svelte/motion` — 상태를 **부드럽게** 보간(spring/tweened)
...
## 접근성(A11y) 고려 — 모션 감소·포커스 보호·시간 제한
...
## 종합 실습 — “카드 그리드: 필터/재배치 + 스크롤 등장 + 상세 모션”
