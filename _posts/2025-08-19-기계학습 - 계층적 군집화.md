...
## 2. 거리와 링크 함수(Linkage): “군집 간 거리”를 어떻게 정의할까?
...
## 3. 수학적 정당화: 덴드로그램·초메트릭·MST

### 3.1 덴드로그램(Dendrogram)과 초메트릭(ultrametric)
...
## 4. 병합적 알고리즘: Lance–Williams 업데이트(일반형)
...
### 10.3 scikit-learn API: AgglomerativeClustering (거리 임계 컷팅)
```python
...
# (B) 거리 임계로 컷 (연결 그래프가 여러 컴포넌트가 되면 군집수 결정)
