...
### 2.2 Row Limiting Clause vs ROWNUM

12c+에서는 `FETCH FIRST N ROWS ONLY` / `OFFSET`이 Row Limiting Clause로 네이티브 지원된다. :contentReference[oaicite:1]{index=1}  
...
Top-N을 하려면 “정렬/인덱스 순서”가 먼저, 그 다음이 ROWNUM/Fetch다. :contentReference[oaicite:2]{index=2}  
...
### 3.3 **ROWID 기반 Late Materialization** (가장 얇은 정렬)
...
### 3.4 “Top-N을 더 **일찍** 적용하기” — 조인/가공 이전에 후보 축소
...
### 4.1 전역 Top-N은 **인덱스 + ROWNUM/FETCH**로 대체(정렬 회피)
...
### 4.2 전역 Top-N + 기간 필터: **(sales_dt, amount DESC) 인덱스 + Stopkey**
...
### 4.3 **그룹별 Top-K(예: prod_id별 Top-3)** — 세 가지 패턴 비교
...
### 5.1 정렬 전에 **행폭 줄이기** (Projection Pushdown)
