...
# .template.config 디렉터리와 template.json 추가
...
## 프로젝트 참조·구성 — `dotnet add reference`, `dotnet sln`
...
## NuGet 패키지 관리 — `dotnet add/remove/list package`, `dotnet nuget *`, `dotnet pack`
...
## 런타임/SDK 정보·업데이트 — `--info`, `--list-*`, `sdk check`, `workload *`
