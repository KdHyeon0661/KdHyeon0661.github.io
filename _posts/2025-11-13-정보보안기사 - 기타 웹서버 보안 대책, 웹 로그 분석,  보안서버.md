...
category: 정보보안기사
---
# SECTION 09 웹 서버 취약점 — 06. 기타 웹서버 보안 대책(아파치 기준) · 07. 웹 로그 분석 · 08. 보안서버(Security Server) 구축

## 06. 기타 웹서버 보안 대책(아파치 httpd 기준)
...
### 6.8 TLS 베스트 프랙티스(요지—세부는 08에서 전체 예시)
```apache
# 현대형 프로토콜/암호 스위트
...
## 08. 보안서버(Security Server) 구축 — Apache + TLS(HTTPS)
...
### 8.2 인증서 발급(예: Let’s Encrypt + Certbot)
```bash
...
## 부록 A) OWASP ModSecurity CRS 적용(선택)
```bash
