...
## `git commit --fixup/--squash` 정확히 알기
...
```bash
# 리베이스 때 autosquash 기본 활성화

git config --global rebase.autosquash true

# git pull의 기본 동작을 rebase로

git config --global pull.rebase true

# rebase 시 변경파일을 임시 저장/복구(충돌 회피에 도움)

git config --global rebase.autoStash true

# push할 때 upstream이 다르면 안전 확인
