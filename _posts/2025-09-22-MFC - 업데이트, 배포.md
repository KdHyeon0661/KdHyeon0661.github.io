...
# MSIX: 패키징·Capabilities·데이터 마이그레이션
...
## App Installer(.appinstaller)로 자동 업데이트
...
### **Package Support Framework(PSF) Fixup** 활용
...
# Squirrel.Windows: 사용자 프로필 설치·델타 업데이트
...
### 앱 빌드 후 `Squirrel --releasify`

PowerShell:
```powershell
# nupkg 준비 (Squirrel-aware)

nuget pack MyApp.nuspec -Version 1.2.3 -OutputDirectory artifacts

# releasify: Setup.exe + RELEASES + delta 패키지 생성
...
# WinSparkle: AppCast 기반 가벼운 자동 업데이트
