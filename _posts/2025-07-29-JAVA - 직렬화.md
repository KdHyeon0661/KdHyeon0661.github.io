...
### 3.1 클래스 진화(필드 추가/삭제/타입 변경)와 호환성
...
## 5. 커스텀 직렬화 — writeObject / readObject / serialPersistentFields

### 5.1 패턴과 시그니처 (정확히 일치해야 함)
```java
...
### 5.3 serialPersistentFields — 스트림 필드 집합 제어
```java
...
## 7. `readResolve` / `writeReplace` — 싱글턴/프록시/불변 보존
...
## 10. 보안 — 절대 **신뢰할 수 없는 입력**을 역직렬화하지 말 것
...
### 19.2 버전 필드로 포맷 제어(Externalizable 스타일)
```java
