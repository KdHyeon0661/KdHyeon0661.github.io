...
## 1. 초기 침투(Initial Access) — 방어 우선 가이드
...
## 2. 권한 상승(Privilege Escalation) — **구성 오류**를 찾고 고친다

### 2.1 Linux: 잘못된 sudoers / SUID / PATH 하이재킹
...
### 2.2 Windows: 서비스 오남용 / 경로 인용 누락 / 과다 ACL

#### (A) 서비스 경로 인용 누락(Unquoted Service Path)
...
## 3. 자격증명 접근(Credential Access) — 접근 자체를 차단하라
...
## 5. 측면 이동(Lateral Movement) — 통신면 제한·로그 상관
...
## 6. 방어 회피(Defense Evasion) — 로깅 끄기 시도 탐지
...
## 8. 메모리 취약점 기초(개념) — **취약 코드 → 방어·수정** 중심

### 8.1 취약 C 프로그램(입력 검증 없음) — **실습은 랩 전용**
```c
