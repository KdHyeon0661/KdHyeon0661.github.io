...
## 초기 침투(Initial Access) — 방어 우선 가이드
...
## 권한 상승(Privilege Escalation) — **구성 오류**를 찾고 고친다
...
## 자격증명 접근(Credential Access) — 접근 자체를 차단하라
...
## 측면 이동(Lateral Movement) — 통신면 제한·로그 상관
...
## 방어 회피(Defense Evasion) — 로깅 끄기 시도 탐지
...
## 메모리 취약점 기초(개념) — **취약 코드 → 방어·수정** 중심

### 취약 C 프로그램(입력 검증 없음) — **실습은 랩 전용**
