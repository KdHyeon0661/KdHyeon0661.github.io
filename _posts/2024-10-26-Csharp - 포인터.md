...
### 4.1 고정 버퍼(fixed buffer) 필드 (unsafe struct 내)
```csharp
...
## 6. sizeof / stackalloc / fixed vs stackalloc
...
## 8. P/Invoke, StructLayout, Marshal, GCHandle

### 8.1 StructLayout — 레이아웃 고정
```csharp
...
## 10. ReadOnlySpan<T>, Span<T>, Memory<T>, IMemoryOwner<T>
...
## 11. 함수 포인터(delegate*) — 호출 오버헤드 최소화
...
## 16. 실전: 바이트 시퀀스에서 구조체 파싱

### 16.1 unsafe 캐스팅 (정렬 보장 가정)
```csharp
using System.Runtime.InteropServices;
