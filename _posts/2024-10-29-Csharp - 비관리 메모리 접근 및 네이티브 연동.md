...
## 2. Marshal: 전통적인 비관리 메모리/마샬링 API
...
## 3. NativeMemory: .NET 6+ 현대적 비관리 메모리 API
...
## 6. 구조체 레이아웃: ABI 일치가 생명

### 6.1 Sequential/Explicit
```csharp
...
## 12. 동적 로딩: `NativeLibrary` (cross-platform)
...
## 13. 함수 포인터(`delegate*`) (고급/unsafe, .NET 5+)
...
## 14. 고성능 입출력: Span + P/Invoke 혼합 레시피

### 14.1 네이티브 압축 API 호출(출력 버퍼 직접 제공)
```csharp
...
## 18. 실전 예제: UTF-8 파일에 안전히 쓰기 (무할당 경로 최소화)
