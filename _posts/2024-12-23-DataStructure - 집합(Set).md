...
## 가족들: `set` vs `multiset` vs `unordered_set` vs `unordered_multiset`
...
## 범위 탐색 패턴 — `lower_bound`/`upper_bound`/`equal_range`
...
## 내부 구조 심화 — 교육용 Red-Black Tree 기반 `set` 구현
...
## 부가: 순서 통계(Order Statistics)가 필요할 때
