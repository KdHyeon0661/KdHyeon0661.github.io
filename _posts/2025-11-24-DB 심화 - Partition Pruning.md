...
## 0. 왜 Partition Pruning이 “성능 절반”을 먹고 들어가는가
...
## 1. Partition Pruning의 분류와 플랜 신호

### 1.1 Static(정적) Pruning
...
## 3. 기본(정적) Partition Pruning

### 3.1 RANGE 파티션: 상수/바인드 기반
...
### 4.4 Subquery Pruning이 잘 안 먹는 케이스
...
## 5. Join Filter / Bloom-Based Pruning
...
### 5.2 차원 → 사실 해시 조인에서 Join Filter 생성
...
### 5.3 달력 차원 조인으로 “날짜 집합 기반” 런타임 프루닝
...
### 8.1 1단계: 플랜에서 “프루닝 신호” 찾기
- `PARTITION RANGE SINGLE/ITERATOR/JOIN`
- `PSTART/PSTOP = 숫자 / KEY / KEY(INLIST)`
- `JOIN FILTER CREATE/USE`

### 8.2 2단계: 실제 실행 후 통계 포함 플랜 확인
