...
### 자율 시스템(AS)과 ISP 계층 구조

인터넷은 하나의 거대한 “평면 네트워크”처럼 보이지만,
실제로는 **수만 개의 자율 시스템(AS, Autonomous System)** 이
**정책(BGP 정책)** 으로 서로 연결된 구조다.

- **AS 정의**
  - 하나의 운영 조직이 관리하는 IP prefix + 라우터 집합
  - 외부에 **일관된 라우팅 정책(one routing policy)** 을 제시
  - 각 AS는 고유한 **ASN(Autonomous System Number)** 을 가진다.

- **규모**
  - 16-bit ASN에서 시작했지만, 이후 32-bit ASN으로 확장
  - 2025년 기준, 전 세계에 **대략 12만 개 정도의 ASN이 할당**되어 있다.

#### ISP 계층: Tier-1 / Tier-2 / Tier-3, Stub / Transit

연구와 운영 보고서에서는 전통적으로 인터넷을 다음과 같이 설명한다.
...
**계층 + 피어링이 섞인 scale-free 그래프**에 가깝다는 결과도 많이 보고된다.

---

### IGP vs EGP, 그리고 유니캐스트 라우팅 프로토콜의 위치
...
- **EGP(BGP)**는
  ...
  를 다루기 때문에 **정책 기반 routing** 이 핵심이다.
...
- **최대 15 hop** 까지만 도달 가능, 16은 무한대(도달 불가)로 간주
- **주기적 업데이트**: 기본 30초마다 전체 라우팅 테이블을 이웃에게 광고
- **RIP v1**: classful, VLSM 지원 X
- **RIP v2**: classless, VLSM, 인증 등 지원 (RFC 2453)
...
### Count-to-Infinity 문제와 15-hop 제한

distance vector의 고전적인 문제는 **경로가 끊겼을 때, 잘못된 경로 정보가 점점 증가하며 수렴이 매우 느려질 수 있다는 것**이다(Count-to-Infinity).
...
- 고속 백본이나 대규모 네트워크에서는
  15-hop 제한, 느린 수렴 때문에 RIP 대신 OSPF/IS-IS를 사용한다.
...
### OSPF 영역(Area)와 LSA 타입

OSPF는 **대규모 네트워크를 area로 분할**하여 확장성과 안정성을 높인다.

- **Backbone area (0)**: 모든 다른 area를 연결하는 중심
- **Regular area**, **Stub area**, **NSSA** 등 다양한 변형

LSA 타입 예시(IPv4 OSPFv2 기준):​
...
서로 다른 AS 사이에서 **IP prefix 도달성(reachability)** 을 교환한다.
...
“**어떤 AS들을 거쳐 왔는지(AS-PATH)**”를 함께 싣는다는 점이 다르다.

각 경로에는 다음과 같은 속성이 붙는다(중요한 것만):

- **AS-PATH**: 이 경로가 지나온 AS 번호들의 리스트
- **NEXT-HOP**: 다음 홉 라우터의 IP
- **LOCAL_PREF**: AS 내부에서 선호도 (값이 **클수록 더 선호**)
- **MED(Multi-Exit Discriminator)**: 인접 AS에게 “이 링크를 우선 사용해 달라”는 힌트 (작을수록 선호)
...
정상 상황에서는 항상 ISP-A 경로가 선택된다.

---

### BGP와 보안, 안정성 이슈 (간단 요약)

BGP는 기본적으로 **경로 광고를 신뢰**하는 설계라서,
- 잘못된 설정이나 악의적 AS가 **거짓 경로를 광고(BGP hijack)** 하면
  전 세계 트래픽이 잘못된 곳으로 흘러갈 수 있다.
...
- **Internet Structure**
  ...
  - 상업적 측면에서는 Tier-1/2/3 ISP 계층 구조로 설명 가능
...
- 그리고 필요하면 **정책 라우팅, 트래픽 엔지니어링**,
  그리고 **보안(RPKI, 필터링)** 을 덧붙여 전체 구조를 완성한다.
